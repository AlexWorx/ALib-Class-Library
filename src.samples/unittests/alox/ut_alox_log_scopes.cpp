...
#endif //  ALOX_DBG_LOG && !ALIB_SINGLE_THREADED && !defined(ALIB_UT_ROUGH_EXECUTION_SPEED_TEST)
}
 ...
 * Log_Prefix
 **********************************************************************************************/
#if ALOX_DBG_LOG_CI && !ALIB_DEBUG_ASSERTION_PRINTABLES
...
PFXCHECK( "One, two, 3*msg*"    ,ml )
    ...
    Log_SetPrefix( nullptr      ) // domain "/PREFIX" nothing here anymore
...
UT_METHOD(Log_ScopeDomains)
{
    ...
    // Thread-related
        ...
        Log_RemoveThreadDomain( "T_MPP",Scope::ThreadInner )  DDCHECK( ""   ,"@/T_O3/MET/T_I/T_I2#"   ,ml )
...
UT_METHOD(Lox_ScopeDomains)
{
    ...
    Lox_SetDomain( "T_O",  Scope::ThreadOuter )  DDCHECK_RL( ""   ,"@/T_O#"                 ,ml )
        ...
        Lox_RemoveThreadDomain( "T_MPP",Scope::ThreadInner )  DDCHECK_RL( ""   ,"@/T_O3/T_I/T_I2#"   ,ml )
