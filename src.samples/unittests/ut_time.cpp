...
UT_METHOD(DateTimeConversion)
{
    ...
    }
    {
        ...
        // check epoch seconds
        {
            ...
            // Test DateTime::Duration
            {
                ...
                timetNowFromSystemUL.QuadPart+= int64_t( (24 + 2) * 3600   +   3 * 60   + 4) * 10000000L;
    ...
    // Ticks converter
    {
        ...
        UT_TRUE( abs( (dateTimeNow - ticksConverted).InNanoseconds()) < 10000 || ALIB_UT_AVOID_ANALYZER_WARNINGS )

        Ticks       ticksBack=      converter.ToTicks( ticksConverted );
        UT_TRUE( abs( (ticksBack - ticksNow).InNanoseconds()) < 10000         || ALIB_UT_AVOID_ANALYZER_WARNINGS )
    }
...
UT_METHOD(Ages)
{
    ...
    // minimum time measuring
    {
        ...
        UT_TRUE ( tt.GetAverage().InAbsoluteMilliseconds()  < 10 || ALIB_UT_AVOID_ANALYZER_WARNINGS   )
    }
...
UT_METHOD(SpeedTestIndexOf)
{
    ...
    for (int run= 4; run < 18; ++run )
    {
        ...
        {
            output.Reset()
                  ._( "Search loops " )            ._( Field( String32( qtyLoops                        ), 6 ) )
                  ._( ":  time needed: " )         ._( Field( String32( stringSample.InNanoseconds()    ), 8 ) )
                  ._( " / "  )                     ._( Field( String32( aStringSample.InNanoseconds()   ), 8 ) )
                  ._( "   Ratio String/AString: " )._( double(stringSample.InNanoseconds()) / double(aStringSample.InNanoseconds()) );
...
void dateFormatCheck( AWorxUnitTesting& ut, CalendarDateTime& ct,  const character * fmt, const character * expected )
{
    String128 res;
    ct.Format( fmt, res );
    UT_PRINT( (String128("CalendarDateTime.Format: ") << fmt << " ->")._(Tab(20)) << res )
...
UT_METHOD(DateFormat)
{
    ...
    //error: dateFormatCheck( ut, ct, A_CHAR("'msH")         ,        A_CHAR("m.H") );
