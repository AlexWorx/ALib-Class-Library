...
#define TESTCLASSNAME       UT_Variables
...
{
    MyPlugin()
    : variables::ConfigurationPlugin(alib::variables::Priority::Environment - 1 )       {}
...
UT_METHOD(Basics)
{
    ...
    //--------------------- test boolean tokens --------------------
    {
        UT_TRUE(  cfg.ParseBooleanToken(A_CHAR("tr")).first )
        UT_TRUE(  cfg.ParseBooleanToken(A_CHAR("1" )).first )
        UT_TRUE(  cfg.ParseBooleanToken(A_CHAR("y" )).first )
        UT_TRUE(  cfg.ParseBooleanToken(A_CHAR("on")).first )
        UT_TRUE(  cfg.ParseBooleanToken(A_CHAR("ok")).first )
        UT_TRUE( !cfg.ParseBooleanToken(A_CHAR("f" )).first )
        UT_TRUE( !cfg.ParseBooleanToken(A_CHAR("fa")).first )
        UT_TRUE( !cfg.ParseBooleanToken(A_CHAR("0" )).first )
        UT_TRUE( !cfg.ParseBooleanToken(A_CHAR("of")).first )
        UT_TRUE( !cfg.ParseBooleanToken(A_CHAR("No")).first )
        UT_TRUE( !cfg.ParseBooleanToken(A_CHAR("-" )).first )
    }
}
...
UT_METHOD(ConfigCommandLineArgs)
{
    ...
    UT_EQ( Priority::CLI,   (vc.Declare(A_CHAR("SingleHyphen" ), A_CHAR("I"  ))).GetPriority()) UT_EQ( 12, vc.GetInt() )
                             vc.Declare(A_CHAR("DoubleHyphen" ), A_CHAR("B"  ));                UT_EQ( true , vc.GetBool())
                                                                                                UT_EQ( 2    , vc.Get<variables::Bool>().TokenIndex)
...
UT_METHOD(ConfigIniFilePlain)
{
    ...
    Path writePath(alib::system::SystemFolders::Temp); writePath.Add(A_PATH("test1.cpy.ini"));
    ...
    UT_EQ(  4, inif.Sections.size() )
    UT_EQ(  0, section->Entries.size() )
...
UT_METHOD(ConfigIniFiles)
{
   ...
   ;
    ...
    Variable vIniFile= variables::CampVariable(BASECAMP, HOME_ENV_NAME, A_CHAR("S") );
