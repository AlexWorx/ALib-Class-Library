    ...
    // ------------- Test a list of bits  ----------
{
    ...
    // ------------- import export ----------
    #define UT_ASSERT_TSIZE(V) static_assert(sizeof(decltype(ex)) == V, "Error");
    {
        BitSet<3> bitSet;
...
UT_METHOD(DbgTypeDemangler)
{
    ...
    #endif
    demangled.Reset() << typeid(typedemangler::TestType);
    UT_EQ(NString("TestType")       , demangled)

    demangled.Reset(lang::DbgTypeDemangler(typeid(typedemangler::TestType::Inner)).Get());
    #if defined(_MSC_VER)
    UT_EQ(NString("struct typedemangler::TestType::Inner"), demangled)
    #else
    UT_EQ(NString(       "typedemangler::TestType::Inner"), demangled)
    #endif
    demangled.Reset() << typeid(typedemangler::TestType::Inner);
    UT_EQ(NString("Inner"), demangled)

    if constexpr (std::same_as<alib::wchar,wchar_t>) {
        #if defined(_MSC_VER)
        ...
        #else
            #if ALIB_C20_MODULES
              // it seems like using clang's libc++, the space disappears?
              UT_TRUE(   NString(   "typedemangler::TestTemplate<alib::strings::TString@ALib.Strings<wchar_t> >").Equals(lang::DbgTypeDemangler(typeid(typedemangler::TestTemplate<alib::strings::TString<alib::wchar>>)).Get())
                      || NString(   "typedemangler::TestTemplate<alib::strings::TString@ALib.Strings<wchar_t>>") .Equals(lang::DbgTypeDemangler(typeid(typedemangler::TestTemplate<alib::strings::TString<alib::wchar>>)).Get()) )
            #else
              // it seems like using clang's libc++, the space disappears?
              UT_TRUE(   NString(   "typedemangler::TestTemplate<alib::strings::TString<wchar_t> >").Equals(lang::DbgTypeDemangler(typeid(typedemangler::TestTemplate<alib::strings::TString<alib::wchar>>)).Get())
                      || NString(   "typedemangler::TestTemplate<alib::strings::TString<wchar_t>>") .Equals(lang::DbgTypeDemangler(typeid(typedemangler::TestTemplate<alib::strings::TString<alib::wchar>>)).Get()) )
            #endif
        #endif
        demangled.Reset() << typeid(typedemangler::TestTemplate<alib::strings::TString<alib::wchar>>);
