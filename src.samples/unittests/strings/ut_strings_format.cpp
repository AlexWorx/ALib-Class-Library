...
#define TESTCLASSNAME       UT_Strings_Format
...
UT_METHOD( Unsorted )
{
    ...
    // int32
    {
        ...
        ms._(); ui=0;       ms._( Dec( ui, 1 ,nf) );   UT_EQ(      A_CHAR("0"), ms )
        ms._(); ui=0;       ms._( Dec( ui, 3 ,nf) );   UT_EQ(    A_CHAR("000"), ms )
        ms._(); ui=10;      ms._( ui    );             UT_EQ(     A_CHAR("10"), ms )
        ms._(); ui=100;     ms._( Dec( ui, 1 ,nf) );   UT_EQ(    A_CHAR("100"), ms )
        ms._(); ui=100;     ms._( Dec( ui, 3 ,nf) );   UT_EQ(    A_CHAR("100"), ms )
        ms._(); ui=100;     ms._( Dec( ui, 4 ,nf) );   UT_EQ(   A_CHAR("0100"), ms )
        ms._(); ui=23;      ms._( ui    );             UT_EQ(     A_CHAR("23"), ms )
        ms._(); ui=99;      ms._( Dec( ui, 5 ,nf) );   UT_EQ(  A_CHAR("00099"), ms )
        ms._();  i=49;      ms._( i     );             UT_EQ(     A_CHAR("49"), ms )
        ms._();  i=-5;      ms._( Dec ( i, 5 ,nf) );   UT_EQ(  A_CHAR("-0005"), ms )
        ms._();  i=-5324;   ms._( Dec ( i, 2 ,nf) );   UT_EQ(  A_CHAR("-5324"), ms )
        ...
        ms._(); ui= std::numeric_limits<uint32_t>::max();    ms._( Dec( ui ,&nfG    ) );   UT_EQ( A_CHAR("4,294,967,295"),  ms )
        ms._(); ui= std::numeric_limits<uint32_t>::min();    ms._( Dec( ui ,&nfG    ) );   UT_EQ(             A_CHAR("0"),  ms )
        ms._();  i= std::numeric_limits<int32_t> ::max();    ms._( Dec ( i ,&nfG    ) );   UT_EQ( A_CHAR("2,147,483,647"),  ms )
        ms._();  i= std::numeric_limits<int32_t> ::min();    ms._( Dec ( i ,&nfG    ) );   UT_EQ(A_CHAR("-2,147,483,648"),  ms )

        ms._(); ui= std::numeric_limits<uint32_t>::max();    ms._( Dec( ui , 12 ,nf ) );   UT_EQ(  A_CHAR("004294967295"),  ms )
        ms._(); ui= std::numeric_limits<uint32_t>::min();    ms._( Dec( ui , 12 ,nf ) );   UT_EQ(  A_CHAR("000000000000"),  ms )
        ms._();  i= std::numeric_limits<int32_t> ::max();    ms._( Dec ( i , 12 ,nf ) );   UT_EQ(  A_CHAR("002147483647"),  ms )
        ms._();  i= std::numeric_limits<int32_t> ::min();    ms._( Dec ( i , 12 ,nf ) );   UT_EQ(  A_CHAR("-02147483648"),  ms )
    }
    ...
    // int64
    {
        ...
        ui=0;       ms.Reset( Dec( ui, 1  ,nf ));  UT_EQ(      A_CHAR("0"), ms )
        ui=0;       ms.Reset( Dec( ui, 3  ,nf ));  UT_EQ(    A_CHAR("000"), ms )
        ui=10;      ms.Reset( ui );                UT_EQ(     A_CHAR("10"), ms )
        ui=100;     ms.Reset( Dec( ui, 1  ,nf ));  UT_EQ(    A_CHAR("100"), ms )
        ui=100;     ms.Reset( Dec( ui, 3  ,nf ));  UT_EQ(    A_CHAR("100"), ms )
        ui=100;     ms.Reset( Dec( ui, 4  ,nf ));  UT_EQ(   A_CHAR("0100"), ms )
        ui=23;      ms.Reset( ui );                UT_EQ(     A_CHAR("23"), ms )
        ui=99;      ms.Reset( Dec( ui, 5  ,nf ));  UT_EQ(  A_CHAR("00099"), ms )
        i =49;      ms.Reset(              i   );  UT_EQ(     A_CHAR("49"), ms )
        i =-5;      ms.Reset( Dec ( i, 5  ,nf ));  UT_EQ(  A_CHAR("-0005"), ms )
        i =-5324;   ms.Reset( Dec ( i, 2  ,nf ));  UT_EQ(  A_CHAR("-5324"), ms )
        ...
        ui= std::numeric_limits<uint32_t>::max();    ms.Reset( Dec(   ui ,&nfG   ));  UT_EQ( A_CHAR("4,294,967,295"),  ms )
        ui= std::numeric_limits<uint32_t>::min();    ms.Reset( Dec(   ui ,&nfG   ));  UT_EQ(             A_CHAR("0"),  ms )
         i= std::numeric_limits<int32_t> ::max();    ms.Reset( Dec(   i  ,&nfG   ));  UT_EQ( A_CHAR("2,147,483,647"),  ms )
         i= std::numeric_limits<int32_t> ::min();    ms.Reset( Dec(   i  ,&nfG   ));  UT_EQ(A_CHAR("-2,147,483,648"),  ms )

        ui= std::numeric_limits<uint32_t>::max();    ms.Reset( Dec(ui, 12,nf     ));  UT_EQ(  A_CHAR("004294967295"),  ms )
        ui= std::numeric_limits<uint32_t>::min();    ms.Reset( Dec(ui, 12,nf     ));  UT_EQ(  A_CHAR("000000000000"),  ms )
         i= std::numeric_limits<int32_t> ::max();    ms.Reset( Dec(i , 12,nf     ));  UT_EQ(  A_CHAR("002147483647"),  ms )
         i= std::numeric_limits<int32_t> ::min();    ms.Reset( Dec(i , 12,nf     ));  UT_EQ(  A_CHAR("-02147483648"),  ms )

        ui= std::numeric_limits<uint64_t>::max();    ms.Reset( Dec(  ui,&nfG     ));  UT_EQ( A_CHAR("18,446,744,073,709,551,615"),  ms )
        ui= std::numeric_limits<uint64_t>::min();    ms.Reset( Dec(  ui,&nfG     ));  UT_EQ(                          A_CHAR("0"),  ms )
         i= std::numeric_limits<int64_t> ::max();    ms.Reset( Dec(  i ,&nfG     ));  UT_EQ(  A_CHAR("9,223,372,036,854,775,807"),  ms )
        ms.SetBuffer(20);
         i= std::numeric_limits<int64_t> ::min();    ms.Reset( Dec(  i ,&nfG     ));  UT_EQ( A_CHAR("-9,223,372,036,854,775,808"),  ms )
         i= std::numeric_limits<int64_t> ::min() + 75008; ms.Reset(Dec(i,&nfG    ));  UT_EQ( A_CHAR("-9,223,372,036,854,700,800"),  ms )
    }
    ...
    // plus sign
    {
        ...
        i= 49;                       ms.Reset( i );                    UT_EQ(     A_CHAR("49"), ms )
        i= 49;      nf.PlusSign=' '; ms.Reset( Dec ( i, -1, &nf  ) );  UT_EQ(    A_CHAR(" 49"), ms )
        i= 49;      nf.PlusSign='+'; ms.Reset( Dec ( i, -1, &nf  ) );  UT_EQ(    A_CHAR("+49"), ms )
        i= 49;      nf.PlusSign=' '; ms.Reset( Dec ( i,  5, &nf  ) );  UT_EQ(  A_CHAR("  049"), ms )
        i= 49;      nf.PlusSign='+'; ms.Reset( Dec ( i,  5, &nf  ) );  UT_EQ(  A_CHAR("+ 049"), ms )
    }
    ...
    // Group character
    {
        ...
        w=  3; ms._(); ui=              100;      ms._( Dec( ui , w, &nf  ) );   UT_EQ(                A_CHAR("100"),  ms )  UT_EQ( w, ms.Length() )
        w=  4; ms._(); ui=              100;      ms._( Dec( ui , w, &nf  ) );   UT_EQ(               A_CHAR(" 100"),  ms )  UT_EQ( w, ms.Length() )
        w=  5; ms._(); ui=              100;      ms._( Dec( ui , w, &nf  ) );   UT_EQ(              A_CHAR("0,100"),  ms )  UT_EQ( w, ms.Length() )
        w=  7; ms._(); ui=           100000;      ms._( Dec( ui , w, &nf  ) );   UT_EQ(            A_CHAR("100,000"),  ms )  UT_EQ( w, ms.Length() )
        w=  8; ms._(); ui=           100000;      ms._( Dec( ui , w, &nf  ) );   UT_EQ(           A_CHAR(" 100,000"),  ms )  UT_EQ( w, ms.Length() )
        w=  9; ms._(); ui=           100000;      ms._( Dec( ui , w, &nf  ) );   UT_EQ(          A_CHAR("0,100,000"),  ms )  UT_EQ( w, ms.Length() )
        w= 11; ms._(); ui=        100000000;      ms._( Dec( ui , w, &nf  ) );   UT_EQ(        A_CHAR("100,000,000"),  ms )  UT_EQ( w, ms.Length() )
        w= 12; ms._(); ui=        100000000;      ms._( Dec( ui , w, &nf  ) );   UT_EQ(       A_CHAR(" 100,000,000"),  ms )  UT_EQ( w, ms.Length() )
        w= 13; ms._(); ui=        100000000;      ms._( Dec( ui , w, &nf  ) );   UT_EQ(      A_CHAR("0,100,000,000"),  ms )  UT_EQ( w, ms.Length() )
        w= 15; ms._(); ui=     100000000000;      ms._( Dec( ui , w, &nf  ) );   UT_EQ(    A_CHAR("100,000,000,000"),  ms )  UT_EQ( w, ms.Length() )
        w= 16; ms._(); ui=     100000000000;      ms._( Dec( ui , w, &nf  ) );   UT_EQ(   A_CHAR(" 100,000,000,000"),  ms )  UT_EQ( w, ms.Length() )
        w= 17; ms._(); ui=     100000000000;      ms._( Dec( ui , w, &nf  ) );   UT_EQ(  A_CHAR("0,100,000,000,000"),  ms )  UT_EQ( w, ms.Length() )

        w=  3; ms._(); i=     -         100;      ms._( Dec(  i , w, &nf  ) );   UT_EQ(               A_CHAR("-100"),  ms )  UT_EQ( w+1, ms.Length() )
        w=  4; ms._(); i=     -         100;      ms._( Dec(  i , w, &nf  ) );   UT_EQ(               A_CHAR("-100"),  ms )  UT_EQ( w  , ms.Length() )
        w=  5; ms._(); i=     -         100;      ms._( Dec(  i , w, &nf  ) );   UT_EQ(              A_CHAR("- 100"),  ms )  UT_EQ( w  , ms.Length() )
        w=  6; ms._(); i=     -         100;      ms._( Dec(  i , w, &nf  ) );   UT_EQ(             A_CHAR("-0,100"),  ms )  UT_EQ( w  , ms.Length() )
        w=  7; ms._(); i=     -      100000;      ms._( Dec(  i , w, &nf  ) );   UT_EQ(           A_CHAR("-100,000"),  ms )  UT_EQ( w+1, ms.Length() )
        w=  8; ms._(); i=     -      100000;      ms._( Dec(  i , w, &nf  ) );   UT_EQ(           A_CHAR("-100,000"),  ms )  UT_EQ( w  , ms.Length() )
        w=  9; ms._(); i=     -      100000;      ms._( Dec(  i , w, &nf  ) );   UT_EQ(          A_CHAR("- 100,000"),  ms )  UT_EQ( w  , ms.Length() )
        w= 10; ms._(); i=     -      100000;      ms._( Dec(  i , w, &nf  ) );   UT_EQ(         A_CHAR("-0,100,000"),  ms )  UT_EQ( w  , ms.Length() )
        w= 11; ms._(); i=     -   100000000;      ms._( Dec(  i , w, &nf  ) );   UT_EQ(       A_CHAR("-100,000,000"),  ms )  UT_EQ( w+1, ms.Length() )
        w= 12; ms._(); i=     -   100000000;      ms._( Dec(  i , w, &nf  ) );   UT_EQ(       A_CHAR("-100,000,000"),  ms )  UT_EQ( w  , ms.Length() )
        w= 13; ms._(); i=     -   100000000;      ms._( Dec(  i , w, &nf  ) );   UT_EQ(      A_CHAR("- 100,000,000"),  ms )  UT_EQ( w  , ms.Length() )
        w= 14; ms._(); i=     -   100000000;      ms._( Dec(  i , w, &nf  ) );   UT_EQ(     A_CHAR("-0,100,000,000"),  ms )  UT_EQ( w  , ms.Length() )
        w= 15; ms._(); i=     -100000000000;      ms._( Dec(  i , w, &nf  ) );   UT_EQ(   A_CHAR("-100,000,000,000"),  ms )  UT_EQ( w+1, ms.Length() )
        w= 16; ms._(); i=     -100000000000;      ms._( Dec(  i , w, &nf  ) );   UT_EQ(   A_CHAR("-100,000,000,000"),  ms )  UT_EQ( w  , ms.Length() )
        w= 17; ms._(); i=     -100000000000;      ms._( Dec(  i , w, &nf  ) );   UT_EQ(  A_CHAR("- 100,000,000,000"),  ms )  UT_EQ( w  , ms.Length() )
        w= 18; ms._(); i=     -100000000000;      ms._( Dec(  i , w, &nf  ) );   UT_EQ( A_CHAR("-0,100,000,000,000"),  ms )  UT_EQ( w  , ms.Length() )
    }
}
...
void intTestRoundtrip( AWorxUnitTesting& ut, int64_t i )
{
    ...
    itAS.Reset("  ")._( Dec( i, &itNF ) );  if( i>=0) {uiBack= itAS.ParseDec  ( 2,     &itNF  );   UT_EQ( i, int64_t(uiBack )) }
    ...
    if( i >= 0 )
    {
        itAS.Reset("0b")._( Bin  (ui, &itNF ) );  uiBack= itAS.ParseBin  ( 2,     &itNF  );        UT_EQ( i, int64_t(uiBack ))
                                                   iBack= itAS.ParseInt  (        &itNF  );        UT_EQ( i, iBack )
                                      itSB= itAS.Substring(2);  UT_TRUE(itSB.ConsumeBin(uiBack, &itNF ))   UT_EQ( i, int64_t( uiBack ) ) UT_TRUE(itSB.IsEmpty())
                                      itSB= itAS;               UT_TRUE(itSB.ConsumeInt( iBack, &itNF ))   UT_EQ( i,                        iBack   ) UT_TRUE(itSB.IsEmpty())

        itAS.Reset("0x")._( Hex  (ui, &itNF ) );  uiBack= itAS.ParseHex  ( 2,     &itNF  );        UT_EQ( i, int64_t(uiBack ))
                                                   iBack= itAS.ParseInt  (        &itNF  );        UT_EQ( i, iBack )
                                      itSB= itAS.Substring(2);  UT_TRUE(itSB.ConsumeHex(uiBack, &itNF ))   UT_EQ( i, int64_t( uiBack ) ) UT_TRUE(itSB.IsEmpty())
                                      itSB= itAS;               UT_TRUE(itSB.ConsumeInt( iBack, &itNF ))   UT_EQ( i,                        iBack   ) UT_TRUE(itSB.IsEmpty())

        itAS.Reset("0o")._( Oct  (ui, &itNF ) );  uiBack= itAS.ParseOct  ( 2,     &itNF  );        UT_EQ( i, int64_t(uiBack ))
                                                           iBack= itAS.ParseInt  (        &itNF  );        UT_EQ( i, iBack )
                                      itSB= itAS.Substring(2);  UT_TRUE(itSB.ConsumeOct(uiBack, &itNF ))   UT_EQ( i, int64_t( uiBack ) ) UT_TRUE(itSB.IsEmpty())
                                      itSB= itAS;               UT_TRUE(itSB.ConsumeInt( iBack, &itNF ))   UT_EQ( i,                        iBack   ) UT_TRUE(itSB.IsEmpty())
    }
}
...
void intTestRoundtripUS( AWorxUnitTesting& ut, uint64_t ui )
{
    ...
    itAS.Reset("0b")._( Bin  ( ui, &itNF ) );  uiBack= itAS.ParseBin ( 2,     &itNF  ); UT_EQ( ui, uiBack )
                                                iBack= itAS.ParseInt (        &itNF  ); UT_EQ( ui, uint64_t(iBack ))
                    itSB= itAS.Substring(2);  UT_TRUE(itSB.ConsumeBin( uiBack,&itNF ))  UT_EQ( ui, uiBack ) UT_TRUE(itSB.IsEmpty())

    itAS.Reset("0x")._( Hex  ( ui, &itNF ) );  uiBack= itAS.ParseHex ( 2,     &itNF  ); UT_EQ( ui, uiBack )
                                                iBack= itAS.ParseInt  (       &itNF  ); UT_EQ( ui, uint64_t(iBack ))
                 itSB= itAS.Substring(2);  UT_TRUE(itSB.ConsumeHex( uiBack   ,&itNF ))  UT_EQ( ui, uiBack ) UT_TRUE(itSB.IsEmpty())

    itAS.Reset("0o")._( Oct  ( ui, &itNF ) );  uiBack= itAS.ParseOct ( 2,     &itNF );  UT_EQ( ui, uiBack )
                                                iBack= itAS.ParseInt  (       &itNF );  UT_EQ( ui, uint64_t(iBack ))
...
UT_METHOD( ConvertIntegers )
{
    ...
    // int detection and string borders
    {
        ...
        }
        {
            ...
            Substring substring= String(A_CHAR("0x1234")).Substring<NC>( 0, 2);
            UT_TRUE( substring.ConsumeInt( result ))
            UT_EQ( 0, result )
            UT_EQ( A_CHAR("x"), substring )
        ...
        }
        {
            ...
            Substring substring= String(A_CHAR("  0x1234")).Substring<NC>( 0, 4);
            UT_TRUE( substring.ConsumeInt( result ))
            UT_EQ( 0, result )
            UT_EQ( A_CHAR("x"), substring )
    ...
    #endif
        intTestRoundtrip( ut,   int_fast64_t(std::rand())
                              * int_fast64_t(std::rand())
                              * int_fast64_t(std::rand()) );
...
UT_METHOD( ConvertFloats )
{
    ...
    // write and parse doubles, non scientific mode
    {
        ...
        // ATTENTION: This may fail on different platforms. As it can be seen, our
        //            rounding seems not precise, as already with 999...4! our code rounds up to 3.0
...
void    checkError (AWorxUnitTesting& ut, Enum expectedException, BoxedObjects&&... args )
{
    ...
    BoxesMA& boxesMA= testFormatter->GetArgContainer();
    boxesMA.Add( std::forward<BoxedObjects>(args) ... );
...
{
    // create argument objects using implicit constructor invocation
    BoxesMA& boxes= testFormatter->GetArgContainer();
...
//--- Test JavaStyle
//--------------------------------------------------------------------------------------------------
UT_METHOD( TestFormatterJavaStyle )
{
    UT_INIT()
    UT_PRINT(  "ALib Format Tests Java Style: Start" )

    alib::FormatterJavaStyle          formatterJS;
    formatterJS.Next.InsertDerived<FormatterPythonStyle>();
    ...
    //===== Hash code 'h'/'H' =========
    NAString tExpect;
    tExpect._() << strings::THex<char>(reinterpret_cast<uinteger>( &formatterJS ) ) ;
    ...
    if(BASECAMP.LocaleFound.IsNotNull() && BASECAMP.LocaleFound.IndexOf<NC, lang::Case::Ignore>(A_CHAR("UTF-8")) >= 0)
    {
...
//--- Test PythonStyle
//--------------------------------------------------------------------------------------------------
UT_METHOD( TestFormatterPythonStyle )
{
    UT_INIT()
    UT_PRINT( "ALib Format Tests Python Style: Start" )

    alib::FormatterPythonStyle formatterPS;
    formatterPS.Next.InsertDerived<FormatterJavaStyle>();
    ...
    // wchar (test this only if the LOCALE was properly set)
        ...
        if(    BASECAMP.LocaleFound.IsNotNull()
            && BASECAMP.LocaleFound.IndexOf<NC, lang::Case::Ignore>(A_CHAR("UTF-8")) >= 0)
        {
            checkFormat(ut,  "x"                         , "{}"           , 'x'          );
            checkFormat(ut,  "x    "                     , "{:5c}"        , 'x'          );
            checkFormat(ut,  NString64()._(L"\u03B1")    , "{:c}"         , L'\u03B1'    ); //greek alpha
