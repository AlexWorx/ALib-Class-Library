...
struct BoxTraits< std::vector<TElem>* >
{
    using                   Mapping=  std::vector<TElem>*;
    static constexpr bool   IsArray=  false;

    static
    void             Write( Placeholder& box,  std::vector<TElem>* const & value ) {
        box.Write( value );
    }

    static
    std::vector<TElem>*     Read( const Placeholder& box) {
        return box.Read<std::vector<TElem>*>();
    }
};
}
DOX_MARKER( [DOX_BOXING_CUSTOM_VECTOR_POINTERS_CUSTOMIZE])
...
DOX_MARKER( [DOX_BOXING_TUT_SURJECTIVE_STATIC_CAST])
    Box     box = int16_t(16);
    integer i16 = int16_t( box.Unbox<integer>() );  // OK
DOX_MARKER( [DOX_BOXING_TUT_SURJECTIVE_STATIC_CAST])
}
...
static_assert( std::same_as<decltype(small),       SmallClass >, "Error" );
static_assert( std::same_as<decltype(big  ), const BigClass*  >, "Error" );
DOX_MARKER([DOX_BOXING_CLASSES_CONST_POINTERS])
}
...
// A templated specialization for a set of types T
template<typename T>

// Using the keyword "requires" , this is a specialization for all type MyBase and all types
// derived of it. A quite powerful C++ language feature!
requires std::derived_from<T, MyBase>

// The specialized version of the traits types
struct BoxTraits<T> {
    // Type mapping is fixed to 'MyBase'.
    using                   Mapping=  MyBase;

    // Mapped as plain type
    static constexpr bool   IsArray=  false;
    ...
    static void     Write( Placeholder& box, const T& src)  {
        box.Write( static_cast<MyBase>( src ) );
    }

    // Read returns 'MyBase'. This implies that only this type can be unboxed, all derived types
    // can not be unboxed.
    // With more complex scenarios, the return type could also be 'T', which
    ...
    static MyBase   Read( const Placeholder& box)  {
        return box.Read<MyBase>();
    }
};
...
void RegisterMyFunctions()
{
    ...
    // Likewise, then the inclusion of "ALib/CallerInfoFunctions.hpp" is not required.

    ALIB_LOCK_RECURSIVE_WITH( alib::monomem::GLOBAL_ALLOCATOR_LOCK )

    // registering FToString for type integer
    alib::boxing::BootstrapRegister<FToString, integer, false >( FToString_integer        );

    // registering FToString for type double
    alib::boxing::BootstrapRegister<FToString, double , false >( FToString_double         );

    // registering FToString for character arrays
    alib::boxing::BootstrapRegister<FToString, char   , true  >( FToString_array<char   > );

    // registering FToString for integer arrays
    alib::boxing::BootstrapRegister<FToString, integer, true  >( FToString_array<integer> );
}
DOX_MARKER([DOX_BOXING_FUNCTIONS_REGISTER])
...
cout << "The mapped type is: " << boxing::debug::TypeName( "char array" ) << endl;
DOX_MARKER([DOX_BOXING_DEBUG_TYPENNAME])
#endif
}
...
cout << boxing::debug::TypeInfo<alib::String>();
DOX_MARKER([DOX_BOXING_DEBUG_TYPEINFO])
#endif
}
...
cout << boxing::debug::DumpVTables(false);
DOX_MARKER([DOX_BOXING_DEBUG_DYNAMICTABLE])
#endif
}
...
cout << boxing::debug::DumpFunctions( boxing::debug::GetKnownFunctionTypes() );
DOX_MARKER([DOX_BOXING_DEBUG_KNOWNFUNCTIONS])
#endif
}
