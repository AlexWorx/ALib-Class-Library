...
//######################################### STEP 1 #################################################
DOX_MARKER( [DOX_EXPR_TUT_FF_FilterSkeleton])
namespace step1 {
struct FileFilter
{
    // Constructor.
...
//######################################### STEP 2 #################################################
DOX_MARKER( [DOX_EXPR_TUT_FF_FilterGeneric])
namespace step2 {
struct FileFilter
{
    Compiler        compiler;
...
DOX_MARKER( [DOX_EXPR_TUT_FF_FilterCheckResultType])
bool Includes(const fs::directory_entry& directoryEntry)
{
    (void) directoryEntry;
...
DOX_MARKER( [DOX_EXPR_TUT_FF_FilterCheckResultType2])
FileFilter( const String& expressionString )
: compiler()
, scope( compiler.CfgFormatter )
{
...
//###################################### STEP 4:  Scope ############################################
DOX_MARKER( [DOX_EXPR_TUT_FF_Scope])
namespace step4 {
...
DOX_MARKER( [DOX_EXPR_TUT_FF_ScopeFF])
namespace step4 {
struct FileFilter
{
    Compiler        compiler;
...
DOX_MARKER( [DOX_EXPR_TUT_FF_PluginSkeleton])
namespace step5 {
struct FFCompilerPlugin : public CompilerPlugin
{
    FFCompilerPlugin( Compiler& compiler )
    : CompilerPlugin( "FF Plug-in", compiler, CompilePriorities::Custom  )
    {}
...
DOX_MARKER( [DOX_EXPR_TUT_FF_PluginCallback])
Box getName( ExpressionScope& scope, ArgIterator argsBegin, ArgIterator argsEnd )
{
    #if !ALIB_CHARACTERS_WIDE
        // Create a copy of the string using the scope string allocator. The returned string is
        // right away boxed as a usual string, aka char[]. 
        return NString(scope.Allocator, dynamic_cast<FFScope&>( scope ).directoryEntry->path().filename().string() );
    #else
        return WString(scope.Allocator, WString512(dynamic_cast<FFScope&>( scope ).directoryEntry->path().filename().string() ) );
    #endif
}
DOX_MARKER( [DOX_EXPR_TUT_FF_PluginCallback])
...
DOX_MARKER( [DOX_EXPR_TUT_FF_PluginOverride])
namespace step5 {
struct FFCompilerPlugin : public CompilerPlugin
{
    FFCompilerPlugin( Compiler& compiler )
    ...
    virtual bool TryCompilation( CIFunction&  ciFunction )                                  override
    {
        ...
        if(       ciFunction.QtyArgs() == 0
             &&   ciFunction.Name.Equals<NC, lang::Case::Ignore>( A_CHAR("Name") )    )
        {
...
DOX_MARKER( [DOX_EXPR_TUT_FF_PluginSkeleton2])
namespace step5 {
struct FileFilter
{
    Compiler            compiler;
    ...
    , ffPlugin( compiler )            // CHANGE 2: Initialize the plug-in with the compiler.
    {
        ...
        // CHANGE 3: Add our custom plug-in to the compiler before compiling the expression
        compiler.InsertPlugin( & ffPlugin );
...
DOX_MARKER( [DOX_EXPR_TUT_FF_PluginCalculus])
struct FFCompilerPlugin : public Calculus
{
    FFCompilerPlugin( Compiler& compiler )
    : Calculus( "FF Plug-in", compiler, CompilePriorities::Custom  )
    {
        Functions=
        {
...
struct FileFilter
{
    ...
    , ffPlugin( compiler )
    {
        ...
        // CHANGE 2: Add our custom plug-in to the compiler before compiling the expression
        compiler.InsertPlugin( & ffPlugin );
...
DOX_MARKER( [DOX_EXPR_TUT_FF_MoreIdentifiers])
namespace
{
    ...
    Box getSize( ExpressionScope& scope, ArgIterator argsBegin, ArgIterator argsEnd )
    {
        return (  dynamic_cast<FFScope&>( scope ).directoryEntry->status().type()
                == fs::file_type::directory )
                ? 0
                : integer(fs::file_size( *dynamic_cast<FFScope&>( scope ).directoryEntry ));
    }
    ...
    {
        return integer( dynamic_cast<FFScope&>( scope ).directoryEntry->status().permissions() );
    }
}
DOX_MARKER( [DOX_EXPR_TUT_FF_MoreIdentifiers])

DOX_MARKER( [DOX_EXPR_TUT_FF_MoreIdentifiersConstants])
namespace
{
    Box constOwnRead  = integer( fs::perms::owner_read  );
    Box constOwnWrite = integer( fs::perms::owner_write );
    Box constOwnExec  = integer( fs::perms::owner_exec  );
    Box constGrpRead  = integer( fs::perms::group_read  );
    Box constGrpWrite = integer( fs::perms::group_write );
    Box constGrpExec  = integer( fs::perms::group_exec  );
    Box constOthRead  = integer( fs::perms::others_read );
    Box constOthWrite = integer( fs::perms::others_write);
    Box constOthExec  = integer( fs::perms::others_exec );
}
DOX_MARKER( [DOX_EXPR_TUT_FF_MoreIdentifiersConstants])


DOX_MARKER( [DOX_EXPR_TUT_FF_MoreIdentifiersPlugin])
...
DOX_MARKER( [DOX_EXPR_TUT_FF_Functions])
...
DOX_MARKER( [DOX_EXPR_TUT_FF_FunctionSignature])
Box*  OneInt[1]= { &Types::Integer  };
DOX_MARKER( [DOX_EXPR_TUT_FF_FunctionSignature])


DOX_MARKER( [DOX_EXPR_TUT_FF_FunctionsPlugin])
struct FFCompilerPlugin : public plugins::Calculus
{
    FFCompilerPlugin( Compiler& compiler )
    : Calculus( "FF Plug-in", compiler, CompilePriorities::Custom )
...
DOX_MARKER( [DOX_EXPR_TUT_FF_PermType])
Box getPerm( ExpressionScope& scope, ArgIterator argsBegin, ArgIterator argsEnd )
{
...
DOX_MARKER( [DOX_EXPR_TUT_FF_PermTypePlugin])
struct FFCompilerPlugin : public plugins::Calculus
{
    FFCompilerPlugin( Compiler& compiler )
    : Calculus( "FF Plug-in", compiler, CompilePriorities::Custom  )
    {
        ...
        constOwnRead  = integer( fs::perms::owner_read  );
        constOwnWrite = integer( fs::perms::owner_write );
        constOwnExec  = integer( fs::perms::owner_exec  );
        constGrpRead  = integer( fs::perms::group_read  );
        constGrpWrite = integer( fs::perms::group_write );
        constGrpExec  = integer( fs::perms::group_exec  );
        constOthRead  = integer( fs::perms::others_read );
        constOthWrite = integer( fs::perms::others_write);
        constOthExec  = integer( fs::perms::others_exec );
...
DOX_MARKER( [DOX_EXPR_TUT_FF_PermTypeAddTypeDef])
...
DOX_MARKER( [DOX_EXPR_TUT_FF_PermTypeOperatorCallbacks])
...
DOX_MARKER( [DOX_EXPR_TUT_FF_PermTypeOperatorTable])
...
DOX_MARKER( [DOX_EXPR_TUT_FF_PermTypeFeedTable])
...
DOX_MARKER( [DOX_EXPR_TUT_FF_AutoCastCallback])
Box perm2Int( ExpressionScope& scope, ArgIterator argsBegin, ArgIterator argsEnd ) {
    return integer( argsBegin->Unbox<fs::perms>() );
}
DOX_MARKER( [DOX_EXPR_TUT_FF_AutoCastCallback])
...
DOX_MARKER( [DOX_EXPR_TUT_FF_AutoCastCompilation])
virtual bool TryCompilation( CIAutoCast& ciAutoCast )    override
{
    ...
    if( ciAutoCast.Operator.Equals<NC>( A_CHAR("Q?T:F") ) )
        return false;
...
DOX_MARKER([DOX_EXPR_TUT_FF_AutoCastCalculus])
AutoCasts=
{
...
#define TESTCLASSNAME       UT_Dox_Expr_Tutorial
...
UT_METHOD( FileSystemIntro )
{
    UT_INIT()
    {
DOX_MARKER([DOX_EXPR_TUT_FF_1])
// search source path from current
auto sourceDir = fs::path(ALIB_BASE_DIR);
sourceDir+= "/src/alib/expressions";
ALIB_ASSERT_ERROR( fs::exists(sourceDir), "UNITTESTS", "Test directory not found: ", sourceDir.c_str() )
...
DOX_MARKER([DOX_EXPR_TUT_FF_1])
    ...
    ut.WriteResultFile( "DOX_EXPR_TUT_FF_INTRO-1.txt", testOutputStreamN.str() );
    testOutputStreamN.str("");


DOX_MARKER([DOX_EXPR_TUT_FF_FilterLoop])
...
DOX_MARKER([DOX_EXPR_TUT_FF_TRUE])
...
DOX_MARKER([DOX_EXPR_TUT_FF_TRUE])
    ...
    ut.WriteResultFile( "DOX_EXPR_TUT_FF_TRUE.txt", testOutputStreamN.str() );
    testOutputStreamN.str("");

DOX_MARKER([DOX_EXPR_TUT_FF_FALSE])
...
DOX_MARKER([DOX_EXPR_TUT_FF_FALSE])
    ...
    ut.WriteResultFile( "DOX_EXPR_TUT_FF_FALSE.txt", testOutputStreamN.str(), "" );
    testOutputStreamN.str("");
...
DOX_MARKER([DOX_EXPR_TUT_FF_Name1])
    ...
    ut.WriteResultFile( "DOX_EXPR_TUT_FF_Name1.txt", testOutputStreamN.str() );
    testOutputStreamN.str("");
...
DOX_MARKER([DOX_EXPR_TUT_FF_Name2])
    ...
    ut.WriteResultFile( "DOX_EXPR_TUT_FF_Name2.txt", testOutputStreamN.str() );
    testOutputStreamN.str("");

DOX_MARKER([DOX_EXPR_TUT_FF_Name3])
...
DOX_MARKER([DOX_EXPR_TUT_FF_Name3])
    ...
    ut.WriteResultFile( "DOX_EXPR_TUT_FF_Name3.txt", testOutputStreamN.str() );
    testOutputStreamN.str("");
    ...
    for( auto& directoryEntry : fs::directory_iterator( sourceDir ) )
        if( filter71.Includes( directoryEntry ) )
            cout << directoryEntry.path().filename().string() << endl;
    ut.WriteResultFile( "DOX_EXPR_TUT_FF_More-1.txt", testOutputStreamN.str() ,"");
    testOutputStreamN.str("");
    ...
    for( auto& directoryEntry : fs::directory_iterator( sourceDir ) )
        if( filter73.Includes( directoryEntry ) )
            cout << directoryEntry.path().filename().string() << endl;
    ut.WriteResultFile( "DOX_EXPR_TUT_FF_More-3.txt", testOutputStreamN.str() ,"") ;
    testOutputStreamN.str("");
    ...
    for( auto& directoryEntry : fs::directory_iterator( sourceDir ) )
        if( filter74.Includes( directoryEntry ) )
            cout << directoryEntry.path().filename().string() << endl;
    ut.WriteResultFile( "DOX_EXPR_TUT_FF_More-4.txt", testOutputStreamN.str() ,"") ;
    testOutputStreamN.str("");
    ...
    for( auto& directoryEntry : fs::directory_iterator( sourceDir ) )
        if( filter75.Includes( directoryEntry ) )
            cout << directoryEntry.path().filename().string() << endl;
    ut.WriteResultFile( "DOX_EXPR_TUT_FF_More-5.txt", testOutputStreamN.str() ,"");
    testOutputStreamN.str("");
    ...
    for( auto& directoryEntry : fs::directory_iterator( sourceDir ) )
        if( filter81.Includes( directoryEntry ) )
            cout << directoryEntry.path().filename().string() << endl;
    ut.WriteResultFile( "DOX_EXPR_TUT_FF_Func-1.txt", testOutputStreamN.str() );
    testOutputStreamN.str("");
    ...
    for( auto& directoryEntry : fs::directory_iterator( sourceDir ) )
        if( filter111.Includes( directoryEntry ) )
            cout << directoryEntry.path().filename().string() << endl;
    ut.WriteResultFile( "DOX_EXPR_TUT_FF_Func-3.txt", testOutputStreamN.str() );
    testOutputStreamN.str("");
    ...
    for( auto& directoryEntry : fs::directory_iterator( sourceDir ) )
        if( filter121.Includes( directoryEntry ) )
            cout << directoryEntry.path().filename().string() << endl;
    ut.WriteResultFile( "DOX_EXPR_TUT_FF_Func-4.txt", testOutputStreamN.str() );
    testOutputStreamN.str("");
    ...
    for( auto& directoryEntry : fs::directory_iterator( sourceDir ) )
        if( filter122.Includes( directoryEntry ) )
            cout << directoryEntry.path().filename().string() << endl;
    ut.WriteResultFile( "DOX_EXPR_TUT_FF_Func-5.txt", testOutputStreamN.str() );
    testOutputStreamN.str("");
    ...
    for( auto& directoryEntry : fs::directory_iterator( sourceDir ) )
        if( filter13.Includes( directoryEntry ) )
            cout << directoryEntry.path().filename().string() << endl;
    ut.WriteResultFile( "DOX_EXPR_TUT_FF_Func-6.txt", testOutputStreamN.str() );
    testOutputStreamN.str("");
    ...
    {
DOX_MARKER([DOX_EXPR_EmptyCompiler])
Compiler compiler;
...
EXPRESSION(
DOX_MARKER([DOX_EXPR_PROS_NASTY])
...
Expression expression= compiler.Compile( A_CHAR("\"Hexadecimal: 0x{:x}\" {} 42") );
DOX_MARKER([DOX_EXPR_OPS_1])
}
    catch(Exception& e)
    {ALIB_LOCK_RECURSIVE_WITH(Formatter::DefaultLock)
...
DOX_MARKER([DOX_EXPR_OPS_3])
    ut.WriteResultFile( "DOX_EXPR_OPS_3.txt", testOutputStreamN.str(), "" );
    testOutputStreamN.str("");
}
    catch(Exception& e)
    {ALIB_LOCK_RECURSIVE_WITH(Formatter::DefaultLock)
...
Expression expression= compiler.Compile(A_CHAR(R"(  *MyNestedExpression  )"));
...
//! [DOX_EXPR_NESTED_OP_2]
    ...
    ut.WriteResultFile( "DOX_EXPR_NESTED_OP_2.txt", testOutputStreamN.str(), "" );
...
//! [DOX_EXPR_NESTED_OP_3]
Expression expression= compiler.Compile(A_CHAR(R"(   2 * *MyNestedExpression    )"));
//! [DOX_EXPR_NESTED_OP_3]
...
cout << "Result: " << expression->Evaluate( scope ) << endl;
    ...
    ut.WriteResultFile( "DOX_EXPR_NESTED_OP_3.txt", testOutputStreamN.str(), "" );
...
//! [DOX_EXPR_NESTED_OP_4]
Expression expression= compiler.Compile( A_CHAR(R"(   *("MyNested" + "Expression")   )") );
//! [DOX_EXPR_NESTED_OP_4]
...
cout << "Result: " << expression->Evaluate( scope ) << endl;
    ...
    ut.WriteResultFile( "DOX_EXPR_NESTED_OP_4.txt", testOutputStreamN.str(), "" );
...
//! [DOX_EXPR_NESTED_OP_5]
Expression expression= compiler.Compile(A_CHAR(R"(    *("MyNested" + ( random >= 0.0 ? "Expression" : "" ))     )") );
//! [DOX_EXPR_NESTED_OP_5]
    ...
    try
    {
//! [DOX_EXPR_NESTED_FUNC_1]
Compiler compiler;
compiler.SetupDefaults();
ExpressionScope scope( compiler.CfgFormatter );

Expression expression= compiler.Compile(A_CHAR(R"(   Expression( "MyNestedExpression", -1 )   )"));
...
//! [DOX_EXPR_NESTED_FUNC_1]
    ...
    ut.WriteResultFile( "DOX_EXPR_NESTED_FUNC_1.txt", testOutputStreamN.str(), "" );


    testOutputStreamN.str("");
    try
    {
//! [DOX_EXPR_NESTED_FUNC_2]
Compiler compiler;
compiler.SetupDefaults();
ExpressionScope scope( compiler.CfgFormatter );
...
//! [DOX_EXPR_NESTED_FUNC_2]
    ...
    ut.WriteResultFile( "DOX_EXPR_NESTED_FUNC_2.txt", testOutputStreamN.str(), "" );


    testOutputStreamN.str("");
    try
    {
//! [DOX_EXPR_NESTED_FUNC_3]
Compiler compiler;
compiler.SetupDefaults();
ExpressionScope scope( compiler.CfgFormatter );
...
//! [DOX_EXPR_NESTED_FUNC_3]
    ...
    ut.WriteResultFile( "DOX_EXPR_NESTED_FUNC_3.txt", testOutputStreamN.str(), "" );
...
ExpressionScope scope( compiler.CfgFormatter );
{
//! [DOX_EXPR_NESTED_FUNC_6]
Expression expression= compiler.Compile(A_CHAR(R"(   Expression( MyNestedExpression, -1 )   )"));
//! [DOX_EXPR_NESTED_FUNC_6]
}
{
//! [DOX_EXPR_NESTED_FUNC_7]
Expression expression= compiler.Compile(A_CHAR(R"(   Expression( ("MyNested" + ( random >= 0.0 ? "Expression" : "" )), -1 )   )"));
//! [DOX_EXPR_NESTED_FUNC_7]
}
    }
    catch(Exception& )
...
Expression expression= compiler.Compile(A_CHAR(R"(   Expression( "MyNestedExpression", -1, throw )   )"));
                   ...
                   bool dontOptimize= true)
{
    ...
    try
    {
        Expression expression= compiler.Compile( expressionString );
        AString listing=  expressions::DbgList(expression);
...
UT_METHOD( VMListings )
{
    ...
    printProgram( ut, A_CHAR("42"),                                               "DOX_EXPR_VM_-1.txt", false  );
    printProgram( ut, A_CHAR("42 * 2"),                                           "DOX_EXPR_VM_-2.txt", false  );
    printProgram( ut, A_CHAR("42 * 2"),                                           "DOX_EXPR_VM_-3.txt"  );
    printProgram( ut, A_CHAR("(((42 * 2) / 5) * (2 + 3) ) * 7"),                  "DOX_EXPR_VM_-4.txt"  );
    printProgram( ut, A_CHAR("Format( \"Result of: {}\", \"2 * 3\", 2 * 3 )") ,   "DOX_EXPR_VM_-5.txt"  );
    printProgram( ut, A_CHAR("true ? 1 : 2")                                  ,   "DOX_EXPR_VM_-6.txt"  );
    printProgram( ut, A_CHAR("true ? 1 : 2")                                  ,   "DOX_EXPR_VM_-6opt.txt", false  );
    printProgram( ut, A_CHAR("*nested")                                       ,   "DOX_EXPR_VM_-7.txt"  );
    printProgram( ut, A_CHAR("Expression(nested,-1,throw)")                   ,   "DOX_EXPR_VM_-8.txt"  );
}
#endif
