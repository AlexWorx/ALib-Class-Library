...
#if ALIB_UT_DOCS && ALIB_UT_ENUMRECORDS
...
DOX_MARKER( [DOX_ENUMS_RECORDS_HEADER_INIT])
DOX_MARKER( [DOX_ENUMS_RECORDS_HEADER_RESOURCES])
...
ALIB_ENUMS_ASSIGN_RECORD( ArithmeticEnum, alib::enumrecords::ERSerializable )
...
ALIB_ENUMS_ASSIGN_RECORD( Bits, alib::enumrecords::ERSerializable )
...
ALIB_ENUMS_ASSIGN_RECORD( BitsParsable, alib::enumrecords::ERSerializable )
...
ALIB_ENUMS_ASSIGN_RECORD( WindowManager::States, alib::enumrecords::ERSerializable )
DOX_MARKER( [DOX_ENUMS_ER_STATES])
...
template<> struct RecordsTraits<simple::TEnum>  : std::true_type               \
{                                                                              \
...
#define  ALIB_ENUMS_ASSIGN_RECORD( TEnum, TRecord )                            \
namespace alib::enumrecords {                                                        \
template<> struct RecordsTraits<TEnum>  :  std::true_type                      \
{                                                                              \
    using  Type=  TRecord;                                                     \
};}
...
{
    cout << alib::enumrecords::GetRecord(fruit).Name << endl;
}
DOX_MARKER( [DOX_ENUMS_RECORDS_FRUITS_GETRECORD])
void invoke()
{
...
void printFruit(Fruits fruit)
{
    const ERFruits* record= alib::enumrecords::TryRecord(fruit);

    if( record != nullptr )
        cout << record->Name << endl;
    else
        cout << "Fruits(" << int(fruit) << ")" << endl;
}
DOX_MARKER( [DOX_ENUMS_RECORDS_FRUITS_TRYRECORD])
void invoke()
{
...
ALIB_ENUMS_ASSIGN_RECORD( Fruits, alib::enumrecords::ERSerializable )
DOX_MARKER([DOX_ENUMS_RECORDS_FRUITS_ASSIGN_ERSERIALZEABLE])
...
bool       success= alib::enumrecords::Parse( input, parsedFruit );
...
requires ( EnumRecords<TEnum>::template AreOfType<ERBase>() )
void acceptBaseOrDerived(TEnum element)
{
    const ERBase* record= alib::enumrecords::TryRecord( element );
...
alib::enumrecords::bootstrap::Bootstrap<Fruits>( Fruits::Apple , A_CHAR("Apple" ) );
alib::enumrecords::bootstrap::Bootstrap<Fruits>( Fruits::Orange, A_CHAR("Orange") );
alib::enumrecords::bootstrap::Bootstrap<Fruits>( Fruits::Banana, A_CHAR("Banana") );
DOX_MARKER([DOX_ENUMS_RECORDS_FRUITS_INIT_3CALLS])
}
...
alib::enumrecords::bootstrap::Bootstrap<Fruits>(
{
...
alib::enumrecords::bootstrap::Bootstrap<WindowManager::States>(
...
alib::enumrecords::bootstrap::Bootstrap<BitsParsable>(
{
...
alib::enumrecords::bootstrap::Bootstrap<Bits>(
{
    { Bits::One  ,   A_CHAR("One")    },
    { Bits::Two  ,   A_CHAR("Two")    },
...
(void) maximized;
    ...
    UT_METHOD( lang_enumops_IterateBitwise )
    {
        UT_INIT()
        UT_PRINT("*** Documentation Sample +**")
    ...
    UT_METHOD( lang_enumops_Parseing )
    {
        UT_INIT()
        UT_PRINT("*** Documentation Sample +**")
        ...
        caseRead= lang::Case(-1);       s= A_CHAR("senitive")   ; UT_TRUE ( enumrecords::Parse(s, caseRead ) )    UT_TRUE( caseRead== lang::Case::Sensitive)  UT_EQ( A_CHAR("itive")   , s)
        caseRead= lang::Case(-1);       s= A_CHAR("ignore")     ; UT_TRUE ( enumrecords::Parse(s, caseRead ) )    UT_TRUE( caseRead== lang::Case::Ignore   )  UT_EQ( A_CHAR("")        , s)
        caseRead= lang::Case(-1);       s= A_CHAR("abc")        ; UT_FALSE( enumrecords::Parse(s, caseRead ) )    UT_TRUE( caseRead== lang::Case(-1)       )  UT_EQ( A_CHAR("abc")     , s)

        lang::ContainerOp copRead;
        copRead= lang::ContainerOp(-1); s= A_CHAR("G")          ; UT_TRUE ( enumrecords::Parse(s, copRead  ) )    UT_TRUE( copRead == lang::ContainerOp::Get       )  UT_EQ( A_CHAR("")        , s)
        copRead= lang::ContainerOp(-1); s= A_CHAR("Ge")         ; UT_TRUE ( enumrecords::Parse(s, copRead  ) )    UT_TRUE( copRead == lang::ContainerOp::Get       )  UT_EQ( A_CHAR("")        , s)
        copRead= lang::ContainerOp(-1); s= A_CHAR("GET")        ; UT_TRUE ( enumrecords::Parse(s, copRead  ) )    UT_TRUE( copRead == lang::ContainerOp::Get       )  UT_EQ( A_CHAR("")        , s)
        copRead= lang::ContainerOp(-1); s= A_CHAR("GETC")       ; UT_TRUE ( enumrecords::Parse(s, copRead  ) )    UT_TRUE( copRead == lang::ContainerOp::GetCreate )  UT_EQ( A_CHAR("")        , s)
        copRead= lang::ContainerOp(-1); s= A_CHAR("GETX")       ; UT_TRUE ( enumrecords::Parse(s, copRead  ) )    UT_TRUE( copRead == lang::ContainerOp::Get       )  UT_EQ( A_CHAR("X")       , s)

        BitsParsable bits;
        bits= BitsParsable(0);  UT_TRUE ( enumrecords::ParseBitwise( s= A_CHAR("onex")     , bits) )       UT_TRUE( bits== BitsParsable::One   )         UT_EQ( A_CHAR("x")      , s)
        bits= BitsParsable(0);  UT_FALSE( enumrecords::ParseBitwise( s= A_CHAR("murx")     , bits) )       UT_TRUE( bits== BitsParsable(0)     )         UT_EQ( A_CHAR("murx")   , s)
        bits= BitsParsable(0);  UT_TRUE ( enumrecords::ParseBitwise( s= A_CHAR("one,twox") , bits) )       UT_TRUE( bits== BitsParsable::One
    ...
    UT_METHOD( lang_enumops_Arithmetic )
    {
        ...
        {ALIB_LOCK_RECURSIVE_WITH( alib::monomem::GLOBAL_ALLOCATOR_LOCK )
            alib::enumrecords::bootstrap::Bootstrap<ArithmeticEnum>(
            {
...
#endif //  ALIB_UT_DOCS && ALIB_UT_ENUMRECORDS
