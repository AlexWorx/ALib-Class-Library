...
UT_METHOD( Std_String )
{
...
#if defined(_MSC_VER)
        //warning: character cannot be represented in the current code page
    ...
    std::u8string   u8Str( u8"Test \u03B1\u03B2\u03B3\u03B4\u03B5 * \U0001F609 * \U0001F607 * \U0001F603 * \U0001F60E" );
    ...
    AString test;
#if !defined(_MSC_VER)
    test._().Append( str.c_str() );                         UT_EQ( compare , test )
    test._().Append( str.c_str(), integer(str.length()));   UT_EQ( compare , test )
    test._() <<      str;                                   UT_EQ( compare , test )
    test._()._<CHK> (str);                                  UT_EQ( compare , test )
    test._()._<NC>  (str);                                  UT_EQ( compare , test )
#endif

    test._().Append( u8Str.c_str() );                       UT_EQ( compare , test )
  //test._().Append( u8Str.c_str(), integer(str.length())); UT_EQ( compare , test )
    test._() <<      u8Str;                                 UT_EQ( compare , test )
    test._()._<CHK> (u8Str);                                UT_EQ( compare , test )
    test._()._<NC>  (u8Str);                                UT_EQ( compare , test )
...
// wide string (test this only if the LOCALE was properly set)
    ...
    {
        test._().Append<CHK>(wStr.c_str(), integer(wStr.length()));     UT_EQ( compare, test )
        test._() <<          wStr;                                      UT_EQ( compare, test )
        test._()._<CHK>     (wStr);                                     UT_EQ( compare, test )
        test._()._<NC>      (wStr);                                     UT_EQ( compare, test )

    #if ALIB_SIZEOF_WCHAR_T == 4
        test._().Append(   u16Str.c_str(), integer(u16Str.length()));   UT_EQ( compare, test )
        test._() <<        u16Str;                                      UT_EQ( compare, test )
        test._()._<CHK>(   u16Str);                                     UT_EQ( compare, test )
        test._()._<NC>(    u16Str);                                     UT_EQ( compare, test )
    #else
        test._().Append(   u32Str.c_str(), integer(u32Str.length()));   UT_EQ( compare, test );
        test._() <<        u32Str;                                      UT_EQ( compare, test );
        test._()._<CHK>(   u32Str);                                     UT_EQ( compare, test );
        test._()._<NC>(    u32Str);                                     UT_EQ( compare, test );
    #endif
    }
#endif
...
UT_METHOD( Std_Other )
{
    ...
    // Creating a String from std::array
    {
        std::array<character, 3> vec= {'a', 'b', 'c', }; UT_EQ( String( A_CHAR("abc")  ), String( vec ) )
    }

    // appending std::array<char> to AString
    {
        AString as;
        std::array<char, 3> vec= {'a', 'b', 'c', };
        as << vec;
        UT_EQ( A_CHAR("abc")   , as )
    }

    // appending std::array<wchar_t> to AString
    {
        AString as;
        std::array<wchar_t, 3> vec= { 'a', 'b', 'c', };
        as << vec;
        UT_EQ( A_CHAR("abc")   , as )
    }

    // appending std::array<xchar> to AString
    {
        AString as;
        std::array<xchar, 3> vec= { A_XCHAR('a'), A_XCHAR('b'), A_XCHAR('c'), };

        as << vec;
        UT_EQ( A_CHAR("abc")   , as )
    }

    // Creating a String from std::span
    {
        char arr[] = {'a', 'b', 'c'};
        std::span<const char> span= arr; UT_EQ( NString( "abc"), NString( span ) )
    }

    // appending std::span<char> to AString
    {
        AString as;
                char arr[] = {'a', 'b', 'c'};
        std::span<const char> span= arr;
        as << span;
        UT_EQ( A_CHAR("abc")   , as )
    }

    // appending std::span<wchar_t> to AString
    {
        AString as;
        wchar_t arr[] = {'a', 'b', 'c'};
        std::span<const wchar_t> span= arr;
        as << span;
        UT_EQ( A_CHAR("abc")   , as )
    }

    // Creating a span from alib::String
    {
        AString as( A_CHAR("abc") );
        std::span<const character> span= as;
        UT_EQ( as, String( span ) )
    }
}
