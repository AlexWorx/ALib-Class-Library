...
struct DynInt
{
    ...
    {
        --instCounter;
        ALIB_ASSERT_ERROR( value != reinterpret_cast<int*>(64), "UT_HASHTABLE",
...

{
    ...
    if(expBucketCount != -1) { UT_EQ( uinteger(expBucketCount)                       , ht.BucketCount() )  }
...
void UT_HashSet(AWorxUnitTesting&  ut, TTable& hashSet)
{
    ...
    // automatic rehash
    {
        ...
        auto inserts= int( hashSet.MaxLoadFactor() * float( hashSet.BucketCount() )
                           - float(hashSet.Size())  );
...
void UT_HashMap(AWorxUnitTesting&  ut, TTable& hashMap)
{
    ...
    // automatic rehash
    {
        hashMap.Emplace( int(1 + hashMap.BucketCount()), 102 );  testConsistency(ut, hashMap,     3 ,  97,  0 );
        hashMap.Emplace( 1                             , 103 );  testConsistency(ut, hashMap,     4 ,  97,  0 );

        auto inserts= int(   hashMap.MaxLoadFactor() * float(hashMap.BucketCount() )
                           - float(hashMap.Size())                                     );
        for( int i= 0 ; i < inserts -1 ; ++i )
