...
ut.EQ( __FILE__, __LINE__,  0,  as.ParseInt()   );
    ...
    // int32
    {
        ...
        ms._(); ui= 0;       ms._( ui    );                UT_EQ(      ASTR("0"), ms );
        ms._(); ui= 0;       ms._( Format( ui, 1 ,nf) );   UT_EQ(      ASTR("0"), ms );
        ms._(); ui= 0;       ms._( Format( ui, 3 ,nf) );   UT_EQ(    ASTR("000"), ms );
        ms._(); ui= 10;      ms._( ui    );                UT_EQ(     ASTR("10"), ms );
        ms._(); ui= 100;     ms._( Format( ui, 1 ,nf) );   UT_EQ(    ASTR("100"), ms );
        ms._(); ui= 100;     ms._( Format( ui, 3 ,nf) );   UT_EQ(    ASTR("100"), ms );
        ms._(); ui= 100;     ms._( Format( ui, 4 ,nf) );   UT_EQ(   ASTR("0100"), ms );
        ms._(); ui= 23;      ms._( ui    );                UT_EQ(     ASTR("23"), ms );
        ms._(); ui= 99;      ms._( Format( ui, 5 ,nf) );   UT_EQ(  ASTR("00099"), ms );
        ms._();  i= 49;      ms._( i     );                UT_EQ(     ASTR("49"), ms );
        ms._();  i= -5;      ms._( Format ( i, 5 ,nf) );   UT_EQ(  ASTR("-0005"), ms );
        ms._();  i= -5324;   ms._( Format ( i, 2 ,nf) );   UT_EQ(  ASTR("-5324"), ms );
        ms._(); ui= std::numeric_limits<uint32_t>::max();    ms._(         ui            );   UT_EQ(    ASTR("4294967295"),  ms );
        ms._(); ui= std::numeric_limits<uint32_t>::min();    ms._(         ui            );   UT_EQ(             ASTR("0"),  ms );
        ms._();  i= std::numeric_limits<int32_t> ::max();    ms._(         i             );   UT_EQ(    ASTR("2147483647"),  ms );
        ms._();  i= std::numeric_limits<int32_t> ::min();    ms._(         i             );   UT_EQ(   ASTR("-2147483648"),  ms );

        ms._(); ui= std::numeric_limits<uint32_t>::max();    ms._( Format( ui ,&nfG    ) );   UT_EQ( ASTR("4,294,967,295"),  ms );
        ms._(); ui= std::numeric_limits<uint32_t>::min();    ms._( Format( ui ,&nfG    ) );   UT_EQ(             ASTR("0"),  ms );
        ms._();  i= std::numeric_limits<int32_t> ::max();    ms._( Format ( i ,&nfG    ) );   UT_EQ( ASTR("2,147,483,647"),  ms );
        ms._();  i= std::numeric_limits<int32_t> ::min();    ms._( Format ( i ,&nfG    ) );   UT_EQ(ASTR("-2,147,483,648"),  ms );

        ms._(); ui= std::numeric_limits<uint32_t>::max();    ms._( Format( ui , 12 ,nf ) );   UT_EQ(  ASTR("004294967295"),  ms );
        ms._(); ui= std::numeric_limits<uint32_t>::min();    ms._( Format( ui , 12 ,nf ) );   UT_EQ(  ASTR("000000000000"),  ms );
        ms._();  i= std::numeric_limits<int32_t> ::max();    ms._( Format ( i , 12 ,nf ) );   UT_EQ(  ASTR("002147483647"),  ms );
        ms._();  i= std::numeric_limits<int32_t> ::min();    ms._( Format ( i , 12 ,nf ) );   UT_EQ(  ASTR("-02147483648"),  ms );
    }
    ...
    // int64
    {
        ...
        ui= 0;       ms._()._( ui );                   UT_EQ(      ASTR("0"), ms );
        ui= 0;       ms._()._( Format( ui, 1  ,nf ));  UT_EQ(      ASTR("0"), ms );
        ui= 0;       ms._()._( Format( ui, 3  ,nf ));  UT_EQ(    ASTR("000"), ms );
        ui= 10;      ms._()._( ui );                   UT_EQ(     ASTR("10"), ms );
        ui= 100;     ms._()._( Format( ui, 1  ,nf ));  UT_EQ(    ASTR("100"), ms );
        ui= 100;     ms._()._( Format( ui, 3  ,nf ));  UT_EQ(    ASTR("100"), ms );
        ui= 100;     ms._()._( Format( ui, 4  ,nf ));  UT_EQ(   ASTR("0100"), ms );
        ui= 23;      ms._()._( ui );                   UT_EQ(     ASTR("23"), ms );
        ui= 99;      ms._()._( Format( ui, 5  ,nf ));  UT_EQ(  ASTR("00099"), ms );
         i= 49;      ms._()._(                 i   );  UT_EQ(     ASTR("49"), ms );
         i= -5;      ms._()._( Format ( i, 5  ,nf ));  UT_EQ(  ASTR("-0005"), ms );
         i= -5324;   ms._()._( Format ( i, 2  ,nf ));  UT_EQ(  ASTR("-5324"), ms );
        ui= std::numeric_limits<uint32_t>::max();    ms._()._(                ui     );  UT_EQ(    ASTR("4294967295"),  ms );
        ui= std::numeric_limits<uint32_t>::min();    ms._()._(                ui     );  UT_EQ(             ASTR("0"),  ms );
         i= std::numeric_limits<int32_t> ::max();    ms._()._(                i      );  UT_EQ(    ASTR("2147483647"),  ms );
         i= std::numeric_limits<int32_t> ::min();    ms._()._(                i      );  UT_EQ(   ASTR("-2147483648"),  ms );

        ui= std::numeric_limits<uint32_t>::max();    ms._()._( Format(   ui ,&nfG   ));  UT_EQ( ASTR("4,294,967,295"),  ms );
        ui= std::numeric_limits<uint32_t>::min();    ms._()._( Format(   ui ,&nfG   ));  UT_EQ(             ASTR("0"),  ms );
         i= std::numeric_limits<int32_t> ::max();    ms._()._( Format(   i  ,&nfG   ));  UT_EQ( ASTR("2,147,483,647"),  ms );
         i= std::numeric_limits<int32_t> ::min();    ms._()._( Format(   i  ,&nfG   ));  UT_EQ(ASTR("-2,147,483,648"),  ms );

        ui= std::numeric_limits<uint32_t>::max();    ms._()._( Format(ui, 12,nf));  UT_EQ(  ASTR("004294967295"),  ms );
        ui= std::numeric_limits<uint32_t>::min();    ms._()._( Format(ui, 12,nf));  UT_EQ(  ASTR("000000000000"),  ms );
         i= std::numeric_limits<int32_t> ::max();    ms._()._( Format(i , 12,nf));  UT_EQ(  ASTR("002147483647"),  ms );
         i= std::numeric_limits<int32_t> ::min();    ms._()._( Format(i , 12,nf));  UT_EQ(  ASTR("-02147483648"),  ms );

        ui= std::numeric_limits<uint64_t>::max();    ms._()._( Format(  ui,&nfG     ));  UT_EQ( ASTR("18,446,744,073,709,551,615"),  ms );
        ui= std::numeric_limits<uint64_t>::min();    ms._()._( Format(  ui,&nfG     ));  UT_EQ(                          ASTR("0"),  ms );
         i= std::numeric_limits<int64_t> ::max();    ms._()._( Format(  i ,&nfG     ));  UT_EQ(  ASTR("9,223,372,036,854,775,807"),  ms );
        ms.SetBuffer(20);
         i= std::numeric_limits<int64_t> ::min();    ms._()._( Format(  i ,&nfG     ));  UT_EQ( ASTR("-9,223,372,036,854,775,808"),  ms );
         i= std::numeric_limits<int64_t> ::min() + 75008; ms._()._(Format(i,&nfG    ));  UT_EQ( ASTR("-9,223,372,036,854,700,800"),  ms );
    }
    ...
    // plus sign
    {
        ...
        i= 49;                            ms._()._( i );                  UT_EQ(     ASTR("49"), ms );
        i= 49;      nf.PlusSign=' '; ms._()._( Format ( i, -1, &nf  ) );  UT_EQ(    ASTR(" 49"), ms );
        i= 49;      nf.PlusSign='+'; ms._()._( Format ( i, -1, &nf  ) );  UT_EQ(    ASTR("+49"), ms );
        i= 49;      nf.PlusSign=' '; ms._()._( Format ( i,  5, &nf  ) );  UT_EQ(  ASTR("  049"), ms );
        i= 49;      nf.PlusSign='+'; ms._()._( Format ( i,  5, &nf  ) );  UT_EQ(  ASTR("+ 049"), ms );
    }
    ...
    // Group character
    {
        ...
        w=  3; ms._(); ui=              100;      ms._( Format( ui , w, &nf  ) );   UT_EQ(                ASTR("100"),  ms ); UT_EQ( w, ms.Length() );
        w=  4; ms._(); ui=              100;      ms._( Format( ui , w, &nf  ) );   UT_EQ(               ASTR(" 100"),  ms ); UT_EQ( w, ms.Length() );
        w=  5; ms._(); ui=              100;      ms._( Format( ui , w, &nf  ) );   UT_EQ(              ASTR("0,100"),  ms ); UT_EQ( w, ms.Length() );
        w=  7; ms._(); ui=           100000;      ms._( Format( ui , w, &nf  ) );   UT_EQ(            ASTR("100,000"),  ms ); UT_EQ( w, ms.Length() );
        w=  8; ms._(); ui=           100000;      ms._( Format( ui , w, &nf  ) );   UT_EQ(           ASTR(" 100,000"),  ms ); UT_EQ( w, ms.Length() );
        w=  9; ms._(); ui=           100000;      ms._( Format( ui , w, &nf  ) );   UT_EQ(          ASTR("0,100,000"),  ms ); UT_EQ( w, ms.Length() );
        w= 11; ms._(); ui=        100000000;      ms._( Format( ui , w, &nf  ) );   UT_EQ(        ASTR("100,000,000"),  ms ); UT_EQ( w, ms.Length() );
        w= 12; ms._(); ui=        100000000;      ms._( Format( ui , w, &nf  ) );   UT_EQ(       ASTR(" 100,000,000"),  ms ); UT_EQ( w, ms.Length() );
        w= 13; ms._(); ui=        100000000;      ms._( Format( ui , w, &nf  ) );   UT_EQ(      ASTR("0,100,000,000"),  ms ); UT_EQ( w, ms.Length() );
        w= 15; ms._(); ui=     100000000000;      ms._( Format( ui , w, &nf  ) );   UT_EQ(    ASTR("100,000,000,000"),  ms ); UT_EQ( w, ms.Length() );
        w= 16; ms._(); ui=     100000000000;      ms._( Format( ui , w, &nf  ) );   UT_EQ(   ASTR(" 100,000,000,000"),  ms ); UT_EQ( w, ms.Length() );
        w= 17; ms._(); ui=     100000000000;      ms._( Format( ui , w, &nf  ) );   UT_EQ(  ASTR("0,100,000,000,000"),  ms ); UT_EQ( w, ms.Length() );

        w=  3; ms._(); i=     -         100;      ms._( Format(  i , w, &nf  ) );   UT_EQ(               ASTR("-100"),  ms ); UT_EQ( w+1, ms.Length() );
        w=  4; ms._(); i=     -         100;      ms._( Format(  i , w, &nf  ) );   UT_EQ(               ASTR("-100"),  ms ); UT_EQ( w  , ms.Length() );
        w=  5; ms._(); i=     -         100;      ms._( Format(  i , w, &nf  ) );   UT_EQ(              ASTR("- 100"),  ms ); UT_EQ( w  , ms.Length() );
        w=  6; ms._(); i=     -         100;      ms._( Format(  i , w, &nf  ) );   UT_EQ(             ASTR("-0,100"),  ms ); UT_EQ( w  , ms.Length() );
        w=  7; ms._(); i=     -      100000;      ms._( Format(  i , w, &nf  ) );   UT_EQ(           ASTR("-100,000"),  ms ); UT_EQ( w+1, ms.Length() );
        w=  8; ms._(); i=     -      100000;      ms._( Format(  i , w, &nf  ) );   UT_EQ(           ASTR("-100,000"),  ms ); UT_EQ( w  , ms.Length() );
        w=  9; ms._(); i=     -      100000;      ms._( Format(  i , w, &nf  ) );   UT_EQ(          ASTR("- 100,000"),  ms ); UT_EQ( w  , ms.Length() );
        w= 10; ms._(); i=     -      100000;      ms._( Format(  i , w, &nf  ) );   UT_EQ(         ASTR("-0,100,000"),  ms ); UT_EQ( w  , ms.Length() );
        w= 11; ms._(); i=     -   100000000;      ms._( Format(  i , w, &nf  ) );   UT_EQ(       ASTR("-100,000,000"),  ms ); UT_EQ( w+1, ms.Length() );
        w= 12; ms._(); i=     -   100000000;      ms._( Format(  i , w, &nf  ) );   UT_EQ(       ASTR("-100,000,000"),  ms ); UT_EQ( w  , ms.Length() );
        w= 13; ms._(); i=     -   100000000;      ms._( Format(  i , w, &nf  ) );   UT_EQ(      ASTR("- 100,000,000"),  ms ); UT_EQ( w  , ms.Length() );
        w= 14; ms._(); i=     -   100000000;      ms._( Format(  i , w, &nf  ) );   UT_EQ(     ASTR("-0,100,000,000"),  ms ); UT_EQ( w  , ms.Length() );
        w= 15; ms._(); i=     -100000000000;      ms._( Format(  i , w, &nf  ) );   UT_EQ(   ASTR("-100,000,000,000"),  ms ); UT_EQ( w+1, ms.Length() );
        w= 16; ms._(); i=     -100000000000;      ms._( Format(  i , w, &nf  ) );   UT_EQ(   ASTR("-100,000,000,000"),  ms ); UT_EQ( w  , ms.Length() );
        w= 17; ms._(); i=     -100000000000;      ms._( Format(  i , w, &nf  ) );   UT_EQ(  ASTR("- 100,000,000,000"),  ms ); UT_EQ( w  , ms.Length() );
        w= 18; ms._(); i=     -100000000000;      ms._( Format(  i , w, &nf  ) );   UT_EQ( ASTR("-0,100,000,000,000"),  ms ); UT_EQ( w  , ms.Length() );
    }
}
...
UT_METHOD( ConvertIntegers )
{
    ...
    {
        {
            int result;
            integer newIdx;
            Substring substring(ASTR("0x"));
            UT_TRUE( substring.ConsumeInt( result ));
            UT_EQ( 0, result );
            UT_EQ( ASTR("x"), substring);

            AString astring(ASTR("0x"));
        ...
        }
        {
            int result;
            integer newIdx;
            Substring substring(ASTR("0xy"));
            UT_TRUE( substring.ConsumeInt( result ));
            UT_EQ( 0, result );
            UT_EQ( ASTR("xy"), substring);

            AString astring(ASTR("0xy"));
            UT_EQ( 0L, astring.ParseInt( &newIdx ));
            UT_EQ( 1, newIdx );
        }
        {
            ...
            Substring substring= String(ASTR("0x1234")).Substring<false>( 0, 2);
            UT_TRUE( substring.ConsumeInt( result ));
            UT_EQ( 0, result );
            UT_EQ( ASTR("x"), substring );

            AString astring(ASTR("0x1234")); astring.SetLength(2);
            UT_EQ( 0L,  astring.ParseInt( &newIdx ));
            UT_EQ( 1,  newIdx );
        }
        ...
        }
        {
            ...
            Substring substring= String(ASTR("  0x1234")).Substring<false>( 0, 4);
            UT_TRUE( substring.ConsumeInt( result ));
            UT_EQ( 0, result );
            UT_EQ( ASTR("x"), substring );

            AString astring(ASTR("  0x1234")); astring.SetLength(4);
            UT_EQ( 0L,  astring.ParseInt( &newIdx ));
            UT_EQ( 3,  newIdx );
        }
    }
    ...
    intTestDetection( ut, ASTR(" - 0xF")                  , -15          , true  , -1);
    intTestDetection( ut, ASTR(" - 0b1")                  ,  -1          , true  , -1);
    intTestDetection( ut, ASTR(" 0x F12A")                ,   0          , true  ,  2);
    intTestDetection( ut, ASTR(" 0zF12A")                 ,   0          , true  ,  2);
    intTestDetection( ut, ASTR(" - 0xF")                  , -0xF         , true  , -1);
    intTestDetection( ut, ASTR(" - 0b1")                  ,  -1          , true  , -1);
    intTestDetection( ut, ASTR(" - 0b111")                ,  -7          , true  , -1);
    intTestDetection( ut, ASTR(" 0b111F")                 ,   7          , true  ,  6);
    intTestDetection( ut, ASTR("0o77")                    ,  63          , true  , -1);
    intTestDetection( ut, ASTR(" \t 0o777")               ,  511         , true  , -1);
    intTestDetection( ut, ASTR(" \t0o777")                ,  511         , true  , -1);
    intTestDetection( ut, ASTR(" \t-0o777")               , -511         , true  , -1);
    intTestDetection( ut, ASTR(" -\t0o777")               , -511         , true  , -1);
    intTestDetection( ut, ASTR(" -FF")                    ,  0           , false ,  0);
...
UT_METHOD( ConvertFloats )
{
    ...
    // parse NaN and INF
    {
        as._()._( ASTR("nan"));    UT_TRUE( isnan( as.ParseFloat( &pos ) ) );             UT_EQ( 3, pos );
        as._()._(ASTR("-nan"));    UT_TRUE( isnan( as.ParseFloat( &pos ) ) );             UT_EQ( 4, pos );
        as._()._( ASTR("nAN"));    UT_TRUE( isnan( as.ParseFloat( &pos ) ) );             UT_EQ( 3, pos );
        as._()._(ASTR("-nAN"));    UT_TRUE( isnan( as.ParseFloat( &pos ) ) );             UT_EQ( 4, pos );

        as._()._( ASTR("inf"));    UT_TRUE( isinf( as.ParseFloat( &pos ) ) );             UT_EQ( 3, pos );
        as._()._(ASTR("-inf"));    UT_TRUE( isinf( as.ParseFloat( &pos ) ) );             UT_EQ( 4, pos );
            ...
            as._()._( ASTR("INf"));    UT_TRUE( as.ParseFloat( &pos ) ==  std::numeric_limits<double>::infinity() );  UT_EQ( 3, pos );
            as._()._(ASTR("-INf"));    UT_TRUE( as.ParseFloat( &pos ) == -std::numeric_limits<double>::infinity() );  UT_EQ( 4, pos );
    ...
    // write NaN and INF
    {
        ...
        as._()._( -std::numeric_limits<double>::infinity()  );    UT_EQ( String16(ASTR("-"))._(NumberFormat::Global.INFLiteral), as );
    }

    // write and parse doubles
    {
        NumberFormat nf;
        nf.DecimalPointChar= '.';  as._()._( ASTR("12345.789"));      UT_EQ( 12345.789,  as.ParseFloat( 0, &nf, &pos ) );   UT_EQ(  0 + as.Length() , pos );
        nf.DecimalPointChar= '@';  as._()._( ASTR("12345@789"));      UT_EQ( 12345.789,  as.ParseFloat( 0, &nf, &pos ) );   UT_EQ(  0 + as.Length() , pos );
        nf.DecimalPointChar= '.';  as._()._( ASTR("12345@789"));      UT_EQ( 12345.0  ,  as.ParseFloat( 0, &nf, &pos ) );   UT_EQ(  5               , pos );
    ...
    // write and parse doubles, non scientific mode
    {
        ...
        {
            NumberFormat nf;
            nf.ExponentSeparator= ASTR("*10^");
            String64 s;
            double v= 7.5E42;
            s._( Format(v, &nf) );
            UT_EQ( ASTR("7.5*10^42"), String(s)  );
...
UT_METHOD( FormatterJavaStyle )
{
    ...
    //===== Hash value 'h'/'H' =========
    NAString tExpect;
    tExpect._() << lib::strings::FormatBase<char>::Hex(reinterpret_cast<uinteger>( &formatterJS ) ) ;
...
UT_METHOD( FormatterPythonStyle )
{
    ...
    //================================== Mixed tests ==============================================
    {
        ...
        AString as;   as << c << '/' << sChar << '/' << uChar << '/' << sChar2; UT_EQ( ASTR("x/121/122/-5"), as );
