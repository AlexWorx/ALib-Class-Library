...
UT_METHOD( CharAt)
{
    ...
    // Delete first
    {
        ...
        subs.ConsumeChars<false>        (2) ;        UT_TRUE( subs == A_CHAR("34567890")  );
        subs.ConsumeCharsFromEnd <false>(3) ;        UT_TRUE( subs == A_CHAR("34567")     );
        subs.ConsumeChars               (2) ;        UT_TRUE( subs == A_CHAR("567")    );
        subs.ConsumeCharsFromEnd        (3) ;        UT_TRUE( subs == A_CHAR("")        );
        subs.ConsumeChars<false>        (0) ;        UT_TRUE( subs == A_CHAR("")        );
        subs.ConsumeCharsFromEnd <false>(0) ;        UT_TRUE( subs == A_CHAR("")        );
...
UT_METHOD( ConsumeNumbers )
{
    ...
    // ConsumeFloat()
    {
        ...
        NumberFormat nf;    nf.Whitespaces= A_CHAR(" ;");
        subs= A_CHAR(" 42.3 ; 0.7 ; 6 ") ; UT_EQ( true ,  subs.ConsumeFloat  ( result, &nf ) );   UT_EQ(    42.3,  result );
...
UT_EQ( A_CHAR("word2 word3"), s );
...

    {
        ...
        UT_EQ( 0, subs.ConsumePartOf(EmptyString()));           UT_EQ(           A_CHAR(""), subs );
    }
