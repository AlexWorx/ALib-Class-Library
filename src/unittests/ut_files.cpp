...
UT_METHOD(FileAndFTree)
{
    ...
    {ALIB_LOCK_RECURSIVE_WITH(lang::format::Formatter::DefaultLock)
        String256 target;
        lang::format::Formatter::Default->Format(target, "{:ta h on gn s dm nal}", file);
        UT_TRUE(target.EndsWith(A_CHAR("expression.hpp")))
        String256 target2;
        lang::format::Formatter::Default->Format(target2, "{}", file);
    ...
    file.Format(A_CHAR("'Size: 's"                             ),fmt,cdc);   UT_EQ(A_CHAR("Size:        15.669KiB"       ), fmt )
    file.Format(A_CHAR("'Size: 's(KiB)"                        ),fmt,cdc);   UT_EQ(A_CHAR("Size:        15.669"          ), fmt )
    file.Format(A_CHAR("'Size: 's(B)"                          ),fmt,cdc);   UT_EQ(A_CHAR("Size:   16,045"               ), fmt )
    file.Format(A_CHAR("'Size: 's(B){15,c}"                    ),fmt,cdc);   UT_EQ(A_CHAR("Size:      16,045    "        ), fmt )
    file.Format(A_CHAR("'Size: 's(iec)"                        ),fmt,cdc);   UT_EQ(A_CHAR("Size:        15.669KiB"       ), fmt)
    file.Format(A_CHAR("'Size: 's(SI)"                         ),fmt,cdc);   UT_EQ(A_CHAR("Size:        16.045kB"        ), fmt)
    file.Format(A_CHAR("'Size: 's(mb)"                         ),fmt,cdc);   UT_EQ(A_CHAR("Size:         0.016"          ), fmt)
    file.Format(A_CHAR("'Size: 's(mib)"                        ),fmt,cdc);   UT_EQ(A_CHAR("Size:         0.015"          ), fmt)
...
// This test would fail on many setups
   ...
   { ALIB_LOCK_WITH(ftree) ftree.Reset(); ftree->MonitorFilesByName( lang::ContainerOp::Insert,
       &secondListener, files::FTreeListener::Event::CreateNode, A_PATH("expression.hpp")); }
   testFScanListener(ut, 3, 36, 0, 1 );
   ...
   testFScanListener(ut, 3, 36, 4, 36 );
...
//   ftree.Reset(); { ALIB_LOCK_WITH(ftree) ftree->MonitorPathSubstring(lang::ContainerOp::Insert, &secondListener, files::FTreeListener::Event::CreateNode, "detail"); }
   testFScan(ut, A_CHAR("name==\"detail\"")      , A_CHAR("name * \"*.hpp\"")          , 3 , 11 );//, false, 0,5);
   ...
   testFScan(ut, nullptr, A_CHAR("date > DateTime(2020 , 1, 1) &&  date < today + days(1)"), 3,  36 );
   testFScan(ut, nullptr, A_CHAR("date > today + days(1)")                                 , 0,   0 );
   testFScan(ut, nullptr, A_CHAR("mdate > DateTime(2020, 1, 1) && mdate < today+ days(1)") , 3,  36 );
   ...
   testFScan(ut, nullptr, A_CHAR("md    > today + days(1)")                                , 0,   0 );
 //tstDoDump= true;
   testFScan(ut, nullptr, A_CHAR("adate > DateTime(2020, 1, 1) && adate < today+ days(1)") , 3,  36 );
