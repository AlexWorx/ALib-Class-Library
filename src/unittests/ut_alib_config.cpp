...
#if !defined(ALIB_UT_SELECT) || defined(ALIB_UT_CONFIG)
...
UT_METHOD(ConfigIniFiles)
{
   ...
   ;
    ...
    ALIB.Config->Load( var.Declare( EmptyString(),            A_CHAR("CUBA")              ) );   UT_EQ( A_CHAR("a country")  , var.GetString() )
    ALIB.Config->Load( var.Declare( EmptyString(),            A_CHAR("cUbA")              ) );   UT_EQ( A_CHAR("a country")  , var.GetString() )
    ALIB.Config->Load( var.Declare( EmptyString(),            A_CHAR("SIZE")              ) );   UT_EQ( A_CHAR("25")         , var.GetString() )
    ALIB.Config->Load( var.Declare( EmptyString(),            A_CHAR("concat"), ','       ) );   UT_EQ( 11 , var.Size())
                                                                                                 UT_EQ( A_CHAR("start =5")   , var.GetString(0))
                                                                                                 UT_EQ( A_CHAR("end   =32")  , var.GetString(1))
    ALIB.Config->Load( var.Declare( A_CHAR("Great Section"),  A_CHAR("SectionVar")        ) );   UT_EQ( A_CHAR("5")          , var.GetString() )
    ALIB.Config->Load( var.Declare( A_CHAR("2nd Section"),    A_CHAR("SectionVar")        ) );   UT_EQ( A_CHAR("6")          , var.GetString() )
    ALIB.Config->Load( var.Declare( A_CHAR("Great Section"),  A_CHAR("SECTION_CONTINUED") ) );   UT_EQ( A_CHAR("yEs")        , var.GetString() )
    ALIB.Config->Load( var.Declare( A_CHAR("Great Section"),  A_CHAR("Tricky")            ) );   UT_EQ( A_CHAR("backslash\\"), var.GetString() )
    ALIB.Config->Load( var.Declare( A_CHAR("Great Section"),  A_CHAR("SECTION_CONTINUED") ) );   UT_TRUE( var.IsTrue() );
    ...
    ALIB.Config->Load  ( var.Declare(EmptyString(), HOME_ENV_NAME) ); UT_EQ( Priorities::Environment, var.Priority );
    ...
    UT_EQ( Priorities::Standard, ALIB.Config->Store( var, A_CHAR("new") ) );
    ALIB.Config->Load  ( var.Declare(A_CHAR("New Section"),  A_CHAR("newvar")) );  UT_EQ( A_CHAR("new"),   var.GetString() )

    var.Declare( EmptyString(),             A_CHAR("newvar"));
    var.Priority= Priorities::Standard;
    UT_EQ( Priorities::Standard, ALIB.Config->Store( var, A_CHAR("aworx")) );
    ALIB.Config->Load  ( var.Declare(EmptyString(),             A_CHAR("newvar")) );  UT_EQ( A_CHAR("aworx"), var.GetString() )
    ...
    var.Priority= Priorities::Standard;
    UT_EQ( Priorities::Standard, ALIB.Config->Store(var) );
    ALIB.Config->Load (  var.Declare( EmptyString(),  A_CHAR("newvarList"))   );


    var.Declare( EmptyString(),   A_CHAR("commented"), ',', A_CHAR("2lines"));
    var.Priority= Priorities::Standard;
    UT_EQ( Priorities::Standard, ALIB.Config->Store(  var, A_CHAR("this is c-line 1 \nand this line 2") ) )
    ...
    ALIB.Config->InsertPlugin( &readBack, Priorities::Standard );
    ALIB.Config->Load( var.Declare( A_CHAR("New Section"),  A_CHAR("newvar")  ) );   UT_EQ( A_CHAR("new")  , var.GetString() )
    ALIB.Config->Load( var.Declare( EmptyString(),          A_CHAR("newvar")  ) );   UT_EQ( A_CHAR("aworx"), var.GetString() )
...
UT_METHOD(ConfigDefaultAndProtected)
{
    ...
    var.DefaultValue._( A_CHAR("Default")  );
    ALIB.Config->StoreDefault(var, A_CHAR("def par"));        UT_EQ( A_CHAR("def par"),   var.GetString() );  UT_EQ( Priorities::DefaultValues   ,var.Priority );

    var.ClearValues();
    var.Add( A_CHAR("def var") );
    ALIB.Config->StoreDefault( var );                 UT_EQ( A_CHAR("def var"),   var.GetString() );  UT_EQ( Priorities::DefaultValues   ,var.Priority );

    var.ClearValues();
    ALIB.Config->StoreDefault( var );                 UT_EQ( A_CHAR("Default"),   var.GetString() );  UT_EQ( Priorities::DefaultValues   ,var.Priority );

    var.ClearValues();
    var.Add( A_CHAR("def var") );
    ALIB.Config->Protect( var );                      UT_EQ( A_CHAR("def var"),   var.GetString() );  UT_EQ( Priorities::ProtectedValues ,var.Priority );
    ALIB.Config->Protect( var,A_CHAR("prot par"));            UT_EQ( A_CHAR("prot par"),  var.GetString() );  UT_EQ( Priorities::ProtectedValues ,var.Priority );
    var.ClearValues();
    ALIB.Config->Protect( var );                      UT_EQ( A_CHAR("Default"),   var.GetString() );  UT_EQ( Priorities::ProtectedValues ,var.Priority );
    var.DefaultValue.SetNull();
    var.ClearValues();
    ALIB.Config->Protect( var );                      UT_EQ( 0, var.Size()                );  UT_EQ( Priorities::ProtectedValues ,var.Priority );
    ALIB.Config->Load( var );                         UT_EQ( A_CHAR("Default"),   var.GetString() );  UT_EQ( Priorities::DefaultValues   ,var.Priority );
...
UT_METHOD(ConfigIteration)
{
    ...
    std::vector<String> args= {A_CHAR("--ALOX_CONSOLE_TYPE=overwritten")};
    cliArgs.SetArgs( &args );
    lib::ALOX.Config->InsertPlugin( &cliArgs, Priorities::CLI + 1 );

    auto* it= lib::ALOX.Config->GetIterator(A_CHAR("ALOX"));
    int cntVars= 0;
...
#endif // !defined(ALIB_UT_SELECT) || defined(ALIB_UT_CONFIG)
