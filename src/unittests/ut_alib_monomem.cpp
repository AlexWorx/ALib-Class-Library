...
#define TESTCLASSNAME       CPP_ALib_Memory
...
// when unit tests are run with a memory checker (e.g. valgrind).
        ...
        class CharSta
        {
            ...
            char val;
            public:
            CharSta() = default; //: val(0) {}
            CharSta( char c ) : val( c )
            {}
...
//char Value( char* c )    { return *c; }
        char Value( CharSta & c )
        { return c.Value(); }

//char Value( CharSta* c ) { return c->Value(); }
...
#undef DBGOUT
        ...
        static_assert( std::is_trivial<CharSta>::value, "error" );
        static_assert( std::is_trivially_default_constructible<CharSta>::value, "error" );
        static_assert( std::is_trivially_constructible<CharSta>::value, "error" );
        static_assert( std::is_trivially_copy_constructible<CharSta>::value, "error" );
        static_assert( std::is_trivially_move_constructible<CharSta>::value, "error" );
        static_assert( std::is_trivially_copyable<CharSta>::value, "error" );
        static_assert( std::is_trivially_copy_assignable<CharSta>::value, "error" );
        static_assert( std::is_trivially_move_assignable<CharSta>::value, "error" );


        static_assert( std::is_copy_assignable<CharSta>::value, "error" );
        static_assert( std::is_copy_assignable<CharDyn>::value, "error" );
        static_assert( std::is_nothrow_copy_assignable<CharSta>::value, "error" );
        static_assert( std::is_nothrow_copy_assignable<CharDyn>::value, "error" );

        static_assert( std::is_copy_constructible<CharSta>::value, "error" );
        static_assert( std::is_copy_constructible<CharDyn>::value, "error" );

        static_assert( std::is_nothrow_copy_constructible<CharSta>::value, "error" );
        static_assert( std::is_nothrow_copy_constructible<CharDyn>::value, "error" );



        static_assert( std::is_move_assignable<CharSta>::value, "error" );
        static_assert( std::is_move_assignable<CharDyn>::value, "error" );
        static_assert( std::is_nothrow_move_assignable<CharSta>::value, "error" );
        static_assert( std::is_nothrow_move_assignable<CharDyn>::value, "error" );

        static_assert( std::is_move_constructible<CharSta>::value, "error" );
        static_assert( std::is_move_constructible<CharDyn>::value, "error" );

        static_assert( std::is_nothrow_move_constructible<CharSta>::value, "error" );
...
DOX_MARKER( [DOX_ALIB_MONOMEM_STDCONTMA_PLACEMENT_NEW] )
    ...
    void ListTest( AWorxUnitTesting & ut )
    {
        ...
        list.EmplaceBack( 'a' );
        testCharList( ut, list, "a", 0 );
        list.Erase( list.begin() );
        testCharList( ut, list, "", 1 );

        list.EmplaceBack( 'a' );
        testCharList( ut, list, "a", 0 );
        list.EmplaceBack( 'b' );
        testCharList( ut, list, "ab", 0 );
        list.Erase( list.begin() );
        testCharList( ut, list, "b", 1 );
        list.Erase( list.begin() );
        testCharList( ut, list, "", 2 );

        list.EmplaceBack( 'a' );
        testCharList( ut, list, "a", 1 );
        list.EmplaceBack( 'b' );
        testCharList( ut, list, "ab", 0 );
        list.Erase( --list.end() );
        testCharList( ut, list, "a", 1 );
        ...
        testCharList( ut, list, "ab", 0 );
        list.EmplaceBack( 'c' );
        testCharList( ut, list, "abc", 0 );
        list.Erase( list.begin() );
        testCharList( ut, list, "bc", 1 );
        list.Erase( list.begin() );
        testCharList( ut, list, "c", 2 );
        list.Erase( list.begin() );
        testCharList( ut, list, "", 3 );

        list.EmplaceBack( 'a' );
        testCharList( ut, list, "a", 2 );
        list.EmplaceBack( 'b' );
        testCharList( ut, list, "ab", 1 );
        list.EmplaceBack( 'c' );
        testCharList( ut, list, "abc", 0 );
        list.Erase( --list.end() );
        testCharList( ut, list, "ab", 1 );
        ...
        UT_EQ( 'c', Value( *it ) )
        it = list.Erase( list.begin(), list.end() );
        UT_TRUE( it == list.begin() )
        UT_TRUE( it == list.end() )

        UT_EQ( 'a', Value( list.EmplaceBack( 'a' ) ) )
        testCharList( ut, list, "a", 2 );
        UT_EQ( 'b', Value( list.EmplaceBack( 'b' ) ) )
        testCharList( ut, list, "ab", 1 );
        UT_EQ( 'c', Value( list.EmplaceBack( 'c' ) ) )
        testCharList( ut, list, "abc", 0 );
        it = list.Erase( ++list.begin(), list.end() );
        testCharList( ut, list, "a", 2 );
        UT_TRUE( it == list.end() )
        ...
        it = list.Emplace( list.begin(), 'c' );
        testCharList( ut, list, "c", 2 );
        UT_EQ( 'c', Value( *it ) )
        it = list.Emplace( list.begin(), 'b' );
        testCharList( ut, list, "bc", 1 );
        UT_EQ( 'b', Value( *it ) )
        it = list.Emplace( list.begin(), 'a' );
        testCharList( ut, list, "abc", 0 );
        UT_EQ( 'a', Value( *it ) )
        list.Clear();
        testCharList( ut, list, "", 3 );
        it = list.Emplace( list.begin(), 'b' );
        testCharList( ut, list, "b", 2 );
        UT_EQ( 'b', Value( *it ) )
        it = list.Emplace( list.end(), 'c' );
        testCharList( ut, list, "bc", 1 );
        UT_EQ( 'c', Value( *it ) )
        it = list.Emplace( list.begin(), 'a' );
        testCharList( ut, list, "abc", 0 );
        UT_EQ( 'a', Value( *it ) )
        it = list.Emplace( list.end(), 'd' );
        testCharList( ut, list, "abcd", 0 );
        UT_EQ( 'd', Value( *it ) )
        list.Reset();
        testCharList( ut, list, "", 0 );
        it = list.Emplace( list.begin(), 'a' );
        testCharList( ut, list, "a", 0 );
        UT_EQ( 'a', Value( *it ) )
        it = list.Emplace( ++it, 'b' );
        testCharList( ut, list, "ab", 0 );
        UT_EQ( 'b', Value( *it ) )
        it = list.Emplace( ++it, 'c' );
        testCharList( ut, list, "abc", 0 );
        UT_EQ( 'c', Value( *it ) )
        list.Clear();
        testCharList( ut, list, "", 3 );
        ...
        list.ReserveRecyclables( 5 );
        testCharList( ut, list, "", 5 );
        list.EmplaceBack( 'd' );
        testCharList( ut, list, "d", 4 );
        list.PushBack( 'e' );
        testCharList( ut, list, "de", 3 );
        list.PushFront( 'b' );
        testCharList( ut, list, "bde", 2 );
        list.Insert( ++list.begin(), 'c' );
        testCharList( ut, list, "bcde", 1 );
        list.Insert( list.end(), 'f' );
        testCharList( ut, list, "bcdef", 0 );
        list.EmplaceFront( 'a' );
        testCharList( ut, list, "abcdef", 0 );
        list.Insert( it = list.end(), 'g' );
        testCharList( ut, list, "abcdefg", 0 );
        list.Insert( it, 'h' );
        testCharList( ut, list, "abcdefgh", 0 );

        it = list.Erase( ++ ++list.begin(),
                         -- --list.end() );
        testCharList( ut, list, "abgh", 4 );
        UT_EQ( 'g', Value( *it ) )
    }
...
// which is ok in general, but not for the unit tests, which rely on identiy-hashing of ints.
struct IntHash
{
...
bool operator==( const DynInt& lhs, int rhs) { return *lhs.value == rhs; }
bool operator!=( const DynInt& lhs, int rhs) { return *lhs.value != rhs; }
bool operator==( int lhs, const DynInt& rhs) { return lhs == *rhs.value; }
bool operator!=( int lhs, const DynInt& rhs) { return lhs != *rhs.value; }
...

{
    ...
    if( expRecycables   != -1 ) { UT_EQ(                       expRecycables  , map.RecyclablesCount() )  }
...
void UT_HashSet(AWorxUnitTesting&  ut, TTable& hashSet)
{
    ...
    // manual rehash
    {
        hashSet.Rehash( 1 );              testConsistency(ut, hashSet,     2 ,  11,  0 );
        hashSet.Rehash( 50 );
        auto cntRecylables= hashSet.RecyclablesCount(); UT_TRUE( cntRecylables > 0 )
                                          testConsistency(ut, hashSet,     2 ,  97,  cntRecylables );
        hashSet.Rehash(1);
        UT_TRUE( cntRecylables < hashSet.RecyclablesCount() )
                 cntRecylables=  hashSet.RecyclablesCount();
                                          testConsistency(ut, hashSet,     2 ,  11,  cntRecylables );
        hashSet.Clear();                  testConsistency(ut, hashSet,     0 ,  11,  cntRecylables + 2 );
        hashSet.Emplace( 1 );             testConsistency(ut, hashSet,     1 ,  11,  cntRecylables + 1 );
        hashSet.Emplace( 2 );             testConsistency(ut, hashSet,     2 ,  11,  cntRecylables     );
        hashSet.Erase  ( 1 );             testConsistency(ut, hashSet,     1 ,  11,  cntRecylables + 1 );
        hashSet.Reserve( 5 );             testConsistency(ut, hashSet,     1 ,  11,  cntRecylables + 1 );
        hashSet.ReserveRecyclables( 5 );  testConsistency(ut, hashSet,     1 ,  11,  cntRecylables + 1 );
        hashSet.Emplace( 3 );             testConsistency(ut, hashSet,     2 ,  11,  cntRecylables + 0 );
        hashSet.Emplace( 4 );             testConsistency(ut, hashSet,     3 ,  11,  cntRecylables - 1 );
        hashSet.Emplace( 5 );             testConsistency(ut, hashSet,     4 ,  11,  cntRecylables - 2 );
        hashSet.Emplace( 6 );             testConsistency(ut, hashSet,     5 ,  11,  cntRecylables - 3 );
        hashSet.Emplace( 7 );             testConsistency(ut, hashSet,     6 ,  11,  cntRecylables - 4 );
        auto
        it= hashSet.Find( 2 );           UT_TRUE(  *it == 2 )
        it= hashSet.Find( 3 );           UT_TRUE(  *it == 3 )
        it= hashSet.Find( 4 );           UT_TRUE(  *it == 4 )
        it= hashSet.Find( 5 );           UT_TRUE(  *it == 5 )
        it= hashSet.Find( 6 );           UT_TRUE(  *it == 6 )
        it= hashSet.Find( 7 );           UT_TRUE(  *it == 7 )
    }

    // automatic rehash
    {
        auto inserts= static_cast<int>(   hashSet.MaxLoadFactor() * hashSet.BucketCount()
                                        - hashSet.Size()                                     );
        ...
        hashSet.Rehash( 1 );              testConsistency(ut, hashSet,    22 ,  11,  -1 );
    }

    // EqualRange
    auto cntRecylables= hashSet.RecyclablesCount();
    {
        cntRecylables+= hashSet.Size();
        ...
        hashSet.Emplace(  1 );
        hashSet.Emplace( 12 );            testConsistency(ut, hashSet,     6 ,  11,  cntRecylables - 6 );
...
void UT_HashMap(AWorxUnitTesting&  ut, TTable& hashMap)
{
    ...
    // automatic rehash
    {
        ...
        auto inserts= static_cast<int>(   hashMap.MaxLoadFactor() * hashMap.BucketCount()
                                        - hashMap.Size()                                     );
        for( int i= 0 ; i < inserts -1 ; ++i )
    ...
    // manual rehash
    {
        hashMap.Rehash( 1 );              UT_EQ (static_cast<uinteger>(hashMap.Size() / hashMap.MaxLoadFactor()),
                                          hashMap.BucketCount() )
                                          testConsistency(ut, hashMap,     22 ,  11,  -1 );

        hashMap.Rehash( 50 );             testConsistency(ut, hashMap,     22 ,  97,  -1 );
        hashMap.Rehash(1);                testConsistency(ut, hashMap,     22 ,  11,  -1 );
    }

    // EqualRange
    auto cntRecylables= hashMap.RecyclablesCount() + hashMap.Size();
    {
        ...
        hashMap.Emplace( 12,  120);       testConsistency(ut, hashMap,     6 ,  11,  cntRecylables - 6 );
...
{
    UT_INIT()
    CharSta avoidUnneededDefaultConstructorWarning;
...
// Assert that the number of recycables is always the same for both sets
...
set1.ReserveRecyclables(10);  UT_EQ( 10, set1.RecyclablesCount() )
