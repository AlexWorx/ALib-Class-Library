...
#   include "ALib.Lang.CIMethods.H"
    ...
    bool ProcessInfo::get( uinteger pid )
    {
        ...
        int st = proc_pidinfo( int(pid), PROC_PIDTBSDINFO, 0, &proc, PROC_PIDTBSDINFO_SIZE);
        if (st != PROC_PIDTBSDINFO_SIZE)
            return false;

        // pid and parent pid
        PID=    pid;
        PPID=   uinteger( proc.pbi_ppid );
        ...
        // get executable filename and path
        {
            ...
            if ( proc_pidpath (int( PID ), pathbuf, PROC_PIDPATHINFO_MAXSIZE) > 0 )
            {
