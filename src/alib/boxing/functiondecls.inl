...
struct FIsNotNull
{
    ...
    /// \ref alib_mod_bs "Bootstrapping" function
...
///
/// \note
///   Compatibility header \implude{Boxing.StdFunctors} specializes
...
struct FIsLess
{
    ...
    #if DOXYGEN
    /// Templated implementation for function \alib{boxing;FIsLess}, usable with boxable types
...
/// \alib{MonoAllocator}. With that, a deep copy of the boxed object can be allocated.
///
...
/// \c uinteger field of the boxes' \alib{boxing;Placeholder} in hexadecimal format.
...
/// that specialized type-traits functor \alib{strings;AppendableTraits} that makes values of the type
...
///    - Chapter \ref alib_format_custom_types of
...
///      box-function \alib{format;FFormat}.
///
/// @tparam TChar      The character type of the destination \alib{strings;TAString;AString} given
...
struct FAppend
{
    ...
    /// \alib{strings;AppendableTraits}, then this static templated function can be used "as is"
    ...
    ///   it is believed that \alib{strings;AppendableTraits} is specialized for the non-pointer type.
    ///   If this is not the case, then two options exist:
    ///   1. Specialize functor \alib{strings;AppendableTraits} for the non-pointer type in parallel.
    ...
    /// Implementation template for box-function \alib{boxing;FAppend} for appendable types wrapped
    /// in \c std::reference_wrapper to
    /// \ref alib_boxing_customizing_identity "bypass custom boxing".
    ///
    /// This static function template can be used for any type \p{TAppendable} that has a
    /// specialization of functor \alib{strings;AppendableTraits} defined and that becomes boxed
    /// in the wrapper type.
    ///
