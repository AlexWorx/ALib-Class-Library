...
/// #"Placeholder::Write(const TInt)" that leverages C++20 function \c std::bit_cast to
...
//==================================================================================================
/// A #"Box::data;protected member" of this union is contained in class
/// #"Box" to store information on a boxed object.
/// This member is passed as an argument to static methods #"%BoxTraits::Write" and
/// #"%BoxTraits::Read" of type-traits struct #"BoxTraits", which implement boxing and unboxing.
///
...
/// has to be implemented by all #"%BoxTraits::Write" and #"%BoxTraits::Read" methods of any
/// specialization of #"BoxTraits". Otherwise, undefined behavior occurs.
///
...
/// - arrays.
///
/// For these types, the default implementation of #"%BoxTraits::Write" and #"%BoxTraits::Read", as
/// given, for example, with macro #"ALIB_BOXING_CUSTOMIZE_TYPE_MAPPING" often is all that is
/// needed.
///
/// ####"Custom" Boxing####
...
/// that the default implementations of box-functions #"FHashcode" and
/// #"FEquals" use only the first \e N relevant bytes. If now, gaps are not written,
...
/// C++ operator \c sizeof reports for the mapped type, then also #"SizeTraits" has
/// to be specialized for that type, so that the method
/// #"Box::GetPlaceholderUsageLength" reports the right value.
/// Note that furthermore method #".Clear", which is used when boxing \e nulled pointers, only
/// clears as much data in this struct as reported by #"SizeTraits".
///
/// ####"Constexpr" Boxing####
...
/// in global and static instances of class #"%Box".
///
/// \see
///   - Chapter #"alib_boxing_customizing" of the Programmer's Manual of
///   \alib_boxing_nl.<br>
///   - Struct #"BoxTraits" and expression #"SizeTraits".
//==================================================================================================
union Placeholder {
    ...
    /// For example, the default implementations of box-functions #"FHashcode" and
    /// #"FEquals" are using the relevant bytes of this placeholder, and those must
  ...
  //################################################################################################
    ...
    /// #"alib_boxing_more_opt_constexpr;constant boxes", which in seldom cases may be required.
    ...
    /// use cases that enable #"alib_boxing_more_opt_constexpr;constant boxes", which in
    /// seldom cases may be required. Furthermore, this way, values boxed like this fulfil one
    /// requirement to produce reliable #"alib_boxing_customizing_placeholder;hash values".
    ///
    ...
    /// #"BoxTraits::Read" is needed. Such an implementation reads the pointer and length
    /// directly from this struct. In other words, there is no overloaded method #"%.Read"
    ...
    /// (I.e. there is no overloaded method #"%.Read" available for arrays.)
    ///
    ...
    /// #"alib_boxing_more_opt_constexpr;constant boxes", which in
...
// On the other hand, it complains if the methods are not doxed. So, we hide them from doxygen.
#if DOXYGEN
    /// Reads a value of type \p{TMapped} from this placeholder.<br>
    /// Internally, this method is implemented using various variants which are selected using
    /// the keyword \c requires.
    ///
    ...
    constexpr TPointer Read()                                                                const {
        if constexpr (std::same_as<TPointer, void*>)
            return VoidP;
        return static_cast<TPointer>( VoidP );
    }
