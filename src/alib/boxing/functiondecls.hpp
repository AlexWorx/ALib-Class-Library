...
//==================================================================================================
/// This built-in #"alib_boxing_functions;box function" constitutes the concept of
...
///
/// A constant function return \c true is given with the static method #"ConstantTrue". This function
...
///   if any function is called on a #"alib_boxing_more_void_void;void box", a
...
struct FIsNotNull {
    ...
    /// This implementation of the function signature returns constant \c true.
    /// It may be registered with custom types that do not provide the concept of being \e nulled.
    /// #"alib_mod_bs;Bootstrapping" function
    /// #"boxing::bootstrap" registers this implementation with type \c bool and all integral,
...
/// be used as key-values of containers like \c std::unordered_map or
/// #"HashTable".
///
...
///
/// \note
///   Compatibility header #"F;ALib.Boxing.StdFunctors.H" specializes
...
///
/// A default implementation is #"BootstrapRegisterDefault;registered" that compares the types
/// (#"Box::IsSameType") and if equal, with arrays compares the array's length,
...
/// #"Box::UnboxSignedIntegral", #"Box::UnboxUnsignedIntegral" and
/// #"Box::UnboxFloatingPoint" and appropriate casting.
...
struct FEquals {
    ...
    #if DOXYGEN
    /// Templated implementation of the function signature, usable with boxable types which have
    /// <c>operator==</c> implemented.
    ///
    /// \note
    ///   This method internally is provided twice: once for types boxed as pointers and one for
    ///   types boxed as values, and selected using keyword \c requires.<br>
    ///   If a type is boxed as a pointer, then \p{TComparable} has to be given as such
    ///   pointer-type.
    ///   For comparison, the unboxed pointers will be dereferenced.
    ///   If one is \e nulled, \c false is returned, if both are \e nulled, true.
    ///
    /// \see
...
///
/// If the types are not the same, the result of the comparison of the runtime type information
/// object is returned. For this, method #"Box::TypeID" is invoked on both boxes
...
/// \c std::less<T> for type #"%Box". This is found in a compatibility header:
///
///   \snippet "DOX_BOXING.cpp"     DOX_BOXING_FISLESS_INCLUDES
///
/// With that, the following code compiles fine:
///   \snippet "DOX_BOXING.cpp"     DOX_BOXING_FISLESS
///
/// It generates the following output:
///
///   \snippet "DOX_BOXING_FISLESS.txt"     OUTPUT
///
...
///
/// \note
///   It is a matter of the compiler (and cannot be determined by the user code) how the
///   types are sorted (outer sorting).<br>
///   Furthermore, the default implementation that simply compares the first #"%lang::uinteger" of
///   the placeholder is unlikely to produce "reasonable" results.
///
/// Type-specific implementations are given and registered for fundamental types. Integrals
/// of different sizes and floating point values will be compared by using
/// #"Box::UnboxSignedIntegral", #"Box::UnboxUnsignedIntegral" and
/// #"Box::UnboxFloatingPoint" and appropriate casting.
///
/// If module \alib_strings is included in the \alibbuild, an implementation for
/// arrays of #"characters::nchar", #"characters::wchar" and #"characters::xchar" is given.
///
/// For custom types, with #".ComparableTypes", a templated implementation is suggested: Rather than
/// implementing a specific box-function, the custom type should implement \c operator< and register
/// an instantiation of the templated function.
...
struct FIsLess {
    ...
    #if DOXYGEN
    /// Templated implementation for function #"FIsLess", usable with boxable types
    /// which have <c>operator\<</c> implemented.
    ///
    /// \note
    ///   This method internally is provided twice, once for types boxed as pointers and one for
    ///   types boxed as values, and selected using template programming.<br>
    ///   If a type is boxed as pointer, then \p{TComparable} has to be given as such a pointer
    ///   type.
...
//==================================================================================================
/// This type declares a built-in #"alib_boxing_functions;box-function".<br>
/// Besides mandatory parameter \p{self}, implementations expect a reference to an allocator of type
/// #"MonoAllocator". With that, a deep copy of the boxed object can be allocated.
///
...
/// boxed array cannot be determined.
///
/// \attention
...
///   that allows later destruction. Alternatively, a custom method could just ensure that
...
///
/// \par Availability
///   This box-function is available only if the module \alib_monomem is included in the
///   \alibbuild.
//==================================================================================================
struct FClone {
    ...
    using Signature = void (*) ( boxing::Box& self, MonoAllocator& allocator );
};
#endif

//==================================================================================================
/// This is one of the built-in #"alib_boxing_functions;box-functions" of \alib_boxing_nl.
/// This function is used to give an answer to the question if a boxed value represents a boolean
...
/// integral value in the #"boxing::Placeholder": If it is not \c 0, \c true is returned, \c false
/// otherwise.<br>
/// For array types, the default implementation returns \c true if method
/// #"Box::UnboxLength" returns a value different to \c 0, otherwise \c false is returned.
//==================================================================================================
struct FIsTrue {
    /// Signature of the invokable function.
    ///
...
#if ALIB_STRINGS
//==================================================================================================
/// Implementations of this #"alib_boxing_functions;box-function" write the content of the
/// data stored in the box to the given #"%AString" object.
///
/// A default implementation is registered. This writes out the raw value of the first
/// \c uinteger field of the boxes' #"boxing::Placeholder" in hexadecimal format.
/// For pointer types, such raw value reflects the memory address of the boxable.
...
/// that specialized type-traits functor #"AppendableTraits" that makes values of the type
/// usable with #"TAString::Append(const TAppendable&)" already.
///
/// \note
///   This is a templated (!) function declaration that defines three different box-functions at
///   once, namely \b FAppend<character>, \b FAppend<characters::complementChar> and
///   \b FAppend<strangeChar>.
///
/// This box-function is usually invoked only indirectly, by "appending" a box to an #"%AString",
/// as shown here:
///
///      Box     box= 42;
///      AString text;
...
///
/// \par Availability
///   This box-function is available only if the module \alib_strings is included in the \alibbuild.
///
/// \see
///    - Manual chapter #"alib_boxing_strings_fappend"
///    - Chapter #"alib_format_custom_types" of
///      the Programmer's Manual of \alib_format, which gives
///      a sample implementation of this function, and introduces a more powerful
///      box-function #"FFormat".
///
/// @tparam TChar      The character type of the destination #"^AString" given with parameter
///                    \p{target}.
...
struct FAppend {
    ...
    ///
    /// @param  self   The box that the function was invoked on.
    /// @param  target The target #"%AString" of character type \p{TChar}.
    using Signature = void (*) ( const Box& self, strings::TAString<TChar, TAllocator>& target );
    ...
    /// #"AppendableTraits", then this static templated function can be used "as is"
    /// and registered with the corresponding mapped type.
    ///
    /// \note
    ///   This method internally is provided twice, once for types boxed as pointers and one for
    ///   types boxed as values, and selected using template programming.<br>
    ///   If a type is boxed as a pointer, then \p{TComparable} has to be given as such a pointer
    ///   type.
    ///   For comparison, the unboxed pointers will be dereferenced. This means that
    ///   it is believed that #"AppendableTraits" is specialized for the non-pointer type.
    ///   If this is not the case, then two options exist:
    ///   1. Specialize functor #"AppendableTraits" for the non-pointer type in parallel.
    ...
    inline static
    void Appendable( const Box& self, strings::TAString<TChar, TAllocator>& target );
    #else
    ...
    /// Implementation template for box-function #"FAppend" for appendable types wrapped
    /// in \c std::reference_wrapper to
    /// #"alib_boxing_customizing_identity;bypass custom boxing".
    ///
    /// This static function template can be used for any type \p{TAppendable} that has a
    /// specialization of functor #"AppendableTraits" defined and that becomes boxed
    /// in the wrapper type.
    ///
    /// @tparam TAppendable   The "appendable" wrapped mapped box type that the function is to be
    ///                       implemented for.
    /// @param self   The box that the function was invoked on.
    /// @param target The target #"%AString" object.
    template<typename TAppendable>
    inline static
    void WrappedAppendable( const Box& self, strings::TAString<TChar, TAllocator>& target);
}; // FAppend
#endif //ALIB_STRINGS
