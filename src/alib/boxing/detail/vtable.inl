...
/** ************************************************************************************************
 * This struct is used internally with \alibmod_nolink_boxing  to provide run-time type information and
...
{
    /** Struct to store a linked list of non-built-in functions. */
    struct Entry
    {
        const std::type_info&   Type;            ///< The function type.
        void*                   Implementation;  ///< The function's implementation.
        Entry*                  Next;            ///< The next entry in the list.
    #if ALIB_DEBUG
        uinteger              DbgCntInvocations; ///< A counter for the number of invocations.
                                                 ///< Available in debug-compilations only.
    #endif
        /**
         * Constructor.
         * @param type            The function type.
         * @param implementation  The function's implementation.
         * @param next            The next entry in the list.
         */
        Entry( const std::type_info& type, void* implementation, Entry*  next )
        :Type          (type          )
...
ALIB_DBG(,DbgCntInvocations(0) )
        ...
        /**
         * Destructor. Deletes the next in the list (recursion).
         */
        ~Entry()
        {
            if( Next )
    ...
    Entry*                   Custom    = nullptr; ///< Linked list of custom function implementations;
...
#if ALIB_MODULE_MEMORY
#   define    ALIB_TEMP_TEST_FCLONE       && !ATMP_EQ(TFDecl, FClone)
#else
#   define    ALIB_TEMP_TEST_FCLONE
#endif
#if ALIB_MODULE_STRINGS
#   define    ALIB_TEMP_TEST_FAPPEND      && !ATMP_EQ(TFDecl, FAppend<character>)
#else
#   define    ALIB_TEMP_TEST_FAPPEND
#endif
    ...
    template<typename TFDecl>  ATMP_T_IF(typename TFDecl::Signature,    !ATMP_EQ(TFDecl, FHashcode )
                                                                     ...
                                                                     && !ATMP_EQ(TFDecl, FIsTrue   )
                                                                        ALIB_TEMP_TEST_FCLONE
                                                                        ALIB_TEMP_TEST_FAPPEND
                                        )
...
#if ALIB_MODULE_STRINGS
#   undef  ALIB_TEMP_TEST_FAPPEND
#endif
#if ALIB_MODULE_MEMORY
#   undef  ALIB_TEMP_TEST_FCLONE
#endif
...
extern ALIB_API  FunctionTable DEFAULT_FUNCTIONS;
     ...
     * Access has to be protected with \alib{boxing::detail,DbgLockMaps}
     */
    extern ALIB_API TypeSet       DBG_KNOWN_FUNCTIONS;

    /**
     * Thread safe access to debug tables \alib{boxing::detail,DBG_KNOWN_FUNCTIONS} and
     * \alib{boxing::detail,DBG_KNOWN_VTABLES}.
     *
     * @param doLock \c true locks, \c false unlocks.
     */
    ALIB_API void DbgLockMaps( bool doLock);
...
struct VTable
{
        ...
        /** Box-functions attached with \alib{boxing,Register}.     */
         ...
         * Note: This is not really need. However, as derived class \b %VTable derives from
         * \alib{singletons,Singleton}, which is a virtual type, having this constructor allows
...
// #################################################################################################
    ...
    /**
     * An array of two maps. The first associates non-array mapped types with the corresponding
     * \e vtable singleton, the second stores the same for mapped array types.
     *
     * Available only in debug-compilations.
     *
     * \see
     *   Function \alib{boxing::detail,DbgRegisterVTable}.
     *
     */
    extern ALIB_API TypeMap<detail::VTable*>    DBG_KNOWN_VTABLES[2];
     ...
     * For this, macro \ref ALIB_BOXING_VTABLE_REGISTER is available, which is empty in
 ...
 *   of the \alibmod_boxing "Programmer's Manual" of module \alibmod_nolink_boxing.
 *
 * ## Friends ##
                ...
                , public Singleton< VTableTT<TPlainOrArray, TMapped> >
{
    ...
    private:
         ...
         * exists).
         */
        inline  ALIB_CPP14_REL_CONSTEXPR VTableTT()
        ...
        {
            ALIB_DBG( DbgRegisterVTable( this, DbgFactoryType::Dynamic );  )
        }
};
...
/**
 * Checks if \alibmod_nolink_boxing was correctly initialized.
 ...
 */
ALIB_API void DbgCheckRegistration( detail::VTable* vtable );
#endif
...
struct  T_VTableFactory
{
     ...
     */
    static ALIB_CPP14_CONSTEXPR inline VTable* Get()
    {
        ALIB_DBG( DbgCheckIsInitialized(); )

        VTable* result=  &VTableTT<typename TMapping::PlainOrArray, typename TMapping::Type>::GetSingleton();

        ALIB_DBG( detail::DbgCheckRegistration( result ); )
...
{ static inline constexpr VTable* Get() { return  &SNGLTN_ ## Identifier; } };   }}}}              \
