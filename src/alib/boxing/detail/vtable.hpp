...
//==================================================================================================
/// This struct is used internally with \alib_boxing_nl to provide runtime type information
/// and virtual method invocations to boxed values.
/// A singleton instance of a derived type of this class is attached to each #"Box".
/// Such derived type is generated using templated derived struct #"detail::VTable".
//==================================================================================================
struct FunctionTable {
  //################################################################################################
  // Fields
  //################################################################################################
    FHashcode         ::Signature    fHashcode = nullptr;  ///< Entry for built-in function #"FHashcode".
    FIsNotNull        ::Signature    fIsNotNull= nullptr;  ///< Entry for built-in function #"FIsNotNull".
    FEquals           ::Signature    fEquals   = nullptr;  ///< Entry for built-in function #"FEquals".
    FIsLess           ::Signature    fIsLess   = nullptr;  ///< Entry for built-in function #"FIsLess".
    FIsTrue           ::Signature    fIsTrue   = nullptr;  ///< Entry for built-in function #"FIsTrue".
    #if ALIB_MONOMEM
    FClone            ::Signature    fClone    = nullptr;  ///< Entry for built-in function #"FClone".
    #endif
    #if ALIB_STRINGS
    FAppend<character,lang::HeapAllocator>                 ///< Entry for built-in function #"FAppend".
                      ::Signature    fAppend   = nullptr;
    #endif
    #if ALIB_DEBUG
...
return reinterpret_cast<typename TFDecl::Signature>( getCustom( typeid(TFDecl) ALIB_DBG(, isInvocation ) ) );
  ...
  //################################################################################################
    ...
    void Set( typename TFDecl::Signature impl )
...
/// This struct is used internally with \alib_boxing_nl to provide runtime type information
/// and virtual method invocations to boxed values.
/// A singleton instance of this struct is attached to each #"Box".
//==================================================================================================
struct VTable {
    ...
    /// \see
    ///   The documentation of #"SizeTraits" provides details on
    ...
    /// is statically or dynamically created. In the case of #"%Unregistered", a customized
    ...
    /// that used this \e vtable. Precisely, the counter is increased with invocations of
    /// #"Box::Unbox",
    /// #"Box::UnboxArray",
    /// #"Box::UnboxElement",
    /// #"Box::TypeID" and
    ...
    /// Constructor.
    ///
    /// @param type             Type information of the derived #"%VTable" type.
    ...
    /// Note: This is not really needed and empty. Because derived class #"%VTable" derives from
    /// virtual type #"Singleton", having this destructor allows cleaning memory
    ...
    /// #"alib::boxing::Box::UnboxLength;Box::UnboxLength"
    /// will return the length of the array and
    /// #"alib::boxing::Box::UnboxElement;Box::UnboxElement" may be used to
    /// access elements of the array.
    ///
    /// @return \c true if this vtable is representing an array type, \c false otherwise.
    bool        IsArray()                                              const { return Mapping > 0; }
    ...
    bool        IsPointer()                        const { return Mapping == MappingType::Pointer; }
    ...
    bool        IsEnum()                              const { return Mapping == MappingType::Enum; }
}; // struct VTable
...
/// For this, macro #"ALIB_BOXING_BOOTSTRAP_VTABLE_DBG_REGISTER" is available, which is empty
/// in release compilations.
///
/// \see
///   Macros ALIB_BOXING_VTABLE_DECLARE and #"alib_mod_boxing;Programmer's Manual" chapter
...
///                       dynamically created from template type #"%VTableUnoptimized".
ALIB_DLL
...
//==================================================================================================
/// This is a templated derivate of class #"%VTable". It is necessary to create singletons for
/// arbitrary mapped types.
///
/// \see
///   Chapter #"alib_boxing_more_opt_staticvt_bg"
///   of the \alib_boxing "Programmer's Manual" of module \alib_boxing_nl.
///
/// ## Friends ##
/// class #"Singleton;Singleton< VTableUnoptimized<TMapped, TIsArray> >"
///
                         ...
                         , public Singleton< VTableUnoptimized<TMapped, TIsArray> >
{
  ...
  private:
    ...
    ///
    /// This constructor is private, and class #"%Singleton" is a friend, which makes each
...
/// This is done with the creation of #"alib_boxing_more_opt_staticvt;dynamic vtables",
...
///
/// \see
///   Manual chapter #"alib_boxing_more_static_instances".
...
//==================================================================================================
/// Method #".Get" of the default version of this struct creates a #"detail::VTable"
/// dynamically.
/// For this, a corresponding templated #"Singleton" of type
/// #"VTableUnoptimized;+<TMapped, TIsArray>" is returned.
///
...
///
/// \see
///   - Chapter #"alib_boxing_more_opt_staticvt_howto" of the Programmer's Manual
///     of \alib_boxing_nl.
///
...
// Therefore, declaring them in "boxingcustoms.hpp.inl." would be too late.
