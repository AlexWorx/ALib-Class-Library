...
///
/// This class implements the interface prototyped with class #"lang::Allocator".
...
///   \p{TAlignment}, which defaults to what is given with the configuration macro
///   #"ALIB_MONOMEM_POOLALLOCATOR_DEFAULT_ALIGNMENT", which in turn defaults to
...
///
/// Chapter #"alib_contmono_intro_recycling" of the joint Programmer's Manual of modules
...
/// "recycling proxy" in front of a #"%MonoAllocator" (or a different,
/// #"alib_contmono_chaining;chained" allocator).
///
/// @see
/// - The #"alib_contmono_intro;Programmer's Manual" of module \alib_monomem_nl.
/// - The "standard" type definition of this template type #"MonoAllocator",
///   which uses an #"HeapAllocator" as the
///   #"alib_contmono_chaining;chained allocator".
///
/// @tparam TAllocator The underlying allocator to use.<br>
///                    With type definition #"PoolAllocator",this is bound to type
///                    #"MonoAllocator".<br>
///                    With type definition #"PoolAllocatorHA",this is bound to type
///                    #"HeapAllocator".<br>
...
///                    Defaults to the configuration macro
///                    #"ALIB_MONOMEM_POOLALLOCATOR_DEFAULT_ALIGNMENT", which in turn defaults
///                    to <c>alignof(uint64_t)</c>.<br>
///                    It is statically asserted that this value is not higher than
///                    field #"Allocator::MAX_ALIGNMENT" of the given chained
///                    \p{TAllocator} and not smaller than <c>alignof(void*)</c>.<br>
///                    See Chapter #"alib_contmono_further_alignment" of the Programmer's Manual
///                    of this module.
//==================================================================================================
...
#endif
{
    ...
    #if !defined(_WIN32) // not a constexpr under windows
  ...
  public:
    ...
    /// already named #"%MonoAllocator". Thus, no constructor argument is available
    /// (in contrast to class #"TMonoAllocator;MonoAllocator").
  ...
  protected:
    ...
    int*        dbgOpenAllocations                                                         =nullptr;
        ...
        /// This member is used with #"ALIB_DEBUG_ALLOCATIONS" to pass the requested size from
  ...
  //========================================== Construction ========================================
    ...
    /// if \p{TAllocator} is default-constructible (e.g., #"HeapAllocator").
  ...
  #if DOXYGEN
    /// See the description of this method with prototype #"Allocator::allowsMemSplit;*".<br>
    ...
    ///   #"alib_contmono_poolallocator_metatrick" of the Programmer's Manual of this module.
    ///
    ...
    template<typename T>
    static constexpr short  GetAllocInformation()                 { return hookIndex<sizeof(T)>(); }

    /// Same as the templated #".GetAllocInformation()" version, but accepts method parameter
    /// \p{size} instead of a template parameter.
    /// Thus, this version is only potentially \c constexpr.
    ///
    ...
    static constexpr short  GetAllocInformation(TIntegral size)  { return hookIndex(size_t(size)); }
    ...
    /// If the configuration macro #"ALIB_DEBUG_ALLOCATIONS" is set, the method will overwrite the freed
    ...
    void FreeByAllocationInfo(int allocInfo, void* mem )                                   {ALIB_DCS
        ...
        *static_cast<void**>(mem)= hooks[allocInfo];
        hooks[allocInfo]= mem;
    }

    /// Deletes all current pool objects with the #"TPoolAllocator::ChainedAllocator".
    ...
    /// Only functional if the  configuration macro #"ALIB_DEBUG_ALLOCATIONS" is set.
    ...
    int     DbgCountedOpenAllocations(size_t size)   { return dbgOpenAllocations[hookIndex(size)]; }
    ...
    /// \p{size}.<br>
    /// Only functional if the  configuration macro #"ALIB_DEBUG_MEMORY" is set.
    /// Otherwise, this is a <c>static constexpr</c> function that returns \c 0.
    /// @see Overloaded method #".DbgStatAllocCounter" which returns the overall number.
    /// @param size The size of the object requested, as given with #".allocate" and #free.
    /// @return The number of non-freed allocations.
    uinteger     DbgStatAllocCounter(size_t size)   { return dbgStatAllocCounter[hookIndex(size)]; }
...
#if !DOXYGEN
extern template ALIB_DLL class  TPoolAllocator<      MonoAllocator, ALIB_MONOMEM_POOLALLOCATOR_DEFAULT_ALIGNMENT>;
extern template ALIB_DLL class  TPoolAllocator<lang::HeapAllocator, ALIB_MONOMEM_POOLALLOCATOR_DEFAULT_ALIGNMENT>;
#endif
...
/// #"alib_contmono_chaining;chained allocator") to type alias #"alib::MonoAllocator;2", which
/// in turn defines its chained allocator to type #"HeapAllocator".
...
/// Type alias in namespace #"%alib" to denote the use of a
/// #"PoolAllocator" with type  #"StdAllocator".
template<typename T>
