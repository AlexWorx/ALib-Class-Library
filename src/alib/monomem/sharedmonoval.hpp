...
//==================================================================================================
/// This templated class is a utility type for #"TMonoAllocator" and supports the
/// following mechanics:
/// - It creates a first #"detail::Buffer;buffer" usable by a #"%MonoAllocator".
/// - A custom type defined by template parameter \p{T} is placed at the beginning of
///   that buffer.
/// - Along with the custom type, the #"%MonoAllocator" that receives this first buffer is likewise
...
/// - This type overloads #".operator->()" and #".operator*()" to access the members of the custom
///   type.
...
///   be used.
///
/// In contrast to the container types #"SharedVal" and #"SharedPtr",
...
///     #"alib_contmono_smv" of the Programmer's Manual of this \alibmod.
...
///     - Class #"TSharedFTree", and
///     - Class #"exc Exception".
///   - Chapter #"alib_contmono_smv_locking" of the Programmer's Manual of module
///     \alib_monomem_nl.
///
/// @tparam T           The custom type that is embedded along with the #"%MonoAllocator".
/// @tparam TAllocator  The #"alib_contmono_chaining;chained allocator" that is used by the
///                     monotonous allocator that this type creates inside its first buffer.<br>
///                     Usually, type #"HeapAllocator" is to be given here.
/// @tparam TLock       The type of #"alib_threads_locks;ALib lock" to embed besides \p{T}.
///                     With the provision of <c>void</c>, the integration of a lock is
///                     suppressed.<br>
///                     See chapter #"alib_contmono_smv_locking" of the Programmer's Manual of
///                     module \alib_monomem_nl for further details.
//==================================================================================================
template<typename T, typename TAllocator, typename TLock>
class TSharedMonoVal {
  protected:
    /// Same as #"TSharedMonoVal::FieldMembersWithLock" but missing the lock.
    ...
    /// #"TSharedMonoVal::FieldMembersNoLock" or
    /// #"TSharedMonoVal::FieldMembersWithLock" is chosen.
  ...
  public:
    ...
    TSharedMonoVal( TAllocator& allocator,
        ...
        members=  static_cast<FieldMembers*>( buffer->allocate( sizeof( FieldMembers),
  ...
  //================================================================================================
    ...
    /// Calls \b Acquire on the embedded instance specified with template parameter \p{TLock}.
    ...
    /// Calls \b TryAcquire on the embedded instance specified with template parameter \p{TLock}.
    /// This method participates in the overload resolution only if \p{TLock} provides an
    /// equivalent method.
    /// @param ci  Caller information. Available only with debug-builds.
    ...
    /// Calls \b Release on the embedded instance specified with template parameter \p{TLock}.
    /// This method participates in the overload resolution only if \p{TLock} provides an
    /// equivalent method.
    /// @param ci  Caller information. Available only with debug-builds.
    void        Release(const CallerInfo& ci)                                        const noexcept;

    /// Calls \b AcquireRecursive on the embedded instance specified with template parameter \p{TLock}.
    /// This method participates in the overload resolution only if \p{TLock} provides an
    /// equivalent method.
    /// @param ci  Caller information. Available only with debug-builds.
    void        AcquireRecursive( const CallerInfo& ci )                             const noexcept;

    /// Calls \b ReleaseRecursive on the embedded instance specified with template parameter \p{TLock}.
    /// This method participates in the overload resolution only if \p{TLock} provides an
    /// equivalent method.
    /// @param ci  Caller information. Available only with debug-builds.
    void        ReleaseRecursive(const CallerInfo& ci)                               const noexcept;

    /// Calls \b %AcquireShared on the embedded instance specified with template parameter \p{TLock}.
    /// This method participates in the overload resolution only if \p{TLock} provides an
    /// equivalent method.
    /// @param ci  Caller information. Available only with debug-builds.
    void        AcquireShared( const CallerInfo& ci )                                const noexcept;

    /// Calls \b %TryAcquireShared on the embedded instance specified with template parameter \p{TLock}.
    /// This method participates in the overload resolution only if \p{TLock} provides an
    /// equivalent method.
    /// @param ci  Caller information. Available only with debug-builds.
    /// @return \c true if the lock was not acquired by a different thread and thus, this call
    ...
    /// Calls \b %ReleaseShared on the embedded instance specified with template parameter \p{TLock}.
    /// This method participates in the overload resolution only if \p{TLock} provides an
    /// equivalent method.
    /// @param ci  Caller information. Available only with debug-builds.
    void        ReleaseShared(const CallerInfo& ci)                                  const noexcept;

    /// Calls \b %TryAcquireTimed on the embedded instance specified with template parameter
    /// \p{TLock}.
    /// This method participates in the overload resolution only if \p{TLock} provides an
    /// equivalent method.
    ...
    /// Calls \b %TryAcquireSharedTimed on the embedded instance specified with template
    /// parameter \p{TLock}.
    /// This method participates in the overload resolution only if \p{TLock} provides an
    /// equivalent method.
    /// @param waitDuration The point in time, when this method stops waiting.
