...
//==================================================================================================
/// Stores the actual state of outer class #"%MonoAllocator".
/// Retrieved method #"TMonoAllocator::TakeSnapshot;*" and #"TMonoAllocator::Reset(Snapshot);*".
//==================================================================================================
class Snapshot {
  protected:
    #if !DOXYGEN
  ...
  public:
    ...
    ///   #"TMonoAllocator::Reset(Snapshot);*" do reset the monotonic allocator to its
...
/// Statistics for type #"TMonoAllocator", receivable with method
/// #"TMonoAllocator::GetStatistics".
/// @see Struct #"DbgStatistics" and corresponding method
///      #"TMonoAllocator::DbgGetStatistics".
struct Statistics {
    ...
    /// #"TMonoAllocator::DbgGetStatistics" needs to be used and its
...
/// Debug statistics for type #"TMonoAllocator", receivable with method
/// #"TMonoAllocator::DbgGetStatistics".
/// Availability depends on configuration macro #"ALIB_DEBUG_MEMORY".
...
//==================================================================================================
/// \par Important Note
///   Please consult the #"alib_contmono_intro;Programmer's Manual" of \alib module
...
/// #"alib_contmono_chaining;chained allocator".
...
///
/// \par Resetting
///   The allocator can be reset to a certain state (see #"TakeSnapshot" and #"Reset(Snapshot)"),
///   which disposes all memory behind the snapshot.
///   Disposed complete buffers will not be freed, but "recycled"  with future allocations.<br>
///   If #"alib_contmono_class_monoalloc_snapshot;nested snapshots" are used in the right
...
///   #"%TMonoAllocator" itself. We name this a <em>"self-contained monotonic allocator"</em>.<br>
///   This feature is leveraged by type #"TSharedMonoVal".
///
/// \par Debug Features
/// Depending on the configuration macro #"ALIB_DEBUG_MEMORY", some metrics on instances of this
...
///
/// \par See also
/// - The #"alib_contmono_intro;Programmer's Manual" of module \alib_monomem_nl.
/// - The "standard" type definition of this template type #"MonoAllocator", which uses
///   an #"HeapAllocator" as the #"alib_contmono_chaining;chained allocator".
///
/// @tparam TAllocator The underlying allocator to use.<br>
///                    With type definition #"MonoAllocator", this is bound to type
///                    #"HeapAllocator".<br>
///                    For general information see chapter #"alib_contmono_chaining"
...
class TMonoAllocator  : public lang::AllocatorMember<TAllocator> {
  ...
  protected:
    ...
    /// @see Method #"DbgLock" and chapter #"alib_contmono_further_debug" of the
    /// Programmer's Manual of this \alibmod_nl.
    bool                    dbgLock                                                          =false;
    #endif

    #if ALIB_DEBUG_MEMORY
    /// Debug statistics measured on the whole runtime of this object.<br>
    /// Availability depends on configuration macro #"ALIB_DEBUG_MEMORY".
  ...
  public:
    ...
    /// Due to the possibility of being self-contained, this type needs special treatment
    /// in respect to the member of type #"lang::DbgCriticalSections". It must be neither derived
    /// nor being a member. Instead, destruction has to be controlled.
    /// Furthermore, the \ref alib_macros_mod_threads "ALib Module Threads Macros"
    /// cannot be used.
  ...
  public:
    ...
    ///
    /// The method #"TMonoAllocator;IsInitialized" can be used to determine if an allocator was
    /// initialized on construction. If not, a placement-new calling a decent constructor is to be
    /// performed before construction.
    ///
    /// @see Chapter #"alib_contmono_globalinstance_initialsize" of the Programmer's Manual
    ///      of module \alib_monomem.
    ///
    /// @param  dbgName     Has to be specified with debug-compilations only.
    ///                     Use the macro #"ALIB_DBG" to pass a constant.
...
ALIB_DBG(,DbgName(dbgName))                                                                       {}
    ...
    /// given \p{initialBuffer} must not be freed to the heap, method #"destructWithExternalBuffer"
    /// has to be called right before the allocator is destructed.
    ///
    /// \note
    ///   This constructor, together with the method #"destructWithExternalBuffer" enables the
    ///   efficient implementation of derived class #"TLocalAllocator", as well as
...
ALIB_DBG(,DbgName(dbgName)) {
    ...
    /// To be used if template parameter \p{TAllocator} does not equal #"HeapAllocator"
...
ALIB_DBG(,DbgName(dbgName)) {
  ...
  //################################################################################################
    ...
    /// e.g., using #"lang::Destruct;*".
    ///
    /// Method #".Reset(Snapshot)" must not be called using its default parameter when an instance
    /// of this type was created by this method. Instead, if reset operations are desired, a
    /// snapshot has to be taken (see method #".TakeSnapshot") right after the invocation of this
    /// method and maybe other initial members that should survive a reset, which then has to be
    /// passed to method #"%.Reset(Snapshot)".<br>
    ...
    ///                              Defaults to \c 200, which doubles buffer size with each
    ///                              next internal buffer allocation.
    /// @return A pointer to an instance of this type #"%MonoAllocator" residing in its first
    ///         created buffer.
    //==============================================================================================
    ALIB_DLL
    ...
    /// parameter.
    /// Used with chained allocators that are default-constructible (e.g., #"%HeapAllocator"):
    ///
    /// @param  dbgName               Has to be specified with debug-compilations only.
    ///                               Use the macro #"ALIB_DBG" to pass a constant.
    /// @param  initialBufferSizeInKB The size of memory the buffers allocated in kilobytes.
    ...
    /// <c>C++ new operator</c>, or #"HeapAllocator", and if the memory is intended to be
    /// freed, this method may not be called. As a sample, class #"TLocalAllocator",
    ...
    /// Tests if the non-initializing constructor #".TMonoAllocator(const char*, std::nullptr_t)"
    /// was used.
  ...
  //################################################################################################
    ...
    ///                  See Chapter #"alib_contmono_further_alignment" of the Programmer's
    ...
    /// This method is empty for this allocator and optimized out. Only if the configuration macro
    /// #"ALIB_DEBUG_ALLOCATIONS" is set, the method will overwrite the freed memory with
    ...
    { return lang::AllocatorInterface<TMonoAllocator>(*this); }
        ...
        /// #"Allocator::allowsMemSplit;*".<br> (Note: This method is static. For technical
        /// reasons this cannot be reflected in this documentation)
        /// @return \c true, except if the configuration macro #"ALIB_DEBUG_ALLOCATIONS" is given.
  ...
  //################################################################################################
    ...
    /// Saves the current state of the allocator and returns this information as a #"%Snapshot"
    ...
    /// Resets this allocator to the given #"Snapshot".
    /// Parameter \p{snapshot} is defaulted with a default-constructed #"%Snapshot", which
    ...
    /// class #"%Snapshot" is only a simple lightweight class that marks the currently
    ...
    /// This method is used by class #"TSharedMonoVal" to avoid the need of storing
    /// a snapshot behind itself.
    ///
    /// If the configuration macro #"ALIB_DEBUG_ALLOCATIONS" is set, then all freed memory is
  ...
  //################################################################################################
    ...
    ///      with configuration macro #"ALIB_DEBUG_MEMORY" set.
    ...
    /// @see Chapter #"alib_contmono_further_debug" of the Programmer's Manual of this
    /// \alibmod_nl.
    ...
    /// For example, this is used with function #"DbgDumpStatistics".
    ...
    /// \par Availability
    ///  This method is included only if the configuration macro #"ALIB_DEBUG_MEMORY" is set.
    ///
    ...
    /// This is a simple helper-type following the
    /// \https{RAII idiom,https://en.cppreference.com/w/cpp/language/raii}.
    /// On construction a snapshot of the given mono allocator is taken and the allocator
    /// is reset to the state on destruction.
    struct Resetter {
        TMonoAllocator&  MA;         ///< The mono allocator to reset on destruction.
        Snapshot         snapshot;   ///< The snapshot taken on construction.

        /// Constructor.
        /// @param ma   The mono allocator to reset on destruction.
        Resetter(TMonoAllocator& ma) : MA{ma}, snapshot{ma.TakeSnapshot()}                        {}

        /// Destructor. Resets the mono allocator given with construciton.
        ~Resetter()                                                          { MA.Reset(snapshot); }
    };
}; // class TMonoAllocator
...
/// placement-new on this instance before #"alib_mod_bs;bootstrapping ALib".
///
/// \see
///  - Chapter #"alib_contmono_globalinstance" of the Programmer's Manual of this \alibmod_nl.
///  - The #"%threads::Lock"-instance #"GLOBAL_ALLOCATOR_LOCK"
extern ALIB_DLL    TMonoAllocator<lang::HeapAllocator>  GLOBAL_ALLOCATOR;

#if !ALIB_SINGLE_THREADED
/// This <em>mutex</em> is used to protect the #"GLOBAL_ALLOCATOR" from racing
/// conditions in multithreaded software.<br>
/// If the configuration macro #"ALIB_DEBUG_CRITICAL_SECTIONS" is set, this lock will be attached
/// to the #"lang::DbgCriticalSections" instance in #".GLOBAL_ALLOCATOR" during bootstrap.
/// Thus, an assertion will be raised if the #"GLOBAL_ALLOCATOR" is used without
/// locking this mutex.<br>
/// This is a recursive lock.
/// This allows using the #"AllocatorInterface;high-level allocation interface" with
...
/// #"alib_contmono_chaining;chained allocator") to type #"HeapAllocator".
using     MonoAllocator =   monomem::TMonoAllocator<lang::HeapAllocator>;

/// Type alias in namespace #"%alib" to denote the use of a #"MonoAllocator"
/// with the type  #"StdAllocator".
template<typename T>
