...
///   \p{TAlignment}, which defaults to what is given with the compiler-symbol
...
///                    Defaults to the compiler-symbol
...
#endif
{
  ...
  public:
    ...
    /// Same as the templated #GetAllocInformation<T>() version, but accepts method parameter
    ...
    /// If the compiler-symbol \ref ALIB_DEBUG_ALLOCATIONS is set, the method will overwrite the freed
    ...
    /// \p{size}.<br>
    /// Only functional if the  compiler-symbol \ref ALIB_DEBUG_MEMORY is set.
    /// Otherwise, this is a <c>static constexpr</c> function that returns \c 0.
