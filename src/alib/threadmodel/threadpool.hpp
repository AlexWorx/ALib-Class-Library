...
//==================================================================================================
#ifndef HPP_ALIB_THREADMODEL_THREADPOOL
#define HPP_ALIB_THREADMODEL_THREADPOOL
#pragma once
#if !defined(DOXYGEN)
...
///    the mechanics provided by \alibmod_nl \alib_monomem.
///
        ...
        if (ctdWorkers == 0)
        {
            ALIB_ASSERT_ERROR(       Strategy.WorkersMax > 0
                                && ( Strategy.Mode != ResizeStrategy::Modes::Fixed
                                     || Strategy.FixedSize > 0   ), "MGTHR/STRGY", NString2K() <<
                "No threads to schedule job. Strategy values:\n"
                "       WorkersMax:    " << Strategy.WorkersMax << "\n"
                "       Strategy.Mode: " << (Strategy.Mode == ResizeStrategy::Modes::Auto ? "Auto" : "Fixed") << "\n"
                "  Strategy.FixedSize: " << Strategy.FixedSize )

                addThread();
        }
        TJob* job= pool().New<TJob>( std::forward<TArgs>(args)... );
        ALIB_ASSERT_ERROR( job->SizeOf()==sizeof(TJob), "MGTHR", NString256() <<
            "Error in ThreadPool::schedule: Job size mismatch. Expected " << sizeof(TJob) <<
            " while virtual method SizeOf returns "<< job->SizeOf() << ".\n"
            "Override this method for job-type <" << typeid(*job) << ">" )

        ALIB_ASSERT_ERROR(  Strategy.WorkersMax > 0, "MGTHR", NString2K() <<
            "Error in ThreadPool::schedule: Job pushed while this pool is shut down already. "
...
#endif // HPP_ALIB_THREADMODEL_THREADPOOL
