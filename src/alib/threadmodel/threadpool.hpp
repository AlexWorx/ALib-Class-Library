...
/// #"alib_mod_threadmodel;Programmer's Manual" of this module for information
/// about how the types found in this namespace are used.
namespace threadmodel {

//==================================================================================================
/// Instances of this thread-class are created by #"ThreadPool"s.
/// Applications may extend this class and override the method #"PrepareJob".
//==================================================================================================
struct PoolWorker : protected Thread {
...
#if ALIB_STRINGS
    String64        nameBuffer;  ///< Buffer to store the thread name given with construction.
    ...
    /// Implementation of #"Thread::Run;*". Retrieves jobs from the pool, invokes
    /// #"PrepareJob", and then calls #"Job::Do;*".
    void         Run()                                                                     override;
...
/// It supports fix-sized or dynamically resizing thread pools to balance workload and optimize
...
///      #"ThreadPool::ResizeStrategy".
///    - Automatically adjusts the number of threads based on workload.
///
/// 2. <b>Jobs</b>:<br>
///    The worker-threads managed by the pool are processing #"Job" objects,
///    which<br>
...
///    Provides #"ThreadPool::Sync;a method to synchronize" all worker threads
...
///
/// @see Chapter #"alib_thrmod_threadpool" of this module's Programmer's Manual
...

{
  ...
  public:
    ...
    struct ResizeStrategy {
        ...
        /// If #".Mode" equals #"%Modes::Fixed", this is used and all other parameters are ignored.
        /// If #".Mode" equals #"%Modes::Auto", this field gives the maximum number of workers that
  ...
  protected:
    ...
    /// Special synchronization job. Pushed with #"ThreadPool::Sync;2" and
    /// #"DedicatedWorker::DeleteJobDeferred;2"
    /// With the latter, the field #".JobToDelete" will be given, otherwise this is nulled.
  ...
  public:
    ...
    /// Creates a worker using the #".pool" allocator.
    /// Derived types may override this method and create a derived #"PoolWorker"-type.
    ...
    /// disadvantage.
    ///
    /// Invoking this method schedules #"JobSyncer;a special synchronization job" in the queue of
    /// this #"%ThreadPool".
    ...
    ///   For each file that meets certain criteria, a job is scheduled in the #"%ThreadPool"
    ...
    ///
    /// \par
    ///   A call to this method resolves this issue by ensuring that all file-reading jobs are
    ...
    ///
    /// If after the call to #"%.WaitForAllIdle" no jobs were scheduled, this method is \b not #
    /// supposed to block.
    ALIB_DLL
    void                    Shutdown();
...
template<>
struct AppendableTraits<alib::threadmodel::ThreadPool, character, lang::HeapAllocator> {
