...
//==================================================================================================
#ifndef HPP_ALIB_THREADMODEL_DEDICATED_WORKER
#define HPP_ALIB_THREADMODEL_DEDICATED_WORKER
#pragma once
...

{
  ...
  public:
    ...
    ALIB_API bool           Remove( DedicatedWorker& thread,
...
///    mechanics, for example, a \alib{threads;Promise} or a \alib{threads;Condition} that a caller
///    can decide to wait on.
///    <br><br>
                      ...
                      , protected TCondition<DedicatedWorker>
{
  ...
  protected:
                           ...
                           TArgs&&... args      )
    {
        ...
        ALIB_ASSERT_ERROR( job->SizeOf()==sizeof(TJob), "MGTHR", NString256() <<
            "Error in DedicatedWorker::schedule: Job size mismatch. Expected " << sizeof(TJob) <<
        ...
        ALIB_ASSERT_ERROR(     GetState() == State::Started
                            || GetState() == State::Running, "MGTHR", NString256() <<
  ...
  public:
    ...
    /// @param threadName    The name of this thread. The string's data has to survive this
    ///                      threads' lifespan.
    ...
    ~DedicatedWorker()                                                                      override
    {
        ALIB_ASSERT_WARNING( Load() == 0, "MGTHR", NString256() <<
         "DedicatedWorker \""<< GetName() <<"\" destructed, while job-queue is not empty." )
    }
...
#endif // HPP_ALIB_THREADMODEL_DEDICATED_WORKER
