...
//==================================================================================================
/// \attention This class belongs to module \alib_threadmodel, which is not in a fully stable and
///            consistent state, yet.
///            Also, this type is considered experimental.
///
/// This #"Singleton;singleton-class" manages worker threads of type
/// #"DedicatedWorker".
/// Such threads are started by adding them to the singleton instance of this type and are stopped
/// with removal.
///
/// Class #"DedicatedWorker" uses this type's #"%PoolAllocator" to create and dispose #"Job"
/// objects and derived types are allowed to allocate resources in its #"%MonoAllocator".
//==================================================================================================
...

{
  ...
  public:
    ...
    /// The thread is stopped by invoking #"DedicatedWorker::ScheduleStop;*" using the given
    ...
    ///                     Use #"%Lowest" (the default) to ensure that all other commands are duly
    ...
    ///                     should be passed using macro #"ALIB_DBG".
    ...
    /// #"DedicatedWorker::ScheduleStop;*".<br>
    /// After this method has been invoked, no further commands should be scheduled, even if
    /// the given priority equals #"%Lowest". This <b>should be assured</b> by the using code.
    ///
...
/// Jobs can have a different #"threadmodel::Priority", and thus may not be executed in the
/// order of insertion.
...
///    usually explicitly belong to the scope of an #"%DedicatedWorker".<br>
///    A #"%Job" - besides its inherited type-ID - contains input data to the #"%DedicatedWorker"
...
///    #"DedicatedWorker::JobTrigger" that has neither input nor output data.)
///    <br><br>
/// 2. The derived type exposes interface methods according to the different jobs available
///    to be scheduled.
///    Here, the #"%Job" is constructed and returned to the caller, usually as a reference
...
///    derived type.
///    <br><br>
/// 3. The #"%Job" objects returned by the interface (usually) contain some sort of thread-safe
///    acknowledge mechanics, for example, a #"Promise" or a #"threads::Event" that a caller can
///    decide to wait on.
///    <br><br>
/// 4. Disposal of #"%Job" instances has to be explicitly performed by the thread that schedules
...
///    The second is #".DeleteJobDeferred", which covers the case that the caller decides not to
///    wait for an acknowledgment.
///    Here, the instance must not be deleted because the #"%DedicatedWorker" will access it when
...
///    The deletion job has a low priority of #"Priority::DeferredDeletion". Jobs scheduled with an
...
///          There, deferred-deletion has a huge negative impact on the performance of the pool.
///          With type #"%DedicatedWorker", the negative impact of deferred deletion is negligible.
///    <br><br>
///
/// 5. The custom types #"%Job" types have to likewise fulfill a few contractual rules.
///    Those are given with the type's #"Job;reference documentation".
///
/// ### Starting and Stopping: ###
/// The class inherits type #"Thread" as a protected base type. With that, the
/// conventional methods to start and stop a thread are hidden.<br>
/// Starting and stopping a #"%DedicatedWorker" is instead performed by adding, respectively
/// removing an instance of this type to singleton class #"DWManager".
///
/// ### Triggering: ###
/// This type implements abstract interface #"Triggered".
/// If this is considered useful by a derived type, then three things have to be performed:
/// - The parameterless job #"DedicatedWorker::JobTrigger" has to be processed
...
/// - The instance of the custom #"%DedicatedWorker" has to be registered by calling method
///   #"Trigger::Add;*".
//==================================================================================================
                      ...
                      , protected TCondition<DedicatedWorker>
{
  ...
  //================================================================================================
    ...
    /// If this #"%DedicatedWorker" is (in addition) attached to a #"DWManager" as a
    /// triggered object, this duration is returned by overridden method
    /// #"Triggered::triggerPeriod;*" to determine the interval between
  ...
  //================================================================================================
    ...
    /// The job sent by methods #"DedicatedWorker::DeleteJobDeferred" and
    /// #"ThreadPool::DeleteJobDeferred".
    ...
    /// The job sent if (optional) trigger-interface #"Triggered::trigger;*"
    /// is invoked.
    ...
    ///         is to be done by either invoking #"DedicatedWorker::DeleteJob"
    ///         or #"DedicatedWorker::DeleteJobDeferred".
  ...
  //================================================================================================
    ...
    /// Schedules #"DedicatedWorker::JobTrigger" need to implement this function and
  ...
  //================================================================================================
    /// This implementation of method #"Thread::Run;*" constitutes a very simple loop
    ...
    /// This flag will be set by the internal job type #"DedicatedWorker::JobStop"
    ...
    /// - #"DedicatedWorker::JobStop", which signals flag #".stopJobExecuted", and
    ///   this way lets calling method #".Run" leave its loop, and
    /// - #"DedicatedWorker::JobDeleter", which deletes the given other job.
    ...
    ///
    /// It is also allowed to implement a custom processing, for example, with #"%JobStop" and thus
  ...
  //================================================================================================
    ...
    /// @param priority The priority of the job. Use #"%Lowest" if it is assured that no
    ...
    ///   the job is processed by this #"%DedicatedWorker".
    ///   For example, if a derived type's interface returns an job instance of type
    ///   #"JPromise", then such a result must only be deleted once the promise is
    ///   fulfilled.
    ///   <p>
    ...
    /// done with #"Priority::DeferredDeletion".<br>
    /// This assures that the job deletion is performed \b after the job execution.
    ///
    /// This method is useful when the thread that schedules a job with this #"%DedicatedWorker" is
    ...
    /// With the second, the derived #"%DedicatedWorker" would delete the given shared data
