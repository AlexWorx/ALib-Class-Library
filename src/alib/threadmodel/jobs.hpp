...
///
/// Defines jobs which are scheduled with instances of types #"ThreadPool" and
/// #"DedicatedWorker".<br>
///
...
/// job definitions.
/// For example, built-in derived type #"JPromise" may be used as the "shared data"
/// between the sender and the processing thread.
/// The constructor of #"%JPromise" accepts a type ID to store.
///
...
///
/// Instances of this class are always #"TPoolAllocator;pool-allocated".
/// Allocation, construction, destruction and deletion are always under the control of either
/// type #"ThreadPool" or #"DedicatedWorker".
...
/// #"alib_contmono_poolallocator_allocinfo;found here").
///
/// Virtual method #"Do" has to be overridden in case a job is to be scheduled with the class
/// #"ThreadPool".
/// In case a derived type is (exclusively) used with type #"DedicatedWorker",
/// it is up to the user of this module whether method #"Do" is implemented or not.
/// If not, then a custom type derived from #"%DedicatedWorker" has to override the method
/// #"DedicatedWorker::process" and handle the job type there.
///
...
struct Job {
    ...
    /// for deferred deletion using #"ThreadPool::DeleteJobDeferred;*"
    ...
    /// #"JPromise" to disable the warnings of deleting an unfulfilled and
    ...
    /// #"DedicatedWorker".
    /// In case it is executed by a #"ThreadPool" it has to be overridden.
    ///
...
{
    /// Constructor.
    /// @param id     The type-info of the derived type. Passed to parent class #"%Job".
