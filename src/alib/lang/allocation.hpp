...
/// Provides debugging utility methods used by allocator types #"HeapAllocator",
/// #"TMonoAllocator", and #"TPoolAllocator".
/// Each method is empty and thus optimized out in case the configuration macro
/// #"ALIB_DEBUG_ALLOCATIONS" is not set.
/// @see Chapter #"alib_contmono_further_debug" of the Programmer's Manual.
struct DbgAlloc {
    ...
    /// #"alib_mod_assert;Raises an ALib error" if the piece of allocated memory is
    ...
    ///                  In case of type #"%MonoAllocator", the debug-name of the instance.
    template<typename TSize>
    static
    void checkMem( void* mem, const TSize size, unsigned char magic, const char* name ) {
        #if ALIB_DEBUG_ALLOCATIONS
            auto* cMem= static_cast<unsigned char*>(mem);
            size_t idx= size;
            while ( idx< size + 4)
                if( cMem[idx++] != magic )
...
///
/// As an exception to the #"alib_manual_appendix_naming;naming conventions used by ALib",
...
/// #"AllocatorInterface" is available with #".operator()()", and such usage is preferred.
///
/// Along with this pure documentation class, a C++20 concept exists with
/// #"IsAllocator", which duly checks if  all entities defined with this type are present
...
struct Allocator
{
    ...
    /// In case of #"HeapAllocator", this type is defined as <c>void</c>.<br>
    /// Allocators provided by \alib allow to access the instance with publicly inherited methods
    /// #"AllocatorMember::GetAllocator" and #"AllocatorMember::AIF".
    ...
    /// #"TPoolAllocator;PoolAllocator"), then this parameter is modified to this higher
    /// value.
    ...
    /// Of the allocators exposed by module \alib_monomem, only #"TMonoAllocator"
    /// returns \c true. This is because this allocator has an empty method #"%.free" and thus
    /// is agnostic about the memory freed.
    /// If the configuration macro #"ALIB_DEBUG_ALLOCATIONS" is set, then also this allocator returns
    /// \c false.
    ///
    ...
    ///
    /// Class #"TMonoAllocator;MonoAllocator" requires a name as the first parameter
    /// with every overloaded constructor. Consequently, this argument has to be passed enclosed
    /// in macro #"ALIB_DBG" (with a trailing comma) to have it pruned with release-builds.
    ///
    /// Class #"TPoolAllocator;PoolAllocator" grabs this name from its chained
    /// allocator, which most often is an already named #"%MonoAllocator". Thus, no constructor
    ...
    ///    this only happens with the use of class #"TPoolAllocator;PoolAllocator", and
    ...
    ///
    /// Within \alib a sample of such a type is class #"^AString": this type requests a string
    /// buffer as needed but acknowledges if the returned buffer is lager.
    ...
    ///
    /// Note that this method is empty and optimized out, when the configuration macro
    /// #"ALIB_DEBUG_ALLOCATIONS" is not set. If set, an \alib_assertion is raised if either the
    ...
    /// If the configuration macro #"ALIB_DEBUG_ALLOCATIONS" is not set, this method is empty and will
...
/// #"lang::Allocator") has to provide only three <b>low-level (!)</b> allocation methods.
/// Those are:
/// - #"Allocator::allocate(size_t& size, size_t alignment)",
/// - #"Allocator::reallocate(void* mem, size_t oldSize, size_t& newSize, size_t alignment)",
/// - #"Allocator::free(void* disposable, size_t size)", and
///
/// Besides these, the C++ function call operator #"Allocator::operator()()" has to be
...
///
/// In contrast to the low-level interface methods #"Allocator::allocate;*" and
/// #"Allocator::reallocate;*", information about a piece of memory returned that is larger
...
///
/// \note
///    In this sample, the destructor is of class \c MyType is emtpy, and furthermore, the method
///    #"%TMonoAllocator::free" is empty. Thus, the whole call to #".Delete" will be
///    optimized out. With other allocators this is different. It is recommended to still include
...
///
///@see
///   - The #"alib_mods_contmono;Programmer's Manual" of module \alib_monomem_nl.
///   - Allocator types #"TMonoAllocator" and #"TPoolAllocator"
///     found in module \alib_monomem_nl.
///   - Class #"HeapAllocator", which is useful for templated types which are agnostic
///     to underlying allocation strategies.
///   - To allocate and create a copy of a character string, the method #"TString::Allocate;*"
///     or constructor #"TString;TString(TAllocator& allocator, const TString<TChar>&)"
///     may be used. Freeing is then to be performed by using #"TString::Free;*".
//==================================================================================================
...
struct AllocatorInterface {
    ...
    /// Constructor. Usually construction is performed by the function operator of
    /// an #"%lang::Allocator", which then passes itself as parameter \p{pAllocator}.
    ...
    {
        auto size= sizeof(T);
        return static_cast<T*>( allocator.allocate(size, alignof(T) ) );
    }
    ...
    /// This method does \b not call a destructor.
    /// To also destruct the elements of an array, use method #".DeleteArray".
    /// @tparam T         The pointer type of parameter \p{array}.
    /// @tparam TIntegral Integral type of parameter \p{size}. Deduced by the compiler.
    /// @param  array     Pointer to the first array element.
    /// @param  length    The size of the given \p{mem}.
...
/// #"lang::Allocator".<br>
/// In fact, all methods that are defined in the non-existing class #"%lang::Allocator" (which is
...
//==================================================================================================
/// Implementation of (prototype-) class #"lang::Allocator" which performs standard
...
/// For example, the type definition #"AString;2" uses this type and thus implements a common
...
/// external instance. If this type #"%HeapAllocator" is used, the constructors that do not
/// expect an allocator become effective.
///
/// @see
/// - Struct #"AllocatorMember", which should be used as a base class to inherit from
///   if an arbitrary allocator is to be stored. A specialization for this #"%HeapAllocator"
///   is given, which does not contain a member and hence avoids increasing the footprint
///   of derived types.
/// - Types #"TMonoAllocator", #"TLocalAllocator", and
///   #"TPoolAllocator" introduced with module \alib_monomem.
//==================================================================================================
struct HeapAllocator {
    /// A magic byte, used with the configuration macro #"ALIB_DEBUG_ALLOCATIONS" to mark
    /// memory and detect out-of-bounds writes.
    /// @see Method #"Allocator::dbgCheckMemory;*".
    ...
    /// @see Field #"Allocator::MIN_ALIGNMENT;*".
    static constexpr size_t             MIN_ALIGNMENT                   = alignof(std::max_align_t);

    /// Evaluates to <c>alignof(std::max_align_t)</c>.
    /// @see Field #"Allocator::MAX_ALIGNMENT;*".
    ...
    inline void*  allocate( size_t size, size_t alignment )                                  const {
        ...
        (void)  alignment;
        void* mem= std::malloc(DbgAlloc::extSize(size));
    ...
    /// #"Allocator::dbgAcknowledgeIncreasedAllocSize;*".
    /// @tparam TSize    The type of parameter \p{allocSize}. (Deduced by the compiler.)
    template<typename TSize>
...
///
/// The class has a specialization with #"lang::AllocatorMember<HeapAllocator>", which does
...
/// through it's alias name #"AString;2", which performs classic heap allocation.
/// Its alias type definition, is given as:
///   \snippet "strings/detail/tastring.hpp"     DOX_MONOMEM_ALLOCMEMBER
/// Obviously the original type #"%TString" has two template parameters, one specifying the
/// character type, the other specifying the allocator. Now looking at the
/// #"TAString;reference documentation of type TAString", it can be seen that this type
/// #"%AllocatorMember" is a protected base class and template parameter \p{TAllocator} is forwarded
/// to this base.
///
...
///
/// @tparam TAllocator The allocator type of the inherited member, as prototyped with class
///                    #"lang::Allocator".
//==================================================================================================
template<typename TAllocator>
struct AllocatorMember {
  ...
  public:
    ...
    /// This is a convenience operator that returns the #"AllocatorInterface" for the
    /// stored allocator (by calling #"Allocator::operator()();*").
    /// @return The allocator interface of the stored allocator.
    AllocatorInterface<TAllocator> AIF()                   const noexcept { return (*allocator)(); }
};

/// Specialization of this helper-struct for type #"HeapAllocator". Does not define a
...
struct AllocatorMember<HeapAllocator>
{
    ...
    /// It is used to receive a non-const reference with the method #".GetAllocator".
    ...
    /// This is a convenience operator which returns the #"AllocatorInterface" for the
    /// stored allocator (by calling #"Allocator::operator()();*").
    /// @return The allocator interface of the stored allocator.
    AllocatorInterface<HeapAllocator> AIF()     const noexcept { return HEAP_ALLOCATOR_INSTANCE(); }
};
...
/// with #"lang::Allocator;ALib Allocators".
///
/// With the inclusion of module \alib_monomem in the \alibbuild, this allocator is suitable to use
/// cases of #"alib_contmono_intro_strictweak;strict or weak monotonic allocation".
///
...
/// named \b Reset (see, for example, #"HashTable::Reset;2"), the C++ standard
/// containers do not. This is a challenge because their internal memory will be invalid
/// with a reset. For example, if allocator type #"MonoAllocator" was used with class
...
///
/// \see
///   - An alternative version that #"alib_contmono_intro_recycling;recycles nodes" (and also
///     bucket arrays in case of <c>std::unordered_set</c> and <c>std::unordered_map</c>), is
///     provided with sibling type #"StdRecyclingAllocator".
///   - Type definitions #"StdMA" and #"StdPA" given with module \alib_monomem.
///   - Further shortcuts for standard container types, given with #"StdVectorMA",
///     #"StdVectorPA", #"StdListMA", #"StdListPA", #"StdDequeMA", and
///     #"StdDequePA".
///
...
struct StdAllocator :  AllocatorMember<TAllocator> {
  ...
  //################################################################################################
    ...
    [[nodiscard]] constexpr
    T*      allocate( size_t n )            { return allocBase::AIF().template AllocArray<T>( n ); }
    ...
    constexpr
    void deallocate( T* p, std::size_t n )                   { allocBase::AIF().FreeArray( p, n ); }

}; // struct StdAllocator


/// Comparison operator for type #"StdAllocator".
...
//==================================================================================================
/// This structs' name stands for "runtime type recycling allocator".
/// The struct encapsulates an allocator and recycles (caches) objects whose size and alignment are
/// only determined and detected at runtime.
/// This type should \e only be used in combination with struct
/// #"StdRecyclingAllocator" or in similar (unlikely) situations when the type of
...
///       only at runtime, is that the implementation of the containers is not standardized.
...
///
/// If the type #"HeapAllocator" is given with the template parameter \p{TAllocator}, this
...
struct RTTRAllocator : AllocatorMember<TAllocator> {
    ...
    #if ALIB_DEBUG
    /// The detected object's runtime type information struct.<br>
  ...
  ALIB_DBG( dbgDetectedObjectTypeInfo        = &dbgTypeInfo;   )
        ...
        if( (size == detectedObjectSize) && (detectedObjectAlignment== alignment) ) {
            ...
            return allocBase::AIF().Alloc( size, alignment );
        }
        #endif
  ...
  //################################################################################################
    ...
    /// Furthermore, a #"alib_mod_assert;warning is raised" in case no previous call to #".Get"
    /// has been performed.
    ///
    ...
    void Recycle( void* mem, size_t size, size_t alignment
        ...
        {
            stack.pushFront( static_cast<Node*>( mem ) );
            #if ALIB_DEBUG_MEMORY
    ...
    /// Furthermore, a #"alib_mod_assert;warning is raised" in case that the provided memory
    ...
    void RecycleChunk( void* memUnaligned, size_t size
        ...
        char* mem = reinterpret_cast<char*>(  (size_t(memUnaligned) + detectedObjectAlignment - 1)
                                             & ~(detectedObjectAlignment -1) );
        ...
        while(size > detectedObjectSize) {
            ...
            mem  =  static_cast<char*>(mem) + detectedObjectSize;
...
/// implementation, this allocator uses type #"RTTRAllocator" which detects
...
///   of C++ standard container types, see the #"alib_mods_contmono;Programmer's Manual" of
...
struct StdRecyclingAllocator {
  ...
  //################################################################################################
    ...
    /// either #"RTTRAllocator::Get;*" or #"RTTRAllocator::AllocUnrelated;*" on the
    /// field #".recycler".
    ///
    /// @param n  The number of requested objects to allocate storage for.
    /// @return Pointer to the start of the array of \p{n} objects of type \p{T}.
    ...
    T*      allocate( size_t n ) {
        if( n == 1 )
            return static_cast<T*>(recycler.Get( sizeof(T), alignof(T)
                                                             ALIB_DBG(, typeid(T))   )   );

        return static_cast<T*>(    recycler.AllocUnrelated( sizeof(T) * n, alignof(T)
                                                                          ALIB_DBG(, typeid(T) )) );
    }

    /// Implementation of <c>std::allocator</c> interface.
    /// Dependent on whether a single or multiple objects are de-allocated (parameter \p{n}), this
    /// method invokes either #"RTTRAllocator::Recycle;*" or
    /// #"RTTRAllocator::RecycleChunk;*" on field #".recycler".
    ///
