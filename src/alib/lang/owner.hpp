...
/// of type #"%CallerInfo".
/// Use the macro #"ALIB_CALLER_PRUNED" to prune those with release-builds.
///
/// In the rather unusual case that it is a runtime decision whether this object should
...
///     They are described and listed in chapter #"alib_threads_locks_auto_owner" of the
///     Programmer's Manual of module \alib_threads.
///   - Besides generic preprocessor macro #"ALIB_OWN", several application-specific alias macros
///     exist.
///     For example, macros
///     - #"ALIB_LOCK",
///     - #"ALIB_LOCK_WITH",
...
/// Similar to class #"lang::Owner", but calls method #"%Lock::TryAcquire" instead of
/// #"%Lock::Acquire".
/// The result is retrievable with method #".IsOwning".
/// @see Chapter #"alib_threads_locks_auto_owner" of the Programmer's Manual of module
///      \alib_threads
/// @tparam TOwnable  The type to own.
///                   Requirements are to have methods #"%Lock::TryAcquire" and #"%Lock::Release"
///                   available.
/// @tparam TOptional If \c true, then checks is performed, whether the given #".owned" is
///                   \e nulled and thus not acquired and released.
///                   If \c false such checks are omitted.
...
/// Similar to class #"lang::Owner", but calls method \b TryAcquireTimed instead of
/// #"%TimedLock::Acquire".
/// The result is retrievable with method #".IsOwning".
/// @see Chapter #"alib_threads_locks_auto_owner" of the Programmer's Manual of module
///      \alib_threads
/// @tparam TOwnable  The type to own.
///                   Requirements are to have methods \b TryAcquireTimed and
///                   \b Release available.
/// @tparam TOptional If \c true, then checks is performed, whether the given #".owned" is
///                   \e nulled and thus not acquired and released.
///                   If \c false such checks are omitted.
...
class OwnerTimed {
  ...
  protected:
    OwnablePointer  owned;      ///< The resource to acquire and release.
    bool            isOwning;   ///< The result of the call to \b TryAcquireTimed.
    #if ALIB_DEBUG
    CallerInfo  dbgCI;      ///< Caller information. Available only with debug-builds.
    #endif
  ...
  public:
    ...
    /// @param  ci         Caller information.
    /// @tparam TTimeValue The type of the parameter \p{time} accepted with construction and passed
    ///                    to the method <b>TOwnable::TryAcquireTimed</b>.<br>
    ///                    Usually this is type #"Ticks" or its inherited type #"^Ticks::Duration".
...
/// Similar to class #"lang::Owner", but calls methods \b AcquireRecursive and
/// \b ReleaseRecursive.
/// @see Chapter #"alib_threads_locks_auto_owner" of the Programmer's Manual of module
///      \alib_threads
/// @tparam TOwnable  The type to own.
...
/// Similar to class #"lang::Owner", but calls methods \b AcquireShared and
/// \b ReleaseShared.
/// @see Chapter #"alib_threads_locks_auto_owner" of the Programmer's Manual of module
///      \alib_threads
/// @tparam TOwnable  The type to own.
...
/// Similar to class #"lang::Owner", but calls method \b TryAcquireShared instead of
/// #"%SharedLock::Acquire" and #"%SharedLock::ReleaseShared" instead of #"%SharedLock::Release".
...
class OwnerTryShared {
  ...
  protected:
    OwnablePointer  owned;      ///< The resource to acquire and release.
    bool            isOwning;   ///< The result of the call to \b TryAcquireShared.
    #if ALIB_DEBUG
    CallerInfo  dbgCI;      ///< Caller information. Available only with debug-builds.
    #endif
...
/// Similar to class #"lang::Owner", but calls method \b TryAcquireSharedTimed instead
/// of \b Acquire and \b ReleaseShared instead of #"%Shared".
/// The result is retrievable with the method #".IsOwning".
/// @see Chapter #"alib_threads_locks_auto_owner" of the Programmer's Manual of module
///      \alib_threads
/// @tparam TOwnable  The type to own.
...
class OwnerSharedTimed {
  ...
  protected:
    OwnablePointer  owned;      ///< The resource to acquire and release.
    bool            isOwning;   ///< The result of the call to \b TryAcquireSharedTimed.
    #if ALIB_DEBUG
    CallerInfo  dbgCI;      ///< Caller information. Available only with debug-builds.
    #endif
  ...
  public:
    ...
    /// the template parameter \p{TOptional} is \c true.
    /// @tparam TTimeValue The type of the parameter \p{time} accepted with construction and passed
    ///                    to method <b>TOwnable::TryAcquireTimed</b>.<br>
    ///                    Usually this is type #"Ticks" or its inherited type #"^Ticks::Duration".
    /// @param time        The duration to wait for, or point in time to wait until.
    /// @param ownable     The ownable to acquire.
    /// @param ci          Caller information.
