...
/// including the header #"F;ALib.Lang.H".
///
...
///
/// \see Macro #"bitsof;bitsof(type)", which works directly on the type.
///
...
/// If the parameter \p{TWidth} is greater or equal to the width of the given \p{TIntegral} type, then
/// all bits are set in the returned value.
///
/// \see While this is the fully templated version, with
///      #"LowerMask(ShiftOpRHS);LowerMask<TIntegral>(ShiftOpRHS)", a runtime version is given.
///
...
/// \p{TIntegral} type.
/// In debug-compilations, an #"alib_mod_assert;error is raised" in that case.
///
/// \see A fully templated version usable when the number of bits are known at compile time,
///      is given with #".LowerMask()".
///
/// @tparam TIntegral The integral type to operate on.
...
constexpr TIntegral LowerMask( ShiftOpRHS width ) {
    ALIB_ASSERT_ERROR( width < bitsof(TIntegral), "LANG",
...
///
/// \see While this is the fully templated version, with
///      #"UpperMask(ShiftOpRHS);UpperMask<TIntegral>(ShiftOpRHS)", a runtime version is given.
///
...
/// If given \p{TWidth} is greater or equal to the width of the given \p{TIntegral} type, then
/// all bits are returned.
///
/// \see While this is the fully templated version and hence explicitly \c constexpr for the
///      reader of a code, with
///      #"LowerBits(ShiftOpRHS, TIntegral);LowerBits<TIntegral>(ShiftOpRHS, TIntegral)",
///      a version is given.
///
...
/// out of the given integral value.
/// Parameter \p{width} <b>must not be greater or equal</b> to the width of the given
/// \p{TIntegral} type. In debug compilations, an #"alib_mod_assert;error is raised".
///
/// \see A fully templated version, usable when the number of bits is known at compile time,
///      is given with #"LowerBits(TIntegral)".
///
...
///   In debug-compilations, this method #"alib_mod_assert;raises an ALib error" in this case,
///   while in release-compilations, the result is <em>'undefined'</em>.
///   With function #"MSB0", an alternative is given which returns \c 0 if the input is \c 0.
///
/// @tparam TIntegral The integral type to operate on.
...
constexpr int CLZ( TIntegral value ) {
      ...
      ALIB_ASSERT_ERROR( value != 0, "LANG",
...
/// Variant of #"CLZ" which tests the given parameter \p{value} on \c 0 and returns
/// <c>sizeof(TIntegral) * 8</c> in this case.
/// Otherwise, returns the result of #"CLZ".
///
/// @tparam TIntegral The integral type to operate on.
...
///   In debug-compilations, this method #"alib_mod_assert;raises an ALib error" in this case,
///   while in release-compilations, the result is <em>'undefined'</em>.
///   With function #"CTZ0", an alternative is given which returns \c 0 if the input is \c 0.
///
/// @tparam TIntegral The integral type to operate on.
/// @param  value     The value to test. Must not be \c 0.
...
/// Variant of #"CTZ" which tests given parameter \p{value} on \c 0 and returns
/// <c>sizeof(TIntegral) * 8</c> in this case.
/// Otherwise, returns the result of #"CLZ".
///
/// @tparam TIntegral The integral type to operate on.
...
/// Internally, this method uses #"CLZ" and returns
///
...
/// Variant of #"MSB" which tests given parameter \p{value} on \c 0 and returns \c 0 in this
/// case. Otherwise, returns the result of #"MSB".
///
/// @tparam TIntegral The integral type to operate on.
