...
///   (on supported platforms, via functions #"CLZ" and #"CTZ") to search the
...
///   #"alib_enums_iter_bitset;address bits with enumeration elements".
...
/// - Methods #"Set" and #"TBitSet::Reset" allow passing several bit indices at once using a comma-separated list.
...
///   #"TBitSet::TBitSet(const std::nullptr_t&)", which
///   avoids code and runtime penalty, in cases where the bits are to be initialized at a later
///   stage.
///
/// In respect to performance, this type equals a typical implementation <c>std::bitset</c>.
///
/// @see
/// - Type definition #"enumops::EnumBitSet", which defines a bitset over
///   #"alib_enums_iter;ALib Iterable Enums" based on this type.<br>
/// - For a quick tutorial on the use of this type (in nice combination with C++ enumerations),
///   see chapter #"alib_enums_iter_bitset" of the
///   Programmer's Manual of module \alib_enumops_nl.
/// - Type definition #"alib::BitSet;BitSet" in namespace #"alib", which defines a set with
///   interface-type <c>int</c>.
/// - Class #"BitBuffer", which implements a sort of stream buffer to store
...
class TBitSet {
  ...
  public:
    ...
    class Reference {
      protected:
        ...
        TBitSetCM*  bitSet;  ///< The #"%TBitSet" this reference addresses.
      ...
      public:
        ...
        /// @return The #"%TBitSet" that this object refers to.
  ...
  //------------------------------------------- operators ------------------------------------------
    ...
    /// @param rhs The other #"%TBitSet" to compare this with.
  ...
  protected:
    /// Implementation of \c std::iterator_traits for class #"%TBitSet".
    ...
    /// - #"TBitSet::Iterator",
    /// - #"TBitSet::ReverseIterator",
    /// - #"TBitSet::ConstIterator", and
    /// - #"TBitSet::ConstReverseIterator".
    ///
    /// @tparam TBitSetCM A constant or mutable TBitSet.
  ...
  //############################## To satisfy concept of  InputIterator ############################
        ...
        /// Retrieves a reference to the internal #"TBitSet::Reference" member that this
        /// iterator uses.
...
/// Performs binary AND operation two #"%TBitSet" objects (of equal size), \p{lhs} and \p{rhs}.
...
/// @tparam TEnd  Template parameter of class #"%TBitSet". Deduced by the compiler.
/// @tparam TBegin        Template parameter of class #"%TBitSet". Deduced by the compiler.
/// @tparam TInterface       Template parameter of class #"%TBitSet". Deduced by the compiler.
/// @return A temporary #"%TBitSet" containing the result of the operation.
ALIB_EXPORT
...
/// Performs binary OR operation two #"%TBitSet" objects (of equal size), \p{lhs} and \p{rhs}.
/// \note This operator function is located in the global namespace. The documentation shows
///       namespace <em>alib</em>, which is done for the purposes of organizing the manual
///       index better.
/// @param lhs               The left-hand side operand.
...
/// Performs binary XOR operation two #"%TBitSet" objects (of equal size), \p{lhs} and \p{rhs}.
/// \note This operator function is located in the global namespace. The documentation shows
///       namespace <em>alib</em>, which is done for the purposes of organizing the manual
///       index better.
/// @param lhs               The left-hand side operand.
