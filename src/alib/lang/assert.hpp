//==================================================================================================
/// \file
/// This header-file is part of module \alib_assert of the \aliblong.
///
...
///
/// Available only if the configuration macro #"ALIB_DEBUG_ASSERTION_PRINTABLES" is given.
...
/// whether the assertion or warning was raised or not -  in the case that the configuration macro
/// #"ALIB_DEBUG_ASSERTION_PRINTABLES" is given.<br>
/// This is used to ensure that all arguments passed are of
/// #"RegisterPrintable;a known type".
...
/// Embeds two flags that allow control of whether function #"Raise" halts on errors,
/// respectively warnings.
/// The default is to halt on errors, but not on warnings.<br>
/// A thread-local instance is given (and internally tested) with the function
...
/// Custom functions may be registered using function #"RegisterPrintable".
...
/// The output stream used to with function #"assert::Raise", when the parameter \p{type}
/// equals \c 0.<br>
...
/// Allows registering custom types to be printable with debug-function #"assert::Raise".
...
/// (The latter usually depends on special the configuration macros like #"ALIB_DEBUG_MEMORY".)
///
/// It may be used by custom code for custom assertions as well.
///
/// This method first checks if the function pointer #"assert::PLUGIN" is set
...
///   - Function #"raise" (called by this method after converting the variadic arguments to
///     an array of <c>std::any</c> objects.
///   - Type definition #"AnyConversionFunc" to register custom printable types.
///   - Function #"RegisterPrintable" to register a conversion function.
///   - Namespace variable #"assert::FORMAT" that defines the output format.
///   - Namespace variables #"STREAM_ERRORS", #"STREAM_WARNINGS", and #"STREAM_MESSAGES" allowing
///     to redirect assertion messages.
...
/// writing the assertion, warning or message text of the functions #"Raise", respectively
/// #"raise".
///
/// With the use of module \alib_alox this pointer can be set to redirect the output to a
/// #"%Lox". Further information on this is found with the chapter
/// #"alib_mod_alox_debug_and_release_logging_ft_4" of the Programmer's Manual of module
/// \alib_alox_nl.
///
