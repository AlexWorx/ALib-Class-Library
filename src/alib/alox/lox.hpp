...
class PrefixLogable : public Box
{
    ...
    : Box( src )
    {
        if ( IsArrayOf<char>() )
        {
            ...
            new (this) Box( copy= new AString( Unbox<NString>() ));
        }
        else if ( IsArrayOf<wchar_t>() )
        {
            // use "placement new" to overwrite the box part of ourselves
            new (this) Box( copy= new AString( Unbox<WString>() ));
        }
    }
...
class Lox
{
    ...
    protected:
        ...
        /// Scope Domains
        core::ScopeStore<NAString*>                 scopeDomains;
        ...
        const aworx::String                         noKeyHashKey                    = ASTR("$");
    ...
    public:
         ...
         ******************************************************************************************/
        const NString   GetName()                         { return scopeInfo.loxName;              }
         ...
         ******************************************************************************************/
        void      SetSourcePathTrimRule( const NString& path,
                                         Inclusion      includeString   = Inclusion::Exclude,
                                         int            trimOffset      = 0,
                                         Case           sensitivity     = Case::Ignore,
                                         const NString& trimReplacement = NNullString,
                                            ...
                                            bool        allowAutoRule   = true                  )
        {
            scopeInfo.SetSourcePathTrimRule( nullptr, allowAutoRule ? Inclusion::Include
                                             ...
                                             Case::Ignore, NNullString, reach, Priorities::NONE  );
        }
         ...
         ******************************************************************************************/
        ALIB_API
        static core::textlogger::TextLogger*  CreateConsoleLogger( const NString& name= nullptr );
         ...
         ******************************************************************************************/
        ALIB_API
        core::Logger*   GetLogger( const NString& loggerName );

        /** ****************************************************************************************
         * Removes a logger from this container.
         * \note
         *  To (temporarily) disable a logger without removing it, a call to
         *  \ref SetVerbosity(core::Logger*,Verbosity,const NString&,Priorities) "SetVerbosity(logger, Verbosity::Off)"
         *   can be used.
         *
         ...
         ******************************************************************************************/
        ALIB_API
        core::Logger*   RemoveLogger( const NString& loggerName );
        ...
        /** ****************************************************************************************
         * Same as \ref #SetVerbosity(core::Logger*,Verbosity,const NString&,Priorities) "SetVerbosity"
         ...
         * using \ref #SetVerbosity(core::Logger*,Verbosity,const NString&,Priorities) "SetVerbosity".
         *
    ...
    #if ALIB_DEBUG
        /** ****************************************************************************************
         * Returns the memory allocator of type \alib{util,MemoryBlocks} used with the different
    ...
    protected:
         ...
         * \ref aworx::lib::lox::core::Domain::Separator "Domain::Separator",
         ...
         * @return The resulting \ref aworx::lib::lox::core::Domain "Domain".
         ******************************************************************************************/
        ALIB_API
        core::Domain*   evaluateResultDomain( const NString& domainPath );
         ...
         ******************************************************************************************/
        ALIB_API
        core::Domain*   findDomain( core::Domain& domainSystem, NString domainPath );
         ...
         ******************************************************************************************/
        ALIB_API
        void            logInternal( Verbosity verbosity, const NString& subDomain, const String& msg );
...
ALIB_LANG_ENUM_IS_BITWISE( aworx::lib::lox::Lox::StateInfo )
ALIB_LANG_ENUM_PARSABLE(   aworx::lib::lox::Lox::StateInfo )
ALIB_LANG_RESOURCED(       aworx::lib::lox::Lox::StateInfo, aworx::lib::ALOX,  ASTR("StateInfo")  )
