...
class Lox {
  ...
  public:
    ...
    ///   - - - - - -| - - - - - - - - - - - - - - - - - - -
    ///   LGR        | Used when \e Loggers are registered, retrieved or removed from a #"%Lox" and when the \e Verbosity of a <em>Log Domain</em> for a \e Logger is changed.<br>In addition used with method #"%Lox"::SetStartTime.
    ...
    ///       setting the root domain of a #"%Lox" to a certain \e Verbosity does \e not affect
  ...
  //################################################################################################
    ...
    /// The name is immutable and all #"%Lox" objects registered with \alox must be unique.
    ...
    /// #"Lox::Register;2" is invoked and the object will be retrievable with the static
    /// method #"Lox::Get;2". In some situations, such 'registration' may not be wanted.
    ///
    ...
    /// Returns a #"%Lox" with the given name.
    /// A #"%Lox" is only found if it was created and registered with \alox using #"Register".
    /// If not found, and parameter \p{create} is \c true (the default), a new #"%Lox" is created,
    /// registered and returned.
    ///
    /// @param name      The name of the #"%Lox" to search and optionally to create.
    ///                  Comparison is case-insensitive.
    /// @param create    Denotes whether a #"%Lox" that was not found is created.
    ///                  Optional and defaults to #"%CreateIfNotExists::No".
    /// @return The #"%Lox" found, \c nullptr in case of failure.
    ALIB_DLL
    static Lox*     Get( const NString&          name,
                         lang::CreateIfNotExists create= lang::CreateIfNotExists::No );

    /// Registers or un-registers a #"%Lox" object statically with \alox.
    /// Once registered, any code entity of the same process is enabled to retrieve
    /// the #"%Lox" using #".Get".<br>
    ...
    ///
    /// If debug-logging is enabled (depends on optional configuration macros) and used, the
    /// singleton of type \c Lox provided for debug-logging is registered. This uses the name
    /// \c "Log".
    ///
    /// Registration is \e not mandatory but done by default by the constructor of class #"%Lox".
    /// Therefore, to keep a #"%Lox" private, an optional parameter is available.
    ///
    /// @param lox       The #"%Lox" to register.
    /// @param operation If #"%ContainerOp::Remove", the given \p{Lox} is deregistered.
    ///                  Defaults to #"%ContainerOp::Insert".
    ALIB_DLL
  ...
  // Interface
  //################################################################################################
    /// Returns the name of this #"%Lox". The name is user-defined, and provided in the
    /// constructor,
    ...
    /// Returns the number of logs that have been performed with this #"%Lox".
    ...
    ///
    /// \attention
    ///   This method was introduced to support resetting the debug #"%Lox" objects in the unit
    ...
    /// logged (e.g., with meta-information of class #"%TextLogger".
    ///
    /// By default such setting affects all instances of class #"%Lox", not only
    ...
    ///                        not.
    ///                        Optional and defaults to #"%Inclusion::Exclude".
    ...
    ///                        Optional and defaults to #"%Case::Ignore".
    /// @param trimReplacement Replacement string for trimmed portion of the path.
    ///                        Optional and defaults to #"%NULL_STRING".
    ...
    /// @param priority        The priority of the setting. Defaults to #"var Priority::Standard".
    ...
    /// If parameter \p{global} is set to #"%Inclusion::Include", the global rules are cleared
    /// in addition.
    ///
    ...
    ///
    /// \see Chapter #"alib_mod_alox_trim_source_path" for more information.
    ///
    ...
    /// configuration variable #"alxcvALOX_CONSOLE_TYPE" is checked. If this variable is not set,
    /// then the decision is made as follows:
    /// - On GNU/Linux OS, a
    ///   #"alib::lox::loggers::AnsiConsoleLogger;AnsiConsoleLogger" is chosen.
    /// - On Windows OS, if a console window is attached, type
    ///   #"alib::lox::loggers::WindowsConsoleLogger;WindowsConsoleLogger" is chosen. If
    ///   no console is attached to the process, instead a
    ///   #"alib::lox::loggers::ConsoleLogger;ConsoleLogger" is returned.
    ///
    ///
    ...
    ///   #"SetVerbosity(int, Verbosity, Priority);SetVerbosity(logger, Verbosity::Off)"
    ///   can be used.
    ///
    ...
    /// #"%Verbosity::Off".
    ///
    /// To deregister a \e Logger with a #"%Lox", use method #".RemoveLogger(detail::Logger*)".
    /// To 'disable' a \e Logger, invoke this method with parameters \p{verbosity} equaling to
    /// #"%Verbosity::Off" and \p{domain} to \c "/".
    ///
    ...
    /// on protecting verbosity settings, consult the #"alib_mod_alox".
    ///
    /// \attention
    ...
    ///   code lines...
    ///
    /// @param logger     The logger to be affected.
    ...
    /// Same as #"SetVerbosity(detail::Logger*,Verbosity,const NString&,Priority)"
    /// but addressing the \e Logger to manipulate by its name.<br>
    /// This variant of the method may only be used after a \e Logger was once 'registered' with
    /// this #"%Lox" using #"SetVerbosity(detail::Logger*,Verbosity,const NString&,Priority)".
    /// @param loggerName The logger to be affected, identified by its name (case
    ///                   insensitive).
    /// @param verbosity  The 'level of verboseness' to be set.
    /// @param domain     The parent (start) domain to be set. The use of absolute paths
    ///                   starting with <c> '/'</c> are recommended.
    ...
    /// Sets in the configuration variable #"Variables::VERBOSITY" of the given logger, the flag
    /// #"CVVerbosities::ExportAll;2" to \c true or \c false.
    /// If \c true is given, furthermore an internal flag is set, which causes the method
    /// #"Variable::IsWriteBack;2" to return \c true.<br>
    /// If the variable is defined already with a higher priority than the given one, then nothing
    /// is done.
    /// @param logger     The logger to be affected.
    /// @param value      The value to set.
    /// @param priority   The priority of the setting. Passed to the method #"Variable::Define;*".
    ALIB_DLL
    void            SetVerbosityExport( detail::Logger*  logger,
                                        bool             value,
                                        Priority         priority=Priority::Standard)
    { detail::LI::SetVerbosityExport(impl, logger, value, priority); }

    /// Same as #"SetVerbosityExport(detail::Logger*,bool value,Priority)"
    /// but addressing the \e Logger by its name.
    /// @param loggerName The logger to be affected, identified by its name (case
    ///                   insensitive).
    /// @param value      The value to set.
    /// @param priority   The priority of the setting. Passed to the method #"Variable::Define;*".
    ALIB_DLL
    void            SetVerbosityExport( const NString&   loggerName,
                                        bool             value,
                                        Priority         priority=Priority::Standard)
    { detail::LI::SetVerbosityExport(impl, loggerName, value, priority); }
    ...
    /// #"SetDomain(const NString&, Scope)" is applicable only for
    ...
    ///
    /// For #"%Lox" objects that should be protected of external manipulation, it is advisable
    /// to remove all <em>Domain Substitution Rules</em> right after the #"%Lox" was created by
    ...
    /// domains of the #"%Lox" can be substituted and then the resulting domains \e Verbosities
    ...
    ///
    /// For more information, consult the chapter #"alib_mod_alox_domain_substitution" of the
    /// Programmer's Manual.
    ///
    ...
    /// use one of the overloaded methods #".SetDomain(const NString&, Scope)" and provide an
    /// empty or \e nulled value for parameter \p{scopeDomain} (the same as how domain paths of
    /// other \e Scopes are removed).
    ///
    /// \note
    ...
    ///   their definition, which is performed using the standard interface that allows 'removing'
    ///   any other <em>Scope Domain</em>. (Passing an empty or \e nulled domain
    ///   path to method #".SetDomain(const NString&, Scope)".)
    ///
    /// @param scopeDomain The domain path to register.
    /// @param scope       Either \e Scope::ThreadOuter or \e Scope::ThreadInner. With other
    ///                    values, an internal error is logged.
    ...
    ///   #"%AString" might be passed wrapped in class \c std::reference_wrapper<AString>.<br>
    ///   For more information, consult manual chapter
    ///   #"alib_mod_alox_prefix_logables_lifecycle"
    ///   as well as chapter #"alib_boxing_customizing_identity" of the Programmer's Manual
    ...
    ///   #"TBoxes" may be passed with parameter \p{logables}, like
    ...
    /// This overloaded version of
    /// #"SetPrefix(const Box&,Scope)" is applicable only for
    /// #"%Scope::ThreadOuter" and #"%Scope::ThreadInner" and allows specifying the thread that
    /// the setting should be associated with.
    ///
    ...
    ///   This means, different to <em>Prefix Logables</em> of type #"%AString" or custom types,
    ///   the life-cycle of the object passed in parameter \p{prefix} is allowed to end
    ///   right after the invocation of this method. For more information, consult manual
    ///   chapter #"alib_mod_alox_prefix_logables_lifecycle" as well as chapter
    ///   #"alib_boxing_customizing_identity" of the Programmer's Manual of module
    ///   \alib_boxing.
    ///
    /// \attention
    ...
    ///                      If given, the associated object of #"Thread" must not
    ...
    /// Stores data encapsulated in an object of class #"Box" which can be retrieved back by
    /// invoking #".Retrieve(const NString&)".
    /// Using the optional \p{key} and \p{scope} offers various possibilities to reference this
    /// data later.<br>
    ...
    ///   passed!<br>
    ///   For more information, consult chapter #"alib_mod_alox_log_data" of the
    ///   Programmer's Manual.
    ///
    ...
    ///                  by this #"%Lox" when overwritten or this lox is deleted.
    ...
    /// #"alib::boxing::Box;Box" which had been stored in a prior call to
    /// #".Store(const Box&, const NString&)".
    /// Using the optional \p{key} and \p{scope} offer various possibilities to reference such
    /// objects.<br>
    ///
    /// \note If no data is found, an \e nulled object is returned. This can be tested using method
    ///       #"alib::boxing::Box::IsNull;Box::IsNull".
    ///
    /// <p>
    /// \note <em>Log Data</em> is a feature provided by \alox to support debug-logging.
    ///       It is not advised to use <em>Log Data</em> to implement application logic.
    ...
    /// * Overloaded version of #".Retrieve(const NString&)" which omits parameter \p{key}.
    ///
    /// \note <em>Log Data</em> is a feature provided by \alox to support debug-logging.
    ///       It is not advised to use <em>Log Data</em> to implement application logic.
    ///
    ...
    ///   your code, \alox provides configuration variable#"alxcvALOX_LOXNAME_DUMP_STATE_ON_EXIT".
  ...
  //################################################################################################
    ...
    /// Note that the #"%Lox" instance has to be acquired before invoking this method and
    ...
    ///
    /// \note
    ///   This method allows a consistent interface of overloaded methods #"%.Info", #"%.Error",
    ...
    /// #"TBoxes", then the contents of this list is inserted into
    ...
    /// ambiguities in respect to domain names.
    ///
    /// If one of the arguments (or a single argument given) is of type #"TBoxes",
    ...
    /// #"alib::lox::Scope;Scope" is given with parameter \p{scope}, then the
    /// counter is associated with the scope.<br>
    /// Finally, parameters \p{key} and \p{scope} can also be used in combination. The key is
    /// then unique in respect to the #"alib::lox::Scope;Scope" provided.
    ///
    ...
    ///   of type #"TBoxes" may be passed with parameter
    ...
    /// Overloaded version of
    /// #"Once(const NString&,Verbosity,const Box&,const String&,Scope,int);Once".
    ///
    /// @param verbosity The verbosity of the <em>Log Statement</em> (if performed).
    /// @param logables  The objects to log (Multiple objects may be provided within
    ///                  container class Boxes.)
    ...
    ///
    /// This method may be used to avoid the execution of more complex logging code (or in very
    /// performance-critical code sections) when such logging would not result in log activity due
    /// to the current log level settings.
    /// @see The method #".GetVerbosity".
    ///
    ...
    /// Returns that verbosity that has at least one active logger in respect to the given
    /// log domain.
    ///
    /// This method may be used to avoid the execution of more complex logging code (or in very
    /// performance-critical code sections) when such logging would not result in log activity due
    /// to the current log level settings.
    /// @see The method #".IsActive".
    ///
    /// @param      domain       The log domain. All rules for resolving the effective log
    ///                          domain apply as with normal log statements.<br>
    ///                          Defaults to \b NULL_NSTRING.
    ...
    ///                          \alox from the optional \p{domain} and all rules that apply
    ///                          at the place of invocation.
    /// @return The "lowest" verbosity that has an active logger.
    Verbosity GetVerbosity( const NString&  domain      = NULL_NSTRING,
                            NAString*       resultDomain= nullptr   )
    { return detail::LI::GetVerbosity( impl, domain, resultDomain ); }
  ...
  //################################################################################################
    ...
    ///
    /// \par Availability
    ///   This method is available only with debug-builds with #"ALIB_DEBUG_MEMORY" set.
    ///
    /// @return The monotonic allocator of this #"%Lox".
    MonoAllocator& DbgGetMonoAllocator()          { return  detail::LI::DbgGetMonoAllocator(impl); }
  #endif
