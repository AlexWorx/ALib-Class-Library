...
/// #"logPlainTextPart" and #"notifyPlainTextLogOp".
//==================================================================================================
class PlainTextLogger : public TextLogger {
  ...
  //################################################################################################
  public:
    /// If this field is set to \c true (the default), all #"alib::lox::ESC;ESC" color and
    ...
    /// Logger are (later) written into a different logger (e.g., as a multi-line message)
  ...
  protected:
    ...
    explicit  PlainTextLogger( const NString& name, const NString& typeName)
  ...
  protected:
    ...
    /// log message is started. It is called exactly once before a series of #"logPlainTextPart"
    /// calls and exactly once after such series. If either the start or one of the calls
    /// to #".logPlainTextPart" returns a negative number, the second invocation that would
    /// indicate the end of a log message is omitted.
    ///
    ...
    ///         #"notifyPlainTextLogOp;notifyPlainTextLogOp(Phase::End)".
    ...
    /// aborted. In this case, no invocation of
    /// #"notifyPlainTextLogOp;notifyPlainTextLogOp(Phase::End)" will be performed.
    ///
  ...
  protected:
    ...
    /// - #"notifyPlainTextLogOp;notifyPlainTextLogOp(true)"
    /// -   #".logPlainTextPart"
    /// -   ...
    /// - #"notifyPlainTextLogOp;notifyPlainTextLogOp(Phase::End)"
    ///
