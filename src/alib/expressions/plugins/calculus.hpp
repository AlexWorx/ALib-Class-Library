...
///
/// In addition, helper-class #"%Calculus" is defined here, which is the foundation for most of
/// the built-in functionality.
///
/// It is strongly recommended to use this helper-class for the creation of custom compiler plug-ins
/// instead of deriving such from class #"CompilerPlugin" directly.
///
...
//==================================================================================================
/// ## 1. Introduction ##
/// This struct specializes #"%CompilerPlugin" and provides generic approaches and the
/// implementation of variants of the method
/// #"CompilerPlugin::TryCompilation(CIUnaryOp&);CompilerPlugin::TryCompilation".
...
/// - #"CompilerPlugin::TryCompilation(CIFunction&)",
/// - #"CompilerPlugin::TryCompilation(CIUnaryOp&)",
/// - #"CompilerPlugin::TryCompilation(CIBinaryOp&)", and
/// - #"CompilerPlugin::TryCompilation(CIAutoCast&)".
///
...
/// 2. Create constant value objects and native C++ callback functions.
...
///   #"alib_expressions_cpcc_calculus"
///   and tutorial section
///   #"alib_expressions_tut_ffext"
///
/// <p>
/// \note
...
/// plug-in should be placed. As explained above, its duty is to fill the vector/hash map using
/// the right combination of operator or function argument types, as well as providing a proper
/// #"alib_expressions_prereq_sb;sample box" that corresponds to the type of the
...
/// While parent class #"%CompilerPlugin" does not make any direct distinction between functions
...
/// Such constant identifiers are supported by populating the table #"ConstantIdentifiers" which is
/// a \c std::vector of elements of type #"%ConstantIdentifierEntry".
/// For details on each "table column", refer to the documentation of the fields of
/// #"Calculus;ConstantIdentifierEntry".
///
/// Non-constant identifiers and functions are supported by populating the table
/// #".Functions", which is a \c std::vector of elements of type #"%FunctionEntry".
/// For details on each "table column", refer to the documentation of the fields of
...
///   \c std::vector of #"alib_expressions_prereq_sb;sample boxes". It is
...
///   signature.
/// - Variadic functions are supported by adding a final \e nulled #"%Box" to the argument list.
...
///   #"%Box" to the signature vector, which leads to functions that accept just any number of any
...
///   function is to be provided. For this, macro #"CALCULUS_CALLBACK" is defined.
///   The macro creates a stringified version of the given function pointer, separated by a comma.
/// - Flag #"FunctionEntry::IsCTInvokable"
///   is a boolean value
...
/// Wherever a second operator argument's type is requested, in the case of unary operators, static
/// type specifier #"Types::Void;*" is to be given.
///
/// The compilation of unary and binary operators is supported by populating the hash map
/// #"Operators".
/// For feeding the map with entries, the following convenience types and methods are provided:
/// - #"AddOperator"<br>
///   This function adds compilation information for a single operator to the field #"Operators".
///
...
/// #"alib_expressions_operators_aliases",
...
/// #"Compilation::AliasEqualsOperatorWithAssignOperator;*" is set in field
/// #"Compiler::CfgCompilation;*".
///
/// #### Aliasing Bitwise Boolean Operators: ####
/// In contrast to the above, compilation flag #"Compilation::AllowBitwiseBooleanOperators;*"
/// affects only built-in type \e boolean - just as the flag's name suggests.
/// The flag is therefore tested only in derived plug-in #"plugins::Arithmetics;*".
...
struct Calculus   : public CompilerPlugin {
    ...
    /// #"CallbackDecl" already at compile-time.
    ///
    ...
    ///
    /// \note
    ///   This type is used with helper-class #"plugins::Calculus;*" but exposed
    ...
    /// Constructor.
    /// @param name      Assigned to the field #"CompilerPlugin::Name;*".
  ...
  //################################################################################################
    ...
    /// An entry of field #"Calculus::ConstantIdentifiers".
    /// Describes how the identifier is recognized and the constant value to return for it.
    ...
    /// An entry of field #"Calculus::Functions". Describes how the function is recognized and
    ...
    struct FunctionEntry {
        ...
        /// \c nullptr or a #"alib_boxing_more_void_void;void box" may be given as the last
        ...
        ///
        /// \see Macro #"CALCULUS_SIGNATURE" which is recommended to be used to pass both
        ///      fields (this and #".SignatureLength"). The macro accepts a C++ array of #"%Box"* and
        ...
        /// library. Use preprocessor macro #"CALCULUS_CALLBACK" to provide this field
        /// together with field #".Callback". The macro selects to prune the name string
        /// in release compilations.
        const char*             DbgCallbackName;
        #endif

        /// The result type given as a pointer to a #"alib_expressions_prereq_sb;sample box".
        ///
        /// \note
        ...
        ///   #"Calculus::ConstantIdentifiers"
        Box*                        ResultType;

        /// Denotes, if the callback function is allowed to be invoked on the
        /// #"expressions::Scope" object used at compile-time.
    ...
    /// Searches in vectors #".Functions" and #".ConstantIdentifiers" for an entry matching \p{name}
    /// and, if found, adds either a constant value or a callback function to \p{ciFunction}.
    ///
    /// This plug-in corrects abbreviated and letter case differences in functions within
    /// in/out parameter #"CIFunction::Name".
    ///
  ...
  public:
    ...
    /// invoked on the #"expressions::Scope" object used at compile-time.
    ...
    /// are constant.
    ///
    /// A fourth tuple member of type #"^String" is available only with debug-builds
    ...
    ///   For unary operators, value #"Types::Void;*" is to be provided.
    ...
    /// - The result type sample box, respectively, if \b callback is \c nullptr, the constant
    ///   result value.
   ...
   /// - The operator that gets aliased.
    ...
    ///                        for.
    ///                        For unary operators, value #"Types::Void;*" is to be
    ///                        provided.
    ...
    /// to #"AddOperators(OperatorTableEntry*" table, size_t length).
    ///
    ...
    /// Templated helper method. Deduces the array size of the given table and passes it
    /// to #"AddOperatorAliases(OperatorAliasTableEntry*" table, size_t length).
    ///
    /// @tparam TCapacity Implicitly deferred size of the array provided.
    /// @param  table     The table containing operator compilation information.
    template<size_t TCapacity>
  ...
  public:
    ...
    ///
    /// The stored element of type #"%Box" may contain either, a constant result value that replaces
    ...
    /// Templated helper method. Deduces the array size of the given table and passes it
    /// to #"AddBinaryOpOptimizations(BinaryOpOptimizationsTableEntry*", size_t).
    ///
    /// @tparam TCapacity Implicitly deferred size of the array provided.
    /// @param  table     The table containing operator compilation information.
    template<size_t TCapacity>
    ...
    /// Searches in #Operators for an entry matching the combination of #"CIBinaryOp::Operator"
  ...
  //################################################################################################
    ...
    struct AutoCastEntry {
        ...
        /// returns a value of type #"Types::Integer;*" which is generated by
        /// taking the #"Box::Data;raw value" of the argument box. This is
        ...
        /// is inserted automatically. Instead of the aforementioned macro #"CALCULUS_CALLBACK"
        /// use macro #"CALCULUS_DEFAULT_AUTOCAST" instead.
        const char*             DbgCallbackName;
        #endif

        /// The result type given a #"alib_expressions_prereq_sb;sample box".
        ///
        /// If field #Callback is \c nullptr to specify the use of the internal, default cast
        /// function, this field will be ignored and #"Types::Integer;*", will
        ...
        ///   #"alib_expressions_details_optimizations_norm".
        ///
        /// \note
    ...
    /// #"CIAutoCast::Operator" and the type(s) that might be auto-cast.
    ///
    ...
    /// #"AutoCastEntry::Type"
    /// and
    /// #"AutoCastEntry::ReverseCastFunctionName"
    /// have to be provided.
    ///
    /// \note
