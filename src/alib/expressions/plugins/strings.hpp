...
//==================================================================================================
/// This built-in #"CompilerPlugin" of \alib_expressions_nl
/// compiles identifiers, functions and operators with character string-type operands or return
/// types.
///
/// By default, this plug-in is #"Compiler::CfgBuiltInPlugins;automatically created"
/// and inserted into each instance of class #"Compiler" with the invocation of
...
/// Integer | \b Integer       | int      | String                  | Parses an integral value from the string. Allows decimal, hexadecimal, octal and binary formats. Literal prefixes default to <c>'0x'</c>, <c>'0o'</c> and <c>'0b'</c>, which can be changed by configuring the field #"Formatter::DefaultNumberFormat" of field #"Scope::Formatter".
/// Float   | \b Float         | float    | String                  | Parses a floating point value from the string. Allows a scientific format and change of decimal point character by configuring the field #"Formatter::DefaultNumberFormat" of field #"Scope::Formatter".
/// String  | \b Hexadecimal   | hex      | Integer                 | Converts an integral value to a hexadecimal string representation. Format options are available with the field #"Formatter::DefaultNumberFormat" of the field #"Scope::Formatter". The literal prefix (e.g., \c "0x") is not written.
/// String  | \b Hexadecimal   | hex      | Integer, Integer        | Converts an integral value to a hexadecimal string representation of the given output width (2nd argument). Format options are available with the field #"Formatter::DefaultNumberFormat" of field #"Scope::Formatter". The literal prefix (e.g., \c "0x") is not written.
/// String  | \b Octal         | oct      | Integer                 | Converts an integral value to a binary string representation. Format options are available with the field #"Formatter::DefaultNumberFormat" of field #"Scope::Formatter". The literal prefix (e.g., \c "0x") is not written.
/// String  | \b Octal         | oct      | Integer, Integer        | Converts an integral value to a binary string representation of the given output width (2nd argument). Format options are available with the field #"Formatter::DefaultNumberFormat" of field #"Scope::Formatter". The literal prefix (e.g., \c "0x") is not written.
/// String  | \b Binary        | bin      | Integer                 | Converts an integral value to an octal string representation. Format options are available with the field #"Formatter::DefaultNumberFormat" of field #"Scope::Formatter". The literal prefix (e.g., \c "0x") is not written.
/// String  | \b Binary        | bin      | Integer, Integer        | Converts an integral value to an octal string representation of the given output width (2nd argument). Format options are available with the field #"Formatter::DefaultNumberFormat" of field #"Scope::Formatter". The literal prefix (e.g., \c "0x") is not written.
...
/// |-------------|----------|------------|----------|--------------------
/// |String       | String   |<b>+</b>  | Integer  | Concatenates an integral value to a string. The number conversion is performed using the field #"Formatter::DefaultNumberFormat" of field #"Scope::Formatter".
/// |String       | String   |<b>+</b>  | Float    | Concatenates a floating point value to a string. The number conversion is performed using the field #"Formatter::DefaultNumberFormat" of field #"Scope::Formatter".
/// |String       | String   |<b>+</b>  | \<any\>  | Converts a boxed value of an arbitrary type to a string and appends the result to another string. The conversion of the boxed value is performed by invoking box-function #"FAppend".
/// |String       | Integer  |<b>+</b>  | String   | Converts an integral value to a string and concatenates another string. The number conversion is performed using the field #"Formatter::DefaultNumberFormat" of field #"Scope::Formatter".
/// |String       | Float    |<b>+</b>  | String   | Converts a floating point value to a string and concatenates another string. The number conversion is performed using the field #"Formatter::DefaultNumberFormat" of field #"Scope::Formatter".
/// |String       | \<any\>  |<b>+</b>  | String   | Converts a boxed value of an arbitrary type to a string and appends another string. The conversion of the boxed value is performed by invoking box-function #"FAppend".
/// |String       | String   |<b>+</b>  | String   | Concatenates two strings.
/// |Boolean      | String   |<b><</b>  | String   | Compares two strings using the method #"TString::CompareTo(const TString&, integer, integer);CompaareTo".
/// |Boolean      | String   |<b><=</b> | String   | Compares two strings using the method #"TString::CompareTo(const TString&, integer, integer);CompaareTo".
/// |Boolean      | String   |<b>></b>  | String   | Compares two strings using the method #"TString::CompareTo(const TString&, integer, integer);CompaareTo".
/// |Boolean      | String   |<b>>=</b> | String   | Compares two strings using the method #"TString::CompareTo(const TString&, integer, integer);CompaareTo".
/// |Boolean      | String   |<b>==</b> | String   | Compares two strings using the method #"TString::Equals".
/// |Boolean      | String   |<b>!=</b> | String   | Compares two strings using the method #"TString::Equals".
...
/// |Boolean      | String   |<b>[]</b> | Integer  | Returns the substring of length \c 1 at index \e rhs in string \e lhs.
///
///
///\I{##############################################################################################}
/// # Notes And Hints #
///
/// \par Compile-Time Invokable:
...
///   compiler.
///
/// \par Underlying String Types:
///   While all string manipulation is based on \alib classes #"^String", #"^AString" and
///   #"^Substring", due to the "harmonizing" way that string classes become boxed, none of these
///   types appears as an expression result type.
///   This means that custom expression functions can unbox strings that have been created
...
///   returned custom strings objects, can seamlessly be used by functions and operators defined
///   here.
///
/// \par
///   Details of this are explained in chapters
///   #"alib_boxing_bijective" and
///   #"alib_boxing_strings;/10. Boxing Character Strings" of the
///   #"alib_mod_boxing;Programmer's Manual" of module \alib_boxing_nl.
///
///
/// \par Determine A String's Length
///   The length of the string can be determined with function \b Length, which is defined with
///   compiler plugin #"plugins::Arithmetics" (because it works on all arrays).
///
/// \par Test For Empty Strings:
///   Unary operator #"DefaultUnaryOperators::BoolNot" (<c>'!'</c>)  may be used to check if a
///   string is not empty. The operator returns \c true  if the string is empty and \c false otherwise.
///
/// \par
...
///   #"plugins::ElvisOperator;elvis operator" is quite useful to avoid empty
...
///
/// \par Case conversion:
///   Unary operators #"DefaultUnaryOperators::Positive" <c>'+'</c> and
///   #"DefaultUnaryOperators::Negative" <c>'-'</c> are alias to functions \b ToUpper and
///   \b ToLower.
...
///
///   - Types #"Types::Integer" and #"Types::Float" are converted
///     to string-types using the field #"Formatter::DefaultNumberFormat" found in member
///     #"Scope::Formatter".
///     With this, detailed options for the number output format are available.
...
///     #"FAppend". For information on how to implement this for custom
...
///
///     1. The custom type:
///        \snippet "ut_expr.cpp"    DOX_EXPRESSIONS_STRINGOPS_IAPPEND_1
///
///     2. Definition of functor #"AppendableTraits" type <b>MyType</b>.
///        Because the type will be boxed as a pointer, we also define the functor for the pointer type:
///        \snippet "ut_expr.cpp"    DOX_EXPRESSIONS_STRINGOPS_IAPPEND_2
///
...
///        \snippet "ut_expr.cpp"    DOX_EXPRESSIONS_STRINGOPS_IAPPEND_3
///
...
///
///   <p>
///   - With type #"Types::Boolean", the #"ResourcePool;resourced"
///     string values generally used with box-function #"FAppend" defined for
...
///     To express other values, #"alib_expressions_builtin_ternary;conditional operator"
...
///   Internally, to perform the comparison, methods #"TString::Equals;String::Equals" and
///   #"TString::CompareTo(const TString&, integer, integer);String::CompareTo" are used.
///
...
///   Being based on classes of underlying module \alib_format, there is a choice between
///   #"FormatterPythonStyle;python style" or
///   #"FormatterJavaStyle;Java/printf style" format strings. It is even possible
...
///   - #"FormatterJavaStyle" for details.
///
/// \par Wildcard Match:
...
///   available with the definition of the configuration macro #"ALIB_FEAT_BOOST_REGEX"
///   and if either #"ALIB_CHARACTERS_WIDE" is \c false or #"ALIB_CHARACTERS_NATIVE_WCHAR"
///   equals \c true.
///
///\I{##############################################################################################}
...
struct Strings   : public plugins::Calculus {
    ...
    /// Overwrites the implementation of class #"Calculus". While this is usually not needed,
    ...
    /// Parent helper-class #"plugins::Calculus" does provide mechanics for such
...
/// number of arguments of arbitrary types to a concatenated string.
///
...
///   with compiler plug-in #"plugins::AutoCast;*".
///
/// @param scope The scope.
/// @param args  The single argument.
/// @return The boxed result string, allocated inside the scope's mono allocator.
//==================================================================================================
ALIB_DLL
...
/// number of arguments according to a given format string.
///
/// The function is compile-time invokable.
///
/// \note
