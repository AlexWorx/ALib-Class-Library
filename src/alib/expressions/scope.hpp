...
/// container #"Scope::NamedResources".
//==================================================================================================
...
/// the method #"ExpressionVal::Evaluate;*".
...
/// in the case that such results are not of a simple type that can be #"Box;boxed by value".
///
...
/// With each reuse, the method #"Scope::Reset" will be invoked internally.
...
/// virtual method #"Compiler::createCompileTimeScope;*".
...
/// code unit anyhow and passed to method #"ExpressionVal::Evaluate;*").
///
/// \see
///   - For details on the use of scopes see the manual chapter #"alib_expressions_scopes".
///   - This is an almost completely public struct.
///     The design rationale behind this is explained in the manual chapter
///     #"alib_expressions_prereq_bauhaus"
//==================================================================================================
struct Scope {
    ...
    struct VMMembers {
        ...
        /// #"plugins::Strings". When wildcard or regex matching is performed on
    ...
    /// The allocated data within this object becomes cleared automatically by method
    /// #"Scope::Reset", at the moment an expression is evaluated the next time (usually with
    /// different custom scope data).
    ///
    /// Note that this allocator is \b not cleared for the compile-time scope instance.
    MonoAllocator&                      Allocator;

    /// This is the argument stack used by class #"detail::VirtualMachine;*" when
    /// evaluating expressions.<br>
    /// With compilation, it used to "simulate" evaluation calls at compile-time.
    StdVectorMA<Box>*                   Stack;

    /// Used to convert numbers to strings and vice versa. In addition, the expression function
    /// \b Format of the built-in compiler plugin #"plugins::Strings" uses this object
    ...
    /// #"FormatterPythonStyle;python style" and
    /// #"FormatterJavaStyle;java/printf-like style".
    ///
    /// The default implementation of method #"Compiler::createCompileTimeScope;*"
    /// provides the field #"Compiler::CfgFormatter;*" with the constructor of the
    ...
    /// Usually, for parameter \p{formatter} field #"Compiler::CfgFormatter;*" should
    /// be provided.
    ///
