...
/// #"Compiler::Repository;*" to enable an \e automated definition and retrieval
...
/// strings, is given with class #"StandardRepository".
///
/// More information about automated nested expressions and the use of this interface class is
/// provided with manual chapter #"alib_expressions_nested_config".
//==================================================================================================
class ExpressionRepository {
  public:
    ...
    /// is not found by method #"Compiler::GetNamed;*".
    ///
...
/// #"alib::expressions;ALib Expressions User Manual".
///
/// ## Friends ##
/// class #"ExpressionVal"
//==================================================================================================
class Compiler  : public lang::PluginContainer<CompilerPlugin, CompilePriorities> {
  ...
  protected:
    ...
    HashMap< MonoAllocator,
             lang::TypeFunctors::Key, NAString,
             lang::TypeFunctors::Hash,
             lang::TypeFunctors::EqualTo  >   typeMap;
  ...
  public:
    ...
    ///
    ///
    /// Flag #"Compilation::DefaultUnaryOperators;*" controls if method
    /// #"%SetupDefaults" adds operators resourced with enumeration
    /// #"expressions::DefaultUnaryOperators".
    ListMA<String>         UnaryOperators;

    /// This public map allows defining alias names for unary operators. Such names
    ...
    /// #"DefaultAlphabeticUnaryOperatorAliases" to this map.
    HashMap<MonoAllocator,
            String, String,
            alib::hash_string_ignore_case<character>,
            alib::equal_to_string_ignore_case<character> > AlphabeticUnaryOperatorAliases;

    /// This public map allows defining alias names for binary operators. Such names
    /// must consist only of alphabetic characters.
    ///
    /// Flag #"Compilation::DefaultAlphabeticOperatorAliases;*" controls if method
    /// #"SetupDefaults" adds the aliases defined with resourced data records of enumeration
    /// #"DefaultAlphabeticBinaryOperatorAliases" to this map.
    HashMap<MonoAllocator, String, String,
            alib::hash_string_ignore_case<character>,
    ...
    /// This method should be invoked before invoking #"SetupDefaults".
    ///
    ...
    void AddUnaryOperator( const String& symbol  ) {
            ...
            for( auto& op : UnaryOperators )
                if( op.Equals<NC>( symbol ) )
                    ALIB_ASSERT_ERROR( false, "EXPR", "Unary operator '{}' already defined.",
                                                      symbol )
        #endif
    ...
    ///
    /// To define a new binary operator, entry may be added before invoking #"SetupDefaults"
    ...
    /// Returns the precedence of the given operator-symbol.
    /// @param symbol        The operator symbol.
    ...
    /// #"BitwiseTraits;Bitwise enum class" that lists the plug-ins built-into
    ...
    enum class BuiltInPlugins {
        ...
        ElvisOperator      = (1 << 1),   ///< Installs #"plugins::ElvisOperator;*".
        AutoCast           = (1 << 2),   ///< Installs #"plugins::AutoCast;*".
        Arithmetics        = (1 << 3),   ///< Installs #"plugins::Arithmetics;*".
        Math               = (1 << 4),   ///< Installs #"plugins::Math;*".
        Strings            = (1 << 5),   ///< Installs #"plugins::Strings;*".
        #if ALIB_CAMP
        DateAndTime        = (1 << 6),   ///< Installs #"plugins::DateAndTime;*". )
        #endif
    };
    ...
    ///   omitted) and instead created once and registered with the compiler using the
    ///   inherited method #"InsertPlugin". In that case, the value
    ///   \c "Responsibility::KeepWithSender" has to be provided as parameter \p{responsibility}.
    ...
    ///
    /// \note
    ///   If changed, the change hast to be made \b before invoking #".SetupDefaults". Otherwise,
    ///   the default configuration for
    ///   #"alib_expressions_operators_verbal;verbal operator aliases" would be set in
    ...
    /// Defaults to "Expression" with a minimum abbreviation of \c 4 characters, ignoring letter
    /// case.<br>
    /// Resourced with the key \c "EF"
    ///
    /// \see Manual chapter #"alib_expressions_nested".
    ...
    /// #"Normalization::UnaryOpSpace;*",
    /// #"Normalization::UnaryOpSpaceIfUnaryFollows;*" or
    /// #"Normalization::BinaryOpSpaces;*" are not set.
    ...
    /// - Used for parsing expression strings (Using the encapsulated #"TNumberFormat;NumberFormat"
    ///   object.)
    ...
    ///
    /// In the constructor of this class, a #"Formatter::Clone;clone" of the
    /// \alib {format::Formatter;Default;default formatter} is copied here.
    ...
    ///
    /// \see Manual chapter #"alib_expressions_nested_config".
  ...
  public:
    ...
    /// type names are read from the #"ResourcePool" of static library object
    /// #"EXPRESSIONS".
    ///
    ...
    ///
    /// @param sample    A #"alib_expressions_prereq_sb;sample value"
    ...
    ///
    /// If given argument \p{box} is #"alib_boxing_more_void_void;a void box", the returned
    /// string is <b>"NONE"</b>.
    ///
    ...
    /// #"Calculus::FunctionEntry") to a string buffer.
    ///
    ...
    /// - If flag #"Compilation::DefaultUnaryOperators;*" is set in field
    ///   #".CfgCompilation", then
    ///   - the unary operators listed in the #"alib_enums_records;enum records" of
    ///     #"expressions::DefaultUnaryOperators" are added using #".AddUnaryOperator".
    ///   - If flag #"Compilation::DefaultAlphabeticOperatorAliases;*" is set in
    ...
    /// - If flag #"Compilation::DefaultBinaryOperators;*" is set in field
    ///   #".CfgCompilation", then
    ///   - the binary operators listed in the data records of enumeration
    ///     #"expressions::DefaultBinaryOperators" are added using #".AddBinaryOperator".
    ///   - If flag #"Compilation::AllowSubscriptOperator;*" is not set, the
    ...
    /// internal #"detail::AST;abstract syntax tree".
    /// This tree is then translated into a #"detail::Program;*" which evaluates
    /// the expression in respect to a #"expressions::Scope" when executed.
    ///
    /// For building the program, attached #"CompilerPlugin" instances
    /// are queried.
    ///
    /// During this process, a #"Normalization;normalized" version of the input
    /// string is always created. While the original input string can
    /// be accessed with #"ExpressionVal::GetOriginalString;*", the normalized
    /// version can be retrieved after the compilation with method
    /// #"ExpressionVal::GetNormalizedString;*". During compilation, the
    /// normalization rules (flags) stored in field #".CfgNormalization" are applied.
    ///
    /// For compilation, a compile-time scope object is created by calling the virtual method
    /// #".createCompileTimeScope". If not overwritten, a standard scope object is used.
    ...
    /// has to be used that overrides method #".createCompileTimeScope" accordingly.
    ///
    ...
    ///         Otherwise, returns \c false.
    bool            RemoveNamed( const String& name )      { return AddNamed( name, NULL_STRING ); }

    /// Returns a named expression previously defined using #".AddNamed".
    /// In the case that no expression with the given \p{name} was defined and optional interface
    ...
    ///
    /// \see
    ///   Manual chapter #"alib_expressions_nested_config".
    ///
    ...
    ///       knowing what to do.
    /// @return The internal monotonous allocator.
  ...
  protected:
    ...
    /// custom scope types, then this method has to be overridden to return such a custom
    /// scope object.
    ///
    /// @param expressionAllocator The allocator of the expression used to allocate the
    ///                            compile-time scope.
    /// @return The default implementation returns the standard scope object.
    ///         For its construction, the field #".CfgFormatter" is provided.<br>
...
class  ProgramBase {
  protected:
    ...
    /// @param ev The expression to receive protected data from.
    /// @return The expression's allocotor.
    MonoAllocator&          getExpressionAllocator(ExpressionVal& ev)       { return ev.allocator; }

    /// Friendship-access method used by derived implementation.
    /// @param ev The expression to receive protected data from.
    /// @return The expression's compile-time scope.
    Scope*                  getExpressionCTScope  (ExpressionVal& ev)         { return ev.ctScope; }

    /// Friendship-access method used by derived implementation.
    /// @param cmp The compiler to receive protected data from.
    /// @return The compiler's plugins.
    Compiler::PluginList&   getCompilerPlugins    (Compiler& cmp)            { return cmp.plugins; }
};
...
struct VirtualMachineBase {
  protected:
    ...
    /// @param ev The expression to receive protected data from.
    /// @return The expression's compile-time scope.
    static Scope*           getExpressionCTScope  (ExpressionVal& ev)         { return ev.ctScope; }
};
...
DOX_MARKER([DOX_MANUAL_ALIASES_COMPILER])
