...
/// #"Compiler". The compiler uses the plug-ins to <em>"compile"</em> single
...
/// #"alib::expressions;User Manual And Tutorial" of module \alib_expressions_nl.
///
/// The overloaded methods #".TryCompilation(CIFunc)" are not abstract, but have a default implementation
...
/// - #"CompilerPlugin::CIBinaryOp" or
/// - #"CompilerPlugin::CIAutoCast".
///
/// These inner types of this struct are derived from likewise inner struct
/// #"CompilerPlugin::CompilationInfo", which provides all output members and
...
///
/// When overriding one of the overloaded methods #".TryCompilation(CIFunc)", it has to be checked
/// if the custom plug-in is responsible for the permutation of given parameter types which
/// are passed.
...
///
/// The output information comprises a #"alib_expressions_prereq_sb;sample box" that
...
/// might be kept \e nulled. In this case, the #"%Box" object that represents the result type
...
///   #"%Box" remains valid during the life-cycle of the expression. This is ensured for all
...
///   assigned object.)<br>
///   Field #"CompileTimeScope"
///   is to be used for compile-time allocations.
///
/// The plug-ins are attached to the #"%Compiler" using a dedicated prioritization defined
/// with (arithmetical) enum type #"CompilePriorities".
...
/// in certain aspects by adding a custom #"%CompilerPlugin" with a higher priority than that of
/// the built-in one.
///
/// \note
///   Instead of deriving from this struct, it is recommended to consider convenience struct
///   #"plugins::Calculus;*" instead.
///
/// \note
...
struct CompilerPlugin : public lang::Plugin<Compiler, CompilePriorities> {
    /// The name of the plug-in. This is used with exception information and with
    /// #"VirtualMachine::DbgList;program listings" which are available in
    ...
    /// #"CompilerPlugin::TryCompilation(CIUnary)".
    struct CompilationInfo {
        /// The scope found here is the same object passed to the method
        /// #"Compiler::Compile;*", which internally invokes the overloaded methods
        /// #"CompilerPlugin::TryCompilation(CIFunc);2", that receive an object of this type.
        ///
        ...
        /// Input: A start iterator to #"alib_expressions_prereq_sb;sample boxes"
    ...
    /// This struct is used with method #"TryCompilation(CIFunction&)" to provide information
    ...
    struct CIFunction     : public CompilationInfo {
        ...
        /// \see See also flag
        ///      #"Compilation::AllowOmittingParenthesesOfParameterlessFunctions" of
        ///      field  \see See also #"Compiler::CfgCompilation.;*".
        bool                            IsIdentifier;
    ...
    /// This struct is used with method #"TryCompilation(CIUnaryOp&)" to provide information
    /// to derived compiler plug-ins, as well as to receive information back.
    ...
    /// This struct is used with method #"TryCompilation(CIBinaryOp&)" to provide information
    /// to derived compiler plug-ins, as well as to receive information back.
    ///
    ...
    ///
    /// In both cases the inherited field #"^.Callback" is to be left \c nullptr. If the result is
    /// constant, inherited field #"%TypeOrValue" is to be set to the constant value.
    ...
    ///
    /// \note
    ///   Class #"plugins::Calculus" which specializes this plug-in class,
    ...
    ///
    /// Built-in compiler plug-in #"plugins::AutoCast" ignores unary operations.
    ...
    struct CIAutoCast  : public CompilationInfo {
        ...
        ///   inherited field #"CompilationInfo::Callback".
        CallbackDecl        CallbackRhs;

        /// Output: Specifies the return type of #".CallbackRhs", respectively, as the name
        /// indicates, the result value in case of constant a result.<br>
        /// In case of constant compile-time values, it might be necessary to allocate
        /// compile-time memory for the values.
        /// For this, the field #"CompilationInfo::CompileTimeScope" is to be used.
        /// \note
        ///   The optional return type and value casting the left-hand side argument is returned with
        ///   inherited field #"CompilationInfo::TypeOrValue".
        Box                 TypeOrValueRhs;
    ...
    /// - on the one hand allow abbreviations and letter case-insensitive identifier recognitions,
    ///   and
    ...
    /// #"ExpressionVal::GetNormalizedString;*", in case flag
    /// #"Normalization::ReplaceFunctionNames;*" is set in field
    /// #"Compiler::CfgNormalization;*".
    ///
    ...
    /// a #"alib_expressions_prereq_sb;sample box" that specifies its return type.
    /// Both are to be stored in output parameter \p{result}.
    ///
    /// Alternatively, if field #"%ArgIsConst" of the given compilation info struct is \c true,
    ...
    /// #"ExpressionVal::GetNormalizedString;*", in case flag
    /// #"Normalization::ReplaceAliasOperators;*" is set in field
    /// #"Compiler::CfgNormalization;*".
    ///
    ...
    /// Both are to be stored in output parameter \p{result}.
    ///
    /// Alternatively, depending on fields #"CIBinaryOp::LhsIsConst" and #"CIBinaryOp::RhsIsConst"
    /// of the given compilation info struct, a constant value might be returned or, as a third
    /// alternative, field #"%NonConstArgIsResult" may be set to \c true, if an "identity" operator
    /// is detected.
    /// For details of the input and output parameters of the function, see struct
    /// #"CompilerPlugin::CIBinaryOp".
    ///
    ...
    /// parameter type #"CompilerPlugin::CIAutoCast".
    ///
    ///
