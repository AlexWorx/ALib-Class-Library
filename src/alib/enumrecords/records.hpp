...
/// is prototyped with #"EnumRecordPrototype"
///
/// \see
///   Please consult chapter #"alib_enums_records" of the Programmer's Manual
///   of \alib_enumrecords_nl for detailed documentation and sample code on this struct and
///   underlying concept.
///
/// \see
///   Macro #"ALIB_ENUMS_ASSIGN_RECORD" offers a well readable alternative to specialize this
...
/// Concept that is satisfied if the type trait #"RecordsTraits" is specialized for the template
/// type \p{TEnum} which specifies type alias #"%RecordsTraits::Type;2".
...
/// #"alib_enums_records;ALib Enum Record".
/// The class inherit's #"Singleton" and has a private default constructor, what
/// makes this type a #"alib_singleton_strict;strict singleton".
///
...
struct EnumRecordHook :  Singleton<EnumRecordHook<TEnum>> {
    ...
    /// #"EnumRecordHook::Node::next" the last element contained in the
  ...
  private:
    ...
    /// #"alib_singleton_strict;strict singleton".
    ///
...
///
/// \see
/// - Function #"enumrecords::TryRecord;*" which returns a pointer to the record and \c nullptr if
///   no record was defined for \p{element}.
/// - Class #"%Enum" of the module \alib_boxing and according namespace functions
///    #"boxing::GetRecord;*" and #"boxing::TryRecord;*", which together allow collecting the
///    necessary runtime information to defer the retrieval of enum records.
///
...
const RecordsTraits<TEnum>::Type& GetRecord( TEnum element ) {
    ...
    return *static_cast<const RecordsTraits<TEnum>::Type*>( result );
}
...
///
/// \see
/// - Function #"enumrecords::GetRecord" which returns a reference and asserts in debug-compilations
///   if no record is defined for \p{element} during bootstrap.
/// - Class #"%Enum" of the module \alib_boxing and the corresponding functions
///   #"boxing::GetRecord;*" and #"boxing::TryRecord;*", which together allow the collection
///   of the necessary runtime information and defer the retrieval of enum records.
///
/// @tparam TEnum     The enumeration type. Deduced from given argument.
/// @param  element   The enum element to search for.
...
const typename RecordsTraits<TEnum>::Type* TryRecord( TEnum element ) {
    ...
    return static_cast<const RecordsTraits<TEnum>::Type*>(
...
///
/// \attention
///   While this type becomes available with the inclusion if the header #"F;ALib.EnumRecords.H",
///   the overloaded #"%Bootstrap" methods (which are to be used solely during bootstrap, with
///   single-threaded access to this backend), become available only with further inclusion of
///   #"F;ALib.EnumRecords.Bootstrap.H".
///
...
/// the variouos #"%Bootstrap" methods must be used only while bootstrapping is done.
///
/// \see Chapter #"alib_enums_records;ALib Enum Records" of the Programmer's Manual of this
///      module.
///
...
struct EnumRecords {
    ...
    /// #"EnumRecords::begin;*" and #"EnumRecords::end;*". In ranged base
    /// <c>for(:)</c> loops, a local instance of type #"%EnumRecords" has to be created.
    ///
    ...
    ///   #"Singleton" and executes in constant time <em>O(1)</em>, in effect almost
    ///   no time.
    ///
    /// \note
    ...
    ///
    /// \see The note documented with the sibling method #".begin".
...
/// with #"alib_enums_records;ALib Enum Records".
///
...
/// This behavior is, for example, implemented with record #"ERException" and various
/// types found in module \alib_app.
///
/// \see For more information, see:
///   - Concept #"IsSerializable".
///   - Chapter #"alib_enums_records;ALib Enum Records" and its subsection
///     #"alib_enums_records_details_serialization".
///   - Specializations
///     #"APPENDABLES::AppendableTraits<TEnum,TChar,TAllocator>" and
///     #"APPENDABLES::AppendableTraits<TBitwiseEnum,TChar,TAllocator>",
///     which allows write enumeration element names to instances of class  #"^AString".
...
struct ERSerializable {
    ...
    /// Built-in basic de-serialization functions #"enumrecords::Parse", #"ParseBitwise" and
    /// #"ParseEnumOrTypeBool" interpret this value as the minimum length (abbreviation)
    ...
    /// Parses the fields of this record from the #"%^Substring" \p{parser} given as reference.
...
/// Concept that is satisfied if the type trait #"RecordsTraits"
/// is specialized for type \p{TEnum} and its type-alias member #"RecordsTraits::Type" evaluates to
/// #"ERSerializable" or a derived type thereof.
/// @tparam TEnum The type to test.
template<typename TEnum>
