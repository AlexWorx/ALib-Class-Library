...
/// including #"F;ALib.EnumRecords.Bootstrap.H".
ALIB_EXPORT namespace alib::enumrecords::bootstrap {

/// This is a pure static type used for parsing enum records from strings.
/// The static fields are initialized with each method of type #"EnumRecords"
/// and therefore implementations of #"EnumRecordPrototype::Parse;*" can rely on finding
/// these fields properly set.
///
/// This concept makes use of the restriction that the setup of
/// #"alib_enums_records;ALib Enum Records" must exclusively be done during
/// #"alib_mod_bs;bootstrapping" of the process, where no multi-threading is active, yet.
///
...
/// that the occurrence of unnecessary characters, indicates an erroneous maintenance of
/// externalized resources, and such should be detected as soon as possible.
///
/// The main interface into this class is given with the overloaded methods #".Get(String&)".
/// Enum Records are usually rather simple structs, consisting of strings, characters, integers
/// and floating point values. If so, a simple list of invocations of #"%Get(String&)" for each
...
///   If (in presence of the module \alib_exceptions) an #"exc Exception" was thrown,
...
///   An end user will not be faced with these exceptions unless an erroneous "roll-out"
///   of software using malicious externalized resources was performed.
///
/// For the last field, the optional parameter \p{isLastField} has to be given as \c true.
/// As an example, the following code shows the implementation of #"Declaration::Parse;*":
/// \snippet "variables/vmeta.cpp"     DOX_ENUMS_RECORD_PARSER
/// This parse method omits reading the value for inherited field
/// #"ERSerializable::MinimumRecognitionLength;*", and sets it to fixed \c 0 instead.
/// This is often an option when custom enumeration record types
/// are derived from #"%ERSerializable" but are not supposed to be deserialized, or deserialization
