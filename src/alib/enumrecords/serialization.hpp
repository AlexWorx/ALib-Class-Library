...
/// #"alib_enums_records;ALib Enum Records" of type #"ERSerializable" (or of a derived
/// type) from a given #"^Substring".
///
/// In debug-builds, the method asserts that at least one record is defined for \p{TEnum}.
///
/// For more information, consult chapter
/// #"alib_enums_records_details_serialization" of the
/// Programmer's Manual of the module \alib_enumrecords_nl.
///
/// \note
///   This namespace function is applicable to #"BitwiseTraits;bitwise enumrecords" as well.
...
//==================================================================================================
/// Repeatedly invokes sibling function #"enumrecords::Parse" until \p{delim} is not found.
/// The enum element values are or'ed in \p{result}.
///
/// In debug-builds, the method asserts that at least one record is defined for \p{TEnum}.
///
/// \note
///   This method is applicable only to #"BitwiseTraits;bitwise enumrecords" that likewise
///   are equipped with #"alib_enums_records;ALib Enum Records" of (derived) type
///   #"ERSerializable".
///
///
/// @tparam TEnum              The enumeration type equipped with <b>ALib Enum Records</b>
///                            of (derived) type #"ERSerializable" and furthermore with a
///                            specialization of #"BitwiseTraits".
/// @tparam TChar              The character type of the \p{input} substring.
///                            Deduced by the compiler.
/// @tparam TSensitivity       The sensitivity of the comparison.
///                            Defaults to #"%Case::Ignore".
...
//==================================================================================================
/// Convenience method that first uses #"enumrecords::Parse" to try and read an element of a C++
/// enum. If this is not successful, an enum of type #"lang::Bool" is tried to be read.
...
///
/// \see
///   For more information, consult chapter #"alib_enums_records_details_serialization" of the
///   Programmer's Manual of the module \alib_resources.
///
/// @tparam TEnum              The enumeration type equipped with <b>ALib Enum Records</b>
///                            of (derived) type #"ERSerializable".
/// @tparam TChar              The character type of the \p{input} substring.
///                            Deduced by the compiler.
/// @tparam TSensitivity       The sensitivity of the comparison.
///                            Defaults to #"%Case::Ignore".
...
///                            only trimmed characters are consumed.
/// @param[out] result         The result enum element given as reference.
/// @return \c true if an element of \p{TEnum} or #"lang::Bool" could be read,
///         \c false otherwise.
//==================================================================================================
template<typename          TEnum,
         typename          TChar,
...
/// Templated specialization of functor #"AppendableTraits" makes elements
/// of C++ enumeration type \p{TEnum} appendable to #"^AString" instances if:
/// - Type \p{TEnum} is equipped with #"alib_enums_records;ALib Enum Records" of type
///   #"ERSerializable" (or a derived type ), and if:
/// - No specialization of the type trait #"BitwiseTraits" exists for \p{TEnum}.
...
///    #"IsSerializable" and #"IsBitwise" to generically specialize
///    the type trait #"AppendableTraits" of the module \alib_strings.
///
/// Member #".operator()" writes the name of the given enumeration value to \p{target}.
/// As with all specializations of functor #"%AppendableTraits", the use of it is done implicitly
/// with various interface method of class #"^AString".
///
...
/// described with methods #"ResourcedType::TypeNamePrefix;*" and
/// #"ResourcedType::TypeNamePostfix;*" - then these resourced strings are written
...
///  #"F;ALib.Resources.H", instead of the header #"F;ALib.EnumRecords.H".<br>
...
///   - Corresponding convenience type  #"ResourcedType".
///   - Some sample code is given with documentation #"alib_enums_records;ALib Enum Records".
///
/// @tparam TEnum      The enumeration type of the element that is to be appended to an #"%AString".
/// @tparam TChar      The character type of the target #"%AString".
/// @tparam TAllocator The allocator type of the target #"%AString", as prototyped with
///                    #"lang::Allocator".
...
struct  AppendableTraits<TEnum, TChar, TAllocator> {
    ...
    /// #"ResourcedType::TypeNamePrefix;prefix" and
    /// #"ResourcedType::TypeNamePostfix;postfix" are written before and after
    ///  the element's name.
    ///
    ...
    ///
    ///
    /// @param target    The #"%AString" that \p{element} is to be appended to.
...
/// This specialization of type-traits-functor #"AppendableTraits" makes elements of
/// C++ enumeration type \p{TBitwiseEnum} appendable to #"^AString" instances if
/// - Type \p{TBitwiseEnum} is equipped with #"alib_enums_records;ALib Enum Records" of type
///   #"ERSerializable" (or a derived type derived from #"%ERSerializable"), and
/// - A specialization of type trait #"BitwiseTraits" exists for \p{TBitwiseEnum}.
///   (For non-bitwise types, a different implementation of this functor is given.)
///
/// Member #".operator()" writes all value names corresponding to the bits set in \p{src},
...
/// but have to be registered with class #"EnumRecords" \b before the
...
/// #"ALIB_ENUMS_MAKE_BITWISE" is used to specialize the type trait #"BitwiseTraits".
/// Next, macro #"ALIB_ENUMS_ASSIGN_RECORD" used to associate the type with enum record type
/// #"ERSerializable":
///
...
/// specification described with the methods #"ResourcedType::TypeNamePrefix;*"
/// and /// #"ResourcedType::TypeNamePostfix;*" - then these resourced strings
...
/// @tparam TChar         The character type of the target #"%AString".
/// @tparam TAllocator    The allocator type of the target #"%AString", as prototyped with the
///                       class #"lang::Allocator".
...
struct  AppendableTraits<TBitwiseEnum, TChar,TAllocator> {
    ...
    ///
    /// @param target    The #"%AString" that \p{elements} is to be appended to.
