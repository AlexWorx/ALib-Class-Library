...
/// functions #"alib::Bootstrap(BootstrapPhases);2" will call this function.
///
/// \see
///   For information about using this method, consult chapter
///   #"alib_mod_bs_nocamps" of the #"alib_manual".
//==================================================================================================
ALIB_DLL
void        bootstrap();
...
///
/// \see
///   Sibling function #"threads::bootstrap".
//==================================================================================================
ALIB_DLL
void        shutdown();
...
/// for an associated #"Thread".
///
/// Objects of this type may be passed to the constructor of class #"Thread". The
/// default implementation of method #"Thread::Run;*" will then invoke
...
class Runnable {
  public:
    ...
    /// The method invoked by the default implementation of #"Thread::Run;*"
...
/// the #"alib_mod_threads".
///
/// ## Friends ##
/// function #"threads::bootstrap;*"
/// function #"threads::shutdown;*"
/// @see
...
class Thread : public Runnable {
  ...
  public:
  #if !ALIB_SINGLE_THREADED
    /// States that follow the lifecycle of the thread. The states are accessible with method
    /// #"GetState".
    enum class State {
  ...
  public:
    ...
    /// The method #"Runnable::Run;2" of \p{target} will be executed upon thread start (unless a
    /// descendant class overrides method #".Run;2" which changes this behavior.
  ...
  //################################################################################################
    ...
    ///
    /// Hence, to have the thread execute something, either a #"Runnable"
  ...
  //################################################################################################
    ...
    /// #"State::Unstarted" to #"State::Terminated".
    /// \note
    ///   For system threads (the thread that executed function <c>main thread</c> and those
    ///   not created using \alib thread features) #"State::Running" is
    ///   returned.
    ...
    ///
    /// @return \c true if the current state of the thread is running or at least started.
    bool                IsAlive()     { return state == State::Started || state == State::Running; }

    /// Starts the execution of the thread. Method #".Run" is invoked by the new system thread,
    /// which - if not overwritten - invokes the method #"Runnable::Run;*".
    ...
    /// When this method is called, the thread's state should be <b>State::Done</b>, which
    /// is the case after method #"%.Run" has exited.
    /// Otherwise, a #"alib_mod_assert;warning is raised".
    ALIB_DLL
    virtual void        Join();
  ...
  public:
    ...
    /// If no instance of class #"%Thread" was created for the current thread, yet, then one
    /// with a negative ID is created, indicating that this thread was not created by \alib.
    ///
    /// Once a #"%Thread" was created, a pointer to this instance stored in the <c>thread_local</c>
    /// namespace variable #"THIS_THREAD" is returned.
    ...
    ///       inlined, but implemented using the macro #"ALIB_DLL".
    ...
    /// - during #"alib_mod_bs;bootstrapping of ALib", which multithreaded
    ///   applications have to perform before starting threads, \c nullptr might be returned as
    ///   well.
    ///
    /// In any other situations, either,
    /// - \p{nativeID} is received from a thread that was started using this class #"%Thread", then
    ...
    /// \see Variants #".SleepMicros", #".SleepNanos", #".Sleep(const Ticks::Duration&)", and
    ///      #".SleepUntil(const Ticks&)".
    ...
    /// Suspends the current thread by calling <em>std::this_thread::sleep_for</em>.
    /// \see Variants #".SleepMillis", #".SleepNanos", #".Sleep(const Ticks::Duration&)", and
    ///      #".SleepUntil(const Ticks&)".
    ...
    /// Suspends the current thread by calling <em>std::this_thread::sleep_for</em>.
    /// \see Variants #".SleepMicros", #".SleepMillis", #".Sleep(const Ticks::Duration&)", and
    ///      #".SleepUntil(const Ticks&)".
    ...
    /// Suspends the current thread by calling <em>std::this_thread::sleep_for</em>.
    /// \see Variants #".Sleep(const Ticks::Duration&)", #".SleepMicros", #".SleepMillis", and
    ///      #".SleepNanos"
