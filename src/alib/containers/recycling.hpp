...
/// of this module.
///
/// In the case #"%Shared" is given, the container type will expose a type definition named
...
///
/// \see
///   Chapter #"alib_contmono_intro_recycling" of the Programmer's Manual
///   for this \alibmod.
//==================================================================================================
...
/// method #"Allocator::allowsMemSplit" (like like #"%MonoAllocator" does), then one
...
void    recycleChunkImpl( lang::SidiListHook<TNode>& recyclables, TChunk* chunk, size_t count ) {
    ...
    if constexpr( alignof(TNode) > alignof(TChunk[1])  ) {
        auto alignedAddr= (reinterpret_cast<size_t>(chunk) + alignof(TNode) - 1)
                        & ~(static_cast<size_t>(alignof(TNode)) - 1U);
        mem = reinterpret_cast<void*>(alignedAddr);
        size-= size_t(static_cast<char*>(mem) - reinterpret_cast<char*>(chunk));
    }

    // create recyclables
    ALIB_DBG( size_t cntRecycledObjects= 0; )
    while(size > sizeof(TNode)) {
        recyclables.pushFront( static_cast<TNode*>( mem ) );
        mem  =  static_cast<char*>(mem) + sizeof(TNode);
        size -= sizeof (TNode);
        ...
        if( cntRecycledObjects <= 0 ) {
            ALIB_WARNING( "MONOMEM/RECYCLER",
              ...
              "  Chunk object:       Type: <{}>\n"
              "    Size, Count, Alignment: {} * {} = {} bytes, alignment: {}\n"
              "           Recyclable Type: <{}>\n"
              "           Size, Alignment: {} bytes, alignment: {}\n"
              "Note: If this recycler is used with a <containers::HashTable>, this message may be\n"
              "      eliminated by reserving a reasonable initial bucket size.",
              &typeid(TChunk), sizeof(TChunk[1]), count, sizeof(TChunk[1]) * count,
              alignof(TChunk[1]), &typeid(TNode), sizeof(TNode), alignof(TNode)  )
        }
    #endif
}
...
/// use private recycling (when their template parameter \p{TRecycling} evaluates to
/// #"Recycling::Private").
///
/// @tparam TAllocator The allocator to store and use.
/// @tparam TNode      The type to recycle. Has to be derived of #"SidiNodeBase".
template<typename TAllocator, typename TNode>
struct RecyclerPrivate  : lang::AllocatorMember<TAllocator> {
  ...
  public:
    ...
    lang::AllocatorInterface<TAllocator> AIF()     const noexcept { return base::GetAllocator()(); }
    ...
    /// This is only done if #"Allocator::allowsMemSplit;*" returns \c true.
    ...
    void RecycleChunk(TChunk* chunk, size_t length )                                      noexcept {
        if constexpr ( TAllocator::allowsMemSplit() )
            recycleChunkImpl( recyclables, chunk, length );
                ...
                base::AIF().FreeArray(chunk, length );
            #else
                base::AIF().template FreeArray(chunk, length );
            #endif
    }
...
/// This is the type exposed to users of the \alib containers in case shared recycling is
/// chosen, for example, with #"List::SharedRecyclerType;*" or
/// #"HashTable::SharedRecyclerType;*".<br>
...
/// Implements internal recycling interface. Used by container types of module \alib_containers_nl that
/// use shared recycling (when their template parameter \p{TRecycling} evaluates to
/// #"Recycling::Shared").
///
/// @tparam TAllocator The allocator to store and use.
/// @tparam TNode      The type to recycle. Has to be derived of #"SidiNodeBase".
...
class RecyclerShared {
  ...
  public:
    ...
    /// @return The allocator interface of the allocator received with construction.
    lang::AllocatorInterface<TAllocator> AIF()        const noexcept { return sr.GetAllocator()(); }
    ...
    /// @return A recycled or allocated element.
    [[nodiscard]]
    TNode*        Get() { return !sr.isEmpty() ? sr.popFront() : sr.AIF().template Alloc<TNode>(); }
...
/// that omit recycling (when their template parameter \p{TRecycling} evaluates to
/// #"Recycling::None").
///
/// @tparam TAllocator The allocator to store and use.
/// @tparam TNode      The type to recycle. Has to be derived of #"SidiNodeBase".
...
struct RecyclerVoid  : lang::AllocatorMember<TAllocator> {
  ...
  public:
    ...
    [[nodiscard]]
    TNode*  Get()                                    { return base::AIF().template Alloc<TNode>(); }
