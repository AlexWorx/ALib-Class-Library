...
//==================================================================================================
#ifndef HPP_ALIB_MONOMEM_CONTAINERS_HASHTABLE
#define HPP_ALIB_MONOMEM_CONTAINERS_HASHTABLE 1
#pragma once
#if !defined(DOXYGEN)
#   include "alib/alib.hpp"
#endif
ALIB_ASSERT_MODULE(CONTAINERS)
#include "alib/containers/detail/hashtablebase.inl"
...
#if ALIB_DEBUG_CONTAINERS &&  ALIB_CAMP
...
///
/// This \alibmod_nl supports compiler symbol \ref ALIB_DEBUG_CONTAINERS which enables extra
/// features.
...
class HashTable : protected detail::HashTableBase<TAllocator,TValueDescriptor,THash,TEqual,THashCaching,TRecycling>
{
  ...
  public:
    ...
    HashTable( float             pBaseLoadFactor = 1.0,
               float             pMaxLoadFactor  = 2.0  );
    #else
        template<Recycling TEnableIf= TRecycling,
            ATMP_T_IF(int, TEnableIf != Recycling::Shared) = 0 >
        explicit
        HashTable( AllocatorType&   pAllocator,
                   float            pBaseLoadFactor = 1.0,
                   float            pMaxLoadFactor  = 2.0  )
    ...
    void            Reset()
    {
        ...
        lang::Destruct(*this);
        if constexpr ( ATMP_EQ( typename base::recyclerType,
                                detail::RecyclerShared<TAllocator ALIB_COMMA Element> ) )
    ...
    ATMP_T_IF(std::pair<Iterator ALIB_COMMA bool>, !ATMP_EQ( TEnableIf, StoredType ))
...
#else
    template<typename TEnableIf_MapMode= MappedType, typename... TArgs>
    ATMP_T_IF(std::pair<Iterator ALIB_COMMA bool>,   !ATMP_EQ(TEnableIf_MapMode, StoredType )  )
    EmplaceOrAssign( const KeyType& key, TArgs&&... args)
    {DCS
        // insert to hash table
    ...
    template<typename TEnableIf_SetMode= MappedType, typename... TArgs>
    ATMP_T_IF(std::pair<Iterator ALIB_COMMA bool>,    ATMP_EQ(TEnableIf_SetMode, StoredType )
                                                   && std::is_constructible< StoredType     ALIB_COMMA
    ...
    template<typename TEnableIf= MappedType, typename... TArgs>
    ATMP_T_IF(std::pair<Iterator ALIB_COMMA bool>, !std::is_constructible< StoredType     ALIB_COMMA
                                                                           const KeyType& ALIB_COMMA
                                                                           TArgs&&...     >::value )
    EmplaceIfNotExistent( const KeyType& key, TArgs&&... args)
    {DCS
...
///
/// \par Availability
///   Available only if compiler symbol \ref ALIB_DEBUG_CONTAINERS is set.
///
/// \see
...
#if ALIB_CAMP
/// Invokes method \alib{containers;DbgGetHashTableDistribution} and creates human-readable output, ready to be
/// logged or written to the console.
///
/// \par Availability
///   Available only if compiler symbol \ref ALIB_DEBUG_CONTAINERS is set and module \alib_basecamp
...
///
/// Before invoking this method, specializations of \alib{strings;T_Append} have to be made
...
#endif // HPP_ALIB_MONOMEM_CONTAINERS_HASHTABLE
