...
//==================================================================================================
/// # Contents #
/// #"alib_ns_containers_hashtable_intro"                   <br>
/// #"alib_ns_containers_hashtable_setandmap"               <br>
/// #"alib_ns_containers_hashtable_single_multi"            <br>
/// #"alib_ns_containers_hashtable_rehashing"               <br>
/// #"alib_ns_containers_hashtable_iterators"               <br>
/// #"alib_ns_containers_hashtable_hashcodes"               <br>
/// &nbsp;&nbsp;#"alib_ns_containers_hashtable_caching"     <br>
/// &nbsp;&nbsp;#"alib_ns_containers_hashtable_hashprecalc" <br>
/// &nbsp;&nbsp;#"alib_ns_containers_hashtable_hashquality" <br>
/// #"alib_ns_containers_hashtable_memory"                  <br>
/// #"alib_ns_containers_hashtable_comparison"              <br>
/// #"alib_ns_containers_hashtable_referencedoc"            <br>
///
/// \I{#############################################################################################}
/// # 1. Introduction # {#alib_ns_containers_hashtable_intro}
...
///
/// \I{#############################################################################################}
/// # 2. Hash Sets vs. Hash Maps# {#alib_ns_containers_hashtable_setandmap}
...
///    Functors \p{THash} and \p{TEqual} consequently expect the #".StoredType".<br>
///
...
///    Consequently, #"%cont HashMap" defines the #".StoredType" as <c>std::pair<TKey, TMapped></c>.<br>
///
...
/// ---------------------------|-------------------------------------|-------------------------
/// Hash Set                   |#"containers::HashSet" (A type definition on #"%HashTable")  | Automaticly chosen as built-in #"TIdentDescriptor"
/// Hash Map                   |#"containers::HashMap" (A type definition on #"%HashTable")  | Automaticly chosen as built-in #"TPairDescriptor"
/// Hash Set with embedded Key |#"HashTable" (The original type)                 | A <b>custom type</b> has to be provided
///
...
///    The following method overloads are exclusively available with hash-<em>maps</em>:"
...
///    - #"TIterator::Mapped"
///    - #"TLocalIterator::Mapped"
///    - #"ElementHandle::Mapped"
///
...
///
/// \I{#############################################################################################}
/// # 3. Single And Multiple Entries # {#alib_ns_containers_hashtable_single_multi}
...
///
/// \I{#############################################################################################}
/// # 4. Re-Hashing # {#alib_ns_containers_hashtable_rehashing}
...
/// specified with #"HashTable::HashTable(float, float);construction" and with the methods
...
///
/// \I{############################################################################################}
/// # 5. Iterators # {#alib_ns_containers_hashtable_iterators}
/// ## 5.1 Iterator Types ##
/// There are two types of iterators provided:
/// - #"HashTable::Iterator" and its constant sibling
///   #"HashTable::ConstIterator", used to iterate over all elements of the
///   hash table,
/// - #"HashTable::LocalIterator" and its constant sibling
///   #"HashTable::ConstLocalIterator" used to iterate over the elements
...
///
///\I{#############################################################################################}
/// # 6. Hash Codes # {#alib_ns_containers_hashtable_hashcodes}
///
///\I{#############################################################################################}
/// ## 6.1 Caching Hash Codes ## {#alib_ns_containers_hashtable_caching}
///
...
/// #"alib_ns_containers_hashtable_rehashing;re-hashed". In addition, the cached
...
///
///\I{#############################################################################################}
/// ## 6.2 Hash Codes Pre-calculation ## {#alib_ns_containers_hashtable_hashprecalc}
...
///
///\I{#############################################################################################}
/// ## 6.3 Hash Code Quality ## {#alib_ns_containers_hashtable_hashquality}
/// To have hash tables perform in constant time <em>O(1)</em> in the average case, a
/// well-implemented calculation of hash-codes has to be provided for template type \p{TKey} with
/// template functor \p{THash}. This is an obligation of the user of this type.
///
/// This \alibmod_nl supports the configuration macro #"ALIB_DEBUG_CONTAINERS" which enables extra
/// features.
...
///
///\I{#############################################################################################}
/// # 7. Memory Use # {#alib_ns_containers_hashtable_memory}
/// With the template parameter \p{TRecycling} being either #"Recycling::Private"
/// (the default) or #"Recycling::Shared", the internal
...
/// type #"Recycling::None" may be used to eliminate this little overhead
...
/// interface method #"Allocator::allowsMemSplit" returns \c true. This is true for type
/// #"%MonoAllocator".
///
...
/// #"TakeSnapshot;snapshot" of the allocator can be taken and
...
///
/// \I{############################################################################################}
/// # 8. Comparison To Standard Library Hash-Types # {#alib_ns_containers_hashtable_comparison}
...
///   #"containers::HashMap" offers exactly this.
...
///   #"alib_ns_containers_hashtable_single_multi".
...
///
/// @see
///   - Chapter #"alib_contmono_containers_types" of the joint Programmer's Manuals of modules
///     \alib_containers and \alib_monomem.
///   - Chapter #"alib_threads_intro_assert_entry" of the Programmer's Manual of module
///     \alib_threads for information about debugging multithreaded access on instances of this
///     type.
///\I{#############################################################################################}
/// # Reference Documentation # {#alib_ns_containers_hashtable_referencedoc}
...
///                          and is published as #"HashTable::HashType;*".
...
///                          and is published as #"HashTable::EqualType;*".
...
class HashTable : protected detail::HashTableBase< TAllocator, TValueDescriptor,
                                                   THash, TEqual,
                                                   THashCaching, TRecycling >    {
  public:
    #if !DOXYGEN
  ...
  //################################################################################################
    ...
    /// #"Caching::Enabled" or if it equals #"Caching::Auto"
    ...
    ///         #"Recycling::None", \c true otherwise.
    ...
    /// A value of this type is returned by the methods #Extract, which allow removing
  ...
  //################################################################################################
    ...
    /// \note
    ///   This constructor is not available if the template argument \p{TRecycling} equals
    ///   #"Recycling::Shared" and if the template argument \p{TAllocator} does not equal
    ///   #"HeapAllocator"
    ///
    ///
    /// @tparam TRequires        Defaulted template parameter. Must not be specified.
    ...
    /// are deleted. The latter is done only if the template parameter \p{TRecycling} is not
    /// #"Recycling::Shared". In this case, the elements are still recycled.
    ///
    ...
    void            Reserve( integer expected, lang::ValueReference reference )                 {DCS
    ...
    ///      Manual.
    ///
    /// @param expected  The expected resulting number (or increase) of elements to be stored in
    ///                  this container.
    /// @param reference Denotes whether \p{expected} is meant as an absolute size or an
    ///                  increase.
    ...
    ///   that specify template parameter \p{TRecycling} as #"Recycling::None".
    ///
    ...
    ///   #"alib_ns_containers_hashtable_rehashing".
    ///
    ...
    { return Insert(value, THash{}(TValueDescriptor().Key(const_cast<StoredType&>(value)))); }

    /// Overloaded version of method #"Insert(const StoredType&)" which
    ...
    ///      #"alib_ns_containers_hashtable_hashprecalc".
    ///
    /// @param  value    A value to copy and insert.
    ...
    ///   elements.
    ///   For more information, see the section #"alib_ns_containers_hashtable_single_multi".
    ///
    /// @param  value   A rvalue reference of contained type \p{StoredType} to insert.
    /// @return An iterator referring to the element added.
    Iterator        Insert( StoredType&& value )
    {
        auto   hashCode = THash{}( TValueDescriptor().Key( static_cast<StoredType&>(value)) );
        return Insert( std::move(value), hashCode );
    }

    /// Overloaded version of method #"HashTable::Insert(StoredType&&)" which
    /// accepts the \p{hashCode} of the given \p{value} as a second parameter.
    ///
    /// @see Use cases of this method are discussed in reference documentation section
    ///      #"alib_ns_containers_hashtable_hashprecalc".
    ...
    /// Inserts the element contained in the given #"HashTable::ElementHandle"
    /// into the hash table.
    ///
    /// \note
    ///   The use of this method may insert elements sharing the same key as already existing
    ...
    ///
    /// <p>
    ///   Objects of type #"%ElementHandle" objects may be received using overloaded methods
    ///   #"HashTable::Extract(ConstIterator)". The combination of
    ///   #"%HashTable::Extract(ConstIterator)" and this method (as well as method
    ///   #"HashTable::InsertIfNotExistent(ElementHandle&)" are the only way to change the
    ///   <em>key-portion</em> of an element without element destruction/re-construction.
    ///
    ...
    /// #"InsertUnique(const StoredType&)" which accepts the
    ...
    ///   after the operation. I.e., method #EqualRange will discontinue functioning properly.
    ///
    /// \attention
    ...
    /// Overloaded version of method #"HashTable::InsertUnique(StoredType&&)"
    ...
    ///   #"alib_ns_containers_hashtable_setandmap;hash map mode".
    ///
    ...
    /// Overloaded version of the method #"InsertOrAssign(const KeyType&, MappedType&&)" which
    ...
    /// Overloaded version of method
    /// #"InsertIfNotExistent(const KeyType&,MappedType&&)" which
    /// accepts the \p{hashCode} of the given \p{key} as a third parameter.
    ///
    /// \par Availability
    ///   This method is only available with
    ...
    /// @return A pair containing an iterator referencing either the element found or the new
    ///         element added. The bool component is \c true if the insertion took place and
    ///         \c false if nothing was changed.
    ...
    /// Overloaded version of method
    /// #"HashTable::InsertIfNotExistent(StoredType&&)" which accepts the
    /// \p{hashCode} of the given \p{key} as a second parameter.
    ///
    /// @see Use cases of this method are discussed in reference documentation section
    ///      #"alib_ns_containers_hashtable_hashprecalc".
    ...
    /// Inserts the element contained in the given #"HashTable::ElementHandle" into
    /// this table, if no equal element exists. In the unsuccessful case, the given
    /// #"%ElementHandle" remains set and can be reused.<br>
    ///
    /// Existing iterators remain valid.
    ///
    /// \note
    ...
    /// \see
    ///   Objects of type #"%ElementHandle" objects may be received using overloaded methods
    ///   #Extract. The combination of #"%HashTable::Extract(ConstIterator)" and this method
    ///   (as well as method #"HashTable::Insert(ElementHandle&)" are the only way to change the
    ///   <em>key-portion</em> of an element without element destruction/re-construction.
    ///
    /// @param  handle   A reference to a handle to an element,  previously received with #Extract.
    ...
    /// Constructs a value within this container \b without checking to place it in the right
    /// position in respect to existing elements with the same <em>key-portion</em>.
    ///
    /// \attention
    ///   This method must only be used if the caller guarantees that no other element is
    ...
    /// \attention
    ///   In debug-compilations, an \alib_assertion is raised if an equal element exists.
    ///   For this reason, performance improvements to method #Insert will be seen only with
    ///   release-compilations.
    ///
    /// \attention
    ///   For more information, see the section #"alib_ns_containers_hashtable_single_multi".
    ...
    ///   #"alib_ns_containers_hashtable_setandmap;hash set mode". For <em>hash map mode</em>,
    ...
    ///
    /// \see
    ///    Method #EqualRange, which in addition returns an iterator pointing behind the last
    ///    element with that key.
    ///
    ///
    /// @param  key   The key to search for.
    /// @return An iterator pointing to the first element with a matching <em>key-portion</em>
    ///         found, respectively, one being equal to #end, if no element was found with \p{key}.
    ...
    /// Overloaded version of method #"Find(const KeyType&)" which
    ...
    /// Overloaded version of method #"Find(const KeyType&)const" which
    /// accepts the \p{hashCode} of the given \p{key} as a second parameter.
    ///
    /// @see Use cases of this method are discussed in reference documentation section
    ///      #"alib_ns_containers_hashtable_hashprecalc".
    ...
    /// Overloaded version of method #"Extract(const KeyType&)" which
    /// accepts the \p{hashCode} of the given \p{key} as a second parameter.
    ///
    /// @see Use cases of this method are discussed in reference documentation section
    ///      #"alib_ns_containers_hashtable_hashprecalc".
    ...
    /// Overloaded version of the method #"erase(const KeyType&)"
    /// which accepts the \p{hashCode} of the given \p{key} as a second parameter.
    ///
    /// @see Use cases of this method are discussed in reference documentation section
    ///      #"alib_ns_containers_hashtable_hashprecalc".
    ...
    ///
    /// \note
    ///   This method is slightly more efficient than the method #erase (const KeyType&), as it will
    ///   not search for the next element with an equal key.<br>
    ///   In debug-compilations, the method asserts that no second element with the same \p{key}
    ///   is available.<br>
    ///   If this table is supposed to
    ///   #"alib_ns_containers_hashtable_single_multi;store only unique elements", the
    ///   use of this method is therefore recommended, as an assertion hints to an erroneous use
    ...
    /// Overloaded version of method #"EraseUnique(const KeyType&)" which
    /// accepts the \p{hashCode} of the given \p{key} as a second parameter.
    ///
    /// @see Use cases of this method are discussed in reference documentation section
    ///      #"alib_ns_containers_hashtable_hashprecalc".
...
ALIB_POP_ALLOWANCE
    ...
    /// #"cbegin(uinteger)const;cbegin(uinteger)".
    ///
    /// The order of the elements that are not erased is preserved, what makes it possible to
    /// erase individual elements while iterating through the container.
    ///
...
///
/// \par Availability
///   Available only if the configuration macro #"ALIB_DEBUG_CONTAINERS" is set.
///
/// \see
///   Sibling namespace functions #"DbgDumpDistribution" and
///   #"DbgDumpHashtable" provided for debugging and optimization.
///
/// @tparam THashtable A specification  of templated type #"HashTable".
...
/// This type definition is a shortcut to #"HashTable", usable if the full
...
///   target type #"HashTable" that accept an object of template type
///   #"%TKey" expect an object of \p{StoredType} when this type is used.<br>
///   In case this is not wanted (or possible), and only the true key-portion should be expected
///   by interface functions such as #"HashTable::Find(const KeyType&);*", the original
///   type has to be used. Here, typically template parameter \p{TValueDescriptor} can be
///   set to #"TSubsetKeyDescriptor".
///
/// \see
///   For a detailed description of this type, see original type
///   #"HashTable", as well as alternative type definition
///   #"containers::HashMap".
///
/// @tparam TAllocator   The #"lang::Allocator;allocator type" to use.
/// @tparam T            The element type stored with this container.
///                      This type is published as #"HashTable::StoredType;*"
///                      and type definition #"HashTable::KeyType;*" becomes
///                      a synonym.
...
/// This type definition is a shortcut to #"HashTable", usable if data
