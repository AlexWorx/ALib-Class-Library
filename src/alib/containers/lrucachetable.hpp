...
/// This type implements a container used for caching objects. When its #".Size" reaches its
/// #".Capacity", one of the least recently used (LRU) objects is replaced with the next
/// insertion of a non-cached object.
///
...
/// - The #"%last" recently used object is always found with only one comparison. The probability
...
/// 1. Class #"OwnerAndGroupResolver" of camp \alib_filetree:
...
///    The default capacity of the #"%LRUCacheTable" chosen by class #"%OwnerAndGroupResolver" is
///    <c>6 x 6</c> and can be changed with
///    #"OwnerAndGroupResolver::SetOwnerCacheCapacity" and
///    #"OwnerAndGroupResolver::SetGroupCacheCapacity".
/// 2. Camp \alib_alox:
///    - With each log operation the placement of the call in a sourcefile (the #"lox::Scope")
///      is evaluated to resolve the resulting
///      #"alib_mod_alox_tak_terminology_domains;Log Domain"\".
...
///
///    The default capacity of the #"%LRUCacheTable" chosen by \alib is likewise <c>4 x 6</c>
...
/// the caller of method #"LRUCacheTable::Try", who, in case of a cache miss, is obligated to duly use a
...
/// While this in most cases is not needed, the central interface method #"LRUCacheTable::Try" also
/// returns an iterator.
/// While this iterator is always valid, a second returned value indicates whether likewise
...
/// #"TForwardIterator;Construct".
/// Furthermore, methods
/// #"TForwardIterator;Key" and
/// #"TForwardIterator;Mapped" allow access to the stored types.
///
/// @see
/// - Type definitions #"containers::LRUCacheSet" and
///   #"containers::LRUCacheMap" based on this type.
/// - The approach to distinguish between "sets", "maps" and "sets with embedded keys" are
///   the same with class #"%HashTable". Therefore, the
///   #"alib_ns_containers_hashtable_setandmap;documentation of class HashTable", which introduces
///   the same template parameter \p{TValueDescriptor} and also provides similar type definitions
///   #"cont HashSet" and #"cont HashMap", is very helpful to understanding the concept of this
///   type.
///
///
/// @tparam TAllocator       The #"lang::Allocator;allocator type" to use.
/// @tparam TValueDescriptor Defines the #".StoredType", #".KeyType", and #".MappedType".
///                          Furthermore, has to provide methods that extract the key and the mapped
///                          value from the stored type.<br>
...
#endif
{
  ...
  //================================================================================================
    ...
    /// #"LRUCacheTable::Iterator" and
    /// #"LRUCacheTable::ConstIterator".
    ///
    ...
    ///
    /// @tparam TConstOrMutable A constant or mutable version of #"LRUCacheTable::StoredType".
    ...
    class TForwardIterator {
      ...
      public:
        ...
        /// This method can be used if method #"LRUCacheTable::Try" indicates a cache miss.
        ...
        ///               instance of type #"LRUCacheTable::StoredType".
  ...
  public:
    ...
    ~LRUCacheTable() {
        if( Capacity() == 0 )
            return;
        Clear();
        allocBase::AIF().template FreeArray<Entry      >(elementPool, Capacity()    );
        allocBase::AIF().template FreeArray<ForwardList>(lists      , capacityLists );
    }
    ...
    void        Reserve( integer newQtyLists, integer newQtyEntriesPerList ) {
        ...
        if( Capacity() != newCapacity) {
            ...
            elementPool = newCapacity ? allocBase::AIF(). template AllocArray<Entry>(newCapacity)
                                      : nullptr;
        }
        ...
        if( capacityLists != newQtyLists) {
            if( capacityLists )
                allocBase::AIF(). template FreeArray<ForwardList>(lists, capacityLists );
            lists= newQtyLists ? allocBase::AIF(). template AllocArray<ForwardList>(newQtyLists)
                               : nullptr;
    ...
    /// 1. No element with a matching \p{key} is found in the cache while not all elements
    ...
    /// 3. The element with a matching \p{key} is found. In this case, the entry is moved to the
    ///    start of the list (if not already there).
    ///
    ...
    /// This can be done using convenient method #"TForwardIterator::Construct".
    /// Note that besides
    /// #"TForwardIterator;operator*", the iterator type
    /// offers methods
    /// #"TForwardIterator;Key" and
    /// #"TForwardIterator;Mapped" to directly access
    ...
    ///         calling #"TForwardIterator::Construct".
    [[nodiscard]]
...
/// This type definition is a shortcut to #"LRUCacheTable", usable if data
...
///   #"LRUCacheTable", as well as alternative type definition
///   #"containers::LRUCacheSet".
///
/// @tparam TAllocator      The #"lang::Allocator;allocator type" to use.
...
/// This type definition is a shortcut to #"LRUCacheTable", usable if the full
...
///   target type #"LRUCacheTable" that accept an object of template type
///   #"%TKey" expect an object of \p{T} when this type type defintion is used.<br>
///   In case this is not wanted (or possible), and only the true key-portion should be expected
///   by interface functions such as #"LRUCacheTable::Try;*", the underlying
///   original type #"LRUCacheTable") has to be used.
///
/// \see
///   For a detailed description of this type, see original type
///   #"LRUCacheTable", as well as alternative type definition
...
///                         This type is published as
///                         #"LRUCacheTable::StoredType;*" and type definition
///                         #"LRUCacheTable::KeyType;*" becomes a synonym.
