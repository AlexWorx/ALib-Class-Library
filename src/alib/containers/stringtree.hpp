...
//==================================================================================================
#ifndef HPP_ALIB_MONOMEM_CONTAINERS_STRINGTREE
#define HPP_ALIB_MONOMEM_CONTAINERS_STRINGTREE 1
#pragma once
#if !defined(DOXYGEN)
...
struct StringTreeNamesDynamic
{
    ...
    using NameStringType = ATMP_IF_T_F ( (TLocalCapacity > 0),
                                         strings::TLocalString<           TChar
                                                               ALIB_COMMA TLocalCapacity
                                                               ALIB_COMMA lang::HeapAllocator>,
...
class StringTree : protected detail::StringTreeBase<TAllocator,T,TNodeHandler,TRecycling>
{
    ...
    public:
        ...
        protected:
            ...
            {
                dbgCheckTree();
                ALIB_ASSERT_ERROR( cmCursor::node != nullptr, "MONOMEM/STRINGTREE",
        ...
        public:
            ...
            template<typename TParent= StringTree>                                             const
            inline
            TParent& Tree();
            ...
            inline
            ATMP_T_IF(TParent&, ATMP_ISOF(TParent, StringTree))
            Tree()                                                                             const
            {
                dbgCheckTree();
                return *static_cast<TParent*>(cmCursor::tree);
            }
            #endif
            ...
            {DCSSHRD
                dbgCheckTreeAndNode();
                ALIB_ASSERT_ERROR( other.node, "MONOMEM/STRINGTREE", "Invalid node given." )
                ALIB_ASSERT_ERROR( cmCursor::tree == other.tree, "MONOMEM/STRINGTREE",
    ...
    /// The mutable version of type \alib{containers;StringTree::TCursor<TConst>}.
    using Cursor       = TCursor<false>;

    /// The constant version of type \alib{containers;StringTree::TCursor<TConst>}.
    ...
    protected:
        ...
        struct RecursionData
        {
            ...
            #else
                 ATMP_IF_T_F(!TConst,       lang::BidiListHook<baseNodeBase>*
                                    , const lang::BidiListHook<baseNodeBase>*  )
                                        childrenUnsorted;
            #endif
    ...
    /// The mutable version of type \alib{containers;StringTree::TRecursiveIterator<TConst>}.
    using RecursiveIterator       = TRecursiveIterator<false>;

    /// The constant version of type \alib{containers;StringTree::TRecursiveIterator<TConst>}.
    ...
    ~StringTree()
    {
        ...
        ALIB_ASSERT_WARNING( basetree::dbgRootDataSet == 0, "MONOMEM/STRINGTREE",
          ...
          "Possible memory leak! To suppress this warning call DestructRootValue() before destruction.\n"
          "In case this is not necessary (because the stored type does not leak if not destructed),\n"
    ...
    {
        #if ALIB_DEBUG
            ALIB_ASSERT_ERROR( basetree::dbgRootDataSet != 0, "MONOMEM/STRINGTREE",
...
#endif // HPP_ALIB_MONOMEM_CONTAINERS_STRINGTREE
