...
//==================================================================================================
#ifndef HPP_ALIB_CONTAINERS_SHARED_PTR
#define HPP_ALIB_CONTAINERS_SHARED_PTR 1
#pragma once
#if !defined(DOXYGEN)
...
class SharedPtr
{
  ...
  public:
    ...
    SharedPtr& operator=(SharedPtr&& other)                                                 noexcept
    {
        ...
        template <typename... TArgs, typename TEnableIf= TAllocator,
                ATMP_IF(     std::is_default_constructible_v<TEnableIf>
                         &&  !ATMP_EQ( std::decay_t<TArgs>... , SharedPtr)
                         &&  (sizeof...(TArgs) > 0)                           ) >
        SharedPtr(TArgs&&... args)
        {
    ...
    #else
        template <typename TDerived, typename TEnableIf= TAllocator, typename... TArgs,
                  ATMP_IF( std::is_default_constructible_v<TEnableIf> )  >
        void InsertDerived(TArgs&&... args)
        {
            // delete any existing
...
#endif // HPP_ALIB_CONTAINERS_SHARED_PTR
