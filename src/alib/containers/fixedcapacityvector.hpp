...
///   each and every complex situation that uses C++20 concepts.
...
/// a #"FixedCapacityVector" as its underlying container type.
...
/// Type alias in namespace #"%alib", which denotes a <c>std::priority_queue</c> using
/// a #"FixedCapacityVector" as its underlying container type.
template<typename T, std::size_t TSize, typename TCompare= std::less<T>>
using FixedSizePriorityQueue = std::priority_queue< T, FixedCapacityVector<T, TSize>, TCompare>;
