...
///                          of the #"%HashTable".
/// @tparam THashCaching     A template parameter of the #"%HashTable" that determines whether
...
/// \note
///   The separation of the internals of class #"%HashTable" to this type in namespace
...
///      #"alib_ns_containers_hashtable_referencedoc;reference documentation"
///      of class #"%HashTable".
//==================================================================================================
template< typename      TAllocator,
          typename      TValueDescriptor,
                                             ...
                                             typename HTElementSelector<TValueDescriptor, THashCaching>::Type >
{
  ...
  //################################################################################################
    ...
    /// Will be exposed by derived class's definitions #"HashTable::Iterator" and
    ...
    /// #"HashTable::LocalIterator" and
    /// #"HashTable::ConstLocalIterator".
    ///
    /// As the name of the class indicates, this iterator satisfies the C++ standard library concept
    /// \https{ForwardIterator,en.cppreference.com/w/cpp/named_req/ForwardIterator}.
    ///
  ...
  //################################################################################################
    ...
    /// The maximum quotient of #"HashTableBase::size" and #"HashTableBase::bucketCount" that
    /// triggers a rehash.
    float               maxLoadFactor;
  ...
  //################################################################################################
    ...
    , size          ( 0 ) {
        buckets          = reinterpret_cast<FwdList*>(&DUMMY_BUCKET);
        bucketCount      = 1;
        size             = 0;
        sizeLimitToRehash= 0;
    }
    ...
    ~HashTableBase() {
        if ( buckets == reinterpret_cast<FwdList*>( &DUMMY_BUCKET ) )
            return;
  ...
  //################################################################################################
    ...
    void            rehash( uinteger newMinBucketCount ) {
        ...
        buckets= base::AIF().template NewArray<FwdList>(bucketCount);
        ...
        if ( oldData != reinterpret_cast<FwdList*>( &DUMMY_BUCKET ) )
    ...
    ///
    /// If no element with key \p{key} is found, both iterators are set to #"HashTable::end".
    ///
    ...
    ///
    /// Note: Used by #"HashTable::EmplaceIfNotExistent(const KeyType&, TArgs &&...);*".
    ///
