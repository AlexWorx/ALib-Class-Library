...
/// \note
///   The separation of the internals of class #"%StringTree" to this type in namespace
///   #"%containers::detail" has no benefit on compilation speed or other positive "technical"
///   effect, nor is it a matter of software design.<br>
///   The motivation is that a user of derived class #"HashTable" finds all interface methods and
///   types in one place, which is not cluttered by the documentation of the internals found here.
...
struct StringTreeBase {
  ...
  //################################################################################################
    ...
    /// definition #"StringTreeNamesDynamic::CharacterType" of template type
    /// \p{TNodeHandler}.
    ...
    /// #"StringTreeNamesDynamic::NameStringType" of template type \p{TNodeHandler}.
    ...
    /// #"StringTreeNamesDynamic::CharacterType" of template type \p{TNodeHandler}.
    using SubstringType     = typename strings::TSubstring<CharacterType>;

    /// The unique key to any element stored in this container.
    /// By being a (second) base type for of type #"StringTreeBase;Node", any
    ...
    struct NodeKey {
        ...
        /// the key of the hash set found with field #"StringTreeBase;nodeTable".
        /// <br>
    ...
    /// This is the base class of the internal node type #"StringTreeBase;Node".
    /// This type implements functionality needed. Derived type #"%Node" then only adds the custom
    /// value \p{T}.
    ///
    /// Objects of this type cannot be received directly and all interface is available
    /// via public type #"StringTree::Cursor;*" only, which holds a pointer to
                      ...
                      public NodeKey
    {
        ...
        /// Implementation of
        /// #"TCursor::AssemblePath(const TCursor)".
        ///
        ...
        ///                      class #"StringTree".
        /// @return The given #"%AString" to allow concatenated operations on it.
  ...
  //################################################################################################
    ...
    /// without prior use of method #"StringTree::ConstructRootValue". Also, the
    /// destructor issues a warning, in case the root node's value was not deleted with
    /// #"StringTree::DestructRootValue".
  ...
  //################################################################################################
    ...
    struct TCursorBase {
        ...
        /// Constant or mutable version of type #"%NodeBase", depending on template parameter
        /// \p{TConst}
        using cmNodeBase = std::conditional_t<!TConst,       NodeBase, const       NodeBase>;

        /// Constant or mutable version of type #"%Node", depending on template parameter \p{TConst}
        ...
        ///                    that is created.
        /// @return A <c>std::pair</c> containing a resulting #"%Node"* and the number of nodes
        ///         created.
        template<typename... TArgs, bool TRequires= !TConst >
        requires TRequires
        std::pair<cmNodeBase*, int> followPathCreate( const NameType& path, TArgs&&... args  ) {
            std::pair<cmNodeBase*, int> result= std::make_pair( node, 0 );
  ...
  //################################################################################################
    ...
    /// In debug-compilations, if it does, a #"alib_mod_assert;warning is raised".
    ///
