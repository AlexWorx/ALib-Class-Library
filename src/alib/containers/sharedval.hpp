...
class SharedVal {
  protected:
    /// Fields if \p{TAllocator} is default-constructible (e.g., #"HeapAllocator").
    ...
    /// Fields if \p{TAllocator} is not default-constructible (not #"HeapAllocator").
    ...
    /// The type of the stored data. Note that we cannot use helper #"AllocatorMember",
    ...
    T* getP()                        const noexcept { return members ? &members->custom : nullptr; }
