...
/// This struct is used to format caller information provided with the type #"CallerInfo".
///
/// Besides using method #".Format" "manually", instances of type #"%CallerInfo" may be passed as
/// arguments to type #"FormatterPythonStyle" using the according placeholder
/// syntax.
/// If done, internally an instance of this type is created and its #".Format" method called.
///
/// \note
///   This is achieved by boxed function #"FFormat_CallerInfo", as
///   explained in chapter #"alib_format_custom_types" of the Programmer's Manual
...
struct FMTCallerInfo {
    ...
    ///  - - - - - - - - - - - - | - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ///   sf | Prints the #"CallerInfo::File;source file name".
    ///   sl | Prints the #"CallerInfo::Line;line number" in the source file.
    ///   sm | Prints the #"CallerInfo::Func;function or method name" in the source file.
    ///   tn | Prints the #"Thread::GetName;ALib tread name". If no thread is set, the string <em>"<None>"</em> is written, which is resourced with <b>"FMTCINT"</b>.
    ///   ti | Prints the #"Thread::GetName;ALib tread ID". If no thread is set, the string <em>"<Null>"</em> is written, which is resourced with <b>"FMTCINR"</b>.
    ...
    ///   yf | Prints the full type name. If the #"%CallerInfo" does originate a static or global function, the string <em>"<None>"</em> is written, which is resourced with <b>"FMTCINY"</b>.<br>(With release-builds, nothing is printed.)
    ///   yn | Prints shortened type name, otherwise like 'yf'.
    ///   ya | Like 'sm' but in case the #"%CallerInfo" originates from a non-static member function, the shortened type name and <b>"::"</b> are prepended.<br>(With release-builds, nothing is printed.)
    ///
...
//==================================================================================================
/// Implementation of #"FFormat" for boxable type #"FMTCallerInfo".<br>
/// Writes the content of \p{box} (which is of type #"%CallerInfo") to the given #"%AString"
/// object \p{target} using a local instance of class #"FMTCallerInfo" and its
/// method #"FMTCallerInfo::Format;*".
///
...
///
/// which results in the same output that is generated if an instance of class #"%CallerInfo" is
/// simply appended to an #"%AString", what in turn is defined with functor
