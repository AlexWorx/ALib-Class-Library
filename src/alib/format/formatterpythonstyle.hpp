...
//==================================================================================================
/// Implements a #"format::Formatter" according to the formatting standards of the
/// \https{Python language,docs.python.org/3.5/library/string.html#format-string-syntax}.
///
/// \note
...
/// but then there are also found some very helpful extensions to that standard.
/// Instead of repeating the complete documentation, please refer to the link above as the
/// foundation and then take note of the following list of differences, extensions and general
/// hints:
///
/// - <b>General Notes:</b>
...
///     indexing. This \alib-implementation does allow it. The automatic index (aka no positional
...
///     attributes #"%ExponentSeparator", #"%NANLiteral" and #"%INFLiteral" of the inherited field
...
///     are added to such.
///     An alias name for \b !Quote is given with \b !Str. As the alias can be abbreviated to \b !s,
...
///     used.<br>
///     However the stored values are cleared, whenever #"^.Format" is invoked on a non-acquired
...
///     a second time to the desired output #"%AString".
///
...
class FormatterPythonStyle : public FormatterStdImpl {
  ...
  protected:
    ...
    ///
    /// @param startIdx  The index of the start of the field written into
    ///                  #"FormatterStdImpl::targetString". A value of \c -1 indicates
    ///                  pre-phase.
    /// @param target    The target string, only if different from the field
    ///                  #"FormatterStdImpl::targetString", which indicates intermediate phase.
