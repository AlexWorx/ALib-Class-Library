...
///
/// This template class provides a map of #"PropertyFormatter" objects that use
/// format strings which are defined by variables of a #"Configuration".
///
/// The use case is about having different versions of how an object is formatted and
/// to have these versions configurable to end users through the \alib variable system.
///
///\I{##############################################################################################}
...
/// #"PropertyFormatter", we are using the sample setup:
///
...
/// We define an enumeration type for this purpose and use macro #"ALIB_ENUMS_ASSIGN_RECORD"
/// to equip the enumeration with #"alib_enums_records;ALib Enum Records" of type
/// #"var Declaration".
/// In addition, macro #"ALIB_RESOURCED" is used to announce that the record data is defined using
...
/// #"alib_mod_bs;bootstrapping".
/// We take the freedom to use the resource pool of camp #"BASECAMP;2":
...
///   i.e., when they open an INI-file. See chapter #"alib_variables_external_ini" of the Programmer's
///   Manual of camp \alib_variables for information about how to write built-in default variables like
...
///
/// With this in place, the #"%PropertyFormatters" can be created and method
/// #".Format" can be used:
///
...
/// \#"AppendableTraits;appendable" to objects of class #"^AString".
/// Such object is given with template type #"PropertyFormatterMapAppendable".
...
/// The sample above showcased how to #"alib_strings_assembly_ttostring;append" a lightweight
/// object of helper-class #"PropertyFormatterMapAppendable" to objects of class #"^AString".
/// A final step is to define the #"alib::boxing;boxing" interface #"FAppend" to make boxed
/// objects of our helper-type applicable.
/// As the type itself is already appendable, we can use a built-in simple macro for that:
///
...
///\I{#############################################################################################}
/// # Module Dependencies #
/// This class is only available if module \alib_variables is included in the \alibbuild.
///
///\I{#############################################################################################}
/// # Reference Documentation #
/// @throws alib::format::FMTExceptions::MissingConfigurationVariable   \I{CLANGDUMMY}
///
...
class PropertyFormatters {
  ...
  public:
    ...
    ///                       #"PropertyFormatter"s.<br>
    ///                       Defaults to \c nullptr which denotes to create a copy of the
    ///                       static formatter given with #"Formatter::DEFAULT;*".
    ...
    ~PropertyFormatters()           { for( auto& formatter : formatters ) delete formatter.second; }

    /// Chooses or - if not yet available - creates the right #"PropertyFormatter"
    /// and invokes its method #"PropertyFormatter::Format;Format".
    ///
        ...
        if( formatter == formatters.end() ) {
            ...
            for( auto varEntryIt=  EnumRecords<TVariables>().begin() ;
                ...
                if( varEntryIt.Enum() == option ) {
                    ...
                    formatter= formatters.emplace( std::make_pair( option,
                                                   new PropertyFormatter(vc, callbacks, Formatter) )
                                                 ).first;
                    break;
                }

            ALIB_ASSERT_ERROR( formatter != formatters.end(), "FORMAT",
               "No entry for option {!Q<>} found in option table of PropertyFormatters.",
               static_cast<std::underlying_type_t<TVariables>>(option) )
        }
...
/// Helper-struct for users of class #"PropertyFormatters".
/// Objects of this type are #"AppendableTraits;appendable" to #"%^AString"s.
///
/// A custom using statement should be used to access this type in a very short fashion,
/// to make "applications" to #"%^AString"s a short call. Alternatively one ore more preprocessor
...
struct PropertyFormatterMapAppendable {
    ...
    TOptionEnum    Option;      ///< The format option defining the #"PropertyFormatter" used.
    TFormattable&  Formattable; ///< The object to be formatted.

    /// Constructor. Just saves the given parameters.
    /// @param pFormatterMap The #"PropertyFormatters" to use.
    /// @param pOption       The format option defining the #"PropertyFormatter" used.
...
//##################################################################################################
// ##### AppendableTraits specialization
//##################################################################################################
...
/// Specialization of functor #"AppendableTraits" for helper-struct
/// #"%PropertyFormatterMapAppendable" used to format objects with
/// #"PropertyFormatters".
/// @tparam TFormattable The formattable object type.
/// @tparam TOptionEnum      The format option enumeration type.
template<typename TFormattable, typename TOptionEnum >
struct       AppendableTraits<format::PropertyFormatterMapAppendable<TFormattable,TOptionEnum>,
                              character, lang::HeapAllocator>                                   {
    /// Invokes the #"PropertyFormatters::Format;*" of the formatter map
    /// encapsulated in \p{src}.
    /// @param target  The target string.
    /// @param src     The helper-struct object.
    void operator()( TAString<character, lang::HeapAllocator>&                   target,
                     const format::PropertyFormatterMapAppendable<TFormattable,
                                                                  TOptionEnum>&  src )   {
        src.FormatterMap.Format( target, src.Option, src.Formattable );
    }
};

/// Specialization of functor #"AppendableTraits" for pointers to helper-struct
/// #"%PropertyFormatterMapAppendable" used to format objects with
/// #"PropertyFormatters".
/// @tparam TFormattable The formattable object type.
/// @tparam TOptionEnum      The format option enumeration type.
