...
/// #"alib_format_intro;standard ALib format strings" is passed.
...
///
/// The only interface method #".Format" then accepts a #"alib::boxing;boxed object" of custom type
/// that is used to collect the format string's placeholder data, by using the stored callback
/// functions.
///
/// The identifiers are denoted by prefixing them with (customizable) character <c>'@'</c>.
///
/// A vector of objects of inner type #"%IdentifierEntry" needs to be defined and passed to the
/// constructor of this class.
/// The field #"IdentifierEntry::Name" contains the \e 'identifier' string which is removed from the
...
/// #"alib_ns_strings_propertyformatter_callbacks;below".
///
...
///
/// Serializing objects in a custom format (e.g., a user could define its own JSon object output)
...
///
/// The return type is #"alib::boxing::Box;Box", which allows the callback function to
...
///
/// The second parameter is an #"%AString" object which optionally can be used to assemble
...
/// the #"%AString" buffer provided in the second parameter is needed to be used
...
///
/// In case of
/// #"FormatterPythonStyle" format strings,
...
///
/// In case of
/// #"FormatterJavaStyle", which uses \b '\%'
...
/// Doing it this way, the \b '\%' symbol acts as a very natural delimiter for the custom
/// identifier.
///
/// Furthermore, the field #"IdentifierEntry::MinimumRecognitionLength" allows abbreviations of
/// identifier names.
/// It denotes the minimum number of characters to be* matched. As in the above sample a
...
/// is available with #"util::ExpressionFormatter".
...
/// @throws Exception #"alib::format::FMTExceptions::UnknownPropertyInFormatString;2" \I{CLANGDUMMY}
//==================================================================================================
class PropertyFormatter {
  public:
        /// The signature of the callback functions.
        /// See #"alib_ns_strings_propertyformatter_callbacks" for more information.
    ...
    struct IdentifierEntry {
        ...
        /// @see #"alib_ns_strings_propertyformatter_callbacks".
        TCallback   Callback;
    };
  ...
  public:
    ...
    ///                            the #"Formatter::DEFAULT;default formatter".
    /// @param ESCCharacter        The prefix used to search identifiers in
    ///                            \p{customFormatString}.<br>
    ///                            Defaults to <c>'@'</c>.
    ///
    /// @throws Exception #"FMTExceptions;FMTExceptions::UnknownPropertyInFormatString".
    ALIB_DLL
