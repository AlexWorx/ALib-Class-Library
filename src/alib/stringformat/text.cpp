...
Text::Text()
: Buffer(text)
, Formatter( STRINGFORMAT.GetDefaultFormatter()  )
{
    text.SetBuffer(2048);
}

Text::Text(AString& externalBuffer)
...
Text& Text::PushIndent( const String& indentFirstLine,
...
Text& Text::PushIndent( uinteger qty, character fillChar )
{
...
Text& Text::PopIndent()
{
    ALIB_ASSERT_ERROR( !IndentSizesFirstLine.empty(), "Text: PopIndent without prior push." )
    IndentFirstLine.DeleteEnd( static_cast<integer>( IndentSizesFirstLine.top() ) );
    IndentSizesFirstLine.pop();

    ALIB_ASSERT_ERROR( !IndentSizesOtherLines.empty(),"Text: PopIndent without prior push." )
...
void   Text::Add( Boxes&  args )
{
    integer startIdx= Buffer.Length();
    Formatter->FormatArgs( Buffer, args );
    integer maxLineWidth;
    Text::FormatParagraph( Buffer, startIdx, LineWidth, JustifyChar, maxLineWidth, IndentFirstLine, IndentOtherLines );
...
void   Text::AddMarked( Boxes&  args )
{
    character searchCharBuf[3];
    ...
    while( parser.IsNotEmpty() )
    {
        ...
        if( parser.CharAt( pos ) == '\n' )
        {
            ...
            Buffer << NewLine();
            integer maxLineWidth;
            Text::FormatParagraph( Buffer, lastTextStart, LineWidth, JustifyChar,
    ...
    {
        integer maxLineWidth;
        Text::FormatParagraph( Buffer, lastTextStart, LineWidth, JustifyChar, maxLineWidth,
...
// #################################################################################################
void  Text::FormatParagraph( AString& text,
