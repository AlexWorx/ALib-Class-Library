...
DOX_MARKER([DOX_ALIB_ENUM_META_DATA_SPECIFCATION_using_enum4])
//...
//...
//...
case END:
        ...
        if( !conversion.ConsumeChar('!') )
            throw Exception(ALIB_CALLER_NULLED, Exceptions::ExclamationMarkExpected,
             ...
             if(  conversion.ConsumePartOf( A_CHAR( "Upper"     ) ) > 0 )   { if (isPostProcess) targetString->ToUpper( startIdx );                 }
                ...
                ||conversion.ConsumePartOf( A_CHAR( "Quote"     ) ) > 0 )   { if (isPostProcess) targetString->InsertAt<false>(A_CHAR( "\"" ), startIdx)
                                                                                                              .Append  <false>(A_CHAR( "\"" ) );  }
