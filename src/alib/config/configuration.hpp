...
class Entry
{
  ...
  friend struct alib::config::detail::ConfigNodeHandler;
...
/// A shortcut to the base class of the base class of class \alib{config;Configuration}.
...
/// \alib{config;Configuration} which recycles variables and their extended information objects.
///
...
struct ConfigNodeHandler
{
    ...
    /// addition recycles any extended information object attached to the \alib{config;Variable}
    /// object.
    /// @param  tree  The instance of struct \alib{containers;detail::StringTreeBase} that invokes
    ///               this method. Any member may be accessed, including
    ///               \alib{containers::detail::StringTreeBase;nodeTable} which contains the
...
///  @see Chapter \ref alib_config_monitoring of the Programmer's Manual of camp \alib_config_nl.
// =================================================================================================
struct ConfigurationListener
{
    ...
    ///                          \alib{config;ConfigurationListener::Event;Event::Definition}.<p>
...
/// The use of this type is documented with the \ref alib_mod_config "Programmer's Manual" of
/// camp \alib_config_nl.
///
...
///  By \ref alib_manual_bootstrapping_customize "customizing the bootstrap process",
...
protected:
    ...
    /// \alib{config;Configuration::StoreDeclaration}.
  ...
  //==================================     Protected Methods     ===================================
    /// Implementation of \alib{config;Configuration::RegisterType}.
    ...
    /// @see Overloaded public methods \alib{config;Configuration::PresetImportString}.
    ...
    ALIB_API  void presetImportString(const String& name, const String& value,
    ...
    ALIB_API  void registerListener( ConfigurationListener*         listener,
    ...
    ALIB_API  void notifyListeners(  int                event,
                                     const Variable&    variable,
  ...
  //=============================       Constructor/destructor     =================================
    ...
    /// -----------------------------------------------|------------------------------
    ///  \alib{config;Priority;Priority::Environment}  | \alib{config;EnvironmentVariablesPlugin}
    ///  \alib{config;Priority;Priority::CLI}          | \alib{config;CLIVariablesPlugin}
    ///
    /// Furthermore all \ref alib_config_types_builtin "built-in variable types" are registered.
    ///
    ...
    /// threads are active) or with prior  locking the \alib{config;Configuration} instance.
    ///
...
//========================================    Interface    =======================================
    ...
    /// the fields (except \alib{config::Declaration;typeName}) are replaced with the
    ...
    ///
    /// @see Chapter \ref alib_config_names_placeholders of the Programmer's Manual of
    ///      camp \alib_config_nl.
    ///
    ...
    const Declaration*  StoreDeclaration( const Declaration* orig, const Box& replacements );
        ...
        /// on the concrete execution path of a run of a software. The rationale to do this is is
        /// twofold:
        ...
        ///    \alib{config;Configuration::PresetImportString}, which in the end leads to correctly
        ...
        ///    source. Populating external configuration files (with the first run of a software),
        ///    tremendously supports an end-user in learning about configuration options.<br>
        ///    For example, built-in class \alib{config;IniFileFeeder} supports such "population"
        ...
        /// - as explained above, the need of method \alib{config;Configuration::PresetImportString}
        ///   is avoided for all variables that do not contain placeholders, and because
        /// - most variables would anyhow be declared later during the run of a software
        ///   and thus, this declaration is just performed earlier and the variable are already in
        ///   place when a software later uses them.
        ///
        /// @see Chapter \ref alib_config_external_ini of the Programmer's Manual of camp
        ///      \alib_config.
        ///
        /// @tparam TEnum Enumeration type equipped with \ref alib_enums_records "ALib Enum Records"
        ///               of type \alib{config;Declaration}.
    ...
    ///   - Chapter \ref  alib_config_external_ini of the Programmer's Manual of camp
    ///        \alib_config.
    /// @param name     The name of the variable.
    ...
    /// @param priority The priority to set for the value.
    ///                 Defaults to \alib{config;Priority;Default}.
    ...
    /// \snippet "alib/config/configuration.cpp"  DOX_CONFIG_DELETE_SAMPLE
    ///
    ///
    ...
    void MonitorDistinctVariable( lang::ContainerOp               insertOrRemove,
                                  ...
                                  const Variable&                 variable       )
    {
        ALIB_ASSERT_WARNING( event != ConfigurationListener::Event::Creation, "CONFIG",
    ...
    void MonitorVariablesByName( lang::ContainerOp              insertOrRemove,
                                 ...
                                 const String&                  variableName )
    {
        ALIB_ASSERT_ERROR( variableName.IsNotEmpty(), "CONFIG", "Empty variable name given.")
    ...
    void MonitorPathPrefix( lang::ContainerOp               insertOrRemove,
                            ...
                            const String&                   pathPrefix      )
    {
        ALIB_ASSERT_ERROR( pathPrefix.IsNotEmpty(), "CONFIG", "Empty path prefix given.")
        registerListener( listener,
                          insertOrRemove,
                          int(event),
                          nullptr, nullptr,
    ...
    void MonitorPathSubstring( lang::ContainerOp               insertOrRemove,
                               ...
                               const String&                   pathSubstring  )
    {
        ALIB_ASSERT_ERROR( pathSubstring.IsNotEmpty(), "CONFIG", "Empty path substring given.")
        registerListener( listener,
                          insertOrRemove,
                          int(event),
                          nullptr, nullptr,
...
//==================================================================================================
/// This is the central interface type into \ref alib_mod_config "ALib Configuration Data".
...
/// a <b>StringTree::Cursor</b> is only necessary for implementing \alib{config;ConfigurationPlugin}
/// or similar extraordinary things. For the common use, the dedicated interface is all that is
/// needed.
///
/// @see
///   - For a quick tutorial about using \alib configuration variables, consult the tutorial-style
///     \ref alib_mod_config "Programmer's Manual" of camp \alib_config_nl.
...
class Variable : protected Configuration::Cursor
{
  ...
  public:
    ...
    /// Same as #Variable(Configuration&), but takes the configuration from an \alibcamp.
    /// @param camp An \alibcamp singleton wich supplies the \alib{config;Configuration} instance.
    explicit Variable(lang::Camp& camp)
    : Cursor(camp.GetConfig().Root())                                                             {}
    ...
    ///                     \alib{config;Configuration} instance.
    /// @param name         The name of the variable.
    /// @param typeName     The type of the variable.
    /// @param defaultValue An optional default value. Defaults to \e nulled string.
    Variable( lang::Camp&   camp,
              const String& name, const String& typeName,
              const String& defaultValue= NULL_STRING )
    : Cursor(camp.GetConfig().Root())                   { Declare( name, typeName, defaultValue ); }


    /// Constructs and declares this variable, without using or allocating a declaration struct.
    /// This overload omits a parameter that specifies a configuration and uses the configuration
    /// object of the \alibcamp singleton \ref alib::BASECAMP.
    /// @param name         The name of the variable.
    /// @param typeName     The type of the variable.
    /// @param defaultValue An optional default value. Defaults to \e nulled string.
    Variable( const String& name, const String& typeName,
              const String& defaultValue= NULL_STRING )
    : Cursor(alib::BASECAMP.GetConfig().Root())         { Declare( name, typeName, defaultValue ); }
    ...
    /// Constructs and declares this variable.
    /// @param camp An \alibcamp singleton which supplies the \alib{config;Configuration} instance.
    /// @param decl The declaration to use.
    Variable(lang::Camp& camp, const Declaration* decl)
    : Cursor(camp.GetConfig().Root())                                            {Declare( decl ); }
    ...
    : Cursor(camp.GetConfig().Root())                       { Declare( Declaration::Get( Enum ) ); }

    #if DOXYGEN
    /// Constructs and declares this variable.
    /// \attention This constructor might allocated a new record in the mono allocator of \p{cfg}.
    ...
    #else
    template<typename TEnum, typename TEnableIf= ATMP_VOID_IF(ATMP_IS_ENUM(TEnum))>
    inline
    Variable(Configuration& cfg, TEnum Enum, const Box replacements )
    ...
    Variable(lang::Camp& camp, TEnum Enum, const Box replacements )
    : Cursor(camp.GetConfig().Root())
    { Declare( camp.GetConfig().StoreDeclaration( Declaration::Get( Enum ), replacements ) ); }
    #endif //if DOXYGEN
    ...
    ///       example, when implementing a custom \alib{config;ConfigurationPlugin}.<br>
  ...
  public:
    ...
    /// In debug compilations, an assertion is raised in case the variable existed but has
    /// a different declaration attached.
    ///
    /// If the variable is created and the \p{declaration} contains a default value, such value is
    /// imported from the string. In this case, \alib{config;Priority;DefaultValues} is returned.
    ///
    ...
    #else
    template<typename TEnum, typename EnableIf=
                            ATMP_VOID_IF( EnumRecords<TEnum>::template AreOfType<Declaration>() ) >
    inline
    Variable&      Declare( TEnum Enum )              {  return Declare( Declaration::Get(Enum) ); }
    #endif //if DOXYGEN
    ...
    template<typename TEnum>
    inline
    ATMP_T_IF( Variable&, EnumRecords<TEnum>::template AreOfType<Declaration>() )
    Declare( TEnum Enum, const Box& replacements )
    {
    ...
    /// \alib{config;Configuration::PresetImportString;preset value} exists for the variable,
    ...
    ///    \alib{config;Configuration::PresetImportString;preset value} is set.
    ///
    /// @param name         The name of the variable.
    /// @param typeName     The type of the variable.
    /// @return \c true if the variable name specified with the given \p{declaration} existed and
    ...
    ///   usage is a contract to make the \alib configuration system secure.
    ...
    ///   \alib{config;Priority;Priority::Protected} is passed, add a simple <c>(void)</c> before
    ///   the call.
    ///
    /// @see
    ///  - Chapter \ref alib_config_definition of the Programmer's Manual for more information.
    ///  - Chapter \ref alib_config_monitoring to learn about how to monitor changes of variables.
    ///
    ...
    /// ('hard-coded' values). Besides the elements in enumeration \alib{config;Priority},
    ...
    /// to \alib{config;Priority;Environment}, then environment variables and CLI arguments may
    /// still overwrite a value. If set to \alib{config;Priority;Environment + 1}, then
    /// environment variables would not overwrite, but still CLI arguments could.
    ///
    /// A priority of \alib{config;Priority;Protected} disallows any code entity to make
    ...
    ///
    /// @return \c true if data contained has a higher priority than \alib{config;Priority;Standard},
    ///         \c false if not.
    inline
    ...
    /// performed by calling \alib{config;VMeta::imPort}  on the virtual meta object
    ...
    ///                 be parsed. Usually, if a \alib{config;ConfigurationPlugin} invokes this method
    ...
    /// This is done by calling \alib{config;VMeta::exPort} on the virtual meta object
    /// associated with this variable's type.
    ///
    ...
    ///               a \alib{config;ConfigurationPlugin} invokes this method to write-back the
    ...
    /// - A configuration plug-in like \alib{config;CLIVariablesPlugin} or
    ///   \alib{config;EnvironmentVariablesPlugin},
    ...
    ///
    /// If this is not done, in debug-compilations an assertion is raised. In release-builds,
    /// it is defined (!) behavior: A default constructed value of \p{T} is returned.
    ...
    /// \ref alib_config_definition of the Programmer's Manual of this \alibcamp.
    ///
    ...
    {
        ALIB_ASSERT_ERROR( !Cursor::IsRoot(), "CONFIG",
                  "Requesting value from undeclared variable: {!Q}", *this )
        ALIB_ASSERT_ERROR( IsDefined(), "CONFIG",
                  "Requesting value from undefined variable. Use 'var.Define();' to mitigate\n"
                  "this error. Variable: ", *this )
        ALIB_ASSERT_ERROR( getMeta()->dbgTypeID() == typeid(T), "CONFIG",
                  "Wrong variable type requested for variable: {!Q}.\n"
                  "  Stored   : {}\n"
                  "  Requested: {}\n",
    ...
    {
        ALIB_ASSERT_ERROR( Cursor::Value().data != nullptr, "CONFIG",
    ...
    operator float           ()                   { return static_cast<float>(Get<double>()); }    ///< @return Calls and returns \ref Get "Get\<double\>()" cast to \c float.
    ...
    operator const Substring ()                   { return Get<AStringPA>(); }                     ///< @return Calls and returns \ref Get "Get\<AStringPA\>()".
                  ...
                  float        GetFloat()         { return static_cast<float>(Get<double>()); }    ///< @return Calls and returns \ref Get "Get\<double\>()" cast to \c float.
    ...
    float         operator= (float         val) { Get<double    >()= static_cast<double>(val); return val; } ///< Calls \ref Get "Get\<double\>()" and assigns \p{val}.        @param val The value to assign. @return The \p{val} given.
    ...
    ///                  Defaults to \alib{config;Priority;DefaultValues}.
    /// @return \c false if data of any priority is available in this variable, \c true if not.
    inline
...
template<typename TEnum>
ATMP_VOID_IF( EnumRecords<TEnum>::template AreOfType<Declaration>() )
Configuration::PreloadVariables()
{
...
///               With the inclusion of module \alib_threads in the \alibdist, the type-alias
...
class TSharedConfiguration : public  monomem::TSharedMonoVal<Configuration, HeapAllocator, TLock>
{
  ...
  public:
    ...
    void DbgCriticalSections(lang::Switch onOff);
    #else
        template<typename TIf= typename Base::LockType>
        ATMP_VOID_IF( !ATMP_EQ(TIf, void) )
        DbgCriticalSections(lang::Switch onOff)
        {
        ...
        template<typename TIf= typename Base::LockType>
        ATMP_VOID_IF( ATMP_EQ(TIf, void) )
        DbgCriticalSections(lang::Switch)                                                         {}
    #endif
...
using Configuration        = config::Configuration;                     ///< Type alias in namespace \b alib.
#if ALIB_THREADS || DOXYGEN
using SharedConfiguration  = config::TSharedConfiguration<SharedLock>;   ///< Type alias in namespace \b alib.
#else
using SharedConfiguration  = config::TSharedConfiguration<void>;
#endif
using ConfigurationListener= config::ConfigurationListener;             ///< Type alias in namespace \b alib.
using Variable             = config::Variable;                          ///< Type alias in namespace \b alib.
...
/// Specialization of functor \alib{strings;T_Append} for type <c>Variable</c>.
template<> struct       T_Append<config::Variable, nchar, lang::HeapAllocator>
{
...
/// Specialization of functor \alib{strings;T_Append} for type <c>Variable</c>.
template<> struct       T_Append<config::Variable, wchar, lang::HeapAllocator>
{
    /// Writes the variable name represented by the given configuration cursor to the given AString.
...
ALIB_BOXING_VTABLE_DECLARE(   alib::config::Configuration::ConstCursor, vt_config_constcursor )
ALIB_BOXING_VTABLE_DECLARE(   alib::config::Variable     , vt_config_variable      )
