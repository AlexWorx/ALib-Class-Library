...
//==================================================================================================
#ifndef HPP_ALIB_CONFIG_VMETA
#define HPP_ALIB_CONFIG_VMETA 1
#pragma once
...
/// \b StringTree nodes of class \alib{config;Configuration}. A pointer to a singleton of this
...
/// \alib{config;Configuration::RegisterType}.
///
/// @see Chapter \ref alib_config_types_custom of the Programmer's Manual of camp \alib_config.
struct VMeta
{
    ...
    /// Types declared with macro \ref ALIB_CONFIG_VARIABLE_DEFINE_TYPE implement this method
    /// rightfully.
    /// @return The type's name.
    virtual String   typeName()                                                           const = 0;

    #if ALIB_DEBUG
    /// Abstract virtual method. Descendants need to return the <c>std::type_info&</c> received with
    /// <c>typeid()</c>. This method is available only in debug-compilations and is used to assert
    /// that the correct types are read from declared variables.<br>
    /// Types declared with macro \ref ALIB_CONFIG_VARIABLE_DEFINE_TYPE implement this method
    /// rightfully.
    ...
    /// the custom type. Types declared with macro \ref ALIB_CONFIG_VARIABLE_DEFINE_TYPE implement
    ...
    /// what is specified with compiler symbol \ref ALIB_MONOMEM_POOLALLOCATOR_DEFAULT_ALIGNMENT.
    ...
    /// lifecycle. However, in this case chapter \ref alib_config_multithreading of the
    ...
    /// \attention
    ///   Types declared with macro \ref ALIB_CONFIG_VARIABLE_DEFINE_TYPE only declare this method.
    ...
    /// are human-readable. See chapter \ref alib_config_external_custom of the Programmer's
    /// Manual for more information.
    ///
    /// \attention
    ///   Types declared with macro \ref ALIB_CONFIG_VARIABLE_DEFINE_TYPE only declare this method.
    ///   An implementation needs to be provided in a compilation unit. Otherwise linking a software
    ///   fails.
    ///
...
namespace alib::config
{
    ...
    /// \alib{config;Configuration::BooleanTokens}. If successful, the index of the pair of
    ...
    struct Bool
    {
        ...
        /// The index in the \alib{config;Configuration::BooleanTokens;list of tokens} found when
    ...
    ///   Built-in type \alib{config;IniFile} processes such codes correctly.
...
{
    ALIB_API String                 typeName()      const  override          { return A_CHAR("I"); }
ALIB_DBG(
    ALIB_API const std::type_info&  dbgTypeID()            override      { return typeid(integer); }
)
    ALIB_API void   construct(void* dest, PoolAllocator&)  override        { new (dest) integer(); }
    ALIB_API void   destruct (void*     , PoolAllocator&)  override                               {}
    ALIB_API size_t size()                                 override      { return sizeof(integer); }
    ALIB_API void   imPort( VDATA*, Configuration&, const StringEscaper&, const String& )  override;
    ALIB_API void   exPort( VDATA*, Configuration&, const StringEscaper&,      AString& )  override;
};
...
{
    ALIB_API String                 typeName()      const  override          { return A_CHAR("F"); }
ALIB_DBG(
    ALIB_API const std::type_info&  dbgTypeID()            override       { return typeid(double); }
)
    ALIB_API void   construct(void* dest, PoolAllocator&)  override         { new (dest) double(); }
    ALIB_API void   destruct (void*     , PoolAllocator&)  override                               {}
    ALIB_API size_t size()                                 override       { return sizeof(double); }
    ALIB_API void   imPort( VDATA*, Configuration&, const StringEscaper&, const String& )  override;
    ALIB_API void   exPort( VDATA*, Configuration&, const StringEscaper&,      AString& )  override;
};
...
{
    ALIB_API String                 typeName()      const  override          { return A_CHAR("S"); }
ALIB_DBG(
    ALIB_API const std::type_info&  dbgTypeID()            override    { return typeid(AStringPA); }
)
    ALIB_API void   construct(void* dest, PoolAllocator& pool)  override
    { new (dest) AStringPA(pool); }
    ALIB_API void   destruct (void* dest, PoolAllocator&)       override
    { reinterpret_cast<AStringPA*>(dest)->~AStringPA();  }
    ALIB_API size_t size()                                 override    { return sizeof(AStringPA); }
    ALIB_API void   imPort( VDATA*, Configuration&, const StringEscaper&, const String& )  override;
    ALIB_API void   exPort( VDATA*, Configuration&, const StringEscaper&,      AString& )  override;
};
...
ALIB_CONFIG_VARIABLE_DEFINE_TYPE(                       alib::config::, Bool                 , A_CHAR("B"  ) )
ALIB_CONFIG_VARIABLE_DEFINE_TYPE(                       alib::boxing::, Box                  , A_CHAR("BOX") )
ALIB_CONFIG_VARIABLE_DEFINE_TYPE_WITH_POOL_CONSTRUCTOR( alib::        , AStringPA            , A_CHAR("S"  ) )
ALIB_CONFIG_VARIABLE_DEFINE_TYPE_WITH_POOL_CONSTRUCTOR( alib::config::, StringVectorComma    , A_CHAR("SV,") )
ALIB_CONFIG_VARIABLE_DEFINE_TYPE_WITH_POOL_CONSTRUCTOR( alib::config::, StringVectorSemicolon, A_CHAR("SV;") )


#endif // ALIB_DOX
#endif // HPP_ALIB_CONFIG_VMETA
