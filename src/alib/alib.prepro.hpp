...
/// Its purposes are:
/// - To identify given #"alib_macros_config" related to module selection and enable
///   corresponding configuration macros (that may be not given) of mandatory dependent modules.
...
//##################################################################################################
// No module selection macro given but threads? -> choose ALL
//##################################################################################################
...
#   define  ALIB_FILETREE            1
#   if !ALIB_SINGLE_THREADED
#       define  ALIB_THREADMODEL     1
#   endif
#elif defined(ALIB_THREADMODEL ) &&  ALIB_SINGLE_THREADED
#   error "ALib Module THREADMODEL requested, while configuration macro ALIB_SINGLE_THREADED is set"
#endif
...
#endif
#if !defined(ALIB_FILETREE        )
#   define ALIB_FILETREE               0
#endif
...
#if ALIB_APP
#   define  IF_ALIB_APP(...)              __VA_ARGS__
#   define  IFNOT_ALIB_APP(...)
#else
#   define  IF_ALIB_APP(...)
#   define  IFNOT_ALIB_APP(...)           __VA_ARGS__
#endif
...
#if ALIB_FILETREE
#   define  IF_ALIB_FILETREE(...)             __VA_ARGS__
#   define  IFNOT_ALIB_FILETREE(...)
#else
#   define  IF_ALIB_FILETREE(...)
#   define  IFNOT_ALIB_FILETREE(...)          __VA_ARGS__
#endif
...
#   else
#       error "Unknown compiler/toolchain. Can't deduce ALIB_CPP_STANDARD. Please provide this configuration macro 'manually' to the compiler."
#   endif
#else
#   if defined(ALIB_INTERNAL_DEDUCED_CPPVER)
...
// C++20-Module compilation mode toggle
#if !defined(ALIB_C20_MODULES) && !DOXYGEN
#   define ALIB_C20_MODULES 0
#endif
#if ((ALIB_C20_MODULES != 0) && (ALIB_C20_MODULES != 1))
...
// Warnings
    ...
    #define ALIB_ALLOW_MACRO_REDEFINITION                                       \
        _Pragma("GCC diagnostic push")                                          \
    ...
    #define ALIB_ALLOW_MACROS_WITH_RESERVED_NAME                                \
        _Pragma("GCC diagnostic push")                                          \

    #define ALIB_ALLOW_OVERLOADED_VIRTUALS                                      \
        _Pragma("GCC diagnostic push")                                          \
    ...
    #define ALIB_ALLOW_FUNCTION_TEMPLATE                                        \
        _Pragma("GCC diagnostic push")                                          \
    ...
    #define ALIB_ALLOW_RESERVED_IDENTIFIER                                      \
        _Pragma("GCC diagnostic push")                                          \
...
#   define   ALIB_DEBUG_CONTAINERS           0
#   pragma message "Configuration Macro ALIB_DEBUG_CONTAINERS set (from outside!) while ALIB_DEBUG is not. The macro got disabled."
#endif
...
#if defined(__clang__)
#   define     ALIB_IDENTIFIER(prefix)  ALIB_ALLOW_RESERVED_IDENTIFIER    \
...
//==================================================================================================
// Used by the macro ALIB_ASSERT_GLOBAL_NAMESPACE for testing.
//==================================================================================================
...
//##################################################################################################
// Macro  introduced by lang::integer, and similar basics
//##################################################################################################
//---------------------- One of the 5 configuration macros given from outside? ---------------------
    ...
    // just check if all are given, that's it for now
        ...
        #error "If one of the Configuration Macros \
'ALIB_SIZEOF_INTEGER', \
'ALIB_SIZEOF_INTGAP', \
'ALIB_INTGAP_TYPE', \
...
//------------ None of the 5 configuration macros given from outside: Platform detection -----------
#else
...
//##################################################################################################
// Macro  introduced by alib::characters
//##################################################################################################
#if defined(ALIB_CHARACTERS_NATIVE_WCHAR)
#   error "Configuration macro ALIB_CHARACTERS_NATIVE_WCHAR must not be passed to the compiler. It is deduced in ALib headers."
#endif
...
//##################################################################################################
// Macro  introduced by lang::Owner
//##################################################################################################
...
//##################################################################################################
// Macro  introduced by lang::DbgCriticalSections
//##################################################################################################
