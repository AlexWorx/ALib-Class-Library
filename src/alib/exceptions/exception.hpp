...
/// struct #"TSharedMonoVal" that class #"exc Exception" is derived from
/// and this way allocated (self-contained) in a #"MonoAllocator".
...
//==================================================================================================
/// An #"alib_enums_records;ALib Enum Record" type used to equip custom enumeration types
/// with records that define an entries of class #"exc Exception".
///
/// Besides the exception entry name, the only field this record adds provides a textual description
/// of an exception entry.
/// If the type trait #"ResourcedTraits" is likewise specialized
...
/// #"ERSerializable::MinimumRecognitionLength;*" is not parsed from the string, but set to
...
struct ERException : public enumrecords::ERSerializable {
    ...
    /// \note
    ///   If the type trait #"ResourcedTraits" is specialized for an
    ...
    /// using enumeration types associated with #"alib_enums_records;ALib Enum Records" of this
    /// type).
    ///
    ...
    /// each parameter of type #"%^String" passed, has to be of "static nature".
...
/// #"alib_exceptions_exceptions;detailed information" about this class and its use.
///
/// In short, this class implements the following "exception paradigm":
/// - There is only one exception type.
/// - That type stores a forward list of #"Message;messages".
...
///   of the exception.
/// - Messages contain IDs of #"Enum;arbitrary scoped enumeration types".
///   This allows structured processing of Exceptions.
///
/// This type uses a smart memory model leveraging class #"TSharedMonoVal"
/// which places all internal data in a first buffer of a #"TMonoAllocator",
/// even the #"%exc Exception" object itself!
/// With that, exceptions usually perform only one single dynamic allocation, even if various
/// messages with various data objects (#"Box;boxes") are attached.
...
///
/// @see Chapter #"alib_exceptions_exceptions" of the Programmer's Manual of
...
class Exception : protected monomem::TSharedMonoVal<detail::ExceptionEntry*, HeapAllocator, void> {
  ...
  public:
    ...
    ///                              #"MonoAllocator" in kilobytes (1024 bytes).
    ...
    /// #"alib_enums_records;ALib Enum Records" according to record type
    /// #"ERException", the first argument added to the message entry
    /// is collected from the corresponding enum record. For more
    /// information consult the #"alib_exceptions_exceptions_res;corresponding section" of the
    /// Programmer's Manual.
    ///
...
//##################################################################################################
    ...
    /// Returns field #"Message::Type;*" of the #"%last" message in the list of
    /// messages that has a positive underlying enum element value.
    ///
    /// The rationale here is explained in the
    /// #"alib_exceptions_exception_types;Programmer's Manual".
    ...
    /// exactly those that are expected by the #"Message::Message;constructor"
    /// of class #"Message".
    ///
    /// The message object itself is created in the inherited monotonic allocator.
    /// After the insertion, method #"TBoxes;CloneAll" is invoked, which
    ...
    /// with #"alib_enums_records;ALib Enum Records" of type #"ERException",
    /// an additional message argument is \b prepended to the message.
    /// This argument is of a string-type and is taken from the field
    /// #"ERException::DescriptionOrItsResourceName" of the associated enum record.
    ...
    ///
    /// If furthermore, the type trait #"ResourcedTraits" is specialized for
    /// enumeration type \p{TEnum}, then the value of #"%DescriptionOrItsResourceName" is not
    /// directly prepended but interpreted as a resource name. In this case the resourced
    /// description is prepended instead.
    ///
    /// For more information, consult chapter #"alib_exceptions_exceptions_res" of the
    /// Programmer's Manual.
    ///
    ...
    #if ALIB_FORMAT
    /// Uses class #"Paragraphs" to write all entries of this
    ...
    /// @return Returns given #"%AString" \p{target} for convenience.
    ALIB_DLL
    AString&  Format( AString& target )                                                       const;

    /// Same as #"Format(AString&)const;Format(AString&)", but writing
    ...
    /// Inline shortcut to #"Format(AString&)const;Format(AString&)"
...
//##################################################################################################
  protected:
    /// Implementation of \c std::ForwardIterator that iterates all #"Message"
    /// entries of an #"%exc Exception".
...
/// Creates an according #"exc Exception" to a corresponding system error number.
/// The following data is attached to the message:<br>
/// 1. <c>"{!Q} ({}){!X}"</c> - Format string used with #"Exception::Format;*"
///    Note that the 3rd placeholder inhibits formatting the 4th. parameter.
/// 2. <c>errorCode.message()</c> - First format parameter.
/// 3. <c>errorCode.category().name()</c> - Second format parameter.
/// 4. <c>errorCode</c> - A copy of the error code.
///
/// The exceptions' enumeration type will be <c>std::errc</c>.
/// The result of function <c>std::error_code::message()</c> is attched as a parameter
/// @param ci    The source location of the exception creation.
/// @param errorCode The system's error number.
/// @return An exception object.
inline
Exception CreateExceptionFromSystemError( const CallerInfo& ci, std::error_code errorCode ){
    return Exception( ci, std::errc(errorCode.value()),
                      "{!Q} ({}){!X}",
                      errorCode.message(),
...
/// Overloaded version, which accepts an integral POSIX system exception.
/// Uses <c>std::make_error_code</c> to create an object of type <c>std::error_code</c>
/// to be passed to the original function.
/// @param ci    The source location of the exception creation.
/// @param errNo The system's error number. Defaults to system macro <c>errno</c>.
/// @return An exception object.
inline
Exception CreateExceptionFromSystemError( const CallerInfo& ci, int errNo= errno ) {
    return CreateExceptionFromSystemError( ci, std::make_error_code(std::errc(errNo)));
}

#if defined(_WIN32)
/// Overloaded version, which accepts a WindowsOS error code.
/// @param ci       The source location of the exception creation.
/// @param winError A WindowsOS error number, i.e. received with <c>GetLastError()</c>.
/// @return An exception object.
inline
Exception CreateExceptionFromWindowsOSError( const CallerInfo& ci, DWORD winError ) {
    return CreateExceptionFromSystemError( ci, std::error_code( static_cast<int>(winError),
                                                                   std::system_category()       ) );
}
#endif
...
#endif
//==================================================================================================
/// Specialization of functor #"AppendableTraits" for type #"exc Exception".
///
/// \par Availability
///   This method is available only if the module \alib_format is included in the \alibbuild.
...
struct AppendableTraits<exceptions::Exception,TChar,TAllocator> {
    /// Invokes #"Exception::Format;*" passing the \p{target}.
    ///
