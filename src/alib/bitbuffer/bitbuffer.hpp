...
class BitRWBase {
  protected:
    ...
    BitBufferBase::Index    idx; ///< The current reading/writing index within #"BitRWBase::bb".
...
class BitWriter : public BitRWBase {
  ...
  public:
    ...
    void    Flush()                                                       { bb.SetWord(idx, word); }
        ...
        ///   #"WriteBits(lang::ShiftOpRHS, TValue)"
        ///   which is to be used when the value is determined only at runtime.
        ///
...
// On the other hand, it complains if the methods are not doxed. So, we hide them from doxygen.
    ...
    ///   available with #".WriteBits(TIntegral)".
...
// On the other hand, it complains if the methods are not doxed. So, we hide them from doxygen.
    ...
    ///   A slightly slower, non-templated version is available with #".ReadBits(lang::ShiftOpRHS)",
    ///   which is to be used when the number of bits to read is determined only at runtime.
    /// @tparam TWidth    The number of bits in \p{value} to write.
