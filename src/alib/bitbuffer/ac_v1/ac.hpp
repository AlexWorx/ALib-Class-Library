...
/// Classes #"ac_v1::ArrayCompressor;*" and #"ac_v1::HuffmanEncoder;*"
...
/// new binary format.
///
/// Type aliases #"alib::ArrayCompressor;2", #"alib::HuffmanEncoder;2" and
/// #"alib::HuffmanDecoder;2" will always refer to the latest version.
///
  ...
  public:
    ...
    class Array {
      ...
      public:
        ...
        : len(length) {
            firstVal= const_cast<char*>( reinterpret_cast<const char*>(arrayStart) );
            distance= sizeof(TUI);
        ...
        : len(length) {
            firstVal= const_cast<char*>( reinterpret_cast<const char*>(&firstValue) );
    ...
    /// This enum is defined to be #"alib_enums_arithmetic;bitwise".
    /// \note
    ///   With the inclusion of module \alib_enumrecords in the \alibbuild, this
    ///   enum suposes over enum records of type #"ERSerializable".
    ///   If furthermore module \alib_camp is included, such records are
    ///   #"alib_resources_details_data;resourced" in class #"BASECAMP;2".
    enum class Algorithm {
        ...
        /// #"BitWriter::WriteBits(TIntegral);WriteBits<TIntegral>(TIntegral)".
        Uncompressed       =   1,
        ...
        /// End of enumeration marker necessary for use of #"ALIB_ENUMS_MAKE_ITERABLE" with
        /// this enum type.
        END_OF_ENUM        = 64,
    };
...
#if ALIB_BOXING
ALIB_BOXING_VTABLE_DECLARE( alib::bitbuffer::ac_v1::ArrayCompressor::Algorithm, vt_bitb_algo  )
#endif

#include "alib/bitbuffer/ac_v1/acalgos.hpp.inl"
...
for( ArrayCompressor::Algorithm algo :  enumops::EnumIterator<ArrayCompressor::Algorithm>() ) {
    ...
    #if ALIB_DEBUG_ARRAY_COMPRESSION
    {
        ...
        if( multipleAlgorithms ) {
            auto readBackAlgo=  Algorithm( 1 << br.ReadBits<3>() );
...
void ArrayCompressor::Decompress(   BitReader&      br,
    ...
    Ticks tm;
    auto algo=  multipleAlgorithms  ?  Algorithm( 1 << br.ReadBits<3>() )
                                    :  algorithmsToTry;
    ALIB_ALLOW_SPARSE_ENUM_SWITCH
    switch( algo ) {
        case Algorithm::Uncompressed:   readUncompressed(br, data ); break;
