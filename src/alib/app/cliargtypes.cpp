...
bool Option::Read( OptionDecl& decl, String& argProbablyReplaced, const integer argNo ) {
    ...
    if( potentialIllegalContinuation ) {
        ...
        if( !isalnum( nextChar ) )
            throw Exception( ALIB_CALLER_NULLED, CLIExceptions::IllegalOptionNameContinuation,
    ...
    if (  argsExpected > integer(argsLeft) )
        throw Exception( ALIB_CALLER_NULLED, CLIExceptions::MissingOptionValue,
...
bool Parameter::Read( ParameterDecl& decl ) {
              ...
              && identifier.StartsWith<CHK,lang::Case::Ignore>( arg ) )  )
    {
        ...
        if ( argsExpected > integer(CmdLine->ArgsLeft.size()) )
            throw Exception( ALIB_CALLER_NULLED, CLIExceptions::MissingParameterValue,
...
void ERExitCodeDecl ::Parse() {
    enumrecords::bootstrap::EnumRecordParser::Get( ERSerializable::EnumElementName        , true   );
}
