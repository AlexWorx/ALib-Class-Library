...
///
/// For example, implicit casting from class #"%AString" to sibling types #"%CString" and
/// #"%^Substring" has to be forbidden, because both are also constructible implicitly from
/// this class and hence an ambiguity between implicit cast and implicit construction would
/// arise.
///
/// Custom classes (i.e., if derived from class #"str TAString") might encounter
/// similar problems.
...
//==================================================================================================
/// This class is the base class of all #"alib_strings_classes;ALib string classes".
...
/// This means there is no interface to change the single two data members #".buffer" and
/// #".length".
/// The immutable nature of this type is lifted by derived types.
/// While the class #"^Substring" allows changing the start and length of the string represented,
/// class #"^AString" holds a copy of the data and consequently allows modifying the string stored.
///
/// \see
...
class TString {
  ...
  protected:
    ...
    union {
        ...
        /// This is, for example, done extensively by the implementation of class #"^AString".
        ///
        /// \note
...
// On the other hand, it complains if the methods are not doxed. So, we hide them from doxygen.
    ...
    /// type that satisfies the concepts #"IsImplicitArraySource" or
    /// #"IsExplicitArraySource".
    ///
    ...
    ///              Requires satisfying the concept #"IsImplicitArraySource".
    ...
    ///
    /// @tparam T    The type of the given \p{src}.
    ///              Requires satisfying the concept #"IsMutableArraySource".
    ///              Deduced by the compiler.
    /// @param  src  The source of the string data to copy.
    template<typename T>
    ...
    ///
    /// @tparam TAllocator The type of the given \p{allocator}, as prototyped with class
    ///                    #"lang::Allocator".
    ///                    Requires satisfying the concept #"IsAllocator".
    ///                    Deduced by the compiler.
  ...
  // On the other hand, it complains if the methods are not doxed. So, we hide them from doxygen.
    ...
    /// This operator requires the type \p{T} satisfying the concepts
    /// #"IsImplicitArrayCast" and #"IsExplicitArrayCast".
    ///
    ...
    ///              Requires satisfying the concept #"IsImplicitArrayCast".
    ///              Deduced by the compiler.
  ...
  //################################################################################################
    ...
    ///   the length.
    ///   This by default is done during #"alib_mod_bs;library initialization",
    ///   if performed on class #"BASECAMP;2".
    ///
    ...
    /// Note that a \e nulled string is also considered #"IsEmpty;empty".
    ///
    /// \see
    ///   Details on the concept of \e nulled and \e empty strings are documented in chapter
    ///   #"alib_strings_details_nulled_vsempty" of this module's
    ///   #"alib_mod_strings;Programmer's Manual".
    ///
  ...
  //################################################################################################
    ...
    ///
    /// @tparam TCheck  Defaults to #"CHK;2", which is the normal invocation mode.
    ///                 If #"NC" is given, no check for an empty string object is performed.
    ...
    ///
    /// @tparam TCheck  Defaults to #"CHK;2", which is the normal invocation mode.
    ///                 If #"NC" is given, no check for an empty or \e nulled object is
    ///                 performed.
    ///
    ...
    ///
    /// \note
    ///   Unlike method #"TString::CharAt", this operator does <em>not</em> perform do range check on
    ///   parameter \p{idx}.
    ...
    ///
    /// \note
    ///   As a result, this operator is equivalent to a call to the non-checking version of method
    ///   #"TString::CharAt;CharAt<NC>". For safe access to characters in the buffer use
    ///   #"TString::CharAt;CharAt<CHK>" which returns <c>'\0'</c> in the case of that \p{idx} is out of
    ///   bounds.
    ///
    /// \note
    ///   Still, in debug-compilations this operator #"alib_mod_assert;raises an ALib error"
  ...
  //################################################################################################
    ...
    /// #"alib_strings_cc_construction_string;string-like object".
    ///
    ...
    ///
    /// @tparam TCheck       Defaults to #"CHK;2", which is the normal invocation mode.
    ///                      If #"NC" is given, no check for a \e nulled is performed
    ...
    ///
    /// @tparam TCheck       Defaults to #"CHK;2", which is the normal invocation mode.
    ///                      If #"NC" is given, no check for a \e nulled object (this) is
    ...
    ///
    /// @tparam TCheck         Defaults to #"CHK;2", which is the normal invocation mode.
    ///                        If #"NC" is given, no check for a \e nulled comparison
    ...
    ///                        value must not be used with <em>TCheck==#"NC"</em>.
    /// @tparam TSensitivity   Determines if comparison is case-sensitive (the default) or not.
    ...
    /// #"alib_strings_cc_construction_string;string-like object" is found at the given
    /// position.
    ///
    /// \note
    ///   The following rules apply:
    ///   - If \p{pos} is out of range or \p{needle} is \e nulled, \c false is returned.
    ///     (This check only done if \p{TCheck} equals #"CHK;2".)
    ...
    /// @tparam TSensitivity Determines if comparison is case-sensitive (the default) or not.
    /// @param needle        The string to compare with. If it is #"IsEmpty;empty", \c true
    ///                      is returned.
    ...
    /// #"alib_strings_cc_construction_string;string-like object" given with parameter
    /// \p{needle}.
    /// In the special case that \p{needle} is #"IsEmpty;empty", \c true is returned.
    ///
    /// @tparam TSensitivity Determines if comparison is case-sensitive (the default) or not.
    /// @tparam TCheck       Defaults to #"CHK;2", which is the normal invocation mode.
    ...
    /// If \p{needle} is #"IsEmpty;empty", \c true is returned.
    ///
    /// @tparam TSensitivity Determines if comparison is case-sensitive (the default) or not.
  ...
  //################################################################################################
    ...
    /// Like #"TString::IndexOf(TChar, integer, integer)" but in case the character is not found,
    /// this method returns the length of this string instead of \c -1.
    ...
    /// Like #"TString::IndexOf(TChar, integer)" but in case the character is not found, this
    ...
    ///
    /// \note
    ///   In derived class #"%CString", a faster version of this method (using \c std::strpbrk()
    ...
    /// Searches the given #"alib_strings_cc_construction_string;string-like object"
    /// in this string.
    ///
    ...
    ///                        \p{startIdx} must be in the range of
    ///                        <c>[0 ...</c> #".Length" <c> - needle.Length()]</c>.
    ...
    /// @param idx    Index pointing to the first character behind the (first) \p{opener}.
    ///
  ...
  //################################################################################################
    ...
    /// <br>Unlike with #".ParseInt(integer, TNumberFormat<TChar>*)" or
    /// #".ParseDec(integer, TNumberFormat<TChar>*)", no sign, whitespaces, or group characters are
    /// accepted.
    ///
    ...
    /// #"TNumberFormat::Computational;NumberFormat::Computational"
    ...
    /// #"TNumberFormat;NumberFormat". All of its interface methods
    /// have a corresponding implementation within class #"%AString".
    ///
    ...
    /// @return The parsed value. In addition, the output parameter \p{newIdx} is set to
    ...
    /// Overloaded version of #"TString::ParseInt(integer, TNumberFormat<TChar>*, integer*)"
    ...
    ///
    /// Parameter \p{numberFormat} defaults to \c nullptr. This denotes static singleton
    /// #"str TNumberFormat::Computational"
    /// which is configured to not using - and therefore also not parsing - grouping characters.
    ///
    /// Optional output parameter \p{newIdx} may be used to detect if parsing was successful.
    /// If not, it receives the value of \p{startIdx}, even if leading whitespaces had been
    ...
    ///
    /// For more information on number conversion, see class
    /// #"TNumberFormat;NumberFormat". All number-parsing interface methods
    ...
    /// #"TString::ParseDec(integer,TNumberFormat<TChar>*,integer*)const;ParseDec"
    /// providing default values for omitted parameters.
    ///
    /// @param numberFormat The format definition. Defaults to \c nullptr.
    /// @param[out] newIdx  Optional output variable that will point to the first
    ...
    /// Overloaded version of
    /// #"TString::ParseBin(integer,TNumberFormat<TChar>*,integer*)const;ParseBin"
    /// providing default values for omitted parameters.
    ///
    /// @param numberFormat The format definition. Defaults to \c nullptr.
    /// @param[out] newIdx  Optional output variable that will point to the first
    ...
    /// Overloaded version of
    /// #"TString::ParseHex(integer,TNumberFormat<TChar>*,integer*)const;ParseHex"
    /// providing default values for omitted parameters.
    ///
    /// @param numberFormat The format definition. Defaults to \c nullptr.
    /// @param[out] newIdx  Optional output variable that will point to the first
    ...
    /// Overloaded version of
    /// #"TString::ParseOct(integer,TNumberFormat<TChar>*,integer*)const;ParseOct"
    /// providing default values for omitted parameters.
    ///
    /// @param numberFormat The format definition. Defaults to \c nullptr.
    /// @param[out] newIdx  Optional output variable that will point to the first
    ...
    /// Overloaded version of
    /// #"TString::ParseFloat(integer,TNumberFormat<TChar>*,integer*)const;ParseFloat"
    /// providing default values for omitted parameters.
    ///
    /// @param numberFormat The format definition. Defaults to \c nullptr.
    /// @param[out] newIdx  Optional output variable that will point to the first
  ...
  //################################################################################################
    ...
    ///
    /// @tparam TAllocator The type of the given \p{allocator}, as prototyped with class
    ///                    #"lang::Allocator". Deduced by the compiler.
    /// @param  allocator  The allocator to use.
    /// @param  copy       The string to copy to the new memory allocated.
    template<typename TAllocator>
    requires alib::lang::IsAllocator<TAllocator>
    ...
    ///   in the case an empty string was given, it is a valid use to allocate and later deallocate
    ///   empty strings.
    /// @tparam TAllocator The type of the given \p{allocator}, as prototyped with the class
    ///                    #"lang::Allocator". Deduced by the compiler.
    /// @param  allocator  The allocator to use.
    template<typename TAllocator>
    requires alib::lang::IsAllocator<TAllocator>
  ...
  //################################################################################################
    ...
    /// Implementation of \c std::iterator_traits for class #"%TString" and its descendents.
    /// Base class #"%^String" exposes #const_iterator which uses
    /// <c>const TChar*</c> and <c>const TChar&</c> for template types \p{TPointer} and
    /// \p{TReference}. Descendant classes may expose a mutable version (e.g., #"^AString").
    ///
  ...
  //################################################################################################
    ...
    /// found only in descendant classes (e.g., #"^AString").
  ...
  protected:
    #if !DOXYGEN
        ALIB_ALLOW_DOCS // needed due to a bug in current clang
    #endif
...
#if !DOXYGEN
template<typename TChar>
struct ArrayTraits<strings::TString<TChar>, TChar> {
...
///
/// The operators use the methods #"TString::Equals" and #"TString::CompareTo(const TString<TChar>)"
/// for evaluation.
///
/// For performance-critical sections, the original methods may be used where template
/// parameter \p{TCheck} can be set to #"NC".
///
...
///       #"%CString", #"%AString" and #"%^Substring" likewise.
///
...
#if DOXYGEN
//==================================================================================================
/// This template class has three specializations for types #"characters::nchar",
/// #"characters::wchar", and #"characters::xchar", which each provides the static method
/// #EmptyString.
...
/// To improve readability, \c constexpr namespace variable are given with #"NULL_STRING",
/// #"NULL_COMPLEMENT_STRING", #"NULL_STRANGE_STRING", #"NULL_NSTRING",
/// #"NULL_WSTRING", and #"NULL_XSTRING".
///
/// @see
///   Manual chapter #"alib_strings_details_constants".
///
/// @tparam TChar The #"alib_characters_chars;character type".
//==================================================================================================
