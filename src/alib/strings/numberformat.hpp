...
/// Flags used with class #"str TNumberFormat".
/// By default, (with construction of an instance of #"%TNumberFormat"), all flags are unset,
...
enum class NumberFormatFlags : uint8_t {
    ...
    /// written using #"detail::WriteFloat".<br>
    /// Defaults to \c false, as some systems will not accept a plus sign on the exponent value.
    /// Note that field #"str TNumberFormat::PlusSign" is not applicable for exponent numbers.
    WriteExponentPlusSign                   = 8,

    /// If this flag ist set, then trailing \c '0' digits in the fractional part of a floating
    /// point value are not written, even if a #"str TNumberFormat::FractionalPartWidth"
    /// is set.
    ...
    /// in the case that #"TNumberFormat::DecMinimumFieldWidth", respectively
    /// #"str TNumberFormat::IntegralPartMinimumWidth", specifies the output to be wider
    /// than the number printed.
    ReplaceLeadingZerosWithSpaces           =32,

    /// If \c true, scientific format is always used.<br>
    /// If \c false (the default), function #"detail::WriteFloat" writes scientific
    /// format only if both fields, #"str TNumberFormat::IntegralPartMinimumWidth" and
    /// #"str TNumberFormat::FractionalPartWidth" are evaluating to \c -1 and only for
    /// numbers smaller than \c 10E-04 or larger than \c 10E+06.<br>
    ///
    /// If one of the fields #"str TNumberFormat::IntegralPartMinimumWidth" or
    /// #"str TNumberFormat::FractionalPartWidth" is set to a positive
    /// value, these limits get extended. Function #"detail::WriteFloat" in this case
...
///
/// In the namespace #"alib::strings::detail;3", corresponding functions that use an instance of
/// this type are implemented. However, those functions are not intended for common use.
/// Instead, the interface of classes
///    #"^String",
///    #"^Substring",
///    #"^AString" or
///    #"format::Formatter"
/// are preferred to write and parse numbers.
/// Also, those accept an object of this type as parameters.
///
/// <b>Defined Singletons and User-Defined Instances:</b><br>
/// Two static singletons of this class, both initialized with function #"Bootstrap",
...
///   provided with fields #".BinFieldWidth", #".HexFieldWidth", and #".OctFieldWidth", denote an
///   \b absolute value.
///   Higher digits of numbers are not written! The advantage of this design is that no
...
/// namespace #"alib::strings::detail;3" which are using this class.
...
/// fields #".BinLiteralPrefix", #".HexLiteralPrefix", and #".OctLiteralPrefix".
/// However, the prefixes \b are identified by function #"detail::ParseInt", which aggregates the
/// other four parsing functions.<br>
...
/// user's code.
///
/// @tparam TChar The #"alib_characters_chars;character type" that this class works on.
...
struct TNumberFormat {
    ...
    /// running process.<br>
    /// Function #"Bootstrap" invokes #".SetFromLocale" on this object and switches grouping
    /// to \e 'on'.
    ///
  ...
  //######################################### string members #######################################
    ...
    /// #"detail::ParseInt" and
    /// #"detail::ParseFloat". In contrast, functions
    /// #"detail::ParseDec",
    /// #"detail::ParseBin",
    /// #"detail::ParseHex", and
    /// #"detail::ParseOct" do not ignore any whitespace characters.
    TCString<TChar>     Whitespaces;

    /// Defines the decimal exponent symbol of string representations of floating point numbers
    /// when written or parsed in scientific notation by functions #"detail::ParseFloat"
    /// and #"detail::WriteFloat".<br>
    /// Function #"detail::ParseFloat" accepts characters 'e' and 'E' in addition to
    ...
    /// Used by function #"detail::ParseInt" to detect binary format of integral values.
    ...
    /// Used by function #"detail::ParseInt" to detect hexadecimal format of integer
    ...
    /// Used by function #"detail::ParseInt" to detect octal format of integral values.
  ...
  //####################################### character members ######################################
    ...
    /// representation with function #"detail::WriteFloat". Also; function #"detail::ParseFloat" uses
    ...
    /// written using #"detail::WriteFloat" or #"detail::WriteDecSigned".<br>
  ...
  //######################################### width members ########################################
    ...
    /// \c 10E+06, get extended. Function #"detail::WriteFloat" in this case keeps
    ...
    /// the limits to switch to scientific notation, which otherwise are fixed \c 10E-04 and
    /// \c 10E+06, get extended. Function #"detail::WriteFloat" in this case keeps non-scientific notation
  ...
  //################################################################################################
    ...
    ///
    /// @param other  The #"%^NumberFormat" object to copy the values from.
    ...
    /// #"TNumberFormat Whitespaces"     |  #"DEFAULT_WHITESPACES"
    ///
    ///
    /// \note
    ///   With static object
    ///   #"alib::strings::TNumberFormat::Computational;2",
    ...
    ///
    /// \note
    ///   Static (global) object #"alib::strings::TNumberFormat::Global;2",
    ///   implements an instance which has the right locale set (provided that function
    ///   #"Bootstrap" was duly invoked by the process).
