...
/// to string classes found in namespace #"alib::strings".
namespace util  {

//==================================================================================================
/// This class operates on strings which contains data separated by a delimiter character.
/// It identifies the substrings between the delimiters as \e tokens of type #"^Substring".
/// After an instance of this class is constructed, three methods are available:
/// - #".HasNext:" Indicates if there are further tokens available.
/// - #".Next:" Sets field #".Actual" (which is of type #"%^Substring") to reference the next token
///   and returns it.<br>
...
///
/// After a token was retrieved, it might be modified using the interface of class #"^Substring"
/// as the tokenizer does not rely on the bounds of the current token when receiving the next.
///
...
/// #"%^Substring" if it seems appropriate. The effect is the same as if the method #".Set" was
...
///                  with type definitions #"alib::TokenizerN" and
///                  #"alib::TokenizerW".
//==================================================================================================
...
class TTokenizer {
  ...
  public:
    ...
    /// Returns the next token, which is afterward also available through the field #".Actual".
    /// If no further token was available, the returned #"^Substring" will be \e nulled.
    /// (see #"TString::IsNull;String::IsNull").
    /// To prevent this, the availability of the next token should be checked with the method
    /// #".HasNext".
    ///
    ...
    ///                   Defaults to #"Whitespaces::Trim;2".
    ...
    /// @return The next token as #"%^Substring". A nulled string is if no next token was
    ///         available.
    ALIB_DLL
