...
///
/// The availability of the class is dependent on the configuration macro #"ALIB_FEAT_BOOST_REGEX"
/// which has to be \c true, and in parallel either #"ALIB_CHARACTERS_WIDE" is \c false or
/// #"ALIB_CHARACTERS_NATIVE_WCHAR" equals \c true.
///
/// Method #".Compile" accepts the pattern string and compiles it to \c boost::regex.
...
///   available in the original \c boost::regex library.
...
class RegexMatcher {
  ...
  public:
    ...
    ///                Defaults to #"%NULL_STRING" to allow parameterless construction, with
    ...
    /// @return The (first) range in \p{haystack} that matches the compiled pattern.
