//==================================================================================================
/// \file
/// This header-file is part of sub-namespace #"alib::strings::util;3" of module \alib_strings of
/// the \aliblong.
///
...
/// (year, month, day, hour, ...).
/// It provides methods to convert to and from objects of type #"time::DateTime".
/// In addition, a method to format the date and time into human-readable string value is available.
///
/// \note
///   The conversion from and into objects of type #"Ticks" is intentionally not supported.
///   In the case that such objects should be used with this class, an additional conversion
///   step has to be performed using class #"TickConverter". In other words, conversion can
...
class CalendarDateTime {
  public:
        ...
        ///  \snippet "strings/util/calendar.cpp"        CDT_MONTH_NAMES
        /// In case the module is included, the same values are resourced in #"BASECAMP;2"
        /// under key <c>"Months"</c>. In both cases a user might change this during bootstrap.
        static ALIB_DLL String MONTH_NAMES[12];

        /// A list of day names used with the method #"CalendarDateTime::Format".
        ///
        /// If module \alib_camp is not included in the \alibbuild, the values default to:
        ///  \snippet "strings/util/calendar.cpp"        CDT_DAY_NAMES
        /// In case the module is included, the same values are resourced in #"BASECAMP;2"
        /// under key <c>"Days"</c>. In both cases a user might change this during bootstrap.
    ...
    /// \attention This value is only set when constructed with a #"%time::DateTime" object and
    ///            set to \c -1 if constructed with single values, or if method #".Clear" is
    ///            invoked.
    int                     DayOfWeek;
    /// Constructs an unset object.
    /// @param init  If #"%Initialization::Default" or \e nulled, #".Clear" is invoked.
    ...
    /// Creates a #"%time::DateTime" object from this calendar date.
    /// \attention
    /// The resolution and possible time range of class #"%time::DateTime" is platform-dependent.
    ...
    ///   yyy...y |The year with a minimum number of digits as the number of y-characters given.
...
/// #"TimePointBase::Duration;DateTime::Duration" and
/// #"TimePointBase::Duration;Ticks::Duration" is supported.
//==================================================================================================
class CalendarDuration {
  public:
    ...
    /// @param init If #"%Initialization::Default" or #"%Nulled", #".Clear" is invoked.
    ///             Otherwise fields are not initialized.
    ///             Defaults to #"%Initialization::Default".
    ...
    /// Invokes #".FromDuration(DateTime::Duration)".
    ...
    /// Constructs the object using the given duration object.
    /// Invokes #".FromDuration(Ticks::Duration)".
    /// @param duration The duration to use for setting the public fields.
...
/// This class internally uses types #"time::DateTime" and #"util::CalendarDateTime"
...
///
/// Another small difference is that field #"CalendarDate;DayOfWeek" is always
...
/// #"util::CalendarDateTime", where a value of \c 0 represents year \b 1900.
...
class CalendarDate {
  ...
  public:
    ...
    /// Constructor taking a #"%time::DateTime" value.
    ...
    /// Sets this class to the date represented by the given #"%time::DateTime" instance.
    /// @param dateTime The value to take the date from.
    ...
    /// @return A corresponding #"%time::DateTime" value.
    ALIB_DLL
