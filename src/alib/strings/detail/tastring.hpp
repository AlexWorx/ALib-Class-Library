...
/// #"AppendableTraits::operator()(TAString<TChar>&,const TAppendable&);operator()",
/// which is invoked by the method #"TAString::Append(const TAppendable&)", when an instance of
/// the type in question is passed.
///
/// For user-defined string-types that get adopted to \alib string system using a specialization of
/// struct #"ArrayTraits", no specialization of this functor is needed, because
/// the method #"Append(const TAppendable&);2" will accept such types likewise.
///
/// \note
...
///
/// \see
///   For details consult chapter #"alib_strings_assembly_ttostring" of the Programmer's Manual
///   of module \alib_strings.
///
/// \see
///   The documentation of built-in specializations for \alib types are collected in
///   sub-namespace #"alib::strings::APPENDABLES;3".
///
...
/// @tparam  TAllocator  The allocator type of the target #"%AString".
//==================================================================================================
...
struct AppendableTraits {
    ...
    /// #"TAString::Append(const TAppendable&)", when an object of type
...
/// #"AppendableTraits" exists, which has a valid call-<c>operator()</c> that accepts
...
//==================================================================================================
/// Specializes base class #"^String" to implement mutable character strings
/// using writable and extendable buffer memory.
///
/// <b>Construction:</b><br>
/// Construction is described in Programmer's Manual, section
/// #"alib_strings_cc_construction_astring".
///
...
///   This class does not manage external buffers. However, if their capacity is exceeded, they
///   will automatically become replaced by an internal buffer.
///   Such a replacement, by default, #"alib_mod_assert;raises a warning" with debug-builds.
...
///
/// The method #".SetBuffer(TChar*)" provides a boolean parameter
/// that also allows letting an object of this class take control of a buffer provided from outside.
/// In this case, the buffer is considered an internal buffer, hence allocated using the same
/// allocator traits (and if not heap allocation, then also the same allocator instance!), rather
/// than an external one.
///
/// The default constructor creates a \e nulled #"%AString" which does not dispose of an allocated
...
/// For example, the move constructor grabs the buffer of a given movable #"%AString", as long
...
/// string content. This helps to distinguish #"%AString" variables from those of the lightweight
...
/// Parent class #"^String" holds its protected field #"TString::buffer;buffer" in an anonymous C++
/// union of two pointers, one typed <em>const char*</em> and the other <em>char*</em>.
...
/// used to notify such change with the #"%AString" object. The latter of course is invokable only
...
/// This class is used to provide a global concept of having a sort of <c>"ToString"</c> method
/// with any C++ class. For this, the class provides method #".Append(const TAppendable&)", which
/// uses C++20 concepts meta programming to accept types with a corresponding specialization of
/// functor #"AppendableTraits".
///
/// The concept is described in detail with chapter #"alib_strings_assembly_ttostring"
/// of this module's #"alib_mod_strings;Programmer's Manual".
///
///
/// @tparam TChar      The character type.
///                    Alias names for specializations of this class using character types
///                    #"characters::character", #"characters::nchar", #"characters::wchar",
///                    #"characters::xchar", #"characters::complementChar" and
///                    #"characters::strangeChar" are provided in namespace #"alib" with type
///                    definitions #"AString", #"NAString" #"WAString", #"XAString",
///                    #"ComplementAString" and #"StrangeAString".
               ...
               , public lang::AllocatorMember<TAllocator>
{
  ...
  protected:
    ...
    /// This field is available only if code selection macro #"ALIB_DEBUG_STRINGS"
    /// is set.
  ...
  //################################################################################################
    ...
    /// every other method of this class, but only if the configuration macro
    /// #"ALIB_DEBUG_STRINGS" is \c true.<br>
    /// Invocations to this method should be performed using macro
    /// #"ALIB_STRING_DBG_CHK".<br>
    ///
    /// \see
    ///   For (a little) more information see
    ///   #"alib_strings_details_debugging" of this module's
    ///   #"alib_mod_strings;Programmer's Manual"
    void        dbgCheck()                                                                    const;

  #endif

  protected:
    /// If \c true, a #"alib_mod_assert;warning is raised" when an external buffer, whose
  ...
  //################################################################################################
    /// Constructs an #"%AString" with the given external buffer and allocator.
    ...
    /// Constructs an #"%AString" with the given external buffer.
    /// The given buffer's life-cycle is considered to be managed externally.<br>
    /// This constructor is protected and provided for derived classes that dispose of
    /// their own buffer.
    ///
  ...
  //################################################################################################
    /// Constructs an empty, \e nulled #"%AString" (does not allocate a buffer).
    ...
    /// allocation strategy that includes the need of storing an allocator type in optionally (!)
    /// inherited field #"AllocatorMember::allocator;2".
    /// For the standard, heap-allocated type #"AString", this constructor is not applicable.
    /// @param pAllocator The allocator to use.
    explicit constexpr
    ...
    /// Move constructor.
    /// See #"alib_ns_strings_astring_copymove;Copy/Move Constructor and Assignment"
    /// for details.
    ...
    /// Constructs the object and uses #".Append(const TAppendable&)" to create a string
  ...
  // On the other hand, it complains if the methods are not doxed. So, we hide them from doxygen.
    ...
    /// #"ArrayTraits", which is used for both;
    ...
    /// If the given other #"%AString" is \e nulled, this object becomes \e nulled. Otherwise,
    ...
    ///   #"alib_ns_strings_astring_copymove;Copy/Move Constructor and Assignment"
  ...
  //################################################################################################
    ...
    /// to an #"%AString" is predictable, then it is advisable to reserve the allocated size
    ...
    ///
    /// If an #"SetBuffer(TChar*);external buffer" is set, in debug-compilations a
    /// #"alib_mod_assert;warning is raised", because usually it
    /// is not wanted that an external buffer becomes replaced during the growth of a string.<br>
    /// Such warnings can be switched off using the method #".DbgDisableBufferReplacementWarning".
    /// For example, in some situations it might be taken into account that instances of derived
    /// type #"TLocalString;LocalString" sometimes may grow more than average and
    /// in such a case, a heap-allocated buffer replaces a local one. By placing a call
    ...
    /// and method #"%DbgDisableBufferReplacementWarning" is optimized out.
    ///
    /// @param newCapacity   The new capacity of the internal buffer.
    ALIB_DLL  void    SetBuffer( integer newCapacity );

    /// This method replaces the current buffer with the one provided.
    ///
    /// The following rules apply:
    ...
    ///
    /// The internal buffer allocation is performed with methods #"Allocator::allocate;*",
    /// #"Allocator::reallocate;*" and #"Allocator::free;*". The latter two are
    /// also used on external buffers that are provided with this method in case parameter
    /// \p{responsibility} is given as #"%Responsibility::Transfer", because in this case such
    /// an externally created buffer is considered to have been allocated using the same
    ...
    /// \note Derived class
    ///       #"TLocalString;LocalString" will report \c false here.
  ...
  //################################################################################################
    ...
    /// \see Chapter
    /// #"alib_ns_strings_astring_write_access;Write Access to the Buffer"
    ...
    /// Provides read/write access to single characters. Overrides #"TString::operator[]"
    ...
    ///   \alib). See #"TString::operator[];String::operator[]" for details.
    ///
    ...
    /// length, for better readability, the use of the method #".ShortenTo" instead of this method is
    ...
    /// #VBuffer.
    ///
    /// @param newLength  The new length of the #"%AString". Must be between 0 and
    ///                   #Capacity.
    ...
    /// #SetLength, it just sets the inherited field #"^.length" to the given value.
    ...
    ///
    /// In debug-compilations, an #"alib_mod_assert;error is raised" if the length provided
    ...
    ///
    /// @param newLength  The new length of this #"%AString". Must be between 0 and the current
    ///                   length.
    /// @return    \c *this to allow concatenated calls.
  ...
  //################################################################################################
    ...
    /// @tparam TCheck      Defaults to #"CHK;2", which is the normal invocation mode.
    ...
    TAString& Append( const TCharSrc* src, integer srcLength ) {
        ...
        // TAString<REAL_XCHAR>
            ...
            static_assert( std::same_as<TChar, REAL_XCHAR>, "Must not happen" );
            // wchar_t -> REAL_XCHAR
    ...
    /// Appends a region of another #"^String".
    ...
    /// this #"%AString".
    /// Type \p{T} needs to satisfy the concept #"str IsAppendable", which is achieved by
    /// specializing the type trait \p{AppendableTraits} accordingly.
    ///
    /// \see
    ///   See chapter #"alib_strings_assembly_ttostring" of this module's
    ///   #"alib_mod_strings;Programmer's Manual" for more information.
    ///
    /// @tparam TCheck      Defaults to #"CHK;2", which is the normal invocation mode.
    ///                     If after the append operation this string is still \e nulled,
    ///                     an initial buffer is allocated and the state of this instance
    ///                     changes to \e empty.
    ///                     If #"NC" is given, a \e nulled string may remain \e nulled.
    ...
    /// #"IsImplicitArraySource" #"IsImplicitZTArraySource".
    ///
    /// @tparam TCheck        Defaults to #"CHK;2", which is the normal invocation mode.
    ///                       If #"NC" is given, a \e nulled string may remain \e nulled,
    ...
    ///                If #"NC" is given, \p{src} is \b not checked for being \c 0.
    ...
    ///                the concept #"ch IsCharacter" are accepted.
    /// @return \c *this to allow concatenated calls.
    template< typename TCheck= CHK>
    ...
    /// @tparam TCheck      Defaults to #"CHK;2", which is the normal invocation mode.
    ///                     If #"NC" is given, checks are omitted.
    ///                     (The specific behavior depends on the overloaded
    ///                     #"%Append(const TAppendable&)" method that is called.)
  ...
  //################################################################################################
    ...
    ///
    /// @tparam TCheck      Defaults to #"CHK;2", which is the normal invocation mode.
    ///                     The value is used with the invocation of method
    ///                     #"%Append(const TAppendable&)".
    /// @tparam TAppendable The type of parameter \p{source}.
    ...
    /// If the position is not between \c 0 and the length of the target, nothing is inserted.
    /// The latter is only asserted in case the template parameter \p{TCheck} is not set to
    /// #"NC".
    /// \note
    ///   To insert a string with replacing a different one at the same time, use the method
    ///   #ReplaceSubstring".
    ///
    /// @tparam TCheck   Chooses checking or non-checking implementation. Defaults to #"CHK;2".
    ...
    TAString&   InsertAt( const TString<TChar>& src, integer pos ) {
        ...
        if constexpr ( TCheck::value ) {
            ...
            ALIB_ASSERT_ERROR( srcLength > 0 && pos >=0 && pos <= base::length, "STRINGS",
               "Non-checking invocation: ","Illegal insertion position 0 <= {} < {}.",
               pos, base::length )
        }
    ...
    /// Inserts the given number of \b uninitialized characters at the given position.
    /// If the position is not between \c 0 and the length of the target, nothing is inserted.
    /// The latter is only asserted in case the template parameter \p{TCheck} is not set to
    /// #"NC".
    ///
    /// \note
    ///   The usage of other <b>Insert</b>-methods is preferred. This method is efficient in cases
    ///   when the replacement consists of different parts and external assembly prior to the
    ...
    TAString&   InsertUninitialized( integer pos, integer size ) {
        ALIB_STRING_DBG_CHK(this)
        if constexpr ( TCheck::value ) {
            if ( size == 0 || pos < 0 || pos > base::length )
                return *this;
        } else {
            ALIB_ASSERT_ERROR( size > 0 && pos >=0 && pos <= base::length, "STRINGS",
               "Non-checking invocation: ","Illegal insertion position 0 <= {} < {}. or size {}",
               pos, base::length, size )
        }
    ...
    ///
    /// @tparam TCheck Defaults to #"CHK;2", which chooses the checking version of the method.
    ...
    /// The non-checking version (\p{TCheck} = #"NC") omits the check, but still allows
    /// leaving parameter \p{regionLength} as default (respectively allows the sum of parameters
    /// \p{regionStart} and \p{regionLength} to be longer than the length of this #"%AString").
    ...
    ///                    Defaults to #"DEFAULT_WHITESPACES".
    /// @return \c *this to allow concatenated calls.
    ALIB_DLL
  ...
  //################################################################################################
    ...
    /// Replaces a region in this object with a given string.
    /// The given region is adjusted to this string's bounds.
    ///
    /// The non-checking version does not adjust the region and
    /// #"alib_mod_assert;raises an ALib error" in debug-compilations if the given region is
    /// out of bounds.
    ///
    /// @tparam TCheck         Chooses checking or non-checking implementation.
    ///                        Defaults to #"CHK;2".
  ...
  public:
    ...
    /// While parent class #"%^String" provides a constant iterator only, this class exposes
    ...
    /// Appends the given character to this #"%AString".<br>
...
struct NoAutoCastTraits< TAString<TChar,TAlloc>,
                         characters::Policy::ExplicitOnly,
                         std::basic_string<TChar>          > : std::true_type {};
}
...
/// This is a simple utility class that can be used in situations where a #"^AString"
...
///
/// \see Macro #"ALIB_STRING_RESETTER" which automatically creates a unique identifier for
///      the otherwise unused instances of this type.
///
/// @tparam TChar      The character type of the #"%AString" that is to be reset.<br>
/// @tparam TAllocator The allocator type of the #"%AString", as prototyped with the class
///                    #"lang::Allocator".
template<typename TChar, typename TAllocator>
class TStringLengthResetter {
  protected:
    TAString<TChar,TAllocator>&  aString;        ///< The #"%AString" to reset.
    integer                      originalLength; ///< The original length of the string.
  ...
  public:
    ...
    /// @return The length of the #"%AString" when this object was constructed.
