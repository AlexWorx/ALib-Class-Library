...
#   error "ALib sources with ending '.inc' must not be included from outside."
#endif
...
/// #"AppendableTraits" which in reality are implemented in parent
/// namespace #"alib::strings;2" (as required by C++ language syntax).<br>
/// The rationale for tricking the documentation to this pseude-namespace, is to twofold:
/// On the one hand to keep namespace #"%alib"::strings clean and on the other to have
    ...
    if ( newCapacity == 0 ) {
        ...
        if( dbgWarnWhenExternalBufferIsReplaced && capacity < 0)
            ALIB_WARNING( "STRINGS",
              "Replacing an external buffer of size {} by an internally managed one of size {}."
              " This may not be wanted: ", -capacity +1, newCapacity +1,
              *this  )
    #endif
...
#if !ALIB_BOXING
template<typename TChar, typename TAllocator>
void AppendableTraits<TField<TChar>, TChar,TAllocator>::operator()(
                                    TAString<TChar,TAllocator>& target, const TField<TChar>& field)
{
