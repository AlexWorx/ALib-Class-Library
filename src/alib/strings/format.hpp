...
/// This is a type purely made to be #"alib_strings_assembly_ttostring;appended" to objects of
/// type #"^AString".
/// Various constructors accept integer and floating point values, along with formatting options.
/// The specialization of functor #"AppendableTraits" will use a given (or defaulted)
/// instance of class #"TNumberFormat;NumberFormat" to format the encapsulated value and
/// append the result to the #"%AString" in question.
///
/// \note
///   Within the same header-file that this class in declared in, there are several
///   specializations of functor #"AppendableTraits" defined for plain integer and
...
///   along with a locale enabled instance of #"%^NumberFormat". For example:
...
class TDec {
  public:
    ...
    int             width;          ///< The minimum width of the number to write.
                                    ...
                                    ///< #"TNumberFormat::DecMinimumFieldWidth;NumberFormat::DecMinimumFieldWidth".
...
/// Implements a temporary object which is #"alib_strings_assembly_ttostring;appended"
/// to instances of type #"^AString".
///
...
struct TEscape {
  public:
    /// The direction of conversion: #"%Switch::On" escapes ascii characters, while
    /// #"%Switch::Off" converts escaped strings to ascii codes.
    lang::Switch pSwitch;
    ...
    /// Constructor. Copies the parameters.
    ///
    /// @param escape        #"%Switch::On" escapes ascii characters (the default),
    ///                      #"%Switch::Off" converts  escaped strings to ascii codes.
...
/// Implements a temporary object which can be #"alib_strings_assembly_ttostring;appended"
/// to instances of type #"^AString".
///
/// Appends an integral value in binary format.
///
/// \see
///   Class #"str TNumberFormat" for more information on formatting options for binary
///   number output.
/// @tparam TChar The #"alib_characters_chars;character type" of the #"%AString" that
///               instances can be "applied" to.
template<typename TChar>
struct TBin {
  public:
    ...
    int             theWidth;   ///< The minimum width of the number to write.
                                ...
                                ///< #"TNumberFormat::BinFieldWidth;NumberFormat::BinFieldWidth".
    TNumberFormat<TChar>* nf;   ///< The number format to use. Defaults to \c nullptr which chooses
                                ///< #"TNumberFormat::Computational;NumberFormat::Computational".
...
///
/// \see
///   Class #"str TNumberFormat" for more information on formatting options for
...
struct THex {
    ...
    int         theWidth;     ///< The minimum width of the number to write.
                              ...
                              ///< #"str TNumberFormat::HexFieldWidth".
    TNumberFormat<TChar>* nf; ///< The number format to use. Defaults to \c nullptr which chooses
                              ///< #"str TNumberFormat::Computational".
    ...
    /// @param overrideWidth Defaults to \c 0 which
    ///                      denotes to choose the value of field
    ///                      #"TNumberFormat::HexFieldWidth;NumberFormat::HexFieldWidth".
    /// @param numberFormat  The number format to use. Defaults to \c nullptr which chooses
    ///                      the static singleton found in
    ///                      #"TNumberFormat::Computational;NumberFormat::Computational".
    template<typename TIntegral>
...
///
/// \see
///   Class #"str TNumberFormat" for more information on formatting options for octal
///   number output.
/// @tparam TChar The #"alib_characters_chars;character type" of the #"%AString" that
///               instances can be "applied" to.
template<typename TChar>
struct TOct {
    ...
    int             theWidth; ///< The minimum width of the number to write.
                              ///< Defaults to \c 0
                              ///< which denotes to choose the value of field
                              ///< #"TNumberFormat::OctFieldWidth;NumberFormat::OctFieldWidth".
    TNumberFormat<TChar>* nf; ///< The number format to use. Defaults to \c nullptr which chooses
                              ///< #"TNumberFormat::Computational;NumberFormat::Computational".
...
/// Implements a temporary object which is #"alib_strings_assembly_ttostring;appended"
/// to instances of type #"^AString".<br>
...
/// Specialization of functor #"AppendableTraits" for type \c Format.
template<typename TChar, typename TAllocator>
struct AppendableTraits<TDec<TChar> ,TChar,TAllocator>
{
    /// Appends a string representation of the value encapsulated in the given \p{src} value.
...
/// Specialization of functor #"AppendableTraits" for type \c TTab.
template<typename TChar, typename TAllocator>
...
/// Specialization of functor #"AppendableTraits" for type \c Escape.
template<typename TChar, typename TAllocator>
...
/// Specialization of functor #"AppendableTraits" for type \c Bin.
template<typename TChar, typename TAllocator>
...
/// Specialization of functor #"AppendableTraits" for type \c Hex.
template<typename TChar, typename TAllocator>
...
/// Specialization of functor #"AppendableTraits" for type \c Oct.
template<typename TChar, typename TAllocator>
...
/// Specialization of functor #"AppendableTraits" for type \c Fill.
template<typename TChar, typename TAllocator>
