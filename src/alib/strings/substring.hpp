...
//==================================================================================================
/// This class specializes parent class #"^String" to allow reduction of the length of the
/// represented string by cutting characters from the front or the end.
/// Such reduction does not affect the character array represented, but only its representation
/// by instances of this type.
///
/// In other words, the difference to base class #"^String" is, that this type allows increasing
/// the pointer to the character array's start and to decrease its stored length.
...
///
/// Like base class #"^String", the class cannot, and therefore does not, verify that the underlying
/// buffer is (still) properly allocated and contains valid data.
...
///
/// @tparam TChar The #"alib_characters_chars;character type" of this string-type.
...
class TSubstring : public TString<TChar> {
  ...
  public:
    ...
    /// Default constructor creating a #"alib_strings_details_nulled" \e "nulled" substring.
    TSubstring()
    : base()                                                                                      {}
    ...
    ///                     Defaults to  #"alib::DEFAULT_WHITESPACES"
    /// @return \c *this to allow concatenated calls.
    ...
    ///                            consume operation. Defaults to #"%Whitespaces::Keep".
    ...
    ///         If this #"%^Substring" is empty or \e nulled, '\0' is returned.
    ...
    /// @param target        An optional target #"%^Substring" that receives the portion that
    ...
    ///                       is performed.
    /// @tparam TTargetData   If \c CurrentData::Keep, #"%AString" \p{target} is not cleared
    ...
    ///                       class #"lang::Allocator". Deduced by the compiler.
    /// @param regionLength   The length of the region at the start to delete.
    /// @param target         A target #"%AString" that receives the portion that
    ...
    ///
    /// @param regionLength    The length of the region at the start to delete.
    /// @param target          A target #"%^String" that receives the portion that
    ///                        is cut from this object.
    /// @param separatorWidth  This width is added to what is cut from this string, while
    ///                        \p{target} still receives the portion defined by \p{regionLength}.
    ///                        Defaults to 0.
    ...
    /// #"alib_enums_records_details_serialization;deserialization of enumeration elements".
    ///
    /// @tparam TSensitivity       The sensitivity of the comparison.
    ///                            Defaults to #"%Case::Ignore".
    /// @tparam TTrimBeforeConsume Determines if the string should be (left-) trimmed before the
    ///                            first character consume operation.
    ///                            Defaults to #"%Whitespaces::Keep".
    /// @param  consumable         The consumable string.
    ...
    /// @return The string consumed. #"%NULL_STRING" on error (start/end character not found)
...
#if !DOXYGEN
template<typename TChar>
struct ArrayTraits<strings::TSubstring<TChar>, TChar> {
