...
//==================================================================================================
/// This class specializes its base class #"^String" in that respect that
...
class TCString : public TString<TChar> {
  ...
  public:
    ...
    ///   In debug-compilations a runtime assertion is raised if the provided buffer is not
    ///   zero-terminated.
...
// On the other hand, it complains if the methods are not doxed. So, we hide them from doxygen.
    ...
    /// type that satisfies the concepts
    /// #"IsImplicitZTArraySource",
    /// #"IsExplicitZTArraySource",
    /// #"IsExplicitZTArraySource",
    /// #"IsMutableZTArraySource", and
    /// #"IsMutableZTArraySource".
    ///
    /// (Internally, for each concept an own constructor implementation is provided.)
    /// Custom types can be enabled for this constructor by specializing the corresponding
    /// traits-types.
    ///
    ...
    /// copies the given string's contents and lets this #"%CString" represent this new
...
// On the other hand, it complains if the methods are not doxed. So, we hide them from doxygen.
    ...
    /// this string instance, selected when type \p{T} satisfys one of he the concepts
    /// - #"IsImplicitZTArrayCast" or #"IsExplicitZTArrayCast"
    ///
    ...
    ///              Requires satisfying the concept #"IsImplicitZTArrayCast".
    ///              Deduced by the compiler.
    ...
    /// #"TString::operator[];String::operator[]" to change the debug assertion
    ...
    /// #"TString::LastIndexOfAny;String::LastIndexOfAny".
    ///
    /// \note
    ///   This method overrides method #"TString::IndexOfAny;String::IndexOf".
    ///   This implementation however expects a #"%CString" with parameter \p{needles}
    ///   (beside the fact that it has to be invoked on a #"%CString" itself).
    ...
    ///   the original method in class #"%^String".
    ///
    ...
    ///
    /// @see Notes in the similar method #"str TString::Free" of the base class, which
...
// Suppress conversion of CString and AString to std::string. The rationale for this is
...
/// #"characters::wchar", and #"characters::xchar", which each provide the following
...
///    - #"EMPTY_CSTRING", #"EMPTY_COMPLEMENT_CSTRING", #"EMPTY_STRANGE_CSTRING",
///      #"EMPTY_NCSTRING", #"EMPTY_WCSTRING", #"EMPTY_XCSTRING",
///    - #"NEW_LINE", #"COMPLEMENT_NEW_LINE", #"STRANGE_NEW_LINE",
///      #"NNEW_LINE", #"WNEW_LINE", #"XNEW_LINE" and
///    - #"DEFAULT_WHITESPACES", #"COMPLEMENT_DEFAULT_WHITESPACES",
///      #"STRANGE_DEFAULT_WHITESPACES", #"NDEFAULT_WHITESPACES", #"WDEFAULT_WHITESPACES",
...
///    #"NULL_NSTRING", #"NULL_WSTRING", and #"NULL_XSTRING". However, those are returning
///    type #"%^String" instead of #"%CString". For \e nulled objects of type #"%CString" use the
///    keyword \c nullptr which performs implicit constexpr creation.
///
/// \see
///   See also manual chapter #"alib_strings_details_constants".
///
