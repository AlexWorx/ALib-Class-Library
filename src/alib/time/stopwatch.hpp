...
//==================================================================================================
/// This class encapsulates a system-dependent timer value of type #"Ticks" and provides
/// some simple interface for measuring multiple time spans and providing their sum, average,
/// minimum and maximum.
/// @see
...
class StopWatch {
  ...
  public:
    ...
    Ticks&       GetStartTime()                                                { return startTime; }
    ...
    void         Start()                                                { startTime= Ticks::Now(); }

    /// Sets the internal value to current system time and clears existing sum, quantity of
    /// samples, minimum, and maximum values.
    void         Reset() {
    ...
    Ticks::Duration  GetCumulated()                                            const { return sum; }
    ...
    Ticks::Duration  GetAverage()                                                            const {
        return cntSamples== 0 ? Ticks::Duration()
                              : ( sum / int64_t(cntSamples) );
    }

    /// Returns the minimum duration of all samples since this instance was created or reset.
    /// If no measurement was performed, the value evaluates to the minimum value storable
    /// in type #"%^Ticks::Duration".
    ///
    /// @return The minimum measured duration.
    Ticks::Duration  GetMinimum()                                              const { return min; }
    ...
    Ticks::Duration  GetMaximum()                                              const { return max; }
};
