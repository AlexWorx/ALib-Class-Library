...
/// \b Filter class for evaluation, and expressions then do not need to return a boolean "yes/no"
/// result.
///
/// @see
...
///
/// All identifier and function names are defined to be matched case-insensitive.
...
class FileExpressions
{
  ...
  public:
    ...
    /// The expression scope used with file expressions. Contains a \b FTree cursor and
    /// the string of the path that the file or folder is located in.
    struct FexScope : expressions::Scope
    {
        friend class Filter;              ///<The filter class needs casting this class to its base.
        File                        Node; ///<A node of a \alib{files;FTree}.
        system::PathString    ParentPath; ///<The absolute path to the parent directory.
