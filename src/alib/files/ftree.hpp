...
struct FTreeNodeHandler
{
    ...
    using CharacterType  = lang::system::PathCharType;

    /// The string-type of a node's name. This is a simple static string, allocated with the
    /// pool allocator.
    using NameStringType =  lang::system::PathString;
...
class FTree : public StringTree<MonoAllocator, FInfo, detail::FTreeNodeHandler>
{
  ...
  protected:
    ...
    struct ListenerRecord
    {
        ...
        lang::system::PathStringPA  fileName;      ///< If given, the file's name to listen to.
        lang::system::PathStringPA  pathPrefix;    ///< If given, the start string of the file path
                                                   ///< to monitor.
        lang::system::PathStringPA  pathSubstring; ///< If given, the substring to match in the path
    ...
    ALIB_API  void registerListener( FTreeListener*                     listener,
    ...
    ALIB_API  void notifyListeners(  FTreeListener::Event               event,
                                     File&                              file,
                                     SharedLock*                        lock,
  ...
  public:
    ...
    void DeleteAllCustomData()
    {
        for( auto& node : nodeTable )
        {
            if( node.data.custom )
            {
                ...
                "This method must only be used if all tree nodes have the same custom data attached" )
    ...
    void Notify( FTreeListener::Event event,
      ...
      void Notify( FTreeListener::Event             event,
                   File&                            file
IF_ALIB_THREADS( , SharedLock*                      lock) ,
                   const lang::system::PathString&  filePath= lang::system::NULL_PATH )
    ...
    bool            HasListeners()                                 { return listeners.Count() > 0; }
    ...
    void MonitorDistinctFile( lang::ContainerOp       insertOrRemove,
                              ...
                              const File&             file       )
    {
        ALIB_ASSERT_WARNING( event != FTreeListener::Event::CreateNode, "CONFIG",
                      "Event::Creation will never be invoked with this listener-registration-type.")
        registerListener( listener,
                          insertOrRemove,
                          event,
                          &file, nullptr,
                          lang::system::NULL_PATH,
                          lang::system::NULL_PATH,
                          lang::system::NULL_PATH );
    }
    ...
    void MonitorFilesByName( lang::ContainerOp                  insertOrRemove,
                             ...
                             const lang::system::PathString&    fileName )
    {
        ALIB_ASSERT_ERROR( fileName.IsNotEmpty(), "CONFIG", "Empty file name given.")
        registerListener( listener, insertOrRemove, event,
                          nullptr, nullptr, fileName,
                          lang::system::NULL_PATH, lang::system::NULL_PATH );
    }
    ...
    ///            \alib{lang::system;Path::MakeReal;real path}.
    /// @param insertOrRemove Denotes whether the listener should be inserted or removed.
    ///                       (Only enum elements \b ContainerOp::Insert or \b ContainerOp::Remove
    ///                       must be passed.)
    /// @param listener       The listener to register.
                            ...
                            const lang::system::PathString&     pathPrefix      )
    {
        ...
        registerListener( listener,
                          ...
                          event,
                          nullptr, nullptr,
                          lang::system::NULL_PATH, pathPrefix, lang::system::NULL_PATH );
    }
...
/// A similar design principle is implemented with class \alib{config;Variable} of module
/// \alib_config. A technical explanation to why base class <b>FTree::Cursor</b> is protected
/// there in the same fashion is \alib{config;Variable::AsCursor;given here}.
///
/// @see
...
class File : protected FTree::Cursor
{
  public:
    ...
    ///       documentation of the same concept found with method \alib{config;Variable::AsCursor}.
    ...
    /// Returns the substring from the beginning of #Name() up to and not including the last
    ...
    lang::system::PathString      Stem()                                                       const
    {
        lang::system::PathString result= Name();
    ...
    lang::system::PathString      Extension()                                                  const
    {
        ...
        return  dotPos < 2 ? lang::system::EMPTY_PATH
    ...
    {
        ALIB_ASSERT_ERROR( Value().custom != nullptr, "FILES",
         "No custom data set." )
...
ALIB_DBG(Value().dbgCustomType= &typeid(TCustom); )
    ...
    ///   dm{DATEFORMAT}|The \alib{files::FInfo;MDate;modification date} of this file. This token is optionally followed by a "DATEFORMAT" string given in curly braces. For specification information, see \alib{lang::format;CalendarDateTime::Format}.</TD> </TR>
    ...
    ///   na            |The \alib{files::File;Name;name} of the file.</TD> </TR>
    ...
    ///   - <c>IEC</c>: Chooses \alib{lang::format;ByteSizeIEC;IEC standard} with automatic
    ...
    ///   - <c>SI</c>: Chooses \alib{lang::format;ByteSizeSI;SI standard} with automatic
    ///     detection of an approiate magnitude. The unit of the magnitude found (<c>"B"</c>,
    ...
    ///   \alib{lang::format::FormatByteSize} is used.<br>
    ///
    /// \par
    ...
    ///      box-function \alib{lang::format;FFormat}. With hat, objects of this type can be used
    ///      as arguments for \alib{lang::format;FormatterPythonStyle}.
    ...
    ///                     chooses \alib{lang::format;NumberFormat::Computational}.
...
/// Specialization of functor \alib{strings;T_Append} for type \alib{files;File}.
template<> struct       T_Append<files::File, nchar, lang::HeapAllocator>
{
...
/// Specialization of functor \alib{strings;T_Append} for type \alib{files;File}.
template<> struct       T_Append<files::File, wchar, lang::HeapAllocator>
{
    /// Writes the file's complete path (including the filename) to the given AString.
