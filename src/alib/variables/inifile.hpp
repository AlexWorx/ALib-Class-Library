...
///   Instead, class #"IniFileFeeder" provides all mechanics to easily use
///   INI-files. For this reason, there is no #"alib_manual_appendix_typealiases;type alias"
...
///   during a run of software. The class performs #"alib_mods_contmono;monotonic allocation"
...
///
/// \attention
///   Such approach is implemented with type #"IniFileFeeder".
///   Hence, the direct use of this class is not recommended, instead use the techniques described
///   in chapter #"alib_variables_external_ini"  of the Programmer's Manual of this \alibcamp.
///
...
///   syntax that software expects. In the case of using class #"Configuration"
///   and associated type #"IniFileFeeder" (the intended case!), the following use-cases
///   can be achieved:
...
///   <em>"writeback"</em> is a token resourced in class #"BASECAMP;2" with key
...
///
/// @see Chapter #"alib_variables_external_ini" of the Programmer's Manual of \alibcamp \alib_variables
...
class IniFile {
  ...
  //======================================= Entry and Section ======================================
    ...
    struct Entry {
        ...
        String Value      = NULL_STRING; ///< The trimmed value. Multiline values are likewise
                                         ///< trimmed and backslashes and line feeds are removed
                                         ///< <c>'\\</c>.
                                         ///< This value is to be used for reading a variable's
                                         ///< content.
        String NewValue   = NULL_STRING; ///< If this value is set, #"RawValue" will be ignored on
                                         ///< writing.
        void*  Custom     = nullptr;     ///< May be used by freely by customers of this class.
                                         ///< Initialized with \c nullptr, but otherwise not
                                         ///< touched.
        int    LineNo     = -1;          ///< If given, the line number in the source file.
        int    FirstLineNo= -1;          ///< Set by the method ".Read" in case the entry was given
                                         ///< more than once.
        bool   WriteBack  = false;       ///< If given, a write back indicator was found for this
                                         ///< entry.
    };
    ...
    struct Section {
      ...
      protected:
        ...
        /// #"List".
        /// @param monoAllocator The allocator of the #"%IniFile".
    ...
    struct Handle {
        Section*    SectionPointer; ///< Pointer to the INI-file's section.
        Entry*      EntryPointer;   ///< Pointer to the entry in the #"SectionPointer".
    };
  ...
  public:
    ...
    /// The file name.
    PathString         FileName                                                            =nullptr;

    /// The file header which will be written out as comment lines with <c>"# "</c> prefixes.
  ...
  public:
    ...
    inline IniFile( const Path&  path )
    : IniFile()                                                                    { Read( path ); }
  ...
  //=========================================== Interface ==========================================
    ...
    ALIB_DLL Handle     SearchEntry (const String& section, const String& name );
  ...
  public:
    ...
    ///
    /// @throws Exception( #"Exceptions::ErrorOpeningFile" ).
    ///         In the case the module \alib_camp is not included in the \alibbuild, then the
    ///         original exceptions of <c>std::ifstream</c> are thrown.
    ALIB_DLL
