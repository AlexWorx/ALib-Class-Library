...
//==================================================================================================
/// This is the entry element type of the #"StringTree" of class
/// #"Configuration".
//==================================================================================================
class Entry {
...
/// #"Configuration" which recycles variables and their extended information objects.
///
/// In respect to the node name allocation, this type behaves like
/// default handler #"StringTreeNamesDynamic". In debug compilations,
/// statistics variables #"DBG_STATS_STRINGTREE_NAMES" and
...
struct ConfigNodeHandler {
    ...
    /// addition recycles any extended information object attached to the #"Variable"
    /// object.
    /// @param  tree  The instance of struct #"detail::StringTreeBase;*" that invokes
    ///               this method. Any member may be accessed, including
    ///               #"StringTreeBase;nodeTable" which contains the
...
/// @see Chapter #"alib_variables_monitoring" of the Programmer's Manual of camp \alib_variables_nl.
//==================================================================================================
struct ConfigurationListener {
    ...
    enum class Event {
        ...
        Deletion,     ///< A variable is deleted. Note that the variable is still valid
                      ///< when the method #"M;Notify" is invoked and will be deleted right after
    ...
    ///                          #"ConfigurationListener::Event;Event::Definition".<p>
...
//==================================================================================================
/// Abstract class that defines the plug-in interface for class #"Configuration".
...
class ConfigurationPlugin  : public lang::Plugin<Configuration, Priority> {
  protected:
    ...
    /// (Namely #"CLIVariablesPlugin" and #"EnvironmentVariablesPlugin".)
...
/// The use of this type is documented with the #"alib_mod_variables;Programmer's Manual" of
/// camp \alib_variables_nl.
///
/// Beside the interface that this class provides, publicly inherited interfaces of base types
/// #"StringTree" and #"PluginContainer" are available for use.
///
/// @see
...
///   By #"alib_mod_bs_customize;customizing the bootstrap process",
                    ...
                    , public lang::PluginContainer<ConfigurationPlugin, Priority>
{
  ...
  public:
    ...
    /// The end of a substitution variable.
    /// If this field is set, then field #"SubstitutionVariableDelimiters" is ignored. If this
    /// field is nullptr (the default) or empty, it is ignored and characters in the field
    ...
    /// This is a list that holds pairs of type #"Token" which is used by the built-in boolean
    /// variable type (type name "B").
  ...
  //===================================== Constructor/destructor ===================================
    ...
    ///
    /// Furthermore all #"alib_variables_types_builtin;built-in variable types" are registered.
    ///
    ...
    /// threads are active) or with prior locking the #"Configuration" instance.
    ///
...
//============================================ Interface ===========================================
    ...
    /// the fields (except #"Declaration::typeName") are replaced with the
    ...
    ///   of this #"%Configuration" object. This imposes a potential memory drain (leak).
    ...
    ///
    /// @see Chapter #"alib_variables_names_placeholders" of the Programmer's Manual of
    ///      camp \alib_variables_nl.
    ///
    ...
    ///                     #"AppendableTraits;appendable" to class #"%AString".
    ///                     To provide more than one object, pass an object of class
    ///                     #"Boxes" or a boxed array.
    ...
    /// twofold:
    /// 1. To allow external configuration plug-ins to set all variable's values, which is not
    ...
    ///    #"Configuration::PresetImportString(StringEscaper*);Configuration::PresetImportString",
    ///    which in the end leads to correctly setting a variable's external value, but which
    ///    creates a little overhead in respect to execution time and memory consumption.
    /// 2. To give external configuration plug-ins the possibility to "populate" their files
    ///    with all known variables, including their default values and comments, again
    ///    independently of a concrete execution path of a run of the software, which might
    ...
    ///    For example, built-in class #"IniFileFeeder" supports such "population"
    ///    of for INI-files through its #"%Export" methods.
    ///
    /// Altogether, the use of this method does not impose much of an overhead, because:
    /// - as explained above, the need of the method
    ///   #"Configuration::PresetImportString(StringEscaper*);Configuration::PresetImportString"
    ...
    ///
    /// @see Chapter #"alib_variables_external_ini" of the Programmer's Manual of camp
    ///      \alib_variables.
    ///
    /// @tparam TEnum Enumeration type equipped with #"alib_enums_records;ALib Enum Records"
    ///               of type #"var Declaration".
    ...
    ///
    /// @see
    ///   - Overloaded public methods #".PresetImportString(StringEscaper*)".
    ...
    /// @param priority The priority to set for the value.
    ///                 Defaults to #"Priority::DefaultValues".
    ...
    /// but does not accept an #"util::StringEscaper" and thus expects the value is in
    /// C++ internal string format (aka 'not escaped').
    /// @param name     The name of the variable.
    /// @param value    The value to import.
    /// @param priority The priority to set for the value. Defaults to #"Priority::DefaultValues".
    ...
    ///
    /// This is a convenience method. Full access to the underlying #"StringTree"
    ...
    /// Utility method that checks if a given value represents boolean \c true.
    ///
...
/// The types' true nature is inherited from class #"StringTree::Cursor;*"
...
/// a <b>StringTree::Cursor</b> is only necessary for implementing #"ConfigurationPlugin"
...
///     #"alib_mod_variables;Programmer's Manual" of camp \alib_variables_nl.
...
///     \alib_camp in the \alibbuild, and the inclusion of the header file #"F;ALib.Camp.H",
///     a set of overloaded namespace functions allow the construction of variables in the
///     #"%Configuration" instance included in \alibcamps. Those are:
...
class Variable : protected Configuration::Cursor {
    ...
    /// cursor operations on #"%Configuration" instances.
  ...
  public:
    /// Default constructor.
    /// \note
    ///   A variable instance that was default-constructed cannot be unless a newly constructed
    ///   instance is assigned at a later stage of the program execution.
    Variable()                                                                             =default;
    ...
    /// @param cursor The #"%StringTree" cursor.
    ...
    ///       #"StringTree::Root;2" after this construction.
    /// @see Overloaded namespace functions #"CampVariable(camp::Camp&);CampVariable" which become
    ///      accessible with the header file #"F;ALib.Camp.H".
    ...
    ///       class #"%Variable" restricts and changes the interface of class #"%StringTree::Cursor"
    ///       in a way that it seems to be a very different entity.
    ///       And this is right in the vast majority of use cases.
    ///       In the remaining cases such access to the cursor features is needed.
    ///       For example, when implementing a custom #"ConfigurationPlugin".<br>
  ...
  public:
    ...
    /// Moves this variable cursor (aka #"StringTree::Cursor;*")
    ...
    /// In debug compilations, an #"alib_mod_assert;error is raised" in case the variable
    ...
    /// imported from the string. In this case, #"Priority::DefaultValues" is returned.
    ///
    ...
    ///
    /// @tparam TEnum         The type of parameter \p{Enum}
    /// @param  Enum          Element of an enum type that represents resourced configuration
    ///                       variables.
    /// @param  replacements  Replacement values.
    /// @return <c>*this</c> to allow concatenated operations.
    template<typename TEnum>
    ...
    /// this is just a path node inside the #"%StringTree" of the #"%Configuration".<br>
    ///
    /// \c false will be returned only if:
    /// - An instance of a variable was created with the constructor that only accepts a
    ///   #"%Configuration" instance (an undeclared variable).
    /// - An instance of a variable was created with the constructor that accepts a cursor
    ///   object of a #"%Configuration", and this cursor does not point to a declared variable.
    /// - Method #".Try(const String&)" was called on an instance of this type, and the given
    ///   variable name did not represent a declared variable.
    ///
    /// @return \c true if the underlying #"%StringTree"::Cursor of the #"%Configuration" that this
    ///         variable is associated with, points to a declared variable.
    ///         Otherwise, \c false is returned.
    bool                IsDeclared()               const { return Cursor::Value().data != nullptr; }

    /// Tries to move the underlying #"%StringTree"::Cursor of the #"%Configuration" that this
    ...
    /// #"PresetImportString(StringEscaper);preset value" exists for the variable,
    ...
    ///    #"PresetImportString(StringEscaper);preset value" is set.
    ///
    /// @param name         The name of the variable.
    /// @param typeName     The type of the variable.
    /// @return \c true if the variable name specified with the given \p{declaration} existed and
  ...
  /// Increases the reported priority of this variable to the value given. If the current value
    ...
    ///   #"Priority::Protected" is passed, add a simple <c>(void)</c> before
    ///   the call.
    ///
    /// @see
    ///   - Chapter #"alib_variables_definition" of the Programmer's Manual for more information.
    ///   - Chapter #"alib_variables_monitoring" to learn about how to monitor changes of
    ///     variables.
    ///
    ...
    /// with an overloaded constructor or method #".Declare(const Declaration*)", which accepts such
    /// struct. Otherwise, a nullptr is returned.
    ///
    ...
    /// ('hard-coded' values). Besides the elements in enumeration #"variables::Priority",
    ...
    /// to #"Priority::Environment", then environment variables and CLI arguments may
    /// still overwrite a value. If set to #"Priority::Environment" <c> + 1</c>, then
    /// environment variables would not overwrite, but still CLI arguments could.
    ///
    /// A priority of #"Priority::Protected" disallows any code entity to make
    ...
    /// Variables which are declared using a #"var Declaration" that
    /// contains a default value, will have this default value parsed and contain a data set of
    /// priority #"Priority::DefaultValues".
    ///
    ...
    ///
    /// @return \c true if data contained has a higher priority than #"var Priority::Standard",
    ///         \c false if not.
    inline
    bool                    IsDefinedExternally()  const { return Priority() > Priority::Standard; }


    /// Returns \c true if the variable is considered to be written back to configuration data
    /// sources.<br>
    /// The built-in variables types return \c false.
    /// The behavior can be changed with custom variable types by overriding the method
    /// #"var VMeta::isWriteBack" for the custom type.<br>
    /// It is not further specified when and how variable values are written back. This is
    /// dependent on the use case and implementation of the data sources.
    ///
    /// The built-in type #"IniFileFeeder" acknowledges and supports this flag.
    /// Class #"App" of the module \alib_app supports writing INI-files with write-back
    /// entries by invoking this method on all exported variables.<br>
    /// A sample of a variable using this feature is found with the module \alib_alox and method
    /// #"Lox::SetVerbosityExport(detail::Logger*);2".
    ///
    /// @return \c true if this variable is considered to be written back to its configuration
    /// source.
    bool    IsWriteBack()           const { return getMeta()->isWriteBack( Cursor::Value().data ); }


    /// Imports a variable value from a serialized string representation. Internally, this is
    /// performed by calling #"VMeta::imPort;*" on the virtual meta object
    ...
    /// @param escaper  The #"%StringEscaper" to use for importing an escaped string. Defaults to
    ///                 \c nullptr which indicates that a programmatically defined C++ string is to
    ///                 be parsed. Usually, if a #"ConfigurationPlugin" invokes this method
    ...
    /// This is done by calling #"VMeta::exPort;*" on the virtual meta object
    /// associated with this variable's type.
    ///
    /// @param dest    The destination string. Must not be reset prior writing, but appended.
    /// @param escaper The #"%StringEscaper" to use for exporting a C++ string to a (probably)
    ...
    ///               a #"ConfigurationPlugin" invokes this method to write-back the
    ...
    /// - A configuration plug-in like #"CLIVariablesPlugin" or
    ...
    ///
    /// If this is not done, in debug-compilations an #"alib_mod_assert;error is raised".
    ...
    /// #"alib_variables_definition" of the Programmer's Manual of this \alibcamp.
    ///
    ...
    /// In other words, providing a wrong type is u33ndefined behavior.
    ///
    /// In debug-compilations, an #"alib_mod_assert;error is raised" if the type does not
...
/// <c>const</c>-version of overloaded method.
    ...
    operator bool            ()                   { return Get<Bool     >(); }              ///< @return Calls and returns #"Variable::Get;Get<Bool>()".
    operator integer         ()                   { return Get<integer  >(); }              ///< @return Calls and returns #"Variable::Get;Get<integer>()".
    operator float           ()                   { return float(Get<double>()); }          ///< @return Calls and returns #"Variable::Get;Get<double>()" cast to \c float.
    operator double          ()                   { return Get<double   >(); }              ///< @return Calls and returns #"Variable::Get;Get<double>()".
    operator const String&   ()                   { return Get<AStringPA>(); }              ///< @return Calls and returns #"Variable::Get;Get<AStringPA>()".
    operator const Substring ()                   { return Get<AStringPA>(); }              ///< @return Calls and returns #"Variable::Get;Get<AStringPA>()".

    bool            GetBool()          { return Get<Bool     >(); }                         ///< @return Calls and returns #"Variable::Get;Get<Bool>()".
    integer         GetInt()           { return Get<integer  >(); }                         ///< @return Calls and returns #"Variable::Get;Get<integer>()".
    float           GetFloat()         { return float(Get<double>()); }                     ///< @return Calls and returns #"Variable::Get;Get<double>()" cast to \c float.
    double          GetDouble()        { return Get<double   >(); }                         ///< @return Calls and returns #"Variable::Get;Get<double>()".
    Box&            GetBox()           { return Get<Box      >(); }                         ///< @return Calls and returns #"Variable::Get;Get<AStringPA>()".
    AStringPA&      GetString()        { return Get<AStringPA>(); }                         ///< @return Calls and returns #"Variable::Get;Get<AStringPA>()".
    StringVectorPA& GetStrings()       { return Get<StringVectorPA>(); }                    ///< @return Calls and returns #"Variable::Get;Get<StringVectorPA>()".
    String&         GetString(int idx) { return Get<StringVectorPA>().at(size_t(idx)); }    ///< @param idx The index of the requested string. @return Calls and returns #"Variable::Get;Get<StringVectorPA>().Lines.at(idx)".
    int             Size()             { return int(Get<StringVectorPA>().size()); }        ///< @return Calls and returns #"Variable::Get;Get<StringVectorPA>().Lines.size()".

    bool          operator= (bool          val) { return Get<Bool      >()= val; }          ///< Calls #"Variable::Get;Get<Bool>()" and assigns \p{val}.                    @param val The value to assign. @return The \p{val} given.
    integer       operator= (integer       val) { return Get<integer   >()= val; }          ///< Calls #"Variable::Get;Get<integer>()" and assigns \p{val}.                 @param val The value to assign. @return The \p{val} given.
    float         operator= (float         val) { Get<double>()= double(val); return val; } ///< Calls #"Variable::Get;Get<double>()" and assigns \p{val}.                  @param val The value to assign. @return The \p{val} given.
    double        operator= (double        val) { return Get<double    >()= val; }          ///< Calls #"Variable::Get;Get<double>()" and assigns \p{val}.                  @param val The value to assign. @return The \p{val} given.
    const String& operator= (const String& val) { return Get<AStringPA>().Reset(val); }     ///< Calls #"Variable::Get;Get<AStringPA>()" and resets the string to \p{val}.  @param val The value to assign. @return The \p{val} given.
...
/// Utility type which implements #"TSharedMonoVal" with class
/// #"Configuration".
/// The result of combining both is an automatic pointer to a #"%Configuration" that is
/// "self-contained" in the first buffer of a #"MonoAllocator" together with the allocator
/// itself.
...
///
/// Along with the #"%Configuration", this shared object includes a #"SharedLock".
...
///               #"alib::SharedConfiguration;2" chooses type #"SharedLock".<br>
...
using Configuration        = variables::Configuration;                     ///< Type alias in namespace #"%alib".
#if !ALIB_SINGLE_THREADED || DOXYGEN
using SharedConfiguration  = variables::TSharedConfiguration<SharedLock>;   ///< Type alias in namespace #"%alib".
#else
using SharedConfiguration  = variables::TSharedConfiguration<void>;
#endif
using ConfigurationListener= variables::ConfigurationListener;             ///< Type alias in namespace #"%alib".
using Variable             = variables::Variable;                          ///< Type alias in namespace #"%alib".
...
/// Specialization of functor #"AppendableTraits" for type <c>Variable</c>.
template<> struct       AppendableTraits<variables::Variable, nchar, lang::HeapAllocator> {
    ...
    /// @param target   The #"%NAString" that #"%Append(const TAppendable&)" was invoked on.
