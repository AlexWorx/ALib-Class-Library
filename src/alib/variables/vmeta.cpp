...
namespace {
void imPortStringVector( VDATA*        data, const StringEscaper& esc,
                         const String& src , const String&        delimiter ) {
    LocalAllocator4K la;
...
void  exPortStringVector( VDATA*    data, const StringEscaper&  esc,
                          AString&  dest, const String&         delimiter ) {
    auto& dataSA= data->As<StringVectorPA>();
    if( !dataSA.empty())
