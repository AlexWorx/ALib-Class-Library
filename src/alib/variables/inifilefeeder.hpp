...
//==================================================================================================
/// Adapter type between class #"IniFile" and class #"Configuration".
/// INI-file contents usually are fed in with bootstrapping an application and optionally be
/// written out when an application ends (to populate empty INI-files or add new variables to
/// existing ones). For this, an instance of this type is to be created locally each time. In other
/// words, no instance of this type needs to be kept in memory during the life-span of an
/// application.
/// @see Chapter #"alib_variables_external_ini" of the Programmer's Manual of camp \alib_variables.
...
class IniFileFeeder {
  ...
  public:
    ...
    /// This value is used with utility method #".AddResourcedSectionComments(ResourcePool&)".
    int                     LineWidth                                                         = 100;
    ...
    /// This flag may be read after invoking the method #"ExportStart", which will try to
    /// read the file first. If the file could not be opened or is empty, this flag is set.
    /// The flag may be used to decide, for example, if a file-comment or section comments
    /// should be generated.
  ...
  public:
    ...
    /// @param pPriority      Stored in the field #".priority". Defaults to #"Priority::ConfigFile".
    ///
    IniFileFeeder( Configuration& pConfiguration, Priority pPriority= Priority::ConfigFile )
    : configuration(pConfiguration)
    , priority     (pPriority)                                                                    {}

    /// Virtual destructor. #"alib_mod_assert;Raises an ALib warning" in debug-compilations,
  ...
  //=========================================== Interface ==========================================
    ...
    ///   #"Variable::Try(const String&);Variable::Try" to find the variable without declaring
    ///   it.<br>
    ///   This method may be used, for example, with camp \alib_expressions to declare nested
    ///   expressions to an #"StandardRepository".
    /// @param sectionName The name of the section to import.
    ...
    /// Otherwise, a #"PresetImportString(StringEscaper);preset string" is set for the
    /// variable.
    /// @return The number of variables imported.
    ALIB_DLL
    int         ImportAll();
    ...
    /// @return The number of entries read. (The result of method #"IniFile::Read;*".)
    int ImportStart( const Path&  path ) {
        ...
        int qtyEntries= 0;
        #if ALIB_CAMP
            try { qtyEntries=  iniFile->Read(path); } catch (Exception& ) {}
        #else
            iniFile->Read(path);
        #endif
        return qtyEntries;
    ...
    int ExportStart( const Path& path ) {
        int qtyEntries= 0;
        #if ALIB_CAMP
        try { qtyEntries= ImportStart(path); }
        catch (Exception& e) {
            if (e.Type() != variables::Exceptions::ErrorOpeningFile)
                throw;
        }
        #else
            qtyEntries= ImportStart(path);
        #endif
        DidNotExistOrWasEmpty= (qtyEntries == 0);
        return qtyEntries;
    }
    ...
    /// Writes all variables below the given #"StringTree::Cursor;*"
    /// of the #"Configuration" into the #"iniFile", previously created with #"ExportStart".
    /// If an INI-file was previously loaded, only those variables which are not existing in the
    ...
    /// value of the variable.
    ///
    /// @param cursor              A cursor into the configuration's #"StringTree".
    ...
    /// This method may be used to set the #"config_IniFile_writebackflag;writeback flag"
    ...
    /// @see Chapter #"alib_variables_external_session" of the Programmer's Manual of module
    /// \alib_variables_nl.
    /// @param path  The path of the variable to export.
    ...
    /// case if a section was read from an INI-file, as even if no comments were given,
    ...
    /// The resource strings found are processed using the method
    /// #"Paragraphs.AddMarked(const BoxedObjects&)".
    /// This allows using text macros like <b>'\@HL'</b> to format the text.
    ///
    /// \see
    ...
    /// @return The number of section comments written.
    ///         This is useful to detect if the INI-file needs to be written.
    ALIB_DLL
    int       AddResourcedSectionComments( ResourcePool&    resourcePool,
                                           const NString&   resourceCategory,
                                           const NString&   resourceNamePrefix  );

    /// This is a shortcut to the overloaded method, accepting a #"ResourceHolder"
    /// instead of parameters \p{resourcePool} and \p{resourceCategory}.
    /// @param resourceHolder      A container holding a resource pool and a fixed category.
    /// @param resourceNamePrefix  A prefix of the resource name.
    /// @return The number of section comments written.
    ///         This is useful to detect if the INI-file needs to be written.
    int       AddResourcedSectionComments( resources::ResourceHolder&  resourceHolder,
                                           const NString&              resourceNamePrefix  ) {
        return AddResourcedSectionComments( resourceHolder.GetResourcePool(),
