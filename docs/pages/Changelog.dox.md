...
\par General:
Unfortunately, we had to drop the support for C++20 module (dual-mode) compilation, which we had 
only introduced a month ago. While the former release was plenty of work but still ran only
with Clang, the newest Clang version follows the standards more strictly, which broke our
current design.
We are not sure how to proceed with this topic. Our experiences are documented in this
\ref alib_c20module_shift "blog page".

\par Module ThreadModel:
With the changes in this release, this module is considered to be in a first stable state.
We removed the warning in the documentation about this module being instable and experimental.
...
- Types that implement the <c>std::allocator</c> contract are now allocator-agnostic and 
  ...
  - \b lang::StdContainerAllocator,
  - \b lang::StdContainerAllocatorRecycling, and
