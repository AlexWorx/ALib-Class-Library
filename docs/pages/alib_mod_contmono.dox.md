...
together:
1. Containers define how software organizes its objects, and
...
- <b>Local- Or Stack-Allocation<br></b>
  ...
  Allocations and deallocations are performed in constant time <b>O(1)</b>, in most cases even
...
\note
  The performance overhead of frequent dynamic memory allocations and corresponding deallocations,
...
by software: At a certain point in execution time, a set of allocations is performed which are
...
3. If a buffer is full, it is added to a list of used buffers for later deallocation
...
\attention
  ...
  deallocation of all allocated memory only happens at the end of the joint life cycle of
...
optimize the performance of software.
...
- The peak memory consumption that has to be taken into account is not higher than the
  peak that occurs with a peak server load. This means it is ignorable and does not contribute
...
A reader could now comment that such a recycling technique is not necessarily related to monotonic
...
Allocators have to provide four basic allocation/deallocation functions, which are quite user-unfriendly.
...
allocation/deallocation methods provided with any \b Allocator.
...
\attention
  ...
  were not so high.
  We had once hoped that with C++17 namespace <c>std::pmr</c> the situation improved, but this
...
Now, software should be free to create and delete arbitrary sections and arbitrary variables
...
A standard container is declared by providing the allocator types \b alib::StdMA<T> as the
allocator template typename. The contained object's type has to be repeated:

 \snippet "ut_stdcontainers.cpp"     DOX_MONOMEM_STDMA_DECL
...
to be passed to the constructor of the container:
 ...
 \snippet "ut_stdcontainers.cpp"     DOX_MONOMEM_STDMA_DEF
                      
Note that we simply pass the "original" allocator type \b MonoAllocator here, which implicitly
constructs the \b StdAllocator, aka \b StdMA with the constructor call.
...
Because this "chain" covers most use cases of monotonic allocation, you could name it the
"natural choice".
...
An expected multi-threading use of software entity can be a driver for both: either for sharing an
...
In debug-compilations \alib_assertions are raised when unsupported alignments are requested.
