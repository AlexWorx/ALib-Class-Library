...
A few further configuration variables that \alib internally uses are defined by camp #"BASECAMP;2".
The variables defined with that module are #"alib_cfgvars;documented here".

<b>Default Behavior:</b><p>
\alox and the \aliblong are designed to run fine without setting configuration variables, because
all of them have #"alib_resources_intro;resourced" default values.
...
- - - - - - - - - - - - -| - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   ...
   Plain      | Plain text \e Logger of type #"loggers::ConsoleLogger" is created.
   Ansi       | A \e Logger of type #"loggers::AnsiLogger" is created.
   Windows    | A \e Logger of type #"loggers::WindowsConsoleLogger" is created.


\I{################################################################################################}
## ALOX/CONSOLE_LIGHT_COLORS ## {#alxcvALOX_CONSOLE_LIGHT_COLORS}
<em>Type</em>: \c "ALOXCLP" (#"textlogger::ColorfulLoggerParameters")
...
color accordingly. E.g., for instances of class #"loggers::AnsiLogger", field
#"AnsiLogger;CFP" is set.
...
Boolean variable that is evaluated in method #"Log::AddDebugLogger;*".
...
Evaluated once in the constructor of class #"loggers::WindowsConsoleLogger".
Overwrites the default value of field #"WindowsConsoleLogger;CodePage".
...
<em>Type</em>: \c "ALOXAS" (#"textlogger::FormatAutoSizes")
...
#"AutoSizes::Export;exports auf the contained AutoSizes objects", separated
 by a semicolon <c>';'</c>.

This variable is used by \e Loggers derived from abstract type #"textlogger::TextLogger".
...
#"Logger::GetName". This allows having different sizes for different \e %Loggers.
...
\note Being a pure convenience feature that optimizes the log output slightly, this variable is
      ...
      Consequently, the variable is defined with #"variables::Priority;Priority::Session" when written by
      a \b TextLogger.
      More information on this difference and how to best store this variable is given
      with chapter #"alib_variables_external_session" of the Programmer's Manual of module
      \alib_variables_nl.

The value of the variable can be retrieved with #"TextLogger::GetAutoSizes".
If a certain set auf sizes is to be used and never changed, then flags
#"AutoSizes::WriteProtected" can be set in one or both \b AutoSizes instances of
this variable. The flag will also be set by the logger, if the configuration variable becomes
defined with a priority higher than #"variables::Priority;Priority::Session".

\see
- Reference documentation of this #"FormatAutoSizes;variable's type"
  for further information.
- Chapter #"alib_mod_alox_apdx_auto_config_list_logables_textlogger".


\I{################################################################################################}
## ALOX/LOGGERNAME/FORMAT ## {#alxcvALOX_LOGGERNAME_FORMAT}
<em>Type</em>: \c "ALOXFMI" (#"textlogger::FormatMetaInfo")
Default Value: <c>"%SF:%SL:%A3%SM %A3[%TC +%TL][%tN][%D]%A1#%# %V: ,[ERR],[WRN],     ,[***]"</c>.

This variable is used by \e Loggers derived of type #"textlogger::TextLogger;*".<br>
...
- #"FormatMetaInfo;VerbosityWarning",
- #"FormatMetaInfo;VerbosityInfo",
- #"FormatMetaInfo;VerbosityVerbose" and
- #"FormatMetaInfo;MsgSuffix"

of struct #"textlogger::FormatMetaInfo" are set according to the variables' content.
...
- #"Logger::GetName" of the \b Logger constructed is used.
- If no configuration setting is found for that name, the value of
  #"Logger::GetTypeName" is used.
...
#"loggers::AnsiConsoleLogger" as long as no specific name is set when such \e %Logger is
...
<em>Type</em>: \c "ALOXFDT" (#"textlogger::FormatDateTime")
Default Value: <c>"yyyy-MM-dd,HH:mm:ss, Days "</c>.

This variable is used and applied in the same way as #"alxcvALOX_LOGGERNAME_FORMAT" and
the fields of struct #"textlogger::FormatDateTime":

- #"FormatDateTime;Date",
- #"FormatDateTime;TimeOfDay" and
- #"FormatDateTime;ElapsedDays".


\I{################################################################################################}
## ALOX/LOGGERNAME/FORMAT_TIME_DIFF ## {#alxcvALOX_LOGGERNAME_FORMAT_TIME_DIFF}
<em>Type</em>: \c "ALOXFTD" (#"textlogger::FormatTimeDiff")
Default Value: <c>"1000,---   , ns, \\u03BCs, ms, s, m, h, days"</c>.

This variable is used and applied in the same way as #"alxcvALOX_LOGGERNAME_FORMAT" and
the fields of struct #"textlogger::FormatTimeDiff":

- #"FormatTimeDiff;Minimum",
- #"FormatTimeDiff;None",
- #"FormatTimeDiff;Nanos",
- #"FormatTimeDiff;Micros",
- #"FormatTimeDiff;Millis",
- #"FormatTimeDiff;Secs",
- #"FormatTimeDiff;Mins",
- #"FormatTimeDiff;Hours", and
- #"FormatTimeDiff;Days".


\I{################################################################################################}
## ALOX/LOGGERNAME/FORMAT_MULTILINE ## {#alxcvALOX_LOGGERNAME_FORMAT_MULTILINE}
<em>Type</em>: \c "ALOXFML" (#"textlogger::FormatMultiLine")
Default Value: <c>"2,ALox: Multi line message follows: ,> ,,nulled,\\r"</c>.

This variable is used and applied in the same way as #"alxcvALOX_LOGGERNAME_FORMAT" and
the fields of struct #"textlogger::FormatMultiLine":

- #"FormatMultiLine;Mode",
- #"FormatMultiLine;Delimiter",
- #"FormatMultiLine;Headline",
- #"FormatMultiLine;Prefix",
- #"FormatMultiLine;Suffix",
- #"FormatMultiLine;DelimiterReplacement",

\I{################################################################################################}
## ALOX/LOGGERNAME/FORMAT_OTHER ## {#alxcvALOX_LOGGERNAME_FORMAT_OTHER}
<em>Type</em>: \c "ALOXFO" (#"textlogger::FormatOther")
<em>Default Value</em>: <c>"---, ---, 3"</c>.

This variable is used and applied in the same way as #"alxcvALOX_LOGGERNAME_FORMAT" and
defines the fields of struct #"textlogger::FormatOther":

- #"FormatOther;NoSourceFileInfo",
- #"FormatOther;NoMethodInfo",
- #"FormatOther;LogNumberMinDigits",

\I{################################################################################################}
## ALOX/LOGGERNAME/VERBOSITY_WITH_LOXNAME ## {#alxcvALOX_LOGGERNAME_VERBOSITY_WITH_LOXNAME}
<em>Type</em>: \c "SV;" (#"StringVectorSemicolon")
<em>Default Value</em>: none.<br>
...
Details are described in chapter #"alib_mod_alox_external_verbosity_configuration_variable"
...
are set using method #"TextLogger::SetReplacement".
...
Details are described in #"alib_mod_alox_domain_substitution_config;Substitution Rules and External Configuration".
...
Details are described in #"alib_mod_alox_trim_source_path_set_config;External Configuration of Trim Rules".
...
- \p{inclusion}<br>
  ...
  (i.e. \b "E" is enough) and is read in case insensitive way. The meaning is the same as the  
  parameter \p{otherPLs} of the method #"Lox::SetPrefix(const Box&, lang::Inclusion);2".
...
If this variable is defined, class Lox will automatically invoke method #"Lox::State;*"
...
#"Lox::State;*" for more information. The identifiers expected in this variables' arguments
are:
ww
...
\b %Lox instance), then method #"Lox::State;*" is invoked, with
