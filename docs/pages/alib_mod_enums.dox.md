...
level of the #"alib_manual_modules_graph;module hierachie".
...
- #"F;ALib.EnumOps.H" for module \alib_enumops_nl, and
- #"F;ALib.EnumRecords.H" for module \alib_enumrecords_nl.
...
Type trait #"ArithmeticalTraits" by default is derived from <c>std::false_type</c>
...
- #"arithmetical::operator>"
- #"arithmetical::operator>="
- #"arithmetical::operator+(TEnum, TE)"
- #"arithmetical::operator-(TEnum, TE)"
- #"arithmetical::operator+=(TEnum, TE)"
- #"arithmetical::operator-=(TEnum, TE)"
...
\attention
  As a result of the #"alib_manual_appendix_tca;type traits" approach used, this does
...
While the specialization of the type trait #"ArithmeticalTraits" is a simple task,
using provided macro #"ALIB_ENUMS_MAKE_ARITHMETICAL" makes the code more readable.
...
(Please do not forget to include the header #"F;ALib.EnumOps.H".)

\I{################################################################################################}
## 2.2 Bitwise Operators ## {#alib_enums_arithmetic_bitwise}

Type trait #"BitwiseTraits" by default is derived from <c>std::false_type</c>
and is otherwise empty. If a specialization for an enumeration type derives from
<c>std::true_type</c> instead, the following set of operators become available to a scoped
enumerations:
...
Therefore, in the sample, macro #"ALIB_ENUMS_MAKE_BITWISE" is used to define the type trait
#"BitwiseTraits" for the type.
...
- #"bitwise::HasBits",
- #"bitwise::CountElements",
- #"bitwise::ToBitwiseEnumeration" and
- #"bitwise::ToSequentialEnumeration"
...
#"IterableTraits" may be specialized for a custom enum type that is not "sparsely"
...
As an alternative, this module provides the type trait #"IterableTraits" which
we specialize for enumeration \c Pets using helper macro #"ALIB_ENUMS_MAKE_ITERABLE" as follows:

 \snippet "DOX_ENUMS.cpp"     DOX_ENUMS_ITER_MAKE_ITERABLE

With that in place, templated class #"EnumIterator" becomes available for the enumeration
...
The reason why this still compiles is that with a specialization of #"IterableTraits", the operators
#"enumops::iterable::operator+;(" and
#"enumops::iterable::operator-;(" become available.
...
invocation might be changed, as <c>Pets::Snake</c> then is not the last in the list.
...
put a new element behind this one. Furthermore, the macro statement would never need to be
changed:
...
The \c std::iterator_traits returned by the methods #"EnumIterator::begin;*" and
#"EnumIterator::begin;*" implements the standard library concept of
...
Iteration works well if the type trait #"BitwiseTraits" is specialized in parallel
to #"IterableTraits".
The restriction described in #"alib_enums_iter_struct", namely that enum types must not be
...
Macro #"ALIB_ENUMS_MAKE_ITERABLE", chooses integral value \c 1 as a start element.
Again, if #"ALIB_ENUMS_MAKE_ITERABLE_BEGIN_END" is used, iteration might start on higher values.

\I{################################################################################################}
## 3.4 Performance Considerations  ## {#alib_enums_iter_perf}
Class #"EnumIterator" is empty in respect to fields. Created on the stack there
is no performance penalty. The same is true for the internal iterator type, which is
returned with class #"EnumIterator::begin;*" and class #"EnumIterator::end;*".
...
#"alib_enums_arithmetic_bitwise". With that, a combination of elements
can easily be stored and tested in an integral value.
...
Let us come back to our #"alib_enums_iter_struct;previous sample" of enum \e Pets:

 \snippet "DOX_ENUMS.cpp"     DOX_ENUMS_ITER_SAMPLE

With the module \alib_enumops, the type definition #"enumops::EnumBitSet" becomes available.
This type simply fills out the right template parameters for target type #"TBitSet".<br>
Those are:
...
- The length of the enumeration by passing the field #"IterableTraits::End;*".
...
Because #"TBitSet" provides efficient bidirectional iterator types that deliver just the bits
...
#"alib::lang::TBitSet;reference documentation" of underlying class \b TBitSet and keep in mind, that
wherever template type \p{TInterface} is mentioned, a C++ enum element can be directly
provided. The only prerequisite is that preprocessor macro #"ALIB_ENUMS_MAKE_ITERABLE"
...
module \alib_enumrecords_nl, which is included with the header #"F;ALib.EnumRecords.H".<br>
This module provides the concept of <b>ALib Enum Records</b>, which again is enabled for a custom
enumeration type by specializing another struct, namely #"RecordsTraits".
...
- The inheritance graph of the record types, may be used to constitute an inheritance relationship
  ...
  elements of arbitrary enum types as an argument, as long as they are of a certain <em>"base type"</em>.
...
A step-by-step sample of this design pattern is explained in chapter #"alib_enums_records_paradigm".

For now, let’s start with the simple things.

\I{################################################################################################}
## 4.1 Equipping An Enumeration With Data Records ## {#alib_enums_records_firststep}

In chapter #"alib_enums_arithmetic_bitwise" a simple enumeration type \b Fruits was
...
For a typical C++ programmer, the purpose of having a language feature that provides runtime
...
Now we specialize the type trait #"RecordsTraits" for enum \b Fruits. The only single
entity in this struct is given with <c>using</c>-statement #"RecordsTraits::Type;*", which
...
The easiest way to do the specialization is by using macro #"ALIB_ENUMS_ASSIGN_RECORD"
as follows:
...
done when #"alib_mod_bs;bootstrapping software".
...
To define enum records, several overloads of namespace function #"bootstrap::Bootstrap(camp::Camp&)"
are provided.
These methods become available with the inclusion of #"F;ALib.EnumRecords.Bootstrap.H":
...
This is using namespace function #"enumrecords::GetRecord" passing the given enum element.
...
#"alib_mod_assert;raises an ALib error" in this case.

Therefore, in situations where a code does not "know" if undefined enum elements are passed,
the way out of this is to use sibling function #"enumrecords::TryRecord". This returns a pointer,
...
Iteration is performed using static templated class #"enumrecords::EnumRecords;EnumRecords<TEnum>"
...
Inner iterator type #"EnumRecords::ForwardIterator;*" offers methods
#"enumrecords::EnumRecords::ForwardIterator;Enum" and
#"enumrecords::EnumRecords::ForwardIterator;Integral"
...
#"alib_enums_records_paradigm" and a curious reader might "fast forward" to that chapter.
...
The clue to this feature is predefined enum record type #"ERSerializable". Besides
field #"ERSerializable::EnumElementName", this record has a second member with
#"ERSerializable::MinimumRecognitionLength". If this is set to a value greater than
...
For parsing enum elements back from strings, templated namespace function #"enumrecords::Parse" is given:
...
- #"APPENDABLES::AppendableTraits<TEnum,TChar,TAllocator>",
...
chapter #"alib_enums_records_paradigm" and which is used with various other \alibmods_nl.
...
#"APPENDABLES::AppendableTraits<TEnum,TChar,TAllocator>",
#"enumrecords::Parse", #"enumrecords::ParseBitwise", and #"enumrecords::ParseEnumOrTypeBool") are 
applicable not only to enum types associated with records of type #"ERSerializable" but also 
with any custom record type that derives from this!
...
If done, then
- Methods #"enumrecords::GetRecord" and #"enumrecords::TryRecord" will retrieve the first record
...
- The additional records can only be retrieved by iterating the records, as explained in
  previous section #"alib_enums_records_access_iteration".

Again a sample helps to quickly understand the rationale and a use case for multiple records.
Built-in \alib enumeration type #"lang::Bool" has two elements; #"lang::Bool::False" and
#"lang::Bool::True".
...
The implementation of appending an element of a serializable enum type, template specialization
#"AppendableTraits<TEnum,TChar,TAllocator>", uses #"enumrecords::TryRecord" and
thus receives the first given name for the elements, namely <c>"False"</c> and <c>"True"</c>.
In contrast, the method #"enumrecords::Parse" iterates over all records and tries to recognize
...
#"ERSerializable::MinimumRecognitionLength;*". This allows string <c>"F"</c> to be parsed
...
quickly be demonstrated with the record definitions of built-in enum type#"ContainerOp":
...
different methods, depending on whether #"BitwiseTraits" is specialized for an enum type
or not (see chapter #"alib_enums_arithmetic_bitwise").
...
#"alib_mod_resources;Externalized String Resources" and, as shown in this chapter, both
...
- #"Bootstrap(TEnum, TArgs&&...)" for a single record, or
- #"Bootstrap(std::initializer_list<Initializer>TEnum>>)"
  for an array of records.

(See chapter #"alib_enums_records_firststep_init" for sample code).

A next method offered accepts a string and two delimiter characters:
- #"Bootstrap(const String& input, character innerDelim, character outerDelim)"
...
Details are given with the non-existing, pure documentation type #"EnumRecordPrototype".
Method \b Parse is parameterless because all parsing information (current remaining input string
and delimiters) is accessible through 100% static helper-type #"bootstrap::EnumRecordParser".
...
records should #"alib_mod_resources;be resourced".
...
which can be accessed after the inclusion of the header #"F;ALib.Camp.H".
...
Module \alib_resources_nl provides the type trait #"ResourcedTraits" to announce
...
- #"Bootstrap(character innerDelim, character outerDelim)"
...
An important advantage of using the type trait #"ResourcedTraits" when used with
...
For this, it ignores the method #"ResourcedTraits::Name;*" and uses the name
...
- #"Bootstrap(camp::Camp&, const NString& name, character innerDelim, character outerDelim)"

This method should be used when software uses class #"Camp" to organize bootstrapping,
...
Further information is given with the #"alib_mod_resources;programmer's manual" of module
\alib_resources_nl.<br>
A comprehensive sample of using \alib resources placed in a custom module is provided with the
#"alib_app_cli_tut;tutorial of ALib Module 'CLI'".
...
<li> A C++ enumeration \e type (!) is used for the name that a) addresses an iterable record list
     ...
     (Technically implemented by using runtime type information, aka keyword <c>typeid</c>).</li>
...
#"alib_enums_records_details_inheritance". The inheritance relationship simply emerges from
...
#"StringTree" of <em>"variables"</em>. These variables can either be
declared in a "hard-coded" fashion or optionally be declared by meta-information struct
#"var Declaration".

This declaration struct extends built-in enum record type #"ERSerializable". While the
...
(See #"Variable;the reference documentation" of class \b Variable.)
...
enum records of type #"var Declaration". If the records are resourced, then variable names,
...
- Special method #"Configuration::PreloadVariables;*" goes even one step further: Just
  all elements, hence all variables of the enumeration types are preloaded with their default value
  with just one invocation.

Another prominent sample is found with class #"exc Exception". 
Furthermore, the classes that handle command-line interface processing in module \alib_app make 
heavy use of this paradigm.
...
#"boxing::Enum". The type is located in module \alib_boxing, because it leverages that
...
the arbitrary type. Along with the underlying integral value of an enum element, runtime
...
With the namespace functions #"enumrecords::GetRecord" and #"enumrecords::TryRecord", such 
postponement is also offered for the concept of <b>ALib Enum Records</b>.
