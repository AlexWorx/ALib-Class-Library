...
The whole \alib project once started with the idea of implementing \alox. For this, some 
utility classes were needed and separated from \alox. Over time, these classes evolved into a
full-fledged library and the relationship somehow turned to the opposite: \alox is now 'just'
one module within \alib.

The library can be compiled to a subset of those modules. Thus, if you are you exclusively
interested in logging, other \alibmods_nl can be excluded from the build. 
