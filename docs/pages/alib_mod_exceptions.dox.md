...
This rather small \alibmod introduces two types, class #"Message" which builds
the foundation for the second and more important type #"exc Exception".

# 2\. Class Message # {#alib_exceptions_message}

When measured in source code, the size of class #"Message" is very short.
Nevertheless, as the type leverages almost all features of module \alib_boxing, it is a quite
powerful tool to have.

The type acts as an information object of the central \b throwable type #"exc Exception",
discussed in the next chapter.

\I{################################################################################################}
## 2.1 Source Location ## {#alib_exceptions_message_location}
Objects of type #"Message" relate to a source code location, which is reflected by field
#"Message::CI".
...
There are two different macros defined for this purpose, #"ALIB_CALLER" and #"ALIB_CALLER_NULLED".
The first macro provides the corresponding built-in configuration macros for the source file,
...
builds, macro #"ALIB_CALLER_NULLED" provides \e nulled values for the arguments.

It depends on the using software which macro should be used. It must be noted that with the use
of macro #"ALIB_CALLER", even in release compilations, information about the name and location
...
Internally, \alib exclusively uses the "pruning" version #"ALIB_CALLER_NULLED".
This is because otherwise using software's executable would include source information of \alib
which may not be wanted.

\note
  There is a third macro existing with #"ALIB_CALLER_PRUNED", which is empty with non-debug
...
To define the type of a message, class #"boxing::Enum" provided by module \alib_boxing
is used, by letting field #"Message::Type;*" be of type \b %Enum.
...
By using the class #"boxing::Enum", there are exactly two hierarchy levels: the first is
accessed with its inherited method #"^Enum::IsType". With that it can be determined if the
"boxed" enumeration element is of a certain scoped enum type. To check both levels at once,
#"Enum::operator==(const)" can be used.

\note
  - While this is not recommended in common use cases, method #"Enum::Integral" allows
  ...
  - It is also not recommended to use non-scoped (anonymous) enumeration types. While two
    elements of two anonymous enum types are well distinguished with #"Enum::operator==(const)",
    the use of the inherited method #"^Enum::IsType" is rather tricky and not well readable: 
    the declaration type of a sample element has to be given as the template parameter to the 
    method.
...
inherits class #"BoxesMA", which itself is a container (vector) of objects of type
#"Box". The interpretation of the data given in a message is defined by an application.
...
As documented with class #"TBoxes", its underlying <c>std::vector</c> may use
either standard dynamic memory or "monotonic allocation" introduced by module \alib_monomem.
With class #"Message", monotonic allocation is chosen. A reference to the allocator has to
...
Because class #"Message" is a #"alib::BoxesMA;vector of boxes" (by inheritance),
...
built-in box-function #"FClone".
This function expects an object of type #"MonoAllocator" that is used to allocate
memory for cloning the contents of boxed data.

\see
  For more information, see the #"FClone;reference documentation" of box-function \b FClone
  and chapter #"alib_boxing_more_iclone" of the Programmer's Manual of module \alib_boxing_nl.
...
Class #"exc Exception" is used within \alib as <b>the one and only type</b> thrown with
C++ keyword \c throw.
...
\alib adheres to a different pattern and class #"exc Exception" supports, respectively
implements this.
...
of type #"Message" and the exception information passed with the constructor of the class,
...
This problem is solved by using type #"Message" as entries of class
#"exc Exception". As discussed in previous chapter #"alib_exceptions_message_types",
this way a two-level hierarchy of exception types is established, where the first level uses
runtime type information and hence no clash of exception types may occur, even between different
code units.
...
This \b Enum is searched and returned by the method #"Exception::Type".
...
many new entries and each entry, as being of type #"Message" in turn may consist of
...
This all is achieved by inheriting class \b Exception from class #"TSharedMonoVal".
Please consult that types' reference documentation as well as chapter
#"alib_contmono_smv" of the Programmer's Manual of module \alib_monomem_nl.

Each time an entry is added with method #"Exception::Add;*", a new message object is
allocated in the monotonic allocator and the values of the given boxes are cloned
(see previous chapter #"alib_exceptions_message_lifecycle").
...
We learned so far that class #"exc Exception" uses a list of entries
of type #"Message", discussed in the #"alib_exceptions_message;first chapter"
of this manual.

Besides the source information and the message types, there is a third aspect that class
#"exc Exception" leverages from its entry type #"Message": the list of
...
- The first argument should be a string which comprises a
  ...
  #"alib_mod_format;ALib Formatters" (or custom formatter syntax implementations).
...
overloaded methods #"Exception::Format;*".
In addition, when using the debug- and release-logging facilities provided with module \alib_alox,
exceptions can be comfortably logged with static method #"LogTools::Exception;*".
...
constituted by method #"Exception::Type;*" that searches the most recent enty with a positive
...
For example, using the #"FormatterPythonStyle;python-style formatter", placeholder
...
In the case an exception should be rethrown, method #"Exception::Add;*" may be used to
...
Being a #"alib_contmono_smv;self-containted type" that implements a shared-pointer
...
\I{################################################################################################}
## 3.7 Resourced Exceptions ## {#alib_exceptions_exceptions_res}
...
#"alib_mod_resources;externalized string resources".

When custom enum types are associated with <em>ALib Enum Records</em> of type #"ERException",
elements of this type become accepted by method #"Exception::Add;*" which internally adds
...
\I{################################################################################################}
## 3.8 Raising ALib Exceptions From System Errors ## {#alib_exceptions_system_errors}

When following the \alib exception paradigm discussed in the previous chapters, it is recommended 
to exclusively throw \alib's \b Exception type. Thus, if a standard library call exists in an
alternative version which allows to get a <c>std::error_code</c> instead of throwing an exception,
that version should be preferred.

The code returned, can then be easily mapped to an \alib exception entry by using the overloaded
namespace function #"CreateExceptionFromSystemError(const CallerInfo&, std::error_code)".

Here is a quick example:

\snippet "ut_lang.cpp"        DOX_EXCEPTION_FROMSYSTEM

If cought like this:

\snippet "ut_lang.cpp"        DOX_EXCEPTION_FROMSYSTEM_CATCH
                 
The contents of the output would be:
                                              
\verbinclude pages/generated/Exceptions_ExceptionFromSystemError.txt
                         
The exception interface allows retrieving the original error information 
(the <c>std::errorcode</c> object), which may be useful for further analysis:

\snippet "ut_lang.cpp"        DOX_EXCEPTION_FROMSYSTEM_CATCH2

These two lines produce:
\verbinclude pages/generated/Exceptions_ExceptionFromSystemError2.txt


An overload of this function that an \c int value, can be likewise used to create exceptions
based on C-macro \c errno.
    
Finally, for WindowsOS software, in addition function 
#"CreateExceptionFromWindowsOSError" is provided.
