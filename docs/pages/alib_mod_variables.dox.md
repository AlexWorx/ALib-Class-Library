...
A simple application needs to include just header #"F;ALib.Variables.H"
\snippet "DOX_VARIABLES.cpp"     DOX_VARIABLES_HEADER

\note OK, a second include is needed with #"F;ALib.Lang.CIFunctions.H", because
      the code following is a global function, namely <c>main()</c>. Background information on this
      is given with chapter #"alib_manual_appendix_callerinfo" of the General ALib Manual.
...
- \alib provides an implementation of the concept of "runtime variables".
- Variables have a name, which is of string-type.
- Variables are associated with \alib camps, in this case the singleton #"BASECAMP;2".<br>
  Well this is not really true: In fact variables are associated with an instance of class
  #"Configuration". Now, because every \alibcamp provides a singleton that disposes over
  an own or a shared instance, in this sample we use an overload of the constructor of class
  #"Variable" that accepts a \b Camp instance instead of a \b Configuration
...
- There is a (convenience) method #"Variable::GetString;*" that receives a string value
  from a variable.

OK, so far so good. But you might wonder: What is this good for? C++ already has variables in
its language. Why should now runtime variables be used?
...
- Central class #"Configuration" used to store and retrieve internal and external
  configuration data using arbitrary sources.
- Abstract plug-in interface #"ConfigurationPlugin" to read from custom external
...
- Possible provision of variable declarations, including
  ...
  as #"alib_enums_records;resourced C++ enumeration values".<br>
...
- Automatic #"alib_variables_values_substitution;variable substitution", which resolves the values of
...
\alibcamp_nl can used in #"alib_variables_usecases;other use-cases" - apart from "just" configuration data.

\I{################################################################################################}
# 2\. Variable Names # {#alib_variables_names}
\I{------------------------------------------------------------------------------------------------}
## 2.1 Building on Class StringTree ## {#alib_variables_names_string_tree}
Variables are stored in and managed by class #"Configuration". This type inherits
...
#"StringTree;reference documentation" now. In short, this type is a hash
...
sampled <c>"MY_VAR"</c> and as implemented with all #"alib_cfgvars;built-in variables"
of \alib.

Built-in plug-ins #"EnvironmentVariablesPlugin" and #"CLIVariablesPlugin",
as well as class #"IniFileFeeder" (all types are discussed in detail in later chapters)
...
- Camp #"alib_mod_camp;Camp": All variables are residing in path <c>"/ALIB/"</c>
- Camp \alib_alox: All variables are residing in path <c>"/ALOX/"</c>

Let us look at variable #"alibcvALIB_LOCALE" which is read by camp class
#"BASECAMP;2" during bootstrap and allows setting a different locale than defaulted
...
#"alxcvALOX_LOGGERNAME_FORMAT". While the documentation here says "/LOGGERNAME/" the resourced
name of the declaration of the variable reads <c>"ALOX/%1/FORMAT"</c>.<br>
Classes #"Variable" and #"var Declaration" both provide overloaded methods
...
#"MonoAllocator" of class \c Configuration. This means, that it is not allowed to
...
create and delete new loggers with different (runtime generated) names.

\note On this topic, please see also the reference documentation of method
      #"Configuration::StoreDeclaration;*", which is used by the overloaded methods which
      accept parameter \p{replacements}.

Parameter \p{replacements} of the various overloaded declaration methods is of type
#"Box". As explained with the #"alib_mod_boxing;Programmer's Manual of module Boxing",
...
Internally, #"alib_boxing_basics;type-guessing" is performed and the following types are
discovered:
- A single \b Box object,
- a C++ array af \b Box objects, and
- instances of #"TBoxes" (either version #"Boxes;2", #"BoxesMA;2" or #"BoxesPA;2").
...
module \alib_alox, shows how two replacements strings are passed. 
The resourced variable, as given in the resource file #"^F;alox/aloxcamp.alibrc", is:

\snippet "alox/aloxcamp.alibrc"     DOX_VARIABLES_REPLACEMENTS1

The declaration of the individual variable for a certain #"detail::Logger" attached to a certain
#"Lox" is as simple as this:
...
With the declaration, the runtime memory allocation of the variable is in fact performed.
...
- The underlying #"StringTree::TCursor;string tree cursor" is moved
...
These four elements can be specified by using an instance of the class #"var Declaration":
...
For this, the concept of #"alib_enums_records;C++ Enum Records", introduced with module
...
This works, because:
1. Type #"camp::Variables" is a C++ enum type and that type was equipped with enum resourced 
   records:
   \snippet "camp/basecamp.hpp"     DOX_VARIABLES_DECLARATION5
2. The resources strings are defaulted during bootstrapping \alib as follows:
   \snippet "camp/basecamp.alibrc"  DOX_VARIABLES_DECLARATION6
3. Finally, during bootstrap, the records are read:
...
The inclusion of the module \alib_camp with the header #"F;ALib.Camp.H", injects various overloads
of the function #"CampVariable(camp::Camp&);CampVariable" into the namespace #"alib::variables;2" 
of this module.
...
Please read #"variables::Priority;this enum's reference documentation" now.
...
#"variables::Priority;Standard". Priority \b Standard is used (as default!) when common source code
assigns a value to a variable.

In turn, \b Standard is superseded by any of the external configuration data sources like
#"variables::Priority;ConfigFile", #"variables::Priority;Environment" or #"variables::Priority;CLI".
...
Enumeration type \b Priority is an #"ArithmeticalTraits;ALib arithmetical enum".
...
With this background information, method #"Variable::Define;*" can be explained.
It has a parameter that was not visible in the hello-world-sample, namely parameter \p{priority},
which is defaulted to #"variables::Priority;Standard". Now, all that this method does is
...
- provide a different value to method #"Variable::Define" when setting variables in
  the code, or
...
Enumeration type #"variables::Priority" defines element \b Protected. It has the maximum
...
#"alib_variables_values_importexport" and #"alib_variables_values_presets".
...
- Declaring a variable creates an entry in the \b StringTree including the variable's data storage.
  The "priority" of the variable is set to #"variables::Priority;Priority::NONE" which denotes
...
with methods #"Variable::IsDeclared;*" and #"Variable::Try(const String&);Variable::Try".
...
----------------------|-----------------------|-----------------------------
 <c>B</c>             | \c bool               | A boolean value, that is parsed with method #"ParseBooleanToken" and optionally written back with #"WriteBooleanToken".
 <c>I</c>             | #"alib::integer;2"    | An integral value which will be parsed when imported from external string values.
 <c>F</c>             | \c double             | A floating-point value which will be parsed when imported from external string values.
 <c>S</c>             | #"AStringPA"          | A string value. This is probably the most often used type.
 <c>SV,</c>           | #"StringVectorPA"     | A list of strings, which is imported by parsing a comma-separated list.
 <c>SV;</c>           | #"StringVectorPA"     | A list of strings, which is imported by parsing a list of strings separated by semicolons.
 <c>BOX</c>           | #"Box"                | A generic variable type using the power of \alib_boxing.

The value of a variable can be accessed with templated method #"Variable::Get".
...
Method #"Variable::GetString;*" is just an inlined shortcut to <b>Get<AStringPA>()</b>.
Similar methods for the other built-in types exist with
#"Variable::GetBool",
#"Variable::GetInt",
#"Variable::GetFloat",
#"Variable::GetDouble",
...
wrong type, #"alib_mod_assert;raises an ALib error" with debug-compilations.
...
\alib_boxing have to be respected. The #"VMeta" implementation (explained only in the
...
- Reference documentation of class #"VMeta".
- Preprocessor macros
  - #"ALIB_VARIABLES_DEFINE_TYPE",
  - #"ALIB_VARIABLES_DEFINE_TYPE_WITH_POOL_CONSTRUCTOR" and
  - #"ALIB_VARIABLES_REGISTER_TYPE".
- Method #"Configuration::RegisterType;*".
- Chapter #"alib_contmono_poolallocator_metatrick" of the Programmer's Manual of module
  \alib_monomem.
...
\note This is due to the use of the #"monomem::TPoolAllocator;PoolAllocator" for allocations and
      recycling.
      This value might be changed by passing a custom value to configuration macro
      #"ALIB_MONOMEM_POOLALLOCATOR_DEFAULT_ALIGNMENT".
      See chapter #"alib_variables_internals_memoman" for further information.
...
1. Declare an ordinary type that determines the variable's contents. The struct used is
   #"textlogger::FormatMetaInfo" and it contains six members, all being of string type
   #"AStringPA".<br>        <br>
2. In the header that declares the struct (outside any namespace), the following macro
   is used to create a custom #"VMeta" derivate:
   \snippet "lox/textlogger/variables.hpp"     DOX_VARIABLES_DEFINETYPE1
   Note that we are using the second version of the macro, because the string members of type
   \b FormatMetaInfo are of pool-allocated #"AStringPA" type.
   Those need a #"monomem::TPoolAllocator;PoolAllocator" with construction and here, with the
...
3. The preprocessor macros are missing to define virtual abstract methods
   #"VMeta::imPort;*" and #"VMeta::exPort;*". Those have to be defined in a
   ...
   #"alib_variables_values_importexport".<br><br>
4. In the bootstrap code of camp class #"ALoxCamp", the variable type is registered with
...
5. Finally, for easy access, class \b TextLogger received inline shortcut method
   #"TextLogger::GetFormatMetaInfo", which just returns the result of
   #"Variable::Get;Get<FormatMetaInfo>()" on the variable which is hold as a member
   of the logger type.
...
for the built-in types (see previous chapter #"alib_variables_types_builtin", are of course
not existing for custom types. Hence, variable value read/write access is always to be made
using templated method #"Variable::Get;Get<T>". In the sample of the previous chapter,
...
default value. For this, either #"Declaration::DefaultValue;*" is used or parameter
...
With method #"Variable::Import;*", string values can be used to set a variable value
even after its declaration and a variable can likewise be serialized into a string value with
method #"Variable::Export;*".
...
\note In this sample, the priority is set to #"variables::Priority;Protected"
      which indicates that no other configuration source is allowed to set any other value.
      See previous chapter #"alib_variables_definition_protected" for more information on this.
...
#"VMeta::imPort;*" and #"VMeta::exPort;*", this camp talks about importing and
...
For escaping strings, virtual utility type #"util::StringEscaper" and its derivate
#"util::StringEscaperStandard", provided by module \alib_strings, is used.
...
Its use is straightforward through overloaded methods 
...
More on using this concept is described in later chapter #"alib_variables_external".
...
#"Configuration::SubstitutionVariableStart" and
#"Configuration::SubstitutionVariableEnd" of class \b %Configuration.
...
A third member is #"Configuration::SubstitutionVariableDelimiters". Please refer
...
#"Configuration" provides a plug-in mechanism. Simple virtual base class
#"ConfigurationPlugin" describes the interface that is to be implemented.
When done, the plugin can be attached with the method #"PluginContainer::InsertPlugin;*".

For two of such sources, built-in solutions exist with types
- #"EnvironmentVariablesPlugin", and
- #"CLIVariablesPlugin"

By default, both plug-ins are created and attached with the construction of a configuration instance 
(respectively when its inherited method #"^Configuration::Reset" is called).
To prevent this, the optional parameter \p{createDefaults} may be given.

\I{------------------------------------------------------------------------------------------------}
## 7.3 Built-in Class IniFileFeeder ## {#alib_variables_external_ini}
As explained in the #"alib_variables_external_general;introduction of this chapter", configuration
...
1. The use of the method 
   #"Configuration::PresetImportString(StringEscaper*);Configuration::PresetImportString". 
   This method stores import strings together with a #"util::StringEscaper" needed for the later
    import in an otherwise hidden branch of the \b StringTree of a \b Configuration object.<br>
   These values are automatically imported at the moment a variable is declared.
2. The use of the method #"Configuration::PreloadVariables;*" \b before feeding external
   data. Note that this method relies on variable declarations, that are duly externalized
   by using #"alib_enums_records;resourced C++ enumeration values", as described in the previous
   chapter #"alib_variables_declaration_decl_resourced".
...
#"IniFileFeeder" can be given and this should be easily understood:

<b>With the start of software:</b>
- During or after bootstrapping of an application, use method
  #"Configuration::PreloadVariables;*" to declare your custom variables.
  Alternatively, if no enum records are in place, do this in a manual way for each variable.
- Create a local instance of class #"variables::IniFileFeeder".
- Use method #"IniFileFeeder::ImportStart" to load an INI-file.
- Use methods named \b Import (or own specialized code) to declare variables in the configuration.
- Call #"IniFileFeeder::ImportEnd" to free all memory allocated by the INI-file loaded.
...
- Use method #"IniFileFeeder::ExportStart" to open the same INI-file as after bootstrap.
...
- Optionally, method #"IniFileFeeder::AddResourcedSectionComments(ResourcePool&)"
  can be invoked to add (externalized) comments to the sections of an INI-file.<br>
  Furthermore, the internal INI-file can be retrieved with #"IniFileFeeder::GetIniFile;*"
  and missing #"IniFile::FileComments" may be set (or other tweaks performed).
- Invoke #"IniFileFeeder::ExportEnd" to write the INI-file.
...
Next, the source code of class #"IniFileFeeder" should be analysed. The good news
here is that most INI-file related code resides in separate class #"IniFile" which
\b IniFileFeeder uses.

With that, the code is easily understandable and rationally short to be used as a jump start.
In addition, information given in chapter #"alib_variables_stringtree" might be helpful.

Another detail to be discussed is that export methods of variable types are allowed to insert
#"NEW_LINE" code into the export string. An external configuration system probably has to
...
lists of values. Built-in class #"IniFileFeeder", respectively the internally used
helper-type #"IniFile", creates duly formatted lines and adds a continuation backslash
...
The built-in string vector types <c>SV,</c> and <c>SV;</c> insert a #"NEW_LINE" code after each
character.
...
- Register a listener for monitoring the branch of variables (and/or specific session-related)
  variables. See upcoming chapter #"alib_variables_monitoring" for information how to do so.
- When notified on changes, create an instance of class #"IniFileFeeder", invoke method
  #"IniFileFeeder::ExportStart" by providing a filename within a temporary folder.
- Then use method #"IniFileFeeder::Export(const Str)"
  to export the subtree of session variables. For example:

        myIniPlugin.Export( "/MYAPP/SESSION" )

  and then write the INI-file with #"IniFileFeeder::ExportEnd"
- The former two steps could also be placed in a background task that defers the writing of the
  ...
  thread-safeness has to be asserted. (See later chapter #"alib_variables_multithreading").

- Of course, when the application starts, the corresponding methods
  #"IniFileFeeder::ImportStart", #"IniFileFeeder::ImportAll" and
  #"IniFileFeeder::ImportEnd" have to be used with a similar \b IniFileFeeder instance,
...
This module provides, for example, built-in type #"IniFileFeeder" which provides an
...
- Equip your software with code that populates INI-files with default values in the case
  ...
  #"alib_variables_external_ini".
- In the previous step, populate the INI-file with all variables, configuration data and session
  data.
- For those variables that resemble session values, invoke the method
  #"IniFileFeeder::SetWriteBackFlag(const Variable&);*". As documented, this method sets the
  #"config_IniFile_writebackflag;writeback flag" only for variables that did not exist
...
#"alxcvALOX_LOGGERNAME_AUTO_SIZES". This is exposed by module \alib_alox.


\I{################################################################################################}
# 8\. The StringTree in Class Configuration # {#alib_variables_stringtree}

It has been mentioned already several times in this manual, that class #"Configuration"
inherits class #"containers::StringTree". A bigger portion of its logic builds
on this base type. Consequently, class #"Variable" encapsulates a
#"StringTree::TCursor;*" and with that it is just a lightweight pair of
pointers (which even fits into a #"Box").
...
When using the StringTree interface, often values of type #"StringTree::Cursor;*"
are involved. This type provides the method #"TCursor::Tree", which, for convenience, allows 
providing an optional template type to statically cast the tree instance to.
...
Class #"Variable" inherits class #"StringTree::TCursor;*"
...
#"Variable::AsCursor;*", which is just a static cast! The cursor can be manipulated
...
If a cursor was generated from one of these variables and the next method #"TCursor::GoToParent"
was invoked, and then this cursor was passed to the constructor of class \b Variable, then this 
variable was illegal.
...
variable can be constructed from the cursor and method #"Variable::IsDeclared"
...
will be found. Beyond this node, class \c Configuration stores information provided with the method
#"Configuration::PresetImportString(StringEscaper)". This node and its children have to be skipped
...
#"ConfigurationListener" is to be created and registered with a configuration instance.
...
method #"Variable::Define;*" in case the method returns \c true. With that, this event
is used to monitor changes of a variable's value.<br>
As explained in chapter #"alib_variables_definition_define", the whole concept of defining variables
...
While there is only one interface type #"ConfigurationListener", which only has
...
1. #"Configuration::MonitorDistinctVariable"<br>
...
2. #"Configuration::MonitorVariablesByName"<br>
   Used for monitoring all variables that share the same name (residing in any path).<br>
   As sample use-case, built-in configuration variable #"alxcvALOX_LOGGERNAME_AUTO_SIZES"
...
5. #"variables::Configuration::MonitorPathSubstring"<br>
...
- Enumeration #"ContainerOp", to specify if a listener should be added or removed
- The listener instance
- The #"ConfigurationListener::Event" to get notified on.
...
registration</b>, along with the first parameter being #"lang::ContainerOp;ContainerOp::Remove".
...
is offered with method #"Configuration::MonitorStop", which removes all registrations
...
class #"StringTree" of camp \alib_containers, it fully implements this camp's
concept of #"alib_contmono_intro_strictweak;weak monotonic allocations". The challenge of
extending the #"alib_contmono_intro_recycling;recycling capabilities" of class \b StringTree
to the user-defined variable content types, was resolved by adding a
#"monomem::TPoolAllocator;PoolAllocator" which uses the #"MonoAllocator" of the
...
its Programmer's Manual provides chapter #"alib_contmono_poolallocator_metatrick", which discusses
...
With the configuration macro #"ALIB_DEBUG_CRITICAL_SECTIONS" set, the underlying \b StringTree receives
assertion mechanics to detect forbidden parallel access. More information on how this works is
provided with chapter #"alib_threads_intro_assert_entry" of the Programmer's Manual of module
\alib_threads.
...
1. Class #"Variable" is a set of two pointers, one to the internal
   #"StringTree" and a second to the specific node in the tree that stores
   a variable's meta-information. <br>
   Once a variable is declared, a corresponding #"Variable" object remains valid,
...
3. Re-definitions of a variable with a higher priority impose, as was explained in this manual,
   ...
   (For example, by calling method #"Variable::IsDefined" may result in undefined values.
...
4. To explicitly mention it: Points \b 1. and \b 2. are also true if a rehash of the underlying
   #"HashTable" of the \b StringTree is re-hashed at the moment further
   variables are inserted into a \b Configuration instance.
5. When accessing the underlying <b>StringTree::Cursor</b> by calling #"Variable::AsCursor;*"
...
6. Methods #"Variable::IsDeclared;*" and #"Variable::Try(const String&);Variable::Try" are safe to 
   be used along with other read operations and read-only cursor movements in the \b StringTree.
...
As described chapter #"alib_mod_bs_customize" of the general manual of \alib,
...
#"alib_cfgvars;reference manual".

The only occurrence of insertions and deletions of variables built-in with any \alibcamp_nl
is when instances of class #"textlogger::TextLogger" are inserted to or removed from a
#"Lox". Now, in common applications, loggers are inserted during bootstrapping and removed
...
Generally spoken, this camp offers named runtime variables, which in contrast to using simple
...
stored by software and as explained in chapter #"alib_variables_multithreading", it is ensured
that these pointers remain valid, even if parallel insertions or deletions of other variables
occur.<br>
Next, by accessing the underlying \b StringTree cursor (with method #"Variable::AsCursor;*")
